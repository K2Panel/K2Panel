...
class BackupManager:
    ...
    def __init__(self, backup_dir: str = "backups", retention: int = 7):
        ...
        self.backup_items = ["data/", "logs/", ".env", "alembic.ini"]
    ...
    def create_backup(self) -> Optional[str]:
        ...
        try:
            ...
            print(
                f"{Colors.BOLD}🔄 بدء عملية النسخ الاحتياطي - Starting Backup{Colors.ENDC}"
            )
            ...
            if not secret_key:
                ...
                hmac_hash = calculate_hmac(
                    str(backup_path), secret_key, algorithm="sha256"
                )
    ...
    def _backup_database(self, temp_dir: Path) -> bool:
        ...
        print(
            f"{Colors.OKCYAN}💾 نسخ احتياطي لقاعدة البيانات ({db_type})...{Colors.ENDC}"
        )
    ...
    def _backup_postgresql(self, temp_dir: Path) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(
                    f"  {Colors.OKGREEN}✓{Colors.ENDC} تم نسخ قاعدة بيانات PostgreSQL"
                )
                return True
            else:
    ...
    def _backup_mysql(self, temp_dir: Path) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, stdout=open(dump_file, "w"), stderr=subprocess.PIPE, timeout=300
            )
    ...
    def list_backups(self) -> List[Dict]:
        ...
        for backup_file in sorted(
            self.backup_dir.glob("backup_*.tar.gz"), reverse=True
        ):
            ...
            backup_info = {
                ...
                "created": datetime.fromtimestamp(backup_file.stat().st_mtime).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "md5": None,
                "environment": None,
    ...
    def display_backups(self):
        ...
        print(f"{Colors.HEADER}{'=' * 100}{Colors.ENDC}")
        print(
            f"📊 إجمالي: {len(backups)} نسخة احتياطية | الحجم الكلي: {format_size(total_size)}"
        )
        print(f"{Colors.HEADER}{'=' * 100}{Colors.ENDC}\n")
    ...
    def cleanup_all(self):
        ...
        confirm = input(
            f"\n{Colors.BOLD}هل تريد المتابعة؟ (yes/no): {Colors.ENDC}"
        ).lower()
    ...
    def _safe_extract_member(
        ...
        try:
            ...
            for allowed_root in RestoreLimits.ALLOWED_ROOTS:
                if root_item == allowed_root or root_item.startswith(
                    allowed_root.rstrip("/") + "/"
                ):
                    allowed = True
                    break
            ...
            if not allowed:
                self.logger.warning(
                    f"رفض: جذر غير مسموح - {member.name} (root={root_item})"
                )
                stats["rejected"] += 1
            ...
            if member.size > RestoreLimits.MAX_FILE_SIZE:
                self.logger.warning(
                    f"رفض: حجم ملف زائد - {member.name} ({format_size(member.size)})"
                )
                stats["rejected"] += 1
                stats["rejected_reasons"][
                    member.name
                ] = f"حجم زائد: {format_size(member.size)}"
                return False
            ...
            for parent in target_path.parents:
                ...
                try:
                    if parent.is_symlink():
                        self.logger.error(
                            f"رفض: الأب symlink - {member.name} (parent={parent})"
                        )
                        stats["rejected"] += 1
            ...
            if member.isdir():
                target_path.mkdir(
                    parents=True, exist_ok=True, mode=RestoreLimits.SAFE_DIR_MODE
                )
                stats["accepted"] += 1
                return True
    ...
    def restore_backup(
        self, backup_file: str, force: bool = False, skip_md5: bool = False
    ):
        """
        ...
        if not force:
            confirm = input(
                f"{Colors.BOLD}هل تريد المتابعة؟ (yes/no): {Colors.ENDC}"
            ).lower()
        ...
        try:
            ...
            if info_file.exists():
                ...
                if format_version == "v2":
                    if not sha256_found or not hmac_found:
                        print(
                            f"{Colors.FAIL}❌ النسخة v2 تتطلب SHA-256 و HMAC!{Colors.ENDC}"
                        )
                        return

                    print(
                        f"{Colors.OKCYAN}🔐 جاري التحقق من SHA-256 + HMAC (v2)...{Colors.ENDC}"
                    )

                    # التحقق من SHA-256 (constant-time comparison)
                    actual_sha256 = calculate_checksum(
                        str(backup_path), algorithm="sha256"
                    )
                    ...
                    if not secret_key:
                        print(
                            f"{Colors.FAIL}❌ SECRET_KEY مطلوب للتحقق من HMAC!{Colors.ENDC}"
                        )
                        return
                    ...
                    if self.config.ENVIRONMENT == "production":
                        ...
                        if secret_key in weak_keys or len(secret_key) < 32:
                            print(
                                f"{Colors.FAIL}❌ SECRET_KEY ضعيف جداً للإنتاج!{Colors.ENDC}"
                            )
                            return

                    actual_hmac = calculate_hmac(
                        str(backup_path), secret_key, algorithm="sha256"
                    )
                    ...
                    print(
                        f"{Colors.OKGREEN}✓ تم التحقق من HMAC بنجاح (الأصالة مؤكدة){Colors.ENDC}"
                    )

                # fallback لـ SHA-256 بدون HMAC (v1 مع SHA-256)
                elif sha256_found and hmac_found:
                    print(
                        f"{Colors.WARNING}⚠️  تحذير: نسخة قديمة بدون version marker{Colors.ENDC}"
                    )
                    print(
                        f"{Colors.OKCYAN}🔐 جاري التحقق من SHA-256 + HMAC...{Colors.ENDC}"
                    )
                    ...
                    secret_key = (
                        self.config.SECRET_KEY or "fallback-key-for-development"
                    )
                    actual_hmac = calculate_hmac(
                        str(backup_path), secret_key, algorithm="sha256"
                    )
                    if hmac.compare_digest(expected_hmac, actual_hmac):
                        ...
                        print(
                            f"{Colors.WARNING}⚠️  فشل HMAC - قد يكون SECRET_KEY مختلف{Colors.ENDC}"
                        )

                # fallback لـ MD5 (نسخ قديمة جداً - v1 قديم)
                elif md5_found:
                    print(
                        f"{Colors.WARNING}⚠️  تحذير: النسخة تستخدم MD5 (deprecated - v1 legacy){Colors.ENDC}"
                    )
                    print(
                        f"{Colors.WARNING}   يُوصى بشدة بإعادة إنشاء النسخة بـ SHA-256 + HMAC{Colors.ENDC}"
                    )
                    ...
                    print(
                        f"{Colors.OKGREEN}✓ تم التحقق من MD5 بنجاح (لكن غير آمن!){Colors.ENDC}"
                    )

                # لا يوجد checksum
                elif not skip_md5:
                    print(
                        f"{Colors.FAIL}❌ ملف .info لا يحتوي على checksum صالح{Colors.ENDC}"
                    )
            ...
            # ملف .info غير موجود
                ...
                if not skip_md5:
                    print(
                        f"{Colors.FAIL}❌ ملف .info غير موجود - لا يمكن التحقق من سلامة النسخة{Colors.ENDC}"
                    )
                    print(f"   استخدم --skip-md5 لتخطي التحقق (غير آمن)")
                    return
                else:
                    print(
                        f"{Colors.WARNING}⚠️  تحذير: تم تخطي التحقق من checksum (--skip-md5){Colors.ENDC}"
                    )
            ...
            with tarfile.open(backup_path, "r:gz") as tar:
                ...
                if len(members) > RestoreLimits.MAX_FILES:
                    raise ValueError(
                        f"عدد الملفات ({len(members)}) يتجاوز الحد الأقصى ({RestoreLimits.MAX_FILES})"
                    )
            ...
            self.logger.info(
                f"تم استرجاع النسخة: {backup_file} (مقبول={stats['accepted']}, مرفوض={stats['rejected']})"
            )
...
أمثلة الاستخدام:
  ...
  python backups/backup_manager.py --restore FILE # استرجاع نسخة احتياطية
    ...
    parser.add_argument(
        "--list", action="store_true", help="عرض قائمة النسخ الاحتياطية"
    )
    parser.add_argument("--cleanup", action="store_true", help="تنظيف النسخ القديمة")
    parser.add_argument(
        "--keep", type=int, metavar="N", help="تغيير عدد النسخ المحفوظة (الافتراضي: 7)"
    )
    parser.add_argument(
        "--restore", type=str, metavar="FILE", help="استرجاع نسخة احتياطية من ملف"
    )
    parser.add_argument(
        "--force", action="store_true", help="تخطي التأكيد اليدوي (مع --restore)"
    )
    parser.add_argument(
        "--skip-md5", action="store_true", help="تخطي التحقق من MD5 (غير آمن)"
    )
    parser.add_argument(
        "--no-color", action="store_true", help="تعطيل الألوان في الإخراج"
    )
