...
class userlogin:
    ...
    def request_post(self, post):
        if not hasattr(post, "username") or not hasattr(post, "password"):
            return (
                public.returnJson(
                    False, public.lang("User name or password cannot be empty!")
                ),
                json_header,
            )
        ...
        if self.limit_address("?") < 1:
            return (
                public.returnJson(
                    False,
                    public.lang(
                        "You have failed to log in many times, please try again in {} seconds!",
                        int(self.limit_expire_time - time.time()),
                    ),
                ),
                json_header,
            )
        ...
        if not re.match(r"^\w+$", post.username):
            return (
                public.return_msg_gettext(
                    False,
                    public.lang(
                        "Disk inode has been exhausted, the panel has attempted to release the inode. Please try again ..."
                    ),
                ),
                json_header,
            )
        ...
        if not last_login_token:
            public.WriteLog(
                "TYPE_LOGIN", "LOGIN_ERR_CODE", ("****", "****", public.GetClientIp())
            )
            return (
                public.returnJson(
                    False,
                    public.lang(
                        "Verification failed, please refresh the page and log in again!"
                    ),
                ),
                json_header,
            )
        ...
        if os.path.exists(user_plugin_file):
            ...
            for u_info in user_list:
                if (
                    public.md5(public.md5(u_info["username"] + last_login_token))
                    == post.username
                ):
                    userInfo = u_info
        else:
        ...
        if "code" in session:
            if session["code"] and not "is_verify_password" in session:
                if not hasattr(post, "code"):
                    return (
                        public.returnJson(
                            False, public.lang("Verification code can not be empty!")
                        ),
                        json_header,
                    )
                ...
                if not public.checkCode(post.code):
                    public.write_log_gettext(
                        "Login",
                        "Verification code is incorrect, Username:{}, Verification Code:{}, Login IP:{}",
                        ("****", "****", public.GetClientIp()),
                    )
                    return (
                        public.returnJson(
        ...
        try:
            if not userInfo:
                ...
                return (
                    public.returnJson(
                        False,
                        public.lang(
                            'wrong user name or password，<span style="color:red;">please refresh the page and try again</span>，You can retry {} more times',
                            num,
                        ),
                    ),
                    json_header,
            ...
            if s_username != post.username or userInfo["password"] != password:
                public.write_log_gettext(
                    "Login",
                    "Password is incorrect, Username:{}, Password:{}, Login IP:{}",
                    ("****", "******", public.GetClientIp()),
                )
                num = self.limit_address("+")
                if not num:
                    return (
                        public.returnJson(
                            False,
                            public.lang(
                                "You failed to log in many times, please try again in {} seconds!",
                                int(self.limit_expire_time - time.time()),
                            ),
                        ),
                        json_header,
                ...
                return (
                    public.returnJson(
                        False,
                        public.lang(
                            "Invalid username or password. You have [{}] times left to try!",
                            str(num),
                        ),
                    ),
                    json_header,
            ...
            if hasattr(post, "vcode"):
                if not re.match(r"^\d+$", post.vcode):
                    return (
                        public.returnJson(
                            False, public.lang("Incorrect format of verification code")
                        ),
                        json_header,
                    )
                ...
                public.writeFile(
                    ...
                    json.dumps({"client_ip": public.GetClientIp(), "add_time": now}),
                )
            ...
            if not os.path.exists(_key_file) or acc_client_ip:
                public.run_thread(
                    ...
                    (
                        ...
                        userInfo["username"],
                        public.GetClientIp(),
                        str(int(request.environ.get("REMOTE_PORT"))),
                    ),
                )
                self.set_cdn_host(post)
            ...
            if stringEx.find("unsupported") != -1 or stringEx.find("-1") != -1:
                ...
                return (
                    public.returnJson(False, public.lang("USER_INODE_ERR")),
                    json_header,
                )
            public.write_log_gettext(
                "Login",
            ...
            if not num:
                return (
                    public.returnJson(
                        False,
                        public.lang(
                            "You have failed to log in many times, please wait {} seconds and try again!",
                            int(self.limit_expire_time - time.time()),
                        ),
                    ),
                    json_header,
    ...
    def request_tmp(self, get):
        try:
            if not hasattr(get, "tmp_token"):
                return (
                    public.returnJson(False, public.lang("Parameter ERROR!")),
                    json_header,
                )
            ...
            if not "tmp_token" in data or not "tmp_time" in data:
                return (
                    public.returnJson(False, public.lang("Verification failed")),
                    json_header,
                )
            if (time.time() - data["tmp_time"]) > 120:
                return (
                    public.returnJson(False, public.lang("Expired Token")),
                    json_header,
                )
            if get.tmp_token != data["tmp_token"]:
                return (
                    public.returnJson(False, public.lang("Invalid Token!")),
                    json_header,
                )
            ...
            return (
                public.returnJson(
                    False, public.lang("Login failed,") + public.get_error_info()
                ),
                json_header,
            )
    ...
    def request_temp(self, get):
        try:
            ...
            if (
                public.M("temp_login")
                .where("state=? and expire>?", (0, s_time))
                .field("id,token,salt,expire")
                .count()
                == 0
            ):
            ...
            ids = public.write_log_gettext(
                "Login",
                "Login succeed, Username: {}, Login IP: {}",
            ...
            public.M("temp_login").where("id=?", (data["id"],)).update(
                {"login_time": s_time, "state": 1, "login_addr": login_addr}
            )
            self.limit_address("-")
            cache.delete("panelNum")
            cache.delete("dologin")
    ...
    def _set_login_session(self, userInfo, acc_client_ip=None):
        try:
            ...
            cache.delete("dologin")
            session["session_timeout"] = time.time() + public.get_session_timeout()
            if "last_login_token" in session:
                del session["last_login_token"]
            self.set_request_token()
            self.login_token()
            ...
            # 返回增加登录地区
            res = (
                public.returnMsg(True, "LOGIN_SUCCESS")
                if not acc_client_ip
                else public.returnMsg(
                    True,
            ...
            session["login_time"] = res[
                "login_time"
            ]  # 记录登录时间，验证客户端时要用，不要删除
