...
class webserver:
    ...
    def __init__(self):
        ...
        self.__webserver_bin = os.path.join(
            self.__panel_path, "webserver/sbin/webserver"
        )  # webserver二进制文件
        self.__webserver_conf = os.path.join(
            self.__panel_path, "webserver/conf/webserver.conf"
        )  # webserver配置文件
        self.__webserver_pid = os.path.join(
            self.__panel_path, "webserver/logs/webserver.pid"
        )  # webserver进程PID
        self.__webserver_ctl = os.path.join(
            self.__panel_path, "script/webserver-ctl.sh"
        )  # webserver控制脚本
        self.__panel_port_file = os.path.join(
            self.__panel_path, "data/port.pl"
        )  # 面板端口文件
        self.__ssl_key_file = os.path.join(
            self.__panel_path, "ssl/privateKey.pem"
        )  # SSL私钥文件
        self.__ssl_crt_file = os.path.join(
            self.__panel_path, "ssl/certificate.pem"
        )  # SSL证书文件
        self.__is_ssl_file = os.path.join(
            self.__panel_path, "data/ssl.pl"
        )  # 是否开启SSL文件
        self.__examples_path = os.path.join(
            self.__panel_path, "webserver/tpls"
        )  # 配置文件模板目录
        self.__webserver_conf_example = os.path.join(
            self.__examples_path, "webserver.conf"
        )  # webserver配置文件模板
        self.__webserver_ssl_conf_example = os.path.join(
            self.__examples_path, "webserver_ssl.conf"
        )  # webserver SSL配置文件模板
        self.__webserver_listen_conf_example = os.path.join(
            self.__examples_path, "webserver_listen.conf"
        )  # webserver监听配置文件模板
        self.__webserver_listen_ssl_conf_example = os.path.join(
            self.__examples_path, "webserver_listen_ssl.conf"
        )  # webserver SSL监听配置文件模板
    ...
    def is_ssl(self):
        ...
        if os.path.exists(self.__is_ssl_file):
            ...
            if os.path.exists(self.__ssl_key_file) and os.path.exists(
                self.__ssl_crt_file
            ):
                return True
        return False
    ...
    def get_http3_header(self):
        ...
        if self.is_ssl():
            ...
            http3_header = 'add_header Alt-Svc \'h3=":{PORT}"; ma=86400,h3-29=":{PORT}"; ma=86400\';'.format(
...
"""
    ...
    def create_conf(self):
        ...
        if not self.conf_example_exists():
            self.print_log(
                "Web server configuration file template not found.", self.__log_error
            )
            return False
        ...
        if not config_example:
            self.print_log(
                "Web server configuration file template read failed.", self.__log_error
            )
            return False
        ...
        config = config_example.format(
            LISTEN=listen_conf, SSL_CONFIG=ssl_conf, HTTP3_HEADER=http3_header
        )
        res = public.WriteFile(self.__webserver_conf, config)
        if not res:
            self.print_log(
                "Write Web server configuration file failed", self.__log_error
            )
            return False
        ...
        if not self.configtest():
            self.print_log(
                "Web server configuration file test failed.", self.__log_error
            )
            return False
    ...
    def chmod_static_files(self):
        ...
        # 设置静态目录链权限
        res = self.exec_shell(
            "chmod 755 /www /www/server /www/server/panel /www/server/panel/BTPanel"
        )
        if res[1] != "":
    ...
    def run_webserver(self):
        ...
        try:
            ...
            if not self.ctl_exists():
                self.print_log(
                    "Web server command line tool not found.", self.__log_error
                )
                return False
            ...
            if not self.chmod_static_files():
                self.print_log(
                    "Web server static file permission setting failed.",
                    self.__log_error,
                )
                return False
            ...
            if not self.create_conf():
                self.print_log(
                    "Web server configuration file creation failed.", self.__log_error
                )
                return False
