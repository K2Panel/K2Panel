...
class panelRun:
    ...
    def create_run(self, get):
        ...
        if not os.path.exists(run_path):
            return public.return_msg_gettext(
                False,
                public.lang("The specified run directory {} does not exist!", run_path),
            )
        ...
        if not re.match(r"^\w+$", run_name):
            return public.return_msg_gettext(
                False,
                public.lang(
                    "The startup item name format is incorrect, support: [a-zA-Z0-9_]!"
                ),
            )
        ...
        if os.path.exists(run_file):
            return public.return_msg_gettext(
                False, public.lang("Launch configuration already exists!")
            )
        ...
        public.write_log_gettext(
            self.__log_name, "Create startup item [] successful!".format(run_title)
        )
    ...
    def modify_run(self, get):
        ...
        if not os.path.exists(run_path):
            return public.return_msg_gettext(
                False, "The specified run directory {} does not exist!", (run_path,)
            )
        ...
        if not os.path.exists(run_file):
            return public.return_msg_gettext(
                False, public.lang("The launch configuration does not exist!")
            )
        ...
        public.writeFile(run_file, run_info)
        public.write_log_gettext(
            self.__log_name, "Modify startup item [{}] successful!", (run_title,)
        )
    ...
    def remove_run(self, get):
        ...
        os.remove(run_file)
        public.write_log_gettext(
            self.__log_name, "Delete startup item [{}] successful!", (run_name,)
        )
    ...
    def set_run_status(self, get):
        ...
        if not os.path.isfile(run_file):
            return public.return_msg_gettext(
                False, public.lang("launch configuration does not exist!")
            )
        ...
        public.write_log_gettext(
            self.__log_name,
            "Setting startup item [{}] status succeeded!",
            (run_info["title"],),
        )
    ...
    def stop_run(self, run_name=None):
        ...
        public.write_log_gettext(
            self.__log_name, "Close startup item [{}] successful!", (run_name,)
        )
        return True
    ...
    def get_run_status(self, run_name):
        ...
        pid = self.get_run_pid(run_name)
        if not pid:
            return public.return_msg_gettext(False, public.lang("Not run"))
        process_info = self.get_process_info(pid)
        if not process_info:
            return public.return_msg_gettext(
                False, public.lang("Unable to get process information")
            )
        return process_info
