...
class datatools:
            ...
            for i in tables:
                if i == 1049:
                    return public.return_msg_gettext(
                        False, public.lang("Database does NOT exist!")
                    )
                if type(i) == int:
                    continue
                ...
                try:
                    ...
                    ret2["type"] = table[0][1]
                    data_size = table[0][6]
                    ret2["rows_count"] = self.DB_MySQL.query(
                        "select count(*) from `{}`.`{}`".format(db_name, i[0])
                    )[0][
                        0
                    ]  # table[0][4]  实时获取行数 @authow hwliang<2021-08-05> 修改
    ...
    def RepairTable(self, get):
        ...
        if m_version.find("5.1.") != -1:
            return public.return_msg_gettext(False, public.lang("Nonsupport mysql5.1!"))
        mysql_table = self.map_to_list(
            self.DB_MySQL.query("show tables from `%s`" % db_name)
        )
        ret = []
                ...
                if len(ret) > 0:
                    for i in ret:
                        self.DB_MySQL.execute(
                            "OPTIMIZE table `%s`.`%s` ENGINE=MyISAM" % (db_name, i)
                        )
                    return True
        return False
