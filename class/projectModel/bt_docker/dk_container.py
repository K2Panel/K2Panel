...
class main:
    ...
    def alter_table(self):
        if (
            not dp.sql("sqlite_master")
            .where("type=? AND name=? AND sql LIKE ?", ("table", "container", "%sid%"))
            .count()
        ):
            dp.sql("container").execute(
                "alter TABLE container add container_name VARCHAR DEFAULT ''", ()
            )
    ...
    def run(self, args):
            ...
            for i in args.ports:
                if dp.check_socket(args.ports[i]):
                    return public.returnMsg(
                        False,
                        "The server port [{}] has been used, please replace it with another port!".format(
                            args.ports[i]
                        ),
                    )
        if not args.image:
            return public.returnMsg(
                False,
                "If there is no image selected, please go to the image tab to pull the image you need!",
            )
        ...
        try:
            ...
            if int(args.cpu_quota) / 100000 > dp.get_cpu_count():
                return public.returnMsg(
                    False, "The CPU quota has exceeded the number of cores available!"
                )
            mem_limit_byte = dp.byte_conversion(args.mem_limit)
            if mem_limit_byte > dp.get_mem_info():
                return public.returnMsg(
                    False, "The memory quota has exceeded the available number!"
                )
            ...
            if res:
                pdata = {"cpu_limit": str(args.cpu_quota), "container_name": args.name}
            ...
            if "Invalid container name" in str(e):
                return public.returnMsg(
                    False,
                    "The container name is illegal, please do not use Chinese container name!",
                )
            if "bind: address already in use" in str(e):
                ...
                return public.returnMsg(
                    False,
                    "Server port {} is in use! Please change other ports".format(port),
                )
            return public.returnMsg(
                False, "Create failed! {}".format(public.get_error_info())
            )
    ...
    def commit(self, args):
        ...
        dp.write_log(
            "Submitting container [{}] as image [{}] succeeded!".format(
                container.attrs["Name"], args.tag
            )
        )
    ...
    def export(self, args):
        ...
        try:
            ...
            f.close()
            return public.returnMsg(
                True, "Successfully exported to: {}".format(file_name)
            )
        except:
            return public.returnMsg(
                False, "Operation failed:" + str(public.get_error_info())
            )
    ...
    def stop(self, args):
        ...
        try:
            ...
            if "No such container" in str(e):
                return public.returnMsg(
                    True,
                    "The container has been stopped and deleted because the container has the option to automatically delete after stopping!",
                )
    ...
    def pause(self, args):
        ...
        try:
            ...
            if "is not running" in str(e):
                return public.returnMsg(
                    False, "The container is not started and cannot be paused!"
                )
    ...
    def get_list(self, args):
        ...
        for attr in attr_list:
            cpu_usage = (
                dp.sql("cpu_stats").where("container_id=?", (attr["Id"],)).select()
            )
