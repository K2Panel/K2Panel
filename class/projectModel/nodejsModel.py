...
class main(projectBase):
    ...
    def get_run_list(self, get):
        ...
        if not os.path.exists(project_cwd):
            return public.return_error("The project directory does not exist!")
        package_file = "{}/package.json".format(project_cwd)
        if not os.path.exists(package_file):
            return (
                {}
            )  # public.return_error('没有在项目目录中找到package.json配置文件!')
        package_info = json.loads(public.readFile(package_file))
        if not "scripts" in package_info:
            return (
                {}
            )  # public.return_error('没有在项目配置文件package.json中找到scripts配置项!')
        if not package_info["scripts"]:
            return {}  # public.return_error('没有找到可用的启动选项!')
...
""".format(
    ...
    def install_packages(self, get):
        ...
        if not os.path.exists(package_file):
            return public.return_error(
                "The package.json configuration file was not found in the project directory!"
            )
        ...
        public.writeFile(
            self._npm_exec_log, "Downloading dependency package, please wait...\n"
        )
        if yarn_bin:
            ...
            public.ExecShell(
                ...
                + "cd {} && {} install 2>&1 >> {}".format(
                    project_find["path"], yarn_bin, self._npm_exec_log
                )
            )
        else:
            public.ExecShell(
                self.get_last_env(nodejs_version)
                + "cd {} && {} install 2>&1 >> {}".format(
                    project_find["path"], npm_bin, self._npm_exec_log
                )
            )
        public.writeFile(
            self._npm_exec_log, "|-Successify --- Command executed! ---", "a+"
        )
        public.WriteLog(
            self._log_name,
        ...
        return public.return_data(
            True, "The dependency package is installed successfully!"
        )
    ...
    def update_packages(self, get):
        ...
        if not os.path.exists(package_lock_file):
            return public.return_error("Please install the dependency package first!")
        nodejs_version = project_find["project_config"]["nodejs_version"]
        npm_bin = self.get_npm_bin(nodejs_version)
        if not npm_bin:
            return public.return_error("The specified nodejs version does not exist!")
        ...
        public.WriteLog(
            self._log_name,
            "Project [{}] update all dependent packages".format(get.project_name),
        )
    ...
    def reinstall_packages(self, get):
        ...
        public.WriteLog(
            self._log_name,
            "Node project: {}, all dependent packages have been reinstalled",
        )
        public.ExecShell(
            self.get_last_env(nodejs_version)
            + "cd {} && {} install &> {}".format(
        ...
        for mod_name in os.listdir(mod_path):
            try:
                ...
                pack_info = {
                    ...
                    "license": (
                        mod_pack_info["license"]
                        if "license" in mod_pack_info
                        else "NULL"
                    ),
    ...
    def install_module(self, get):
        ...
        if os.path.exists(filename):
            return public.return_error("The specified module has been installed!")
        ...
        if yarn_bin:
            public.ExecShell(
                self.get_last_env(nodejs_version)
                + "cd {} && {} add {} &> {}".format(
                    project_find["path"], yarn_bin, mod_name, self._npm_exec_log
                )
            )
        else:
            public.ExecShell(
                self.get_last_env(nodejs_version)
                + "cd {} && {} install {} &> {}".format(
                    project_find["path"], npm_bin, mod_name, self._npm_exec_log
                )
            )
        if not os.path.exists(filename):
            return public.return_error("Failed to install the specified module!")
        public.WriteLog(
            self._log_name,
            "Node project {}, {} module installation is complete!".format(
                get.project_name, mod_name
            ),
        )
    ...
    def uninstall_module(self, get):
        ...
        filename = "{}/node_modules/{}/package.json".format(project_cwd, mod_name)
        if not os.path.exists(filename):
            return public.return_error("The specified module is not installed!")
        ...
        if yarn_bin:
            result = public.ExecShell(
                self.get_last_env(nodejs_version)
                + "cd {} && {} remove {}".format(
                    project_find["path"], yarn_bin, mod_name
                )
            )
        else:
            result = public.ExecShell(
                self.get_last_env(nodejs_version)
                + "cd {} && {} uninstall {}".format(
                    project_find["path"], npm_bin, mod_name
                )
            )
        if os.path.exists(filename):
            result = "\n".join(result)
            if result.find("looking for funding") != -1:
                return public.return_error(
                    "This module is dependent on other installed modules and cannot be uninstalled!"
                )
            return public.return_error("Unable to uninstall this module!")

        public.WriteLog(
            self._log_name,
            "Node project {}, {} module uninstallation completed!".format(
                get.project_name, mod_name
            ),
        )
    ...
    def upgrade_module(self, get):
        ...
        public.WriteLog(
            self._log_name,
            "Node project {}, {} module update completed!".format(
                get.project_name, mod_name
            ),
        )
    ...
    def create_project(self, get):
        ...
        if not isinstance(get, public.dict_obj):
            return public.return_error(
                "The parameter type is wrong, need dict obj object"
            )
        ...
        if not os.path.exists(get.project_cwd):
            return public.return_error(
                "The project directory does not exist: {}".format(get.project_cwd)
            )

        # 端口占用检测
        if self.check_port_is_used(get.get("port/port")):
            return public.return_error(
                "This port is already occupied, please modify your project port, port: {}".format(
                    get.port
                )
            )
        ...
        if get.bind_extranet == 1:
            ...
            if not public.is_apache_nginx():
                return public.return_error("Please install Nginx or Apache first")
        for domain in domains:
            domain_arr = domain.split(":")
            if public.M("domain").where("name=?", domain_arr[0]).count():
                return public.return_error(
                    "Domain name already exists: {}".format(domain)
                )
        pdata = {
        ...
        public.WriteLog(
            self._log_name, "Add Node.js project {}".format(get.project_name)
        )
    ...
    def modify_project(self, get):
        ...
        """
        if not isinstance(get, public.dict_obj):
            return public.return_error("The parameter type is wrong, need dict obj")
        if not self.is_install_nodejs(get):
            return public.return_error(
                "Please install nodejs version manager before installing at least one nodejs"
            )
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.return_error(
                "Item does not exist: {}".format(get.project_name)
            )
        ...
        if hasattr(get, "port"):
            if int(project_find["project_config"]["port"]) != int(get.port):
                if self.check_port_is_used(get.get("port/port"), True):
                    return public.return_error(
                        "The port is already occupied, please modify your port, port: {}".format(
                            get.port
                        )
                    )
        ...
        self.set_config(get.project_name)
        public.WriteLog(
            self._log_name, "Modify Node.js project {}".format(get.project_name)
        )
        if rebuild:
            self.rebuild_project(get.project_name)
    ...
    def remove_project(self, get):
        ...
        if not project_find:
            return public.return_error(
                "The specified item does not exist: {}".format(get.project_name)
            )
        ...
        if os.path.exists(log_file):
            os.remove(log_file)
        public.WriteLog(
            self._log_name, "Delete Node.js project {}".format(get.project_name)
        )
    ...
    def project_get_domain(self, get):
        ...
        """
        project_id = (
            public.M("sites").where("name=?", (get.project_name,)).getField("id")
        )
        domains = (
            public.M("domain").where("pid=?", (project_id,)).order("id desc").select()
        )
        project_find = self.get_project_find(get.project_name)
    ...
    def project_add_domain(self, get):
        ...
        for domain in domains:
            domain = domain.strip()
            if not domain:
                return public.return_error("Domain name cannot be empty", data="")
            domain_arr = domain.split(":")
            if len(domain_arr) == 1:
            ...
            if not public.M("domain").where("name=?", (domain_arr[0],)).count():
                ...
                public.WriteLog(
                    self._log_name,
                    "Successfully added the domain [{}] to the project [{}]".format(
                        domain, get.project_name
                    ),
                )
                success_list.append(domain)
            else:
                public.WriteLog(
                    self._log_name, "Domain [{}] already exists".format(domain)
                )
        ...
        if success_list:
            ...
            return public.return_data(
                True,
                "[{}] domain names added successfully, [{}] failed!".format(
    ...
    def project_remove_domain(self, get):
        ...
        if project_find["project_config"]["bind_extranet"]:
            if len(project_find["project_config"]["domains"]) == 1:
                return public.return_error(
                    "At least one domain name is required for the mapped project"
                )
        domain_id = (
            public.M("domain")
            .where("name=? AND pid=?", (domain_arr[0], project_id))
            .getField("id")
        )
        if not domain_id:
        ...
        public.WriteLog(
            self._log_name,
            "From project: [{}], delete domain name [{}]".format(
    ...
    def bind_extranet(self, get):
        ...
        project_find = self.get_project_find(project_name)
        if not project_find:
            return public.return_error("Item does not exist")
        if not project_find["project_config"]["domains"]:
            return public.return_error(
                "Please add at least one domain name in the [Domain Management] option"
            )
        ...
        public.WriteLog(
            self._log_name, "Node project{}, enable mapping".format(project_name)
        )
    ...
    def clear_nginx_config(self, project_find):
        ...
        if os.path.exists(config_file):
            os.remove(config_file)
        rewrite_file = "{panel_path}/vhost/rewrite/node_{project_name}.conf".format(
    ...
    def exists_nginx_ssl(self, project_name):
        ...
        @return tuple
        """
        config_file = "{}/nginx/node_{}.conf".format(
            public.get_vhost_path(), project_name
        )
        if not os.path.exists(config_file):
            return False, False
    ...
    def exists_apache_ssl(self, project_name):
        ...
        @return bool
        """
        config_file = "{}/apache/node_{}.conf".format(
            public.get_vhost_path(), project_name
        )
        if not os.path.exists(config_file):
            return False, False
    ...
    def unbind_extranet(self, get):
        ...
        public.WriteLog(
            self._log_name, "Node project {}, disable the mapping".format(project_name)
        )
        ...
        for i in self._pids:
            try:
                ...
                if p.cwd() == project_find["path"]:
                    pname = p.name()
                    if (
                        pname in ["node", "npm", "pm2", "yarn"]
                        or pname.find("node ") == 0
                    ):
    ...
    def kill_pids(self, get=None, pids=None):
        ...
        if get:
            pids = get.pids
        if not pids:
            return public.return_data(True, "No process")
    ...
    def start_project(self, get):
        ...
        npm_bin = self.get_npm_bin(nodejs_version)
        project_script = (
            project_find["project_config"]["project_script"].strip().replace("  ", " ")
        )
        if project_script[:3] == "pm2":  # PM2启动方式处理
            project_script = project_script.replace(
                "pm2 ",
                "pm2 -u {} -n {} ".format(
        ...
        if not project_script:
            return public.return_error("No startup script configured")
...
""".format(
        ...
        # 处理前置权限
        public.ExecShell(
            "chown -R {user}:{user} {project_cwd}".format(
        ...
        public.ExecShell(
            "chmod 755 {} {} {}".format(
                self._nodejs_path, public.get_setup_path(), "/www"
            )
        )
        public.set_own(
            script_file,
        ...
        try:
            ...
            return public.return_error(
                "Startup failed <br>{}".format(public.GetNumLines(log_file, 20))
            )
        ...
        if not pids:
            if os.path.exists(pid_file):
                os.remove(pid_file)
            return public.return_error(
                "failed to activate<br>{}".format(public.GetNumLines(log_file, 20))
            )
    ...
    def stop_project(self, get):
        ...
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.return_error("Project does not exist")
        project_script = (
            project_find["project_config"]["project_script"].strip().replace("  ", " ")
        )
        pid_file = "{}/{}.pid".format(self._node_pid_path, get.project_name)
...
cd {}
{}""".format(
                    last_env, project_find["path"], project_script
                )
            )
        else:
            pid_file = "{}/{}.pid".format(self._node_pid_path, get.project_name)
    ...
    def get_project_log(self, get):
        ...
        if not os.path.exists(log_file):
            return public.return_error("Log file does not exist")
    ...
    def get_process_info_by_pid(self, pid):
        ...
        try:
            ...
            with p.oneshot():
                ...
                process_info["io_write_bytes"], process_info["io_read_bytes"] = (
                    self.get_io_speed(p)
                )
    ...
    def get_cpu_precent(self, p):
        ...
        percent = round(
            100.00
            * (process_cpu_time - old_process_cpu_time)
            / (new_time - old_time)
            / psutil.cpu_count(),
            2,
        )
    ...
    def get_project_find(self, project_name):
        ...
        project_info = (
            public.M("sites")
            .where("project_type=? AND name=?", ("Node", project_name))
            .find()
        )
        if not project_info:
            return False
    ...
    def get_project_info(self, get):
        ...
        project_info = (
            public.M("sites")
            .where("project_type=? AND name=?", ("Node", get.project_name))
            .find()
        )
        if not project_info:
            return public.return_error("The specified item does not exist!")
    ...
    def get_project_stat(self, project_info):
        ...
        """
        project_info["project_config"] = json.loads(project_info["project_config"])
        project_info["run"] = self.get_project_run_state(
        ...
        if project_info["run"]:
            ...
            project_info["load_info"] = self.get_project_load_info(
                project_name=project_info["name"]
            )
        ...
        if project_info["load_info"]:
            ...
            if project_info["listen"]:
                project_info["listen_ok"] = (
                    project_info["project_config"]["port"] in project_info["listen"]
                )
        return project_info
    ...
    def get_project_listen(self, project_name):
        ...
        project_config = json.loads(
            public.M("sites").where("name=?", project_name).getField("project_config")
        )
    ...
    def set_project_listen(self, get):
        ...
        project_config = json.loads(
            public.M("sites")
            .where("name=?", get.project_name)
            .getField("project_config")
        )
        project_config["listen_port"] = get.port
        public.M("sites").where("name=?", get.project_name).save(
            "project_config", json.dumps(project_config)
        )
        public.WriteLog(
            self._log_name,
            "Modify the port of the project ["
            + get.project_name
            + "] to ["
            + get.port
            + "]",
    ...
    def set_project_nodejs_version(self, get):
        ...
        public.WriteLog(
            self._log_name,
            "Modify the nodejs version of the project ["
            + get.project_name
            + "] to ["
            + get.nodejs_version
            + "]",
    ...
    def check_port_is_used(self, port, sock=False):
        ...
        project_list = (
            public.M("sites")
            .where("status=? AND project_type=?", (1, "Node"))
            .field("name,path,project_config")
            .select()
        )
            ...
            try:
                ...
                if not project_state:
                    ...
                    if not result["status"]:
                        error_count += 1
                        error_msg = (
                            "Automatically start Nodej project ["
                            + project_name
                            + "] failed!"
                        )
                        public.WriteLog(self._log_name, error_msg)
                        public.print_log(
                            error_msg + ", " + result["error_msg"], "ERROR"
                        )
                    else:
                        success_count += 1
                        success_msg = (
                            "Automatically start the Nodej project ["
                            + project_name
                            + "] successfully!"
                        )
