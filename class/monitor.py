...
class Monitor:
    ...
    def _statuscode_distribute_site(self, site_name):
        ...
        try:
            ...
            select_sql = "select time/100 as time1, sum(status_401), sum(status_500), sum(status_502), sum(status_503) from request_stat where time between {} and {}".format(
                start_date, end_date
            )

            db_path = os.path.join(
                "/www/server/total/", "logs/{}/logs.db".format(site_name)
            )
    ...
    def _statuscode_distribute(self, args):
        ...
        for site in sites:
            site_name = site["name"]
            day_401, day_500, day_502, day_503 = self._statuscode_distribute_site(
                site_name
            )
    ...
    def get_exception(self, args):
        data = {
            "mysql_slow": self._get_slow_log_nums(args),
            "php_slow": self._php_count(args),
            "attack_num": self._get_attack_num(args),
            "cc_attack_num": self._get_cc_attack_num(args),
        }
    ...
    def get_spider(self, args):
        ...
        for site_info in sites:
            ...
            try:
                ...
                start_date, end_date = self.get_time_interval(time.localtime())
                select_sql = "select time, spider from request_stat where time between {} and {}".format(
                    start_date, end_date
                )
    ...
    def get_spider_old(self, args):
        ...
        for site in sites:
            site_name = site["name"]
            file_name = (
                "/www/server/total/total/" + site_name + "/spider/" + today + ".json"
            )
    ...
    def get_request_count_by_hour(self, args):
        ...
        for site_info in sites:
            ...
            try:
                site_name = site_info["name"]
                start_date, end_date = self.get_time_interval(time.localtime())
                select_sql = "select time, req from request_stat where time between {} and {}".format(
                    start_date, end_date
                )
                db_path = os.path.join(
                    "/www/server/total/", "logs/{}/logs.db".format(site_name)
