...
class Firewalld(Base):
    ...
    def input_port(self, info: dict, operation: str) -> dict:
        ...
        if info["Protocol"].find("/") != -1:
            stdout, stderr = public.ExecShell(
                "{cmd_str} --zone=public --{operation}-port={port}/{prot} --permanent".format(
        ...
        if "/" in info["Protocol"]:
            ...
            for pp in info["Protocol"]:
                if not pp in ["tcp", "udp"]:
                    return self._result(
                        False, "设置出站端口失败, err: 协议不支持 {}".format(pp)
                    )

                stdout, stderr = public.ExecShell(
                    "{cmd_str} --permanent --direct --{operation}-rule ipv4 filter OUTPUT {priority} -p {prot} --dport {port} -j {strategy}".format(
                        cmd_str=self.cmd_str,
                        operation=operation,
                ...
                if stderr:
                    return self._result(
                        False, "设置出站端口失败, err: {}".format(stderr)
                    )
        else:
            stdout, stderr = public.ExecShell(
                "{cmd_str} --permanent --direct --{operation}-rule ipv4 filter OUTPUT {priority} -p {prot} --dport {port} -j {strategy}".format(
    ...
    def set_rich_rule(self, info: dict, operation: str) -> dict:
        ...
        stdout, stderr = public.ExecShell(
            "{} --zone=public --{}-rich-rule='{}' --permanent".format(
                self.cmd_str, operation, rule_str
            )
        )

        if stderr:
            return self._result(
                False, "设置规则：{} 失败, err: {}".format(operation, rule_str, stderr)
            )
    ...
    def output_rich_rules(self, info: dict, operation: str) -> dict:
        ...
        rich_rules = (
            self.cmd_str
            + " --permanent --direct --{0}-rule ipv4 filter OUTPUT".format(operation)
        )
        if "Priority" in info:
        ...
        try:
            ...
            for elem in root:
                ...
                # 2024/3/22 下午 3:01 复杂的规则配置
                    ...
                    rule = {
                        "Family": (
                            elem.attrib["family"] if "family" in elem.attrib else "ipv4"
                        )
                    }
                    for subelem in elem:
                        ...
                        ports.append(
                            {
                                ...
                                "Port": rule["port"]["port"],
                                "Strategy": (
                                    rule["Strategy"] if "Strategy" in rule else "accept"
                                ),
                                "Family": (
                                    rule["Family"] if "Family" in rule else "ipv4"
                                ),
                                "Address": (
                                    rule["Address"] if "Address" in rule else "all"
                                ),
                                "Chain": "INPUT",
                            }
                        )
                ...
                # 2024/3/25 下午 2:57 端口转发规则
                    ...
                    port = {
                        ...
                        "Protocol": (
                            ...
                            if "protocol" in elem.attrib
                            else "tcp"
                        ),
                        "S_Address": "",
                        "S_Port": elem.attrib["port"] if "port" in elem.attrib else "",
                        "T_Address": (
                            elem.attrib["to-addr"] if "to-addr" in elem.attrib else ""
                        ),
                        "T_Port": (
                            elem.attrib["to-port"] if "to-port" in elem.attrib else ""
                        ),
                    }
    ...
    def port_forward(self, info: dict, operation: str) -> dict:
        ...
        if "S_Address" in info and info["S_Address"] != "":
            ...
            if info["Protocol"].find("/") != -1:
                ...
                if "success" not in stdout and stderr:
                    ...
                    return self._result(
                        False, "设置端口转发失败, err: {}".format(stderr)
                    )
        ...
        # 2024/3/25 下午 6:08 处理没有源地址的情况
            ...
            if info["Protocol"].find("/") != -1:
                stdout, stderr = public.ExecShell(
                    "{} --zone=public --{}-forward-port='port={}:proto={}:toport={}:toaddr={}' --permanent".format(
                        self.cmd_str,
                        operation,
                        info["S_Port"],
                        "udp",
...
if __name__ == "__main__":
    ...
    if args[1] == "status":
        ...
        if len(args) < 6:
            print(
                "Usage: output_rich_rules Family Address Port Protocol Strategy Priority"
            )
            sys.exit(1)
        print(
            firewall.output_rich_rules(
                {
                    "Family": args[2],
                    "Address": args[3],
                    "Port": args[4],
