...
class Iptables(Base):
    def __init__(self):
        ...
        self.protocol = {"6": "tcp", "17": "udp", "0": "all"}
    ...
    def version(self):
        ...
        """
        try:
            result = public.ExecShell("iptables -v 2>&1|awk '{print $2}'|head -1")[
                0
            ].replace("\n", "")
            if result == "":
    ...
    def parse_rules(self, stdout):
        ...
        for line in lines:
            if line.startswith("Chain"):
                ...
                or line.strip() == ""
                or "source" in line
                or "Warning: iptables-legacy tables present" in line
            ):
                # 过滤表头,空行,警告
                continue
            else:
        ...
        try:
            ...
            for line in lines:
                ...
                if not rule_info[4] in self.protocol:
                    continue
                if (
                    not "dpt" in rule_info[-1]
                    and not "-" in rule_info[-1]
                    and not ":" in rule_info[-1]
                ):
                    continue
    ...
    def set_chain_port(self, info, operation, chain):
        ...
        try:
            if not chain in ["INPUT", "OUTPUT"]:
                return self._result(
                    False, "设置端口规则失败:{}".format("不支持的链类型")
                )

            if info["Protocol"] not in ["tcp", "udp"]:
                return self._result(
                    False, "设置端口规则失败:{}".format("不支持的协议类型")
                )
            if info["Strategy"] == "accept":
                ...
                return self._result(
                    False, "设置端口规则失败:{}".format("不支持的策略类型")
                )
    ...
    def set_chain_rich_port(self, info, operation, chain):
        ...
        try:
            ...
            if "Address" in info and public.is_ipv6(info["Address"]):
                return self._result(
                    False, "设置端口规则失败:{}".format("不支持的IPV6地址")
                )
        ...
        try:
            ...
            if ":" in info["Address"] or "-" in info["Address"]:
                ...
                rule = "{} -t filter {} {} -m iprange --src-range {} -j {}".format(
                    self.cmd_str, operation, chain, info["Address"], info["Strategy"]
                )
            else:
    ...
    def port_forward(self, info, operation):
        ...
        try:
            ...
            parm = {"table": "nat", "chain_name": "PREROUTING", "rule": rule}
    ...
    def get_nat_prerouting_rules(self):
        ...
        """
        try:
            port_forward_rules = self.list_rules(
                {"table": "nat", "chain_name": "PREROUTING"}
            )
            rules = []
...
if __name__ == "__main__":
    ...
    if len(args) < 2:
        ...
        print(
            "port_forward: port_forward <S_Address> <S_Port> <T_Address> <T_Port> <Protocol> <Operation>"
        )
    ...
    if args[1] == "list_rules":
        ...
        if len(args) < 8:
            print(
                "传参使用方法: port_forward <S_Address> <S_Port> <T_Address> <T_Port> <Protocol> <Operation>"
            )
            sys.exit(1)
        ...
        info = {"Protocol": args[2], "Port": args[3], "Strategy": args[4]}
