...
# k2panel
# -------------------------------------------------------------------
# Copyright (c) 2014-2099 k2panel(http://binarjoinanalyticnl.nl) All rights reserved.
# -------------------------------------------------------------------
...
class main(Base):
    ...
    def check_port_exist(self, get):
        ...
        for item in port_rules_list:
            if (
                item["Port"] == get.port
                and item["Address"] == get.address
                and item["Protocol"] == get.protocol
                and item["Strategy"] == get.strategy
                and item["Chain"] == get.chain
            ):
                return False
    ...
    def check_forward_exist(self, get):
        ...
        for item in forward_rules_list:
            if (
                item["S_Address"] == get.S_Address
                and item["S_Port"] == get.S_Port
                and item["T_Address"] == get.T_Address
                and item["T_Port"] == get.T_Port
            ):
                return False
        ...
        try:
            ...
            for i in range(len(data)):
                ...
                for j in range(len(domain_data)):
                    if (
                        "domain" in domain_data[j]
                        and data[i]["address"] in domain_data[j]["domain"]
                    ):
    ...
    def structure_port_return_data(self, list_port, rule_db, query):
        ...
        for j in range(len(list_port)):
            ...
            list_port[j]["domain"] = ""
            if (
                list_port[j]["Port"].find(":") != -1
                or list_port[j]["Port"].find("-") != -1
                or list_port[j]["Port"].find("/") != -1
                or list_port[j]["Port"].find(".") != -1
            ):
                ...
                try:
                    if (
                        not ":" in list_port[j]["Port"]
                        or not "-" in list_port[j]["Port"]
                    ):
                        list_port[j]["status"] = self.CheckPort(
                            int(list_port[j]["Port"]), list_port[j]["Protocol"]
                        )
                    else:
                        list_port[j]["status"] = -1
                except:
            ...
            for i in range(len(rule_db)):
                if (
                    rule_db[i]["ports"] == list_port[j]["Port"]
                    and rule_db[i]["protocol"] == list_port[j]["Protocol"]
                    and rule_db[i]["address"].lower() == list_port[j]["Address"].lower()
                    and rule_db[i]["types"] == list_port[j]["Strategy"]
                    and rule_db[i]["chain"] == list_port[j]["Chain"]
                ):
            ...
            if query != "":
                if (
                    query in list_port[j]["Port"]
                    or query in list_port[j]["brief"]
                    or query in list_port[j]["Address"]
                ):
    ...
    def structure_ip_return_data(self, list_ip, rule_db, query):
        ...
        for j in range(len(list_ip)):
            ...
            for i in range(len(rule_db)):
                if (
                    rule_db[i]["address"] == list_ip[j]["Address"]
                    and rule_db[i]["types"] == list_ip[j]["Strategy"]
                    and rule_db[i]["chain"] == list_ip[j]["Chain"]
                ):
        ...
        if len(new_list) > 0 or query != "":
            return public.return_area(
                sorted(new_list, key=lambda x: x["addtime"], reverse=True), "Address"
            )

        return public.return_area(
            sorted(list_ip, key=lambda x: x["addtime"], reverse=True), "Address"
        )
    ...
    def structure_forward_return_data(self, list_forward, rule_db, query):
        ...
        for j in range(len(list_forward)):
            ...
            for i in range(len(rule_db)):
                if (
                    rule_db[i]["T_Address"] == list_forward[j]["T_Address"]
                    and rule_db[i]["S_Port"] == list_forward[j]["S_Port"]
                    and rule_db[i]["T_Port"] == list_forward[j]["T_Port"]
                ):
            ...
            if query != "":
                if (
                    query in list_forward[j]["brief"]
                    or query in list_forward[j]["S_Address"]
                    or query in list_forward[j]["S_Port"]
                    or query in list_forward[j]["T_Address"]
                    or query in list_forward[j]["T_Port"]
                ):
    ...
    def import_rules(self, get):
        ...
        if not self.get_firewall_status():
            return public.returnMsg(
                False, public.lang("Please start the firewall before importing rules!")
            )
    ...
    def set_nline_port_ip(self, get):
        ...
        for addr in address:
            ...
            if not result["status"]:
                failed_list.append({"address": addr, "msg": result["msg"]})
        if len(failed_list) > 0:
            return public.returnMsg(
                True,
                public.lang(
                    "The setup succeeded , the following rule failed :{}", failed_list
                ),
            )
    ...
    def exec_port_rule(self, get):
        ...
        if get.operation == "add" and not self.check_port_exist(get):
            return public.returnMsg(
                False, public.lang("端口{}已存在，请勿重复添加", get.port)
            )
        ...
        if get.address != "all" and "\n" in get.address:
            ...
            get.address != "all"
            and not public.checkIp(get.address)
            and not public.is_ipv6(get.address)
        ):
    ...
    def check_ips(self, get):
        ...
        if get.address != "all" and "\n" in get.address:
            ...
            for addr in address:
                if (
                    addr != "all"
                    and not public.checkIp(addr)
                    and not public.is_ipv6(addr)
                ):
                    return public.returnMsg(False, public.lang("指定IP地址格式错误"))
        elif get.address != "all" and "," in get.address:
            address = get.address.split(",")
    ...
    def modify_port_rule(self, get):
        ...
        if "address" in get.new_data:
            ...
            if not self.check_ips(get)["status"]:
                return public.returnMsg(
                    False, public.lang("修改后的指定IP地址格式错误")
                )
    ...
    def modify_domain_port_rule(self, get):
        ...
        if address == "":
            return public.returnMsg(
                False, public.lang("域名: 【{}】解析失败", get.domain)
            )
    ...
    def remove_port_db(self, get, protocol, addtime, domain):
        ...
        @return dict{"status":True/False,"msg":"提示信息"}
        """
        public.M("firewall_new").where(
            "ports=? and protocol=? and address=? and types=? and chain=?",
    ...
    def set_port_db(self, get):
        ...
        if get.operation == "add":
            ...
            if not query_result:
                get.domain = get.get("domain/s", "")
                domain = (
                    "{}|{}".format(get.domain, get.address) if get.domain != "" else ""
                )
            ...
            if query_result:
                if get.protocol == "tcp/udp" and self._isFirewalld:
                    self.remove_port_db(
                        get, "tcp", query_result["addtime"], query_result["domain"]
                    )
                    self.remove_port_db(
                        get, "udp", query_result["addtime"], query_result["domain"]
                    )
                else:
                    self.remove_port_db(
                        get,
                        get.protocol,
                        query_result["addtime"],
                        query_result["domain"],
                    )
    ...
    def set_ip_db(self, get):
        ...
        if get.operation == "add":
            ...
            query_result = (
                ...
                .where(
                    "address=? and types=? and domain=? and chain=?",
                    (get.address, get.strategy, domain, get.chain),
                )
                .find()
            )
            ...
            public.M("firewall_ip").where(
                "address=? and types=? and chain=?",
                (get.address, get.strategy, get.chain),
            ).delete()
            get.domain = get.get("domain/s", "")
            if get.domain != "":
                public.M("firewall_domain").where("domain=?", (get.domain)).delete()
    ...
    def set_nline_ip_rule(self, get):
        ...
        for addr in address:
            ...
            if args.chain == "INPUT":
                ...
                result = self.firewall.output_rich_rules(
                    info=info, operation=args.operation
                )
    ...
    def set_mask_ip_rule(self, get):
        ...
        if get.operation == "add" and not self.check_ip_exist(get):
            return public.returnMsg(
                False, public.lang("目标地址{}已存在，请勿重复添加", get.address)
            )
    ...
    def set_ip_rule(self, get):
        ...
        if get.address != "all" and "\n" in get.address:
            ...
            if get.chain == "INPUT":
                ...
                result = self.firewall.output_rich_rules(
                    info=info, operation=get.operation
                )
    ...
    def port_forward_list(self, get):
        ...
        if type(list_port_forward) == list and type(forward_db) == list:
            return self.structure_forward_return_data(
                list_port_forward, forward_db, query=get.query
            )
        return []
    ...
    def set_port_forward(self, get):
        ...
        if get.operation == "add":
            ...
            if not self.check_forward_exist(get):
                return public.returnMsg(
                    False, public.lang("端口转发规则已存在，请勿重复添加")
                )
