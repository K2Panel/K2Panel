...
# K2Panel
# -------------------------------------------------------------------
# Copyright (c) 2015-2017 K2Panel(k2panel.online) All rights reserved.
# -------------------------------------------------------------------
# | Author: hwliang <hwl@k2panel.online>
# -------------------------------------------------------------------
        ...
        try:  # 表单验证
            ...
            if args["def_name"].find("__") != -1:
                return public.return_status_code(
                    1000, "调用的方法名称中不能包含“__”字符"
                )
            if not re.match(r"^\w+$", args["mod_name"]):
                return public.return_status_code(
                    1000, r"调用的模块名称中不能包含\w以外的字符"
                )
            if not re.match(r"^\w+$", args["def_name"]):
                return public.return_status_code(
                    1000, r"调用的方法名称中不能包含\w以外的字符"
                )
        except:
