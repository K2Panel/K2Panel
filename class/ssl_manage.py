...
class _SSLDatabase:
    ...
    def init_db(self):
        ...
        index_sql_str = (
            "CREATE INDEX IF NOT EXISTS 'hash_index' ON 'ssl_info' ('hash');"
        )
            ...
            if "no such column: ps" in str(e):
                try:
                    public.M("ssl_info").execute(
                        "ALTER TABLE 'ssl_info' ADD 'ps' TEXT DEFAULT ''", ()
                    )
                except Exception as e:
                    pass
...
class SSLManger:
    ...
    def get_cert_for_deploy(cls, ssl_hash: str) -> Dict:
        ...
        if not isinstance(data["privkey"], str) or not isinstance(
            data["fullchain"], str
        ):
    ...
    @staticmethod
    def ssl_hash(
        cert_filename: str = None, certificate: str = None, ignore_errors: bool = False
    ) -> Optional[str]:
    ...
    def get_cert_info_by_hash(self, cert_hash):
        ...
        record = (
            public.M("ssl_info").where("hash=?", (cert_hash,)).field("id, ps").find()
        )
    ...
    def save_by_file(
        ...
        if not os.path.isfile(cert_filename) or not os.path.isfile(
            private_key_filename
        ):
    ...
    def save_by_data(
        ...
        if not certificate.startswith("-----BEGIN") or not private_key.startswith(
            "-----BEGIN"
        ):
            raise ValueError(public.lang("Certificate format error"))
        if cloud_id is None:
    ...
    def get_cert_list(
        self, param: Optional[Tuple[str, List]] = None, force_refresh: bool = False
    ) -> List:
    ...
    def _get_cert_list(cls, param: Optional[Tuple[str, List]]) -> List:
        ...
        if (
            ...
            and isinstance(param[1], (tuple, list))
        ):
        ...
        for value in res:
            ...
            for f_str in format_time_strs:
                try:
                    end_time = int(
                        (
                            datetime.strptime(value["not_after"], f_str).timestamp()
                            - today_time
                        )
                        / (60 * 60 * 24)
                    )
    ...
    def _refresh_ssl_info_by_cloud(self):
        ...
        if key is None or iv is None:
            raise ValueError(
                public.lang(
                    "not logged in, so it's impossible to connect to the cloud!"
                )
            )
        ...
        for ssl_id in all_ids:
            if ssl_id["id"] not in change_set:
                ssl_db.connection().where("id = ?", (ssl_id["id"],)).update(
                    {"cloud_id": -1}
                )
    ...
    def _get_ssl_by_local_data(self):  # 从本地获取可用证书
        ...
        if os.path.isfile(panel_pem_file) and os.path.isfile(panel_key_file):
            try:
                self.save_by_file(
                    panel_pem_file, panel_key_file, other_data={"use_for_panel": 1}
                )
            except:
                pass
    ...
    def change_use_for_site(cls, site_id, ssl_id=None, ssl_hash=None, is_add=True):
        ...
        if site_id in site_ids and is_add is False:
            ...
            up_res = (
                ssl_db.connection()
                .where("id = ?", (target["id"],))
                .update({"use_for_site": json.dumps(site_ids)})
            )
    ...
    def get_all_site_ssl(self):
        ...
        for site in all_sites:
            prefix = (
                ...
                else site["project_type"].lower() + "_"
            )
    ...
    def _get_site_ssl_info(site_name, prefix="") -> Optional[Tuple[str, str]]:
        ...
        if webserver == "nginx":
            conf_file = "{}/vhost/nginx/{}{}.conf".format(
                public.get_panel_path(), prefix, site_name
            )
        elif webserver == "apache":
            conf_file = "{}/vhost/apache/{}{}.conf".format(
                public.get_panel_path(), prefix, site_name
            )
        else:
            conf_file = "{}/vhost/openlitespeed/detail/{}.conf".format(
                public.get_panel_path(), site_name
            )
    ...
    def remove_cert(self, ssl_id=None, ssl_hash=None, local: bool = False):
        ...
        if user_info is None:
            raise ValueError(
                public.lang("not logged in, thus unable to upload to the cloud!")
            )
        ...
        if target["cloud_id"] != -1:
            ...
            try:
                ...
                if local:
                    raise ValueError(
                        public.lang("Local file del success. But cloud file del fail.")
                    )
                raise ValueError(
                    public.lang(
                        "Failed to connect to the cloud. Unable to delete data on the cloud."
                    )
                )

            ssl_db.connection().where("id = ?", (target["id"],)).update(
                {"cloud_id": -1}
            )
    ...
    def upload_cert(self, ssl_id=None, ssl_hash=None):
        ...
        if key is None or iv is None:
            raise ValueError(
                False, public.lang("not logged in, thus unable to upload to the cloud!")
            )
