...
class main(databaseBase):
    ...
    def AddDatabase(self, args):
        ...
        if re.match(r"^\d+", data_name):
            return public.returnMsg(
                False, public.lang("SQLServer databases cannot start with numbers!")
            )
        ...
        if len(password) < 8 or len(password) > 128 or reg_count < 3:
            return public.returnMsg(
                False,
                public.lang(
                    "SQLServer password complexity policy does not match, should be 8-128 characters, and contain any 3 of them in upper case, lower case, digits, special symbols!"
                ),
            )
    ...
    def ToBackup(self, args):
        ...
        backupName = (
            session["config"]["backup_path"] + "/database/sqlserver/" + fileName
        )

        mssql_obj = self.get_mssql_obj_by_sid(find["sid"])

        if not int(find["sid"]):
            ret = mssql_obj.execute(
                "backup database %s To disk='%s'" % (find["name"], backupName)
            )
            ...
            return public.returnMsg(
                False,
                public.lang("Operation failed. Remote database cannot be backed up."),
            )
        ...
        if os.path.getsize(backupName) < 2048:
            return public.returnMsg(
                True,
                public.lang(
                    "The backup file size is smaller than 2Kb. Check the backup integrity."
                ),
            )
        else:
    ...
    def InputSql(self, get):
        ...
        if ext == "zip":
            try:
                ...
                return public.returnMsg(
                    False,
                    public.lang(
                        "The import failed because the file is not a valid ZIP file."
                    ),
                )
        ...
        mssql_obj.execute(
            "ALTER DATABASE %s SET OFFLINE WITH ROLLBACK IMMEDIATE" % (find["name"])
        )
        mssql_obj.execute(
            "use master;restore database %s from disk='%s' with replace, MOVE N'%s' TO N'%s',MOVE N'%s_Log'  TO N'%s' "
            % (find["name"], file, find["name"], data[0][0], find["name"], data[1][0])
        )
        mssql_obj.execute("ALTER DATABASE %s SET ONLINE" % (find["name"]))

        public.WriteLog(
            "TYPE_DATABASE",
            "Description Succeeded in importing database [{}]".format(name),
        )
    ...
    def ToDataBase(self, find):
        ...
        self.__CreateUsers(
            find["name"], find["username"], find["password"], "127.0.0.1"
        )
    ...
    def ResDatabasePassword(self, args):
        ...
        try:
            if not newpassword:
                return public.returnMsg(
                    False,
                    public.lang(
                        "The password of database [' + username + '] cannot be empty."
                    ),
                )
        ...
        if not find:
            return public.returnMsg(
                False,
                public.lang(
                    "Modify the failure，The specified database does not exist."
                ),
            )
    ...
    def get_root_pwd(self, args):
        ...
        if not ret:
            return public.returnMsg(
                False,
                public.lang(
                    "The SQL Server is not installed or started. Install or start it first"
                ),
            )
    ...
    def set_root_pwd(self, args):
        ...
        try:
            ...
            if len(re.search(r"^[\w@\.]+$", password).groups()) > 0:
                return public.returnMsg(
                    False,
                    public.lang(
                        "saThe password cannot be empty or have special symbols"
                    ),
                )
        except:
            return public.returnMsg(
                False,
                public.lang("saThe password cannot be empty or have special symbols"),
            )
        ...
        return public.returnMsg(
            True, public.lang("The password of sa is changed successfully.")
        )
    ...
    def get_database_size_by_id(self, args):
        ...
        try:
            ...
            tables = mssql_obj.query(
                "select name,size,type from sys.master_files where type=0 and name = '{}'".format(
                    name
                )
            )
    ...
    def __CreateUsers(self, data_name, username, password, address):
        ...
        mssql_obj = self.get_mssql_obj_by_sid(self.sid)
        mssql_obj.execute(
            "use %s create login %s with password ='%s' , default_database = %s"
            % (data_name, username, password, data_name)
        )
        mssql_obj.execute(
            "use %s create user %s for login %s with default_schema=dbo"
            % (data_name, username, username)
        )
        mssql_obj.execute(
            "use %s exec sp_addrolemember 'db_owner','%s'" % (data_name, data_name)
        )
    ...
    def check_cloud_database_status(self, conn_config):
        ...
        try:
            ...
            sql_obj = panelMssql.panelMssql().set_host(
                conn_config["db_host"],
                conn_config["db_port"],
                conn_config["db_name"],
                conn_config["db_user"],
