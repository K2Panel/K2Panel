...
# -------------------------------------------------------------------
# Author: hezhihong <bt_ahong@binarjoinanalyticnl.nl>
# -------------------------------------------------------------------
...
class panelPgsql:
    ...
    def __init__(self):
        ...
        if (
            ...
            and self.__DB_HOST == "localhost"
            and self.__DB_PASS == None
        ):
    ...
    def __Conn(self):
        ...
        try:
            ...
            self.__DB_CONN.set_isolation_level(
...
class main(databaseBase, panelPgsql):
    ...
    def GetCloudServer(self, args):
        ...
        if (
            check_result != 0
            and not public.M("database_servers").where("db_type=?", "pgsql").count()
        ):
            return []
    ...
    def AddDatabase(self, args):
        ...
        if test_str != None:
            return public.returnMsg(
                False,
                public.lang("The database name cannot contain special characters"),
            )
    ...
    def ToBackup(self, args):
        ...
        if not find["password"].strip():
            return public.returnMsg(
                False,
                public.lang("The database password is empty. Set the password first."),
            )
        ...
        if int(find["sid"]):
            ...
            shell = '{} "host={} port={} user={} dbname={} password={}" > {}'.format(
                sql_dump,
                info["db_host"],
                info["db_port"],
                info["db_user"],
            ...
            shell = (
                '{} "host=127.0.0.1 port={} user={} dbname={} password={}" > {}'.format(
                    sql_dump,
                    port["data"],
                    find["username"],
                    find["name"],
    ...
    def ToDataBase(self, find):
        ...
        if (
            isError != None
            and isError["status"] == False
            and isError["msg"] == "指定数据库已存在，请勿重复添加."
        ):
            return 1
    ...
    def ResDatabasePassword(self, args):
        ...
        if not newpassword:
            return public.returnMsg(
                False, public.lang("The database password cannot be empty.")
            )
        ...
        sql_obj = self.get_sql_obj_by_sid(find["sid"])
        result = sql_obj.execute(
            "alter user {} with password '{}';".format(username, newpassword)
        )
        isError = self.IsSqlError(result)
        if isError != None:
            return isError
    ...
    def get_root_pwd(self, args):
        ...
        if check_result != 0:
            return public.returnMsg(
                False,
                public.lang(
                    "If PgSQL is not installed or started, install or start it first"
                ),
            )
        password = ""
    ...
    def set_root_pwd(self, args):
        ...
        if len(password) < 8:
            return public.returnMsg(
                False, public.lang("The password must not be less than 8 digits.")
            )
        check_result = os.system("/www/server/pgsql/bin/psql --version")
        if check_result != 0:
            return public.returnMsg(
        ...
        if not os.path.isfile(path):
            return public.returnMsg(
                False,
                public.lang(
                    "{}File does not exist, please check the installation is complete!",
                    path,
                ),
            )
    ...
    def __CreateUsers(self, data_name, username, password, address):
        ...
        sql_obj.execute(
            "GRANT ALL PRIVILEGES ON DATABASE {} TO {};".format(data_name, username)
        )
        return True
