...
class databaseBase:
    ...
    def get_base_list(self, args, sql_type="mysql"):
        ...
        if search:
            where += "AND (name like '%{search}%' or ps like '%{search}%')".format(
                search=search
            )
        ...
        for sdata in rdata["data"]:
            ...
            sdata["backup_count"] = (
                public.M("backup").where("pid=? AND type=1", (sdata["id"])).count()
            )
    ...
    def add_base_database(self, get):
        ...
        if (
            public.M("databases")
            .where("name=? or username=?", (data_name, username))
            .count()
        ):
    ...
    def delete_base_backup(self, get):
        ...
        public.WriteLog(
            "TYPE_DATABASE", "DATABASE_BACKUP_DEL_SUCCESS", (name, filename)
        )
    ...
    def check_cloud_args(self, get, nlist=[]):
        ...
        for key in nlist:
            if not key in get:
                return public.returnMsg(
                    False,
                    public.lang("Parameter passing error, missing parameter {}!", key),
                )
    ...
    def AddBaseCloudServer(self, get):
        ...
        if isinstance(result, int):
            public.WriteLog(
                "Database manager",
                "Add remote MySQL server[{}:{}]".format(get["db_host"], get["db_port"]),
            )
    ...
    def RemoveBaseCloudServer(self, get):
        ...
        if not id:
            return public.returnMsg(
                False, public.lang("Parameter passed error, please try again!")
            )
        db_find = public.M("database_servers").where("id=?", (id,)).find()
        if not db_find:
            return public.returnMsg(
                False, public.lang("The specified remote server does not exist!")
            )
        ...
        if isinstance(result, int):
            public.WriteLog(
                "Database manager",
                "Delete remote MySQL server [{}:{}]".format(
    ...
    def ModifyBaseCloudServer(self, get):
        ...
        if isinstance(result, int):
            public.WriteLog(
                "Database manager",
                "Modify the remote MySQL server[{}:{}]".format(
    ...
    def IsSqlError(self, mysqlMsg):
        if mysqlMsg:
            ...
            if "MySQLdb" in mysqlMsg:
                return public.returnMsg(
                    False, public.lang("MySQLdb component is missing!")
                )
            if "2002," in mysqlMsg:
                return public.returnMsg(False, public.lang("ERROR to connect database"))
            if "2003," in mysqlMsg:
                return public.returnMsg(
                    False,
                    public.lang(
            ...
            if "1045," in mysqlMsg:
                return public.returnMsg(False, public.lang("MySQL password error."))
            if "1040," in mysqlMsg:
                return public.returnMsg(
                    False,
                    public.lang(
            ...
            if "using password:" in mysqlMsg:
                return public.returnMsg(
                    False, public.lang("Database password is incorrect!")
                )
            ...
            if "1142" in mysqlMsg:
                return public.returnMsg(
                    False,
                    public.lang("Insufficient permissions, please use root user."),
                )

            if "DB-Lib error message 20018" in mysqlMsg:
                return public.returnMsg(
                    False, public.lang("Create failed, SQL Server requires GUI support")
                )
