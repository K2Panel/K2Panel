...
class panelRedisDB:
    ...
    def get_options(self, get=None):
        ...
        result = {}
        redis_conf = public.readFile(
            "{}/redis/redis.conf".format(public.get_setup_path())
        )
        if not redis_conf:
            return False
...
class main(databaseBase):
    ...
    def InputSql(self, get):
        ...
        public.ExecShell(
            "chown redis.redis {dump} && chmod 644 {dump}".format(dump=dst_path)
        )
    ...
    def get_backup_list(self, get):
        ...
        if hasattr(get, "sort"):
            nlist = sorted(
                nlist, key=lambda data: data["mtime"], reverse=get["sort"] == "desc"
            )
        return nlist
