    ...
    def __Conn(self, auth):
        ...
        if not self.__DB_CLOUD:
            ...
            if os.path.exists(path):
                self.__DB_PASS = public.readFile(path)
...
class main(databaseBase):
    ...
    def __set_auth_open(self, status):
        ...
        if status:
            conf = re.sub(
                r"authorization\s*\:\s*disabled", "authorization: enabled", conf
            )
        else:
            conf = re.sub(
                r"authorization\s*\:\s*enabled", "authorization: disabled", conf
            )
    ...
    def set_auth_status(self, get):
        ...
        if not public.process_exists("mongod"):
            return public.returnMsg(
                False, public.lang("Mongodb service has not been started yet!")
            )
    ...
    def get_obj_by_sid(self, sid=0, conn_config=None):
        ...
        if sid:
            if not conn_config:
                conn_config = public.M("database_servers").where("id=?", sid).find()
            db_obj = panelMongoDB()
    ...
    def AddDatabase(self, args):
        ...
        try:
            ...
            return public.returnMsg(
                False, public.lang("Database type sid needs int type!")
            )
        ...
        if hasattr(args, "contact"):
            site = (
                public.M("sites").where("id=?", (args.contact,)).field("id,name").find()
            )
            if site:
        ...
        if auth_status:
            db_obj.command(
                ...
                pwd=password,
                roles=[
                    {"role": "dbOwner", "db": data_name},
                    {"role": "userAdmin", "db": data_name},
                ],
            )
    ...
    def DeleteDatabase(self, args):
        ...
        find = (
            public.M("databases")
            .where("id=?", (id,))
            .field("id,pid,name,username,password,type,accept,ps,addtime,sid,db_type")
            .find()
        )
        if not find:
            return public.returnMsg(
    ...
    def InputSql(self, args):
        ...
        if not os.path.exists(file):
            return public.returnMsg(
                False, public.lang("The import path doesn't exist!")
            )
        if not os.path.isfile(file):
            return public.returnMsg(
                False, public.lang("Only compressed files can be imported!")
            )
        find = (
            public.M("databases")
            .where("name=? AND LOWER(type)=LOWER('MongoDB')", (name,))
            .find()
        )
        if not find:
        ...
        if not os.path.exists(mongorestore_obj):
            return public.returnMsg(
                False,
                public.lang(
                    "Lack of backup tools, please install MongoDB through [APP Store] first!"
                ),
            )
        ...
        if len(split_tmp[1:]) == 2 and split_tmp[1] not in ["json", "csv"]:
            return public.returnMsg(
                False, f".{ext_err} This file format is not currently supported！"
            )
        ...
        if os.path.isdir(input_path):  # zip,gz,bson
            ...
            public.ExecShell(shell)
        public.WriteLog(
            "Database manager", "Import database [{}] succeeded".format(name)
        )
    ...
    def ToBackup(self, args):
        ...
        find = (
            public.M("databases")
            .where("id=? AND LOWER(type)=LOWER('MongoDB')", (id,))
            .find()
        )
        if not find:
            return public.returnMsg(
        ...
        if self.get_local_auth(get):
            if not info["db_password"]:
                return public.returnMsg(
                    False,
                    public.lang(
                        "Password authentication has been enabled. The password cannot be empty when the database is backed up. Please set a password and try again!"
                    ),
                )
            shell = "{}  -h {} --port {} -u {} -p {} -d {} -o {} ".format(
                sql_dump,
                info["db_host"],
                info["db_port"],
                info["db_user"],
            ...
            shell = "{}  -h {} --port {} -d {} -o {} ".format(
                sql_dump, info["db_host"], info["db_port"], find["name"], backupName
            )

        ret = public.ExecShell(shell)
        if not os.path.exists(backupName):
            return public.returnMsg(
                False, public.lang("Database backup failed, file does not exist")
            )
        ...
        if os.path.getsize(backupFile) < 1:
            return public.returnMsg(
                True,
                public.lang(
                    "The backup is executed successfully, the backup file is less than 1b, please check the backup integrity."
                ),
            )
        else:
    ...
    def SyncToDatabases(self, get):
        ...
        if type == 0:
            data = (
                sql.field("id,name,username,password,accept,type,sid,db_type")
                .where("type=?", ("MongoDB",))
                .select()
            )
            ...
            for value in data:
                find = (
                    sql.where("id=?", (value,))
                    .field("id,name,username,password,sid,db_type,accept,type")
                    .find()
                )
        ...
        if auth_status:
            ...
            try:
                db_obj.command(
                    ...
                    pwd=find["password"],
                    roles=[
                        {"role": "dbOwner", "db": find["name"]},
                        {"role": "userAdmin", "db": find["name"]},
                    ],
                )
            except:
                pass
    ...
    def SyncGetDatabases(self, get):
        ...
        try:
            ...
            return public.returnMsg(
                False, public.lang("The database type SID requires an INT!")
            )
        if not public.process_exists("mongod") and not int(get.sid):
            return public.returnMsg(
                False, public.lang("The Mongodb service is not enabled!")
            )
        auth_status = self.get_local_auth(get)
        data = (
            self.get_obj_by_sid(self.sid)
            .get_db_obj("admin", auth=auth_status)
        ...
        for item in data["databases"]:
            dbname = item["name"]
            if sql.where("name=?", (dbname,)).count():
                continue
    ...
    def ResDatabasePassword(self, args):
        ...
        try:
            if not newpassword:
                return public.returnMsg(
                    False,
                    public.lang(
                        "Modify the failure，The database[' + username + ']password cannot be empty."
                    ),
                )
            if len(re.search(r"^[\w@\.]+$", newpassword).groups()) > 0:
                return public.returnMsg(
                    False,
                    public.lang(
                        "The database password cannot be empty or contain special characters"
                    ),
                )
        ...
        find = (
            public.M("databases")
            .where("id=?", (id,))
            .field("id,pid,name,username,password,type,accept,ps,addtime,sid")
            .find()
        )
        if not find:
            return public.returnMsg(
                False,
                public.lang(
                    "The modification failed because the specified database does not exist."
                ),
            )
        ...
        auth_status = self.get_local_auth(args)
        if auth_status:
            db_obj = self.get_obj_by_sid(find["sid"]).get_db_obj(
                username, auth=auth_status
            )
            try:
            ...
            return public.returnMsg(
                False, public.lang("Password access is not enabled for the database.")
            )
