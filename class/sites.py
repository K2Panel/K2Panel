...
class sites:
    ...
    def create_site(self, get):
        ...
        if domainFormat:
            return public.ReturnMsg(False, "SITE_DOMAIN_FORMAT_ERR", (domainFormat,))
        domainExists = self.domain_exists(pdata["domains"])
        if domainExists:
            return public.ReturnMsg(False, "SITE_DOMAIN_EXIST", (domainExists,))
...
server
{
    ...
    def domain_exists(self, domains):
        ...
        for domain in domains:
            ...
            pid = (
                sql.table("domain")
                .where("name=? and port=?", (tmp[0], tmp[1]))
                .getField("pid")
            )
            if pid:
    ...
    def _generate_nginx_conf(self, siteName):
        ...
        if siteInfo:
            ...
            siteBinding = (
                public.M("binding")
                .where("pid=?", (siteInfo["id"],))
                .field("domain,port,path")
                .select()
            )
        siteConfig = {
            "siteId": 2,
            "defaultDoc": "index.php index.html index.htm default.php default.htm default.html",
            "fpmConfig": {"type": "PHP", "version": "5.4"},
            "ssl": {"open": False, "cert": "", "privateKey": "", "pool": ""},
            "binDingSsl": {
                "/www": {"open": False, "cert": "", "privateKey": "", "pool": ""}
            },
            "redirect": [
                {
                    "code": 301,
            ...
            "networkLimit": {"open": False, "perServer": 500, "perIp": 25, "rate": 512},
        }
