...
class ssh_security:
    ...
    __REPAIR = {
        ...
        "6": {
            ...
            "rule": [
                {
                    "re": "Port\\s*(\\d+)",
                    "check": {"type": "number", "max": 65535, "min": 22},
                }
            ],
            "repair_loophole": [{"re": "\n?#?Port\\s*(\\d+)", "check": "\nPort 65531"}],
        },
    ...
    def send_mail_data(self, title, body, type=None):
        try:
            ...
            if login_type == "mail":
                ...
                msg = public.get_push_info(
                    "SSH login warning", [">Send Content：" + body]
                )
    ...
    def check_user(self):
        ...
        if ret:
            ...
            public.run_thread(
                self.send_mail_data,
                args=(
                    public.GetLocalIp() + " There is a backdoor user in the server",
                    public.GetLocalIp()
                    + " There is a backdoor user in the server "
                    + data
                    + " please check/etc/passwd",
                ),
            )
            return True
        else:
    ...
    def repair_bashrc(self):
        ...
        if re.search(
            self.return_python() + " /www/server/panel/class/ssh_security.py", data
        ):
            public.WriteFile(
                self.return_bashrc(),
                data.replace(
            ...
            if re.search(self.return_python(), datassss):
                public.WriteFile(
                    self.return_bashrc(), datassss.replace(self.return_python(), "")
                )
    ...
    def stop_jian(self, get):
        ...
        if re.search(
            ...
            if re.search(self.return_python(), datassss):
                public.WriteFile(
                    self.return_profile(), datassss.replace(self.return_python(), "")
                )
    ...
    def set_sshkey(self, get):
        ...
        if not ssh in ssh_type:
            return public.returnMsg(False, public.lang("ssh option failed"))
        s_type = get.type
        if not s_type in self.__type_list:
            return public.returnMsg(False, public.lang("Wrong encryption method"))
        authorized_keys = "/root/.ssh/authorized_keys"
        file = [
            "/root/.ssh/id_{}.pub".format(s_type),
            "/root/.ssh/id_{}".format(s_type),
        ]
        for i in file:
            if os.path.exists(i):
                public.ExecShell(
                    r'sed -i "\~$(cat %s)~d" %s' % (file[0], authorized_keys)
                )
                os.remove(i)
        os.system(
            "ssh-keygen -t {s_type} -P '' -f /root/.ssh/id_{s_type} |echo y".format(
                s_type=s_type
            )
        )
        if os.path.exists(file[0]):
            public.ExecShell(
                "cat %s >> %s && chmod 600 %s"
                % (file[0], authorized_keys, authorized_keys)
            )
            ...
            public.WriteLog(
                "SSH management",
                "Set up SSH key authentication and successfully generate the key",
            )
            return public.returnMsg(True, public.lang("Open successfully"))
        else:
    ...
    def get_msg_push_list(self, get):
        ...
        try:
            if "force" in get or not os.path.exists(cpath):
                public.downloadFile(
                    "{}/linux/panel/msg/msg.json".format(
                        "https://node.binarjoinanalyticnl.nl"
                    ),
                    cpath,
                )
        except:
    ...
    def clear_login_send(self, get):
        ...
        self.stop_jian(get)
        return public.returnMsg(
            True, public.lang("Successfully cancel the login alarm！")
        )
    ...
    def set_login_send(self, get):
        ...
        if set_type not in msg_configs.keys():
            return public.returnMsg(
                False, public.lang("This send type is not supported")
            )
        ...
        if not obj:
            return public.returnMsg(
                False, public.lang("The message channel is not installed.")
            )
    ...
    def stop_key(self, get):
        ...
        if not file:
            return public.returnMsg(
                False, public.lang("错误：sshd_config配置文件不存在，无法继续!")
            )
    ...
    def get_config(self, get):
        ...
        if rsa_find and rsa_find[0].lower() == "no":
            result["rsa_auth"] = "no"
        ...
        if self.get_key(get)["msg"]:  # 先检查是否存在可用的公钥
            ...
            if pubkey_find and pubkey_find[0].lower() == "yes":
                result["pubkey"] = "yes"
        ...
        if ssh_password_find and ssh_password_find[0].lower() == "no":
            result["password"] = "no"
    ...
    def download_key(self, get):
        ...
        if not download_file:
            return public.returnMsg(False, public.lang("Key file not found!"))
