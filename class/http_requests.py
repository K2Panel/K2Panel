...
class http:
    ...
    def get(self, url, timeout=(6, 60), headers={}, verify=False, type="python"):
        ...
        if type in ["python", "src", "php"]:
            ...
            try:
                ...
                result = requests.get(
                    url, timeout=timeout, headers=get_headers(headers), verify=verify
                )
        ...
        if type in ["python", "src", "php"]:
            ...
            try:
                ...
                result = requests.post(
                    url, data, timeout=timeout, headers=headers, verify=verify
                )
            except:
                try:
                    # IPV6？
    ...
    def download_file(
        ...
        data=None,
        timeout=1800,
        speed_file="/dev/shm/download_speed.pl",
    ):
        """
        @name 下载文件
        ...
        if data is None:
            ...
            res = requests.post(
                url, data, headers=headers, timeout=timeout, stream=True
            )
    ...
    def _post_py2(self, url, data, timeout, headers, verify):
        ...
        return response(
            r_response.read(), r_response.getcode(), r_response.info().headers
        )
    ...
    def _post_py3(self, url, data, timeout, headers, verify):
        ...
        req = urllib.request.Request(
            url, self._str_py_post(data, headers), headers=headers
        )
        try:
            if not verify:
                context = ssl._create_unverified_context()
                r_response = urllib.request.urlopen(
    ...
    def _post_curl(self, url, data, timeout, headers, verify):
        ...
        result = public.ExecShell(
            "{} -X POST -sS -i --connect-timeout {} {} {} '{}' 2>&1".format(
                self._curl_bin() + _ssl_verify, timeout, headers_str, pdata, url
            )
        )[0]
...
?>""".format(
        ...
        if php_version.find("/www/server/php") != -1:
            result = public.ExecShell(
                php_version + " " + tmp_file + " --post='" + data + "'"
            )[0]
        else:
            result = public.request_php(
                php_version, "/http.php", "/dev/shm", "POST", {"data": data}
            )
    ...
    def _get_curl(self, url, timeout, headers, verify):
        ...
        if not verify:
            _ssl_verify = " -k"
        result = public.ExecShell(
            "{} -sS -i --connect-timeout {} {} {} 2>&1".format(
                self._curl_bin() + " " + str(_ssl_verify), timeout, headers_str, url
            )
        )[0]
        r_body, r_headers, r_status_code = self._curl_format(result)
        return response(r_body, r_status_code, r_headers)
...
?>""".format(
    ...
    def _curl_bin(self):
        ...
        curl_bin = "curl"
        for cb in c_bin:
            if os.path.exists(cb) and public.ExecShell(cb + " -V")[0] != "":
                curl_bin = cb
            ...
            for line in tmp.split("\r\n"):
                if line.find("HTTP/") != -1:
                    if line.find("Continue") != -1:
                        continue
                    status_code = int(
                        re.search(r"HTTP/[\d\.]+\s(\d+)", line).groups()[0]
                    )
                    break
            if status_code == 100:
                status_code = 200
    ...
    def _str_post(self, pdata, headers):
        str_pdata = ""
        if (
            headers.find("application/jose") != -1
    ...
    def _str_py_post(self, pdata, headers):
        if "Content-Type" in headers:
            if (
                headers["Content-Type"].find("application/jose") != -1
...
def post(url, data={}, timeout=(15, 120), headers={}, verify=False, s_type=None):
    ...
    try:
        return p.post(
            url, data, timeout, get_headers(headers), verify, get_stype(s_type)
        )
    except:
