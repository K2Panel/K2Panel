...
class send_mail:
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.__mail_config):
            ...
            try:
                ...
                if (
                    "qq_mail" in qq_mail_info
                    and "qq_stmp_pwd" in qq_mail_info
                    and "hosts" in qq_mail_info
                ):
        ...
        if not os.path.exists(self.__dingding_config):
            ...
            try:
                ...
                if (
                    "dingding_url" in dingding_info
                    and "isAtAll" in dingding_info
                    and "user" in dingding_info
                ):
        ...
        if not os.path.exists(self.__weixin_config):
            ...
            try:
                ...
                if (
                    "corpid" in weixin_info
                    and "corpsecret" in weixin_info
                    and "user_id" in weixin_info
                    and "agentid" in weixin_info
                ):
    ...
    def get_settings(self):
        ...
        ret["user_mail"] = {
            "user_name": user_mail,
            "mail_list": self.__mail_list,
            "info": self.get_user_mail(),
        }
    ...
    def qq_smtp_send(self, email, title, body):
        if (
            "qq_mail" not in self.__qq_mail_user
            or "qq_stmp_pwd" not in self.__qq_mail_user
            or "hosts" not in self.__qq_mail_user
        ):
            return -1
        ret = True
        if not "port" in self.__qq_mail_user:
            self.__qq_mail_user["port"] = 465
        try:
            msg = MIMEText(body, "html", "utf-8")
            msg["From"] = formataddr(
                [self.__qq_mail_user["qq_mail"], self.__qq_mail_user["qq_mail"]]
            )
            if type(email) == str:
            ...
            if int(self.__qq_mail_user["port"]) == 465:
                server = smtplib.SMTP_SSL(
                    str(self.__qq_mail_user["hosts"]),
                    str(self.__qq_mail_user["port"]),
                    timeout=5,
                )
            else:
                server = smtplib.SMTP(
            ...
            server.login(
                self.__qq_mail_user["qq_mail"], self.__qq_mail_user["qq_stmp_pwd"]
            )
            if type(email) == str:
                server.sendmail(
                    self.__qq_mail_user["qq_mail"], [email.strip()], msg.as_string()
                )
            elif type(email) == list:
    ...
    def dingding_insert(self, url, atall, user="1"):
        qq_stmp_info = {
            "dingding_url": url.strip(),
            "isAtAll": str(atall).strip(),
            "user": str(user).strip(),
        }
    ...
    def dingding_send(self, content):
        if (
            "dingding_url" not in self.__dingding_info
            or "isAtAll" not in self.__dingding_info
            or "user" not in self.__dingding_info
        ):
            return -1
        if "weixin.qq.com" in self.__dingding_info["dingding_url"]:
            ...
            data = {
                "msgtype": "markdown",
                "markdown": {"title": "Server notification", "text": content},
                "at": {
        ...
        try:
            x = requests.post(
                url=self.__dingding_info["dingding_url"],
    ...
    def add_weixin_user(self, uid):
        if (
            "corpid" not in self.__weixin_info
            or "corpsecret" not in self.__weixin_info
            or "user_id" not in self.__weixin_info
            or "agentid" not in self.__weixin_info
        ):
            return False
        if uid in self.__weixin_info["user_id"]:
            return False
    ...
    def get_token(self):
        ...
        values = {
            "corpid": self.__weixin_info["corpid"],
    ...
    def send_msg(self, content):
        ...
        data = {
            "touser": self.return_weixin_user(),
            "toparty": "1",
            "msgtype": "text",
