...
class system:
    ...
    def GetConcifInfo(self, get=None):
        ...
        if not "email" in session["config"]:
            session["config"]["email"] = (
                public.M("users").where("id=?", ("1",)).getField("email")
            )
        data = {}
        data = session["config"]
            ...
            try:
                if os.path.exists(configFile):
                    ...
                    if rtmp:
                        phpversion = rtmp.groups()[0]
                    conf = public.readFile(
                        "/www/server/panel/vhost/openlitespeed/listen/888.conf"
                    )
                    rep = r"address\s+\*\:(\d+)"
                    rtmp = re.search(rep, conf)
                    if rtmp:
        ...
        tmp["version"] = public.xss_version(
            public.readFile(self.setupPath + "/" + data["webserver"] + "/version.pl")
        )
        tmp["status"] = False
        ...
        tmp["version"] = public.xss_version(
            public.readFile(self.setupPath + "/tomcat/version.pl")
        )
        data["tomcat"] = tmp
        ...
        tmp["version"] = public.xss_version(
            public.readFile(self.setupPath + "/mysql/version.pl")
        )
        ...
        tmp["version"] = public.xss_version(
            public.readFile(self.setupPath + "/pure-ftpd/version.pl")
        )
        tmp["status"] = os.path.exists("/var/run/pure-ftpd.pid")
        data["pure-ftpd"] = tmp
        data["panel"] = self.GetPanelInfo()
        data["systemdate"] = public.ExecShell('date +"%Y-%m-%d %H:%M:%S %Z %z"')[
            0
        ].strip()
    ...
    def GetCpuInfo(self, interval=1):
        ...
        if not used:
            used = self.get_cpu_percent_thead(interval)
    ...
    def get_cpu_percent(self):
        ...
        try:
            percent = round(
                100.00
                * (
                    (new_process_time - old_process_time)
            ...
            if os.path.exists("/proc/diskstats"):
                ...
                for disk_name in diskio_2.keys():
                    ...
                    diskInfo[disk_name]["read_count"] = int(
                        (
                            diskio_2[disk_name].read_count
                            - diskio_1[disk_name].read_count
                        )
                        / stime
                    )
                    diskInfo[disk_name]["write_count"] = int(
                        (
                            diskio_2[disk_name].write_count
                            - diskio_1[disk_name].write_count
                        )
                        / stime
                    )
                    diskInfo[disk_name]["read_bytes"] = int(
                        (
                            diskio_2[disk_name].read_bytes
                            - diskio_1[disk_name].read_bytes
                        )
                        / stime
                    )
                    diskInfo[disk_name]["write_bytes"] = int(
                        (
                            diskio_2[disk_name].write_bytes
                            - diskio_1[disk_name].write_bytes
                        )
                        / stime
                    )
                    ...
                    diskInfo[disk_name]["write_time"] = int(
                        (
                            diskio_2[disk_name].write_time
                            - diskio_1[disk_name].write_time
                        )
                        / stime
                    )
                    ...
                    if (
                        diskInfo["ALL"]["write_time"]
                        < diskInfo[disk_name]["write_time"]
                    ):
                        diskInfo["ALL"]["write_time"] = diskInfo[disk_name][
                            "write_time"
                        ]
    ...
    def GetNetWork(self, get=None):
        ...
        if get != False:
            ...
            for disk in self.GetDiskInfo2(False):
                disk["size"].append(
                    int(disk["size"][0]) - (int(disk["size"][1]) + int(disk["size"][2]))
                )  # 计算系统占用
                disk["size"] = list(
                    map(
                        lambda num: (
                            f"{round(int(num) / 1048576, 2)}G"
                            if str(num).isdigit() and str(num).find("G") == -1
                            else num
                        ),
                        disk["size"],
                    )
    ...
    def GetNetWorkOld(self):
        ...
        networkInfo = {}
        networkInfo["upTotal"] = networkInfo["downTotal"] = networkInfo["up"] = (
            networkInfo["down"]
        ) = networkInfo["downPackets"] = networkInfo["upPackets"] = 0
    ...
    def ServiceAdmin(self, get=None):
        ...
        if get.name == "phpmyadmin":
            ...
            return public.return_msg_gettext(
                True, public.lang("Executed successfully!")
            )
        ...
        if get.name == "apache" or get.name == "httpd":
            ...
            if not os.path.exists(self.setupPath + "/apache/bin/apachectl"):
                return public.return_msg_gettext(
                    True, public.lang("Execution failed, check if Apache installed")
                )
            ...
            result = public.ExecShell(
                "ulimit -n 8192 ; " + self.setupPath + "/apache/bin/apachectl -t"
            )
            if result[1].find("Syntax OK") == -1:
                public.write_log_gettext(
                    "Software manager", "Execution failed: {}", (str(result),)
                )
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Apache rule configuration error: <br><a style='color:red;'>{}</a>",
                        result[1].replace("\n", "<br>"),
                    ),
                )
        ...
        # 检查nginx配置文件
            ...
            if not os.path.exists("/dev/shm/nginx-cache/wp"):
                public.ExecShell(
                    "mkdir -p /dev/shm/nginx-cache/wp && chown -R www.www /dev/shm/nginx-cache"
                )
            ...
            if result[1].find("perserver") != -1:
                ...
                return public.return_msg_gettext(
                    True,
                    public.lang(
                        "Configuration file mismatch caused by reinstalling Nginx fixed"
                    ),
                )
            ...
            if result[1].find("successful") == -1:
                ...
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Nginx rule configuration error: <br><a style='color:red;'>{}</a>",
                        result[1].replace("\n", "<br>"),
                    ),
                )
        ...
        if execStr == "/etc/init.d/pure-ftpd reload":
            execStr = (
                self.setupPath
                + "/pure-ftpd/bin/pure-pw mkdb "
                + self.setupPath
                + "/pure-ftpd/etc/pureftpd.pdb"
            )
        ...
        if get.type != "test":
            public.write_log_gettext(
                "Software manager", "Executed successfully [{}]!", (execStr,)
            )
        ...
        if get.type != "stop":
            ...
            if not self.check_service_status(get.name):
                if (
                    len(result[1]) > 1
                    and get.name != "pure-ftpd"
                    and get.name != "redis"
                ):
                    return public.returnMsg(
                        False,
                        public.lang(
                            "<p>failed to activate: <p>"
                            + result[1].replace("\n", "<br>")
                        ),
                    )
                else:
                    return public.returnMsg(
                        False, public.lang("{} service failed to start", get.name)
                    )
        else:
            if self.check_service_status(get.name):
                return public.returnMsg(False, public.lang("Service stop failed!"))
    ...
    def RestartServer(self, get):
        if not public.IsRestart():
            return public.return_msg_gettext(
                False,
                public.lang("Please run the program when all install tasks finished!"),
            )
        public.ExecShell("sync && init 6 &")
        return public.return_msg_gettext(
            True, public.lang("Command sent successfully!")
        )
    ...
    def ReMemory(self, get):
        ...
        if not os.path.exists(scriptFile):
            public.downloadFile(
                public.GetConfigValue("home") + "/script/rememory.sh", scriptFile
            )
    ...
    def RepPanel(self, get):
        ...
        public.ExecShell(
            ...
            + public.get_url()
            + "/install/update_7.x_en.sh && bash update.sh"
        )
        self.ReWeb(None)
        return True
