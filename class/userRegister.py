...
class userRegister:
    ...
    def toRegister(self, post):
        try:
            ...
            if not hasattr(post, "email") or not hasattr(post, "password"):
                return public.return_msg_gettext(
                    False, public.lang("User email or password cannot be empty!")
                )
            ...
            if not emailformat.search(post.email):
                return public.return_msg_gettext(
                    False, public.lang("Please enter your vaild email")
                )
        ...
        try:
            ...
            # sUrl = 'http://dev.binarjoinanalyticnl.nl/api/user/register_on_panel'
            # 判断 普通版 or Pro版
            if hasattr(post, "isPro") and post.isPro:
                sUrl = "{}/pro/api/user/register_on_panel".format(
    ...
    def getToken(self, get):
        ...
        try:
            # APIURL1 = 'http://dev.binarjoinanalyticnl.nl/api/user/login'
            ...
            if get.isPro:
                if result.get("err_no", None) == 2002:
                    return public.return_message(
                        -1, 0, public.lang("The email has not been validated")
                    )
