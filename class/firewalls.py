...
class firewalls:
    ...
    def SetFirewallStatus(self, get=None):
        ...
        public.write_log_gettext(
            "Firewall manager", "{} system firewall!", (status_msg[status],)
        )
        return public.return_msg_gettext(
            True, "{} system firewall!", (status_msg[status],)
        )
    ...
    def AddDropAddress(self, get):
        if not self.CheckFirewallStatus():
            return public.return_msg_gettext(
                False, public.lang("The system firewall is not open")
            )
        import time
        import re
        ...
        if not public.check_ip(ip_format):
            return public.return_msg_gettext(
                False, public.lang("IP address you entered is illegal!")
            )
        if ip_format in ["0.0.0.0", "127.0.0.0", "::1"]:
            return public.return_msg_gettext(
                False, public.lang("Disabling this IP will cause your server to fail")
            )
        address = get.port
        ...
        if self.__isUfw:
            if public.is_ipv6(ip_format):
                ...
                public.ExecShell(
                    "{} insert 1 deny from {} to any".format(self.__ufw, address)
                )
        else:
            if self.__isFirewalld:
    ...
    def DelDropAddress(self, get):
        ...
        if self.__isUfw:
            public.ExecShell(
                "{} delete deny from {} to any".format(self.__ufw, address)
            )
        else:
            if self.__isFirewalld:
                if public.is_ipv6(ip_format):
    ...
    def AddAcceptPort(self, get):
        ...
        is_exists = (
            public.M("firewall").where("port=? or port=?", (port, src_port)).count()
        )
        if is_exists:
            return public.return_msg_gettext(
        ...
        public.WriteLog(
            "TYPE_FIREWALL", "Successfully accepted port [{}]!".format(port)
        )
        addtime = time.strftime("%Y-%m-%d %X", time.localtime())
        if not is_exists:
    ...
    def DelAcceptPort(self, get):
        ...
        if public.is_ipv6(port):
            return self.DelDropAddress(get)  # 如果是ipv6地址，则调用DelDropAddress
        ...
        try:
            if (
                ...
                return public.return_msg_gettext(
                    False, public.lang("Failed,cannot delete current port of the panel")
                )
            if self.__isUfw:
    ...
    def SetSshPort(self, get):
        ...
        if port in ports:
            return public.return_msg_gettext(
                False, public.lang("Do NOT use common default port!")
            )
        ...
        public.M("firewall").where(
            "ps=? or ps=? or port=?",
            ("SSH remote management service", "SSH remote service", port),
        ).delete()
        public.M("firewall").add(
            "port,ps,addtime",
            (
