...
class panelSetup:
    ...
    def init(self):
        ...
        if os.path.exists("data/recycle_bin.pl"):
            g.recycle_bin_open = 1

        g.recycle_bin_db_open = 0
        if os.path.exists("data/recycle_bin_db.pl"):
            g.recycle_bin_db_open = 1
        g.is_aes = False
        self.other_import()
        return None
    ...
    def other_import(self):
        ...
        if g.o:
            ...
            if os.path.exists(css_file):
                g.other_css.append("/static/other/{}".format(css_name))

            js_name = "js/{}.js".format(g.o)
            js_file = s_path.format(js_name)
            if os.path.exists(js_file):
                g.other_js.append("/static/other/{}".format(js_name))
...
class panelAdmin(panelSetup):
    ...
    def setSession(self):
        ...
        session["bt_help"] = public.lang(
            "For Support|Suggestions, please visit the K2Panel Forum"
        )
    ...
    def checkWebType(self):
        ...
        if not "webversion" in session:
            if os.path.exists(
                self.setupPath + "/" + session["webserver"] + "/version.pl"
            ):
        ...
        try:
            ...
            if not "login" in session:
                ...
                if not public.check_client_hash():
                    return self.to_login(
                        public.get_admin_path(),
                        "Client verification failed, please log in again",
                    )
            ...
            if login_token and login_token != public.get_login_token_auth():
                return self.to_login(
                    public.get_admin_path(),
                    "Login verification failed, please log in again",
                )
        ...
        if not "client_bind_token" in get:
            ...
            if not public.get_error_num(num_key, 20):
                return public.returnJson(
                    False, "20 consecutive verification failures, prohibited for 1 hour"
                )

            if not public.is_api_limit_ip(
                api_config["limit_addr"], client_ip
            ):  # client_ip in api_config['limit_addr']:
                public.set_error_num(num_key)
                return public.returnJson(
                    False,
                    "%s[" % public.lang("IP validation failed, your access IP is")
                    + client_ip
                    + "]",
                )
        else:
