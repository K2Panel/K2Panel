...
class panelMessage:
    ...
    def get_cloud_messages(self, args):
        ...
        try:
            ...
            if ret:
                return public.returnMsg(True, public.lang("同步成功1!"))
            data = {}
    ...
    def get_messages(self, args=None):
        ...
        if not data:
            ...
            data = (
                public.M("messages")
                .where("state=? and expire>?", (1, int(time.time())))
                .order("id desc")
                .select()
            )
    ...
    def remove_message_level(self, level):
        ...
        @return bool
        """
        public.M("messages").where(
            "(level=? or level=? or level=? or level=?) and state=?",
            (level, level + "15", level + "7", level + "3", 1),
        ).delete()
        return True
