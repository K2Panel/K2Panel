...
class ajax:
        ...
        try:
            if not os.path.exists("/www/server/nginx/sbin/nginx"):
                return public.return_msg_gettext(
                    False, public.lang("Nginx is not install")
                )
            process_cpu = {}
            worker = (
                int(
                    public.ExecShell("ps aux|grep nginx|grep 'worker process'|wc -l")[0]
                )
                - 1
            )
            workermen = (
            ...
            public.write_log_gettext(
                "Get Info", "Nginx load status acquisition failed:{}", (ex,)
            )
            return public.return_msg_gettext(
                False, public.lang("Data acquisition failed!")
            )
    ...
    def GetPHPStatus(self, get):
        ...
        try:
            ...
            public.write_log_gettext(
                "Get Info",
                "PHP load status acquisition failed: {}",
                (public.get_error_info(),),
            )
            return public.return_msg_gettext(
                False, public.lang("PHP load status acquisition failed!")
            )
    ...
    def CheckStatusConf(self):
        ...
        if os.path.exists(filename):
            if public.ReadFile(filename).find("nginx_status") != -1:
                return
    ...
    def GetSoftList(self, get):
        ...
        data = json.loads(tmp)
        tasks = (
            public.M("tasks").where("status!=?", ("1",)).field("status,name").select()
        )
        for i in range(len(data)):
            data[i]["check"] = (
                public.GetConfigValue("root_path") + "/" + data[i]["check"]
            )
            for n in range(len(data[i]["versions"])):
                ...
                for task in tasks:
                    ...
                    if data[i]["name"] == "PHP":
                        if (
                            tmp1[0].lower() == data[i]["name"].lower()
                ...
                if data[i]["name"] == "PHP":
                    ...
                    checkFile = data[i]["check"].replace(
                        "VERSION", data[i]["versions"][n]["version"].replace(".", "")
                    )
                else:
                    data[i]["task"] = isTask
    ...
    def CheckLibInstall(self, checks):
        for cFile in checks:
            if os.path.exists(cFile):
                return public.GetMsg("Already installed")
    ...
    def GetLibOpt(self, status, libName):
        ...
        if status == public.GetMsg("Not installed"):
            optStr = (
                '<a class="link" href="javascript:InstallLib(\''
                + libName
                + "');\">"
                + public.GetMsg("Uninstallaton succeeded")
                + "</a>"
            )
        else:
            libConfig = public.GetMsg("Old configuration")
            if libName == "beta":
                libConfig = public.GetMsg("Beta tester profile")

            optStr = (
                '<a class="link" href="javascript:SetLibConfig(\''
                + libName
                + "');\">"
                + libConfig
                + '</a> | <a class="link" href="javascript:UninstallLib(\''
                + libName
                + "');\">"
                + public.lang("Uninstallaton succeeded")
                + "</a>"
            )
        return optStr
    ...
    def GetQiniuAS(self, get):
        filename = (
            public.GetConfigValue("setup_path") + "/panel/data/" + get.name + "As.conf"
        )
    ...
    def SetQiniuAS(self, get):
        ...
        conf = (
            get.access_key.strip()
            + "|"
            + get.secret_key.strip()
            + "|"
            + get.bucket_name.strip()
            + "|"
            + get.bucket_domain.strip()
        )
        public.writeFile(filename, conf)
        if not os.path.exists(filename):
        ...
        if result[0].find("ERROR:") == -1:
            public.write_log_gettext(
                "Plugin manager", "Set plugin [" + info["name"] + "]AS!"
            )
            return public.return_msg_gettext(True, public.lang("Successfully set"))
        return public.return_msg_gettext(
            False,
            "ERROR: Unable to connect to the {} server, please check if the [AK/SK/Storage] setting is correct!",
            (info["name"],),
        )
    ...
    def SetBeta(self, get):
        ...
        if data["status"]:
            public.writeFile(
                "data/beta.pl", get.bbs_name + "|" + get.qq + "|" + get.email
            )
        return data
    ...
    def GetQiniuFileList(self, get):
        try:
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Failed to get the list, please check if the [AK/SK/Storage] setting is correct!"
                ),
            )
    ...
    def KillProcess(self, get):
        ...
        if name == "python":
            return public.return_msg_gettext(
                False, public.lang("Error, cannot end task processes!")
            )

        p.kill()
        public.write_log_gettext(
            "Task manager", "Ended processes[{}][{}] Successfully!", (get.pid, name)
        )
        return public.return_msg_gettext(
            True, "Ended processes[{}][{}] Successfully!", (get.pid, name)
        )
    ...
    def GetNetWorkIo(self, get):
        ...
        data = (
            public.M("network")
            .dbfile("system")
            .where("addtime>=? AND addtime<=?", (get.start, get.end))
    ...
    def GetDiskIo(self, get):
        ...
        __OPT_FIELD = "*"
        tmp_cols = (
            public.M("diskio").dbfile("system").query("PRAGMA table_info(diskio)", ())
        )
        cols = []
        for col in tmp_cols:
            if len(col) > 2:
        ...
        data = (
            ...
            .dbfile("system")
            .query(
                "SELECT diskio.*,process_top_list.disk_top from diskio inner join process_top_list on diskio.addtime=process_top_list.addtime where diskio.addtime>={} AND diskio.addtime<={} ORDER BY diskio.addtime desc;".format(
                    get.start, get.end
                ),
                (),
            )
        ...
        if (
            isinstance(data, str)
            and data.find("error: no such table: process_top_list") != -1
        ):
            return (
                public.M("diskio")
                .dbfile("system")
    ...
    def GetCpuIo(self, get):
        ...
        __OPT_FIELD = "*"
        tmp_cols = (
            public.M("cpuio").dbfile("system").query("PRAGMA table_info(cpuio)", ())
        )
        cols = []
        for col in tmp_cols:
            if len(col) > 2:
        ...
        data = (
            ...
            .dbfile("system")
            .query(
                "SELECT cpuio.*,process_top_list.cpu_top,process_top_list.memory_top from cpuio inner join process_top_list on cpuio.addtime=process_top_list.addtime where cpuio.addtime>={} AND cpuio.addtime<={} ORDER BY cpuio.addtime desc;".format(
                    get.start, get.end
                ),
                (),
            )
    ...
    def get_load_average(self, get):
        __OPT_FIELD = "*"
        tmp_cols = (
            public.M("load_average")
            .dbfile("system")
            .query("PRAGMA table_info(load_average)", ())
        )
        cols = []
        for col in tmp_cols:
            if len(col) > 2:
        ...
        data = (
            ...
            .dbfile("system")
            .query(
                "SELECT load_average.*,process_top_list.cpu_top from load_average inner join process_top_list on load_average.addtime=process_top_list.addtime where load_average.addtime>={} AND load_average.addtime<={} ORDER BY load_average.addtime desc;".format(
                    get.start, get.end
                ),
                (),
            )
    ...
    def get_process_tops(self, get):
        ...
        """
        data = (
            public.M("process_tops")
            .dbfile("system")
            .where("addtime>=? AND addtime<=?", (get.start, get.end))
            .field("id,process_list,addtime")
            .order("id asc")
            .select()
        )
    ...
    def get_process_cpu_high(self, get):
        ...
        """
        data = (
            public.M("process_high_percent")
            .dbfile("system")
            .where("addtime>=? AND addtime<=?", (get.start, get.end))
            .field("id,name,pid,cmdline,cpu_percent,memory,cpu_time_total,addtime")
            .order("id asc")
            .select()
        )
        return self.ToAddtime(data)
                ...
                try:
                    if types:
                        ...
                        if "memory_top" in data[i]:
                            data[i]["memory_top"] = json.loads(data[i]["memory_top"])
    ...
    def apple_beta(self, get):
        try:
            ...
            try:
                ...
                return public.return_msg_gettext(
                    False, public.lang("Fail to connect to the server!")
                )
        except:
            return public.return_msg_gettext(
                False, public.lang("Please bind your account first!")
            )
    ...
    def to_beta(self):
        try:
            ...
            p_data["username"] = userInfo["username"]
            public.HttpPost(
                public.GetConfigValue("home") + "/api/panel/to_beta", p_data, 5
            )
        except:
            pass
    ...
    def get_beta_logs(self, get):
        try:
            data = json.loads(
                public.HttpGet(
                    "{}/api/panel/getBetaVersionLogs".format(self.__official_url)
                )
            )
            return data
        except:
        ...
        try:
            if not public.IsRestart():
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Please run the program when all install tasks finished!"
                    ),
                )
            import json

            conf_status = (
                public.M("config").where("id=?", ("1",)).field("status").find()
            )
            if (
                int(session["config"]["status"]) == 0
                and int(conf_status["status"]) == 0
            ):
                public.arequests(
                    "get",
                    "{}/api/setupCount/setupPanel?type=Linux".format(
                        self.__official_url
                    ),
                )
            ...
            if "updateInfo" in session and hasattr(get, "check") == False:
                ...
                data["system"] = (
                    panelsys.GetSystemVersion()
                    + "|"
                    + str(mem.total / 1024 / 1024)
                    + "MB|"
                    + str(public.getCpuType())
                    + "*"
                    + str(psutil.cpu_count())
                    + "|"
                    + str(public.get_webserver())
                    + "|"
                    + session["version"]
                )
            ...
            if (
                ...
                if os.path.getsize("panel.zip") < 1048576:
                    return public.return_msg_gettext(
                        False,
                        public.lang(
                            "File download failed, please try again or update manually!"
                        ),
                    )
                cmd = "unzip -o panel.zip -d {}/ && chmod 700 {}/panel/BT-Panel".format(
                    setupPath, setupPath
                )
                ...
                return public.return_msg_gettext(
                    True, "Successful to update to {}", (updateInfo["version"],)
                )
    ...
    def delClose(self, get):
        ...
        public.add_security_logs(
            "Clear the log",
            "The number of log entries cleared is:{}".format(public.M("logs").count()),
        )
        # 清空日志
    ...
    def change_phpmyadmin_ssl_port(self, get):
        ...
        try:
            ...
            return public.return_msg_gettext(
                False, public.lang("Please enter the correct port number")
            )
        for i in ["nginx", "apache"]:
            ...
            if get.port in rulePort:
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Please do NOT use the usual port as the phpMyAdmin port!"
                    ),
                )
            if i == "nginx":
            ...
            if i == "apache":
                ...
                public.write_log_gettext(
                    "Software manager",
                    "Modified access port to {} for phpMyAdmin!",
                    (get.port,),
                )
    ...
    def set_phpmyadmin_ssl(self, get):
        ...
        if not os.path.exists("/www/server/panel/ssl/certificate.pem"):
            return public.return_msg_gettext(
                False,
                public.lang(
                    "The panel certificate does not exist. Please apply for the panel certificate and try again."
                ),
            )
        if get.v == "1":
            # 获取auth信息
...
{}
       ...
       DirectoryIndex index.php index.html index.htm default.php default.html default.htm
        ...
        return public.return_msg_gettext(
            True,
            public.lang(
                "Open successfully, please manually release phpmyadmin ssl port"
            ),
        )
    ...
    def setPHPMyAdmin(self, get):
        ...
        if hasattr(get, "phpversion"):
            if public.get_webserver() == "nginx":
                filename = (
                    public.GetConfigValue("setup_path") + "/nginx/conf/enable-php.conf"
                )
                conf = public.readFile(filename)
                rep = r"(unix:/tmp/php-cgi.*\.sock|127.0.0.1:\d+)"
                conf = re.sub(
                    rep, public.get_php_proxy(get.phpversion, "nginx"), conf, 1
                )
            elif public.get_webserver() == "apache":
                rep = r"(unix:/tmp/php-cgi.*\.sock\|fcgi://localhost|fcgi://127.0.0.1:\d+)"
                conf = re.sub(
                    rep, public.get_php_proxy(get.phpversion, "apache"), conf, 1
                )
            else:
            ...
            public.write_log_gettext(
                "Software manager",
                "Modified PHP runtime version to PHP-{} for phpMyAdmin!",
                (get.phpversion,),
            )
            return public.return_msg_gettext(True, public.lang("Setup successfully!"))
        ...
        deny all""".format(
                ...
                sub_string = """{}
        Deny from all
        Allow from 127.0.0.1 ::1 localhost""".format(
                    src_string, pma_path
                )
                if conf.find(sub_string) != -1:
            ...
            public.serviceReload()
            public.write_log_gettext(
                "Software manager", "phpMyAdmin already {}!", (msg,)
            )
    ...
    def GetWarning(self, get):
        ...
        if not os.path.exists(warningFile):
            return public.return_msg_gettext(
                False, public.lang("Warning list does NOT exist!")
            )
        import json, time
    ...
    def GetRedisStatus(self, get):
        ...
        data = public.ExecShell(
            "/www/server/redis/src/redis-cli -p " + port + password + " info"
        )[0]
        res = [
            "tcp_port",
    ...
    def check_user_auth(self, get):
        ...
        try:
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Account binding has expired, please re-bind on the [Settings] page!"
                ),
            )
        ...
        resp = public.HttpPost.post(
            "{}/api/user/verifyToken".format(self.__official_url),
            headers=url_headers,
            verify=False,
        )
        resp = resp.json()
    ...
    def php_info(self, args):
        ...
        if (
            not os.path.exists("/etc/redhat-release")
            and public.get_webserver() == "openlitespeed"
        ):
            php_ini = (
                php_path
                + php_version
        ...
        tmp = public.ExecShell(
            php_bin + " -c {} /www/server/panel/class/php_info.php".format(php_ini)
        )[0]
        ...
        result["phpinfo"]["keys"] = {
            "1cache": "Buffer",
            "2crypt": "Encryption and decryption library",
            "0db": "Database-driven",
            "4network": "Network Communication Library",
            "5io_string": "File and string processing libraries",
            "3photo": "Image processing library",
            "6other": "Other third-party libraries",
        }
    ...
    def get_lines(self, args):
        if not os.path.exists(args.filename):
            return public.returnMsg(False, public.lang("Logs emptied"))
    ...
    def get_pay_type(self, get):
        ...
        plu_panel = panelPlugin.panelPlugin()
        plugin_list = plu_panel.get_cloud_list()
        if not "pro" in plugin_list:
            plugin_list["pro"] = -1

        for item in data:
            if "list" in item:
                item["list"] = self.__get_home_list(
                    item["list"], item["type"], plugin_list, plu_panel
                )
                if item["type"] == 1:
                    if len(item["list"]) > 4:
            ...
            {"type": -1},
            {
                ...
                "list": [
                    ...
                    },
                    {
                        ...
                        "pluginType": "pro",
                        "eventList": [
                            {"event": "WebsiteReport('$siteName')", "version": "5.0"}
                        ],
                    },
                    {
                        "title": "统计",
    ...
    def ignore_version(self, get):
        ...
        return public.return_msg_gettext(
            True,
            public.lang(
                "Ignore success, this version will no longer be reminded to update."
            ),
        )
