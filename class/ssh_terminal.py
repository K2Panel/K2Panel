...
class ssh_terminal:
    ...
    def __init__(self):
        ...
        if (
            ...
            .where("type=? AND name=?", ("table", "ssh_login_record"))
            .count()
        ):
            public.M("").execute(
                """CREATE TABLE ssh_login_record (
            ...
            public.M("").execute(
                "CREATE INDEX ssh_login_record ON ssh_login_record (addr);"
            )
        self.time = time.time()

    def record(self, rtype, data):
        if (
            os.path.exists(public.get_panel_path() + "/data/open_ssh_login.pl")
            and self._video_addr
        ):
    ...
    def connect(self):
        ...
        if not self._host:
            return public.return_msg_gettext(
                False, public.lang("Wrong connection address")
            )
        ...
        while num < 5:
            ...
            try:
                ...
                if num == 5:
                    ...
                    return returnMsg(
                        False, "Connection failure: {}", (self._host, self._port)
                    )
                else:
                    time.sleep(0.2)
        ...
        try:
            ...
            if not self._pass and not self._pkey:
                ...
                return public.return_msg_gettext(
                    False,
                    "Password or private key cannot both be empty: {}:{}",
                ...
                try:
                    if self._key_passwd:
                        pkey = paramiko.RSAKey.from_private_key(
                            p_file, password=self._key_passwd
                        )
                    else:
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        return public.returnMsg(
                                            False,
                                            public.lang(
                                                "Incorrect private key password:{}", ex
                                            ),
                                        )
                                    elif (
                                        ex.find("encountered RSA key, expected DSA key")
                                        != -1
                                    ):
                                        pkey = paramiko.RSAKey.from_private_key(
                                            p_file, password=self._key_passwd
                                        ...
                                        return public.returnMsg(
                                            False,
                                            public.lang("Private key error: {}", ex),
                                        )
                            else:
                                pkey = paramiko.DSSKey.from_private_key(p_file)
                if not pkey:
                    return public.returnMsg(False, public.lang("Private key error!"))
            ...
            if e.find("websocket error!") != -1:
                return public.return_msg_gettext(
                    True, public.lang("connection succeeded")
                )
            ...
            if e.find("Authentication failed") != -1:
                ...
                if self._key_passwd:
                    ...
                    if sshd_config and sshd_config.find("ssh-dss") == -1:
                        return returnMsg(
                            False,
                            "The private key verification fails, the private key may be incorrect, or the ssh-dss private key authentication type may not be enabled in the /etc/ssh/sshd_config configuration file",
                        )
                    return returnMsg(
                        False,
                        "Authentication failed, please check whether the private key is correct: {}".format(
                            e
                            + ","
                            + self._user
                            + "@"
            ...
            if e.find("Bad authentication type; allowed types") != -1:
                ...
                return public.return_msg_gettext(
                    False, "Unsupported authentication type: {}", (str(e))
                )
            if e.find("Connection reset by peer") != -1:
                self.debug(
                    public.lang("The target server actively refused the connection")
                )
                return public.return_msg_gettext(
                    False,
                    public.lang("The target server actively refused the connection"),
            ...
            if e.find("Error reading SSH protocol banner") != -1:
                ...
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "The protocol header response timed out, and the network quality with the target server was too bad: {}",
                        str(e),
                    ),
                )
            if e.find("encountered RSA key, expected DSA key") != -1:
                ...
                return public.return_msg_gettext(
                    False,
                    public.lang("Private keys may require password access: {}", str(e)),
                )
            ...
            if not e:
                ...
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "The SSH protocol handshake timed out, and the network quality with the target server is too bad"
                    ),
                )
            err = public.get_error_info()
            self.debug(err)
            return public.return_msg_gettext(
                False, public.lang("unknown error: {}", str(err))
            )

        self.debug(
            public.lang(
                "The authentication is successful and the session channel is being constructed"
            )
        )
        ...
        public.write_log_gettext(
            self._log_type,
            "Successfully logged in to the SSH server [{}:{}]",
            (self._host, str(self._port)),
        )
        ...
        self._video_addr = (
            "/www/server/panel/plugin/jumpserver/static/video/%s.json"
            % str(int(self._connect_time))
        )
        ...
        if self._host in ["127.0.0.1", "localhost"]:
            try:
                ...
                if os.path.exists(s_file):
                    ...
                    self._old_conf = True
                    return
                ssh_key_type_file = "{}/data/ssh_key_type.pl".format(
                ...
                if os.path.exists(ssh_key_type_file):
                    ...
                    if ssh_key_type_new:
                        ssh_key_type = ssh_key_type_new.strip()
                ...
                if not self._pass or not self._pkey or not self._user:
                    ...
                    id_rsa_file = [
                        ...
                        home_path + "/.ssh/id_ecdsa",
                        home_path + "/.ssh/id_rsa",
                        home_path + "/.ssh/id_rsa_bt",
                    ]
                    if ssh_key_type:
                        id_rsa_file.insert(
                            0, home_path + "/.ssh/id_{}".format(ssh_key_type)
                        )
                    for ifile in id_rsa_file:
                        if os.path.exists(ifile):
                            self._pkey = public.readFile(ifile)
            ...
            while self._ws.connected:
                ...
                if not resp_line:
                    if not self._tp.is_active():
                        ...
                        self._ws.send(
                            public.lang(
                                "The connection is disconnected, press enter to try to reconnect!"
                            )
                        )
                        self.close()
                        return
    ...
    def send(self):
        ...
        try:
            ...
            if ex.find("_io.BufferedReader") != -1:
                self.debug(
                    public.lang(
                        "An error occurred while reading data from websocket. Retrying"
                    )
                )
                self.send()
                return
                ...
                self.debug(
                    public.get_msg_gettext(
                        "An error occurred while writing data to the buffer: {}",
                        (str(ex),),
                    )
                )
    ...
    def history_recv(self, recv_data):
        ...
        if self._last_cmd_tip == 1:
            if not recv_data.startswith("\r\n"):
                self._last_cmd += (
                    recv_data.replace("\u0007", "").replace("\x07", "").strip()
                )
    ...
    def debug(self, msg):
        ...
        """
        msg = "{} - {}:{} => {} \n".format(
            public.format_date(), self._host, self._port, msg
        )
...
class ssh_host_admin(ssh_terminal):
    ...
    def get_host_find(self, args):
        ...
        if not os.path.exists(info_file):
            return public.return_msg_gettext(
                False, public.lang("The specified SSH information does not exist!")
            )
    ...
    def modify_host(self, args):
        ...
        if args.host != args.new_host:
            ...
            if os.path.exists(info_file):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "The specified host address has been added to other SSH information!"
                    ),
                )
        ...
        public.write_log_gettext(
            self._log_type, "Modify the SSH information of HOST: {}", (args.host,)
        )
    ...
    def create_host(self, args):
        ...
        self.save_ssh_info(args.host, host_info)
        public.write_log_gettext(
            self._log_type, "Add the SSH information of HOST: {}", (str(args.host),)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def remove_host(self, args):
        ...
        public.write_log_gettext(
            self._log_type, "Delete the SSH information of HOST: {}", (str(args.host),)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def set_sort(self, args):
        ...
        if not "sort_list" in args:
            return public.return_msg_gettext(
                False, public.lang("Please pass in the [sort_list] field")
            )
    ...
    def create_command(self, args):
        ...
        if self.command_exists(command, args.title):
            return public.return_msg_gettext(
                False, public.lang("The specified command name already exists")
            )
        ...
        public.write_log_gettext(
            self._log_type, "Add common commands [{}]", (str(args.title),)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def get_command_find(self, args=None, title=None):
        ...
        for cmd in command:
            if cmd["title"] == title or cmd["title"] == args.title:
                return cmd
        return public.return_msg_gettext(
            False, public.lang("The specified command does not exist")
        )
    ...
    def modify_command(self, args):
        ...
        self.save_command(command)
        public.write_log_gettext(
            self._log_type, "Modify common commands [{}]", (str(args.title),)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def remove_command(self, args):
        ...
        self.save_command(command)
        public.write_log_gettext(
            self._log_type, "Delete common commands [{}]", (str(args.title),)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
