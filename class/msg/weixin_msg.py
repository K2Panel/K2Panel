...
class weixin_msg:
    ...
    def get_config(self, get):
        ...
        if self.__weixin_info:
            ...
            data["list"]["default"] = {
                "title": title,
                "data": data["weixin_url"],
                "state": self.__weixin_info["state"],
            }
    ...
    def set_config(self, get):
        ...
        if hasattr(get, "title"):
            ...
            if len(title) > 7:
                return public.returnMsg(
                    False, public.lang("Note name cannot exceed 7 characters")
                )
        ...
        if not self.__weixin_info:
            self.__weixin_info = {}
        if not "list" in self.__weixin_info:
            self.__weixin_info["list"] = {}
        ...
        try:
            info = public.get_push_info(
                "Message channel configuration reminder",
                [">configuration status: <font color=#20a53a>Success</font>\n\n"],
            )
        ...
        if ret["status"]:
            ...
            if ret["success"] <= 0:
                return public.returnMsg(
                    False,
                    public.lang(
                        "Failed to add, please check whether the URL is correct"
                    ),
                )
            ...
            return public.returnMsg(
                False,
                public.lang("Failed to add, please check whether the URL is correct"),
            )
    ...
    def send_msg(self, msg, to_user="default"):
        ...
        if not self.__weixin_info:
            return public.returnMsg(
                False, public.lang("Information is not configured correctly.")
            )
        ...
        if "state" in self.__weixin_info and self.__weixin_info["state"] == 0:
            return public.returnMsg(
                False,
                public.lang(
                    "Notifications have been turned off, please turn them on and try again."
                ),
            )
        ...
        ret = public.returnMsg(
            True,
            "Send completed, send successfully {}, send failed {}.".format(
                success, error
            ),
        )
        ret["success"] = success
    ...
    def _write_log(self, module, msg, res):
        ...
        log = "[{}] sent to {}, sending content: [{}]".format(
            module, user, public.xsssec(msg)
        )
