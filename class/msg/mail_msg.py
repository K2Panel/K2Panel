...
class mail_msg:
    ...
    def __init__(self):
        ...
        if os.path.exists("data/stmp_mail.json"):
            self.__mail_config["send"] = json.loads(
                public.readFile(self.__mail_send_conf)
            )

        if os.path.exists("data/mail_list.json"):
            self.__mail_config["receive"] = json.loads(
                public.readFile(self.__mail_receive_conf)
            )

        if not "send" in self.__mail_config:
            self.__mail_config["send"] = {}
        if not "receive" in self.__mail_config:
            self.__mail_config["receive"] = {}

        if (
            "qq_mail" not in self.__mail_config["send"]
            or "qq_stmp_pwd" not in self.__mail_config["send"]
            or "hosts" not in self.__mail_config["send"]
        ):
            self.__mail_config = None
        ...
        if self.__mail_config:
            ...
            if "receive" in data:
                if not os.path.exists(self.__mail_receive_conf):
                    public.writeFile(
                        self.__mail_receive_conf, json.dumps(data["receive"])
                    )
    ...
    def set_config(self, get):
        ...
        if hasattr(get, "send"):
            if (
                not hasattr(get, "qq_mail")
                or not hasattr(get, "qq_stmp_pwd")
                or not hasattr(get, "hosts")
                or not hasattr(get, "port")
            ):
                return public.returnMsg(
                    False, public.lang("Please fill in the complete information")
                )
            mail_config = {
        ...
        if ret["status"]:
            ...
            if ret["success"] <= 0:
                return public.returnMsg(
                    False,
                    public.lang(
                        "Sending failed, please check whether the sender configuration or receiver information is correct."
                    ),
                )

            public.writeFile(
                self.__mail_send_conf, json.dumps(self.__mail_config["send"])
            )
            public.writeFile(
                self.__mail_receive_conf, json.dumps(self.__mail_config["receive"])
            )
    ...
    def send_msg(self, msg, title="K2Panel panel message push", to_email=None):
        ...
        if not self.__mail_config:
            return public.returnMsg(
                False, public.lang("Email information is not configured correctly.")
            )

        if not "port" in self.__mail_config["send"]:
            self.__mail_config["send"]["port"] = 465
        ...
        for email in receive_list:
            ...
            try:
                ...
                data["From"] = formataddr(
                    [
                        self.__mail_config["send"]["qq_mail"],
                        self.__mail_config["send"]["qq_mail"],
                    ]
                )
                data["To"] = formataddr(
                    [self.__mail_config["send"]["qq_mail"], email.strip()]
                )
                data["Subject"] = title
                if int(self.__mail_config["send"]["port"]) == 465:
                    server = smtplib.SMTP_SSL(
                        str(self.__mail_config["send"]["hosts"]),
                        str(self.__mail_config["send"]["port"]),
                    )
                else:
                    server = smtplib.SMTP(
                        str(self.__mail_config["send"]["hosts"]),
                ...
                server.login(
                    self.__mail_config["send"]["qq_mail"],
                    self.__mail_config["send"]["qq_stmp_pwd"],
                )
                server.sendmail(
                    self.__mail_config["send"]["qq_mail"],
                    [
        ...
        try:
            if res:
                ...
                public.WriteLog(
                    "Alarm notification",
                    "[Email] No sender configured, please configure first.",
                )
        except:
            pass
        ...
        result = public.returnMsg(
            True,
            "The sending is complete, a total of [{}] items were sent, [{}] succeeded, and [{}] failed.".format(
                total, sucess, error
            ),
        )
        result["list"] = ret_msg
