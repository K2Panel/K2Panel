...
class wx_account_msg:
    ...
    def get_config(self, get):
        ...
        if os.path.exists(self.conf_path):
            ...
            title = "默认"
            if "res" in data and "nickname" in data["res"]:
                title = data["res"]["nickname"]
    ...
    def get_web_info(self, get):
        if self.user_info is None:
            return public.returnMsg(
                False, public.lang("The user binding information was not obtained")
            )
        url = "https://wafapi2.binarjoinanalyticnl.nl/api/v2/user/wx_web/info"
        data = {
        ...
        try:
            ...
            public.WriteFile(
                ...
                json.dumps(
                    {
                        "success": False,
                        "res": "The link to the cloud failed, please check the network,请检查网络",
                    }
                ),
            )
            return public.returnMsg(
                False,
                public.lang("The link to the cloud failed, please check the network"),
            )
        ...
        try:
            ...
            public.WriteFile(
                ...
                json.dumps(
                    {
                        "success": False,
                        "res": "The link to the cloud failed, please check the network",
                    }
                ),
            )
            return public.returnMsg(
    ...
    def get_auth_url(self, get):
        ...
        url = "https://wafapi2.binarjoinanalyticnl.nl/api/v2/user/wx_web/get_auth_url"
        data = {
            "uid": self.user_info["uid"],
            "access_key": "B" * 32,
            "serverid": self.user_info["server_id"],
    ...
    def send_msg(self, msg):
        ...
        if self.user_info is None:
            return public.returnMsg(
                False, public.lang("No user information was obtained")
            )

        msg, title = self.get_send_msg(msg)
        url = "https://wafapi2.binarjoinanalyticnl.nl/api/v2/user/wx_web/send_template_msg_v2"
        datassss = {
            "first": {
                "value": "堡塔主机告警",
            },
        ...
        data = {
            ...
            "data": base64.b64encode(json.dumps(datassss).encode("utf-8")).decode(
                "utf-8"
            ),
        }
        ...
        try:
            ...
            result = public.returnMsg(
                True, "Send complete.Send success :{}, send failure :{}", success, error
            )
            ...
            return public.returnMsg(
                False,
                public.lang(
                    "WeChat message delivery failed. --> {}", public.get_error_info()
                ),
            )
