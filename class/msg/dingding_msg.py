...
class dingding_msg:
    ...
    def __init__(self):
        try:
            ...
            if (
                not "dingding_url" in self.__dingding_info
                or not "isAtAll" in self.__dingding_info
                or not "user" in self.__dingding_info
            ):
    ...
    def set_config(self, get):
        ...
        self.__dingding_info = {
            "dingding_url": get.url.strip(),
            "isAtAll": atall,
            "user": user,
            "title": title,
        }
    ...
    def get_send_msg(self, msg):
        ...
        try:
            ...
            if msg.find("####") >= 0:
                ...
                info = public.get_push_info(
                    "Alarm Mode Configuration Reminder", [">Send Content: " + msg]
                )
                msg = info["msg"]
        except:
            pass
    ...
    def send_msg(self, msg, to_user="default"):
        ...
        if not self.__dingding_info:
            return public.returnMsg(
                False, public.lang("DingTalk information is incorrectly configured.")
            )
        ...
        if isinstance(self.__dingding_info["user"], int):
            return public.returnMsg(
                False,
                public.lang(
                    "DingTalk configuration error, please reconfigure the DingTalk robot."
                ),
            )

        at_info = ""
        for user in self.__dingding_info["user"]:
            if re.match("^[0-9]{11,11}$", str(user)):
                at_info += "@" + user + " "
        ...
        data = {
            "msgtype": "markdown",
            "markdown": {"title": "server notification", "text": msg},
            "at": {
