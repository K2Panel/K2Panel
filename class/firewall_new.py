...
class firewalls:
    ...
    def GetList(self, get=None):
        try:
            ...
            for i in range(len(data["ports"])):
                #
                tmp = self.CheckDbExists(
                    data["ports"][i]["port"], data["ports"][i]["protocol"]
                )
                # | id | port  | ps  | addtime   | ports | protocol | address_ip | types |
                if not tmp:
                    public.M("firewall").add(
                        "port,ps,addtime", (data["ports"][i]["port"], "", addtime)
                    )
            ...
            data["data"] = (
                public.M("firewall")
                .limit(data["page"]["shift"] + "," + data["page"]["row"])
                .order("id desc")
                .select()
            )
            for i in range(len(data["data"])):
                if (
                    ...
                    data["data"][i]["status"] = public.check_port_stat(
    ...
    def CheckDbExists2(self, protocol, type, port, address):
        ...
        for dt in data:
            if (
                dt["ports"] == port
                and dt["protocol"] == protocol
                and dt["types"] == type
                and dt["address_ip"] == address
            ):
                return dt
        return False
    ...
    def AddDropAddress(self, get):
        ...
        if not re.search(rep, get.port):
            return public.return_msg_gettext(
                False, public.lang("IP address youve entered is illegal!")
            )
        address = get.port
        ...
        public.write_log_gettext(
            "Firewall manager", "Successfully blocked IP [{}]!", (address,)
        )
        addtime = time.strftime("%Y-%m-%d %X", time.localtime())
    ...
    def AddAcceptPort(self, get):
        ...
        if types not in type_list:
            return public.return_msg_gettext(
                False, public.lang("The port exists, no need to repeat the release!")
            )
        ...
        if types == "tcp":
            if flag:
                if public.M("firewall").where("port=?", (port,)).count() > 0:
                    return public.return_msg_gettext(
                        False,
                        public.lang("The port exists, no need to repeat the release!"),
                    )
            else:
                if (
                    public.M("firewall").where("port=? and type='tcp'", (port,)).count()
                    > 0
                ):
                    return public.return_msg_gettext(
                        False,
            ...
            if flag:
                if public.M("firewall").where("port=?", (port,)).count() > 0:
                    return public.return_msg_gettext(
                        False, "The port exists, no need to repeat the release!"
                    )
            else:
                if (
                    public.M("firewall").where("port=? and type='udp'", (port,)).count()
                    > 0
                ):
                    return public.return_msg_gettext(
                        False,
        ...
        public.write_log_gettext(
            "Firewall manager", "Successfully accepted port [{}]!", (port,)
        )
        addtime = time.strftime("%Y-%m-%d %X", time.localtime())
        result = public.M("firewall").add(
            "port,ps,addtime,types", (port, ps, addtime, types)
        )
        # return result
        self.FirewallReload()
    ...
    def DelAcceptPort(self, get):
        ...
        try:
            if port == public.GetHost(True):
                return public.return_msg_gettext(
                    False,
                    public.lang("Failed，cannot delete current port of the panel!"),
                )
            if self.__isUfw:
            ...
            public.write_log_gettext(
                "Firewall manager",
                "Successfully deleted accepted port [{}] on firewall!",
                (port,),
            )
            public.M("firewall").where("id=?", (id,)).delete()
    ...
    def SetSshPort(self, get):
        ...
        if int(port) < 22 or int(port) > 65535:
            return public.return_msg_gettext(
                False, public.lang("Port range must be between 22 and 65535!")
            )
        ...
        self.FirewallReload()
        public.M("firewall").where("ps=?", (public.lang("SSH Server"),)).setField(
            "port", port
        )
        public.write_log_gettext(
            "Firewall manager", "Successfully changed SSH port to [{}]!", (port,)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def AddSpecifiesIp(self, get):
        ...
        if not re.search(rep2, get.address):
            return public.return_msg_gettext(
                False, public.lang("IP address is illegal!")
            )
        import time
        ...
        query_result = (
            ...
            .where(
                ...
                (ports, address_ip, protocol, types),
            )
            .count()
        )
        # 这里大于0 表示存在
        ...
        if self.__isUfw:
            ...
            if self.__isFirewalld:
                port = ports.replace(":", "-")
                self.__Obj.Add_Port_IP(
                    port=ports, address=address_ip, pool=protocol, type=types
                )
            else:
                if type == "accept":
                    public.ExecShell(
        ...
        public.write_log_gettext(
            "Firewall manager", "Successfully accepted port [{}]!", (ports,)
        )
        addtime = time.strftime("%Y-%m-%d %X", time.localtime())
        result = public.M("firewall").add(
    ...
    def DelSpecifiesIp(self, get):
        ...
        if protocol not in protocol_list:
            return public.return_msg_gettext(
                False, public.lang("Specified protocol does NOT exist!")
            )
        if self.__isUfw:
            if type == "accept":
                public.ExecShell(
                    "ufw delete allow proto "
                    + protocol
                    + " from "
                    + address_ip
                    + " to any port "
