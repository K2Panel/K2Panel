...
class main(dataBase):
    ...
    def get_data_list(self, get):
        try:
            ...
            table_header = config().get_table_header(
                public.to_dict_obj({"table_name": "mysqlTableColumn"})
            )
            if not table_header:
                if table_header["mysqlTableColumn"] == "":
                    table_header["mysqlTableColumn"] = "{}"
                table_header["mysqlTableColumn"] = json.loads(
                    table_header["mysqlTableColumn"]
                )
                table_header = [
                    i["title"] if "title" in i else i["label"]
                    for i in table_header["mysqlTableColumn"]
                    if (
            ...
            if user_Data != "all":
                data_list = [
                    i
                    for i in data_list
                    if str(i["id"]) in user_Data.get("databases", [])
                ]
            # 用户权限处理结束

            [
                ids.append(str(val["id"])) or names.append(val["name"])
    ...
    def get_database_backup_count(self, ids):
        ...
        """
        try:
            res = public.M("backup").query(
                "SELECT pid,COUNT(*) as count FROM backup WHERE pid in ({}) AND type='1' GROUP BY pid".format(
                    ",".join(ids)
                )
            )
