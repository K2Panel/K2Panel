...
class main(dataBase):
    ...
    def get_data_list(self, get):
        ...
        if user_Data != "all" and table in ["sites"]:
            get.data_list = [
                i for i in get.data_list if str(i["id"]) in user_Data.get(table, [])
            ]
        # 用户权限处理结束
    ...
    def get_page_data(self, get, where_sql, params, result="1,2,3,4,5,8"):
        ...
        if "search_key" in get and get["search_key"]:
            data["search_history"] = public.get_search_history(
                get.table, get["search_key"]
            )
        return data
    ...
    def get_sort_data(self, data):
        ...
        if "plist" in data:
            ...
            data["data"].sort(
                key=lambda x: (x.get("sort", 0) == sort_reverse, x[sort_key]),
                reverse=reverse,
            )
    ...
    def _get_search_where(self, table, search):
        ...
        if search:
            ...
            wheres = {
                ...
                "backup": ("pid=?", (search,)),
                "users": (
                    "id='?' OR username=?{}".format(conditions),
                    (search, search),
                ),
                "domain": (
                    "pid=? OR name LIKE ?{}".format(conditions),
                    (search, "%" + search + "%"),
                ),
