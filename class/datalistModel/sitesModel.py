...
class main(dataBase):
    ...
    def __init__(self):
        ...
        self.default_quota = {
            "used": 0,
            "size": 0,
            "quota_push": {"size": 0, "used": 0},
            "quota_storage": {"size": 0, "used": 0},
        }
    ...
    def get_data_list(self, get):
        ...
        for val, key in self.custom_list.items():
            ...
            if key == "net":
                ...
                custom_conf[key] = {
                    i: self.get_site_net(site_netinfo, i) for i in names
                }
        ...
        for val in data_list:
            if "project_config" in val.keys():
                ...
                if (
                    "type" in val["project_config"].keys()
                    and "PHPMOD" == val["project_config"]["type"]
                ):
                    val = self.get_php_mod_status(val)
                    val["status"] = (
                        1 if realserver.daemon_status(val["name"])["status"] else 0
                    )
                    public.M(get.table).where("id=?", (val["id"],)).setField(
                        "status", val["status"]
                    )
            val["backup_count"] = (
                public.M("backup").where("pid=? and type=?", (val["id"], "0")).count()
            )
            val["domain"] = domain_obj.where("pid=?", (val["id"],)).count()
            val["ssl"] = self.get_site_ssl_info(val["name"])
            val["php_version"], val["php_version_status"] = self.get_php_version(
                val["name"]
            )
            if (
                "project_config" in val.keys()
                and "type" in val["project_config"].keys()
                and "PHPMOD" == val["project_config"]["type"]
            ):
                if (
                    "php_version" in val["project_config"].keys()
                    and len(val["project_config"]["php_version"]) > 1
                ):
                    val["php_version"] = (
                        val["project_config"]["php_version"][0]
                        + "."
                        + val["project_config"]["php_version"][1]
                    )
                    val["php_version_status"] = os.path.exists(
                        "/www/server/php/{}/bin/php".format(
                            val["project_config"]["php_version"]
                        )
                    )
            ...
            try:
                val["proxy"] = False
                proxy_list = self.site_obj.GetProxyList(
                    public.to_dict_obj({"sitename": val["name"]})
                )
                for i in proxy_list:
                    if i["type"] == 1:
                        val["proxy"] = True
            ...
            try:
                val["redirect"] = False
                redict_list = self.site_obj.GetRedirectList(
                    public.to_dict_obj({"sitename": val["name"]})
                )
    ...
    def get_header_list(self, get):
        ...
        try:
            ...
            table_header = config().get_table_header(
                public.to_dict_obj({"table_name": "phpTableColumn"})
            )
            if table_header["phpTableColumn"]:
                table_header["phpTableColumn"] = json.loads(
                    table_header["phpTableColumn"]
                )
                table_header = [
                    i["subtitle"] if "subtitle" in i else i["label"]
                    for i in table_header["phpTableColumn"]
                    if (i.get("isCustom", False) == True)
                    and ("subtitle" in i.keys() or "label" in i.keys())
                ]
            else:
    ...
    def get_php_version(self, siteName):
        try:
            ...
            conf = public.readFile(
                "{}/vhost/{}/{}.conf".format(spath, self.web_server, siteName)
            )
            ...
            return tmp[0][0] + "." + tmp[0][1], os.path.exists(
                "/www/server/php/{}/bin/php".format(tmp[0])
            )
        except:
            return "静态", True
    ...
    def get_search_where(self, get):
        ...
        nlist = (
            public.M("domain")
            .where("name LIKE ?{}".format(conditions), ("%{}%".format(get.search),))
            .field("pid")
            .select()
        )
        for val in nlist:
