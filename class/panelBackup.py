...
class backup:
    ...
    def __init__(self, cloud_object=None, cron_info={}):
        ...
        if (
            ...
            .where("type=? AND name=? AND sql LIKE ?", ("table", "backup", "%cron_id%"))
            .count()
        ):
            public.M("backup").execute(
                "ALTER TABLE 'backup' ADD 'cron_id' INTEGER DEFAULT 0", ()
            )
    ...
    def echo_end(self):
        print("=" * 90)
        print(
            "|-" + public.lang("Backup completed") + "[{}]".format(public.format_date())
        )
        print("=" * 90)
        print("\n")
    ...
    def GetDiskInfo2(self):
        ...
        temp = public.ExecShell(
            "df -T -P|grep '/'|grep -v tmpfs|grep -v 'snap/core'|grep -v udev|grep -v overlay"
        )[0]
        tempInodes = public.ExecShell(
            "df -i -P|grep '/'|grep -v tmpfs|grep -v 'snap/core'|grep -v udev|grep -v overlay"
        )[0]
        temp1 = temp.split("\n")
    ...
    def backup_path(self, spath, dfile=None, exclude=[], save=3, echo_id=None):
        ...
        if not dfile:
            fname = "path_{}_{}.tar.gz".format(
                dirname, public.format_date("%Y%m%d_%H%M%S")
            )
        ...
        if self._cloud:
            self.echo_info(
                public.lang("Uploading to {}, please wait ...", self._cloud._title)
            )
            if self._cloud.upload_file(dfile, "path"):
                self.echo_info(
                    public.lang("Successfully uploaded to {}", self._cloud._title)
                )
            else:
                ...
                self.send_failture_notification(
                    error_msg, target=f"{spath}|path", remark=remark
                )
                return False
        ...
        if not self._cloud:
            ...
            backups = (
                ...
                .where(
                    ...
                    (cron_id, "2", 0, spath, "%{}%".format(self._cloud._name)),
                )
                .field("id,name,filename")
                .select()
            )
    ...
    def delete_old(self, backups, save, data_type=None, site_name=None):
        if type(backups) == str:
            self.echo_info(
                public.lang("Failed to clean expired backup, error: {}", backups)
            )
            return
        self.echo_info(
            public.lang(
                "Keep the latest number of backups: {} copies",
                str(save),
            )
        )
        ...
        for i in range(len(backups)):
            if (
                data_type == "database" and backups[i]["name"][:3] == "db_"
            ):  # 数据库备份
                new_backups.append(backups[i])
            elif (
                data_type == "site"
                and backups[i]["name"][:4] == "web_"
                and backups[i]["name"][-7:] == ".tar.gz"
            ):  # 网站备份
                new_backups.append(backups[i])
            ...
            for backup in backups:
                ...
                if os.path.exists(backup["filename"]):
                    try:
                        ...
                        self.echo_info(
                            public.lang(
                                "Expired backup files have been cleaned from disk: {}",
                                backup["filename"],
                            )
                        )
                    except:
                ...
                if self._cloud:
                    ...
                    self.echo_info(
                        public.lang(
                            "Expired backup files have been cleaned from {}: {}",
                            self._cloud._title,
                            backup["name"],
                        )
                    )
    ...
    def backup_path_to(self, spath, dfile, exclude=[], siteName=None):
        if not os.path.exists(spath):
            self.echo_error(
                public.lang("The specified directory {} does not exist!", spath)
            )
            return False
        ...
        stime = time.time()
        self.echo_info(
            public.lang("Start compressing files: {}", public.format_date(times=stime))
        )
        if os.path.exists(dfile):
            os.remove(dfile)
        public.ExecShell(
            ...
            + " '"
            + dirname
            + "' 2>{err_log} 1> /dev/null".format(err_log=self._err_log)
        )
        ...
        try:
            ...
            if find["project_type"] == "WP2":
                try:
                    ...
                    self.echo_info(
                        public.lang("Backup wordpress [{}] successfully", siteName)
                    )
                    self.echo_end()
            ...
            spath = find["path"]
            pid = find["id"]
            fname = "web_{}_{}.tar.gz".format(
                siteName, public.format_date("%Y%m%d_%H%M%S")
            )
            ...
            if self._cloud:
                ...
                if self._cloud.upload_file(dfile, "site"):
                    ...
                    remark = "Backup to " + self._cloud._title
                    self.send_failture_notification(
                        error_msg, target=f"{siteName}|site", remark=remark
                    )
                    return False
            ...
            if not self._cloud:
                backups = (
                    public.M("backup")
                    .where(
                        "cron_id=? and type=? and pid=? and filename NOT LIKE '%|%'",
                        (cron_id, "0", pid),
                    )
                    .field("id,name,filename")
                    .select()
                ...
                backups = (
                    ...
                    .where(
                        ...
                        (cron_id, "0", pid, "%{}%".format(self._cloud._name)),
                    )
                    .field("id,name,filename")
                    .select()
                )
            ...
            self.send_failture_notification(
                "site {} {}".format(siteName, e), target=siteName
            )
    ...
    def backup_database_all(self, save=3, echo_id=None):
        databases = (
            public.M("databases").where("type=?", "MySQL").field("name").select()
        )
        ...
        for database in databases:
            ...
            self.save_backup_status(
                result, target=f"{database['name']}|database", msg=self._error_msg
            )
    ...
    def backup_site_all(self, save=3, echo_id=None):
        ...
        for site in sites:
            ...
            self.save_backup_status(
                result, target=f"{site['name']}|site", msg=self._error_msg
            )
    ...
    def backup_database(self, db_name, dfile=None, save=3, echo_id=None):
        try:
            ...
            if db_find["type"] != "MySQL":
                ...
                print(
                    "|-{}The database does not currently support backup".format(
                        db_find["type"]
                    )
                )
                return False
            ...
            if is_cloud_db:
                ...
                if not self._db_mysql or not self._db_mysql.set_host(
                    conn_config["db_host"],
                    ...
                    error_msg = "Failed to connect to remote database [{}:{}]".format(
                        conn_config["db_host"], conn_config["db_port"]
                    )
                    print(error_msg)
                    return False
            ...
            if p_size == None:
                error_msg = public.get_msg_gettext(
                    "The specified database [ {} ] has no data!", (db_name,)
                )
                self.echo_error(error_msg)
                self.send_failture_notification(error_msg, target=f"{db_name}|database")
                return False
            ...
            self.echo_info(
                public.get_msg_gettext("Database character set: {}", (character,))
            )
            disk_path, disk_free, disk_inode = self.get_disk_free(dfile)
            self.echo_info(
            ...
            if disk_path:
                if disk_free < p_size:
                    ...
                    self.echo_error(error_msg)
                    self.send_failture_notification(
                        error_msg, target=f"{db_name}|database"
                    )
                    return False
            ...
            self.echo_info(
                public.get_msg_gettext(
                    "Start exporting database: {}", (public.format_date(times=stime),)
                )
            )
            if os.path.exists(dfile):
                os.remove(dfile)
            ...
            self.echo_info(
                public.get_msg_gettext(
                    "Compression completed, took {} seconds, compressed package size: {}",
                    (str(compressed_time), str(public.to_size(gz_size))),
                )
            )
            if self._cloud:
                self.echo_info(
                    public.get_msg_gettext(
                        "Uploading to {}, please wait ...", (self._cloud._title,)
                    )
                )
                if self._cloud.upload_file(dfile, "database"):
                    self.echo_info(
                        public.get_msg_gettext(
                            "Successfully uploaded to {}", (self._cloud._title,)
                        )
                    )
                else:
                    if hasattr(self._cloud, "error_msg"):
                    ...
                    remark = "Backup to " + self._cloud._title
                    self.send_failture_notification(
                        error_msg, target=f"{db_name}|database", remark=remark
                    )
                    return False
            ...
            if self._cloud:
                filename = dfile + "|" + self._cloud._name + "|" + fname
            self.echo_info(
                public.get_msg_gettext("Database has been backed up to: {}", (dfile,))
            )
            if os.path.exists(self._err_log):
                os.remove(self._err_log)
            ...
            if not self._cloud:
                ...
                backups = (
                    ...
                    .where(
                        "cron_id=? and type=? and pid=? and filename LIKE ?",
                        (cron_id, "1", pid, "%{}%".format(self._cloud._name)),
                    )
                    .field("id,name,filename")
                    .select()
                )
            ...
            self.send_failture_notification(
                public.get_msg_gettext("Database {} does not exist", (db_name,)),
                target=db_name,
            )
            self.echo_error(
                public.get_msg_gettext("Database {} does not exist", (db_name,))
    ...
    def generate_success_title(self, task_name):
        ...
        title = public.get_msg_gettext(
            "{}-{} The task was executed successfully", (server_ip, task_name)
        )
        return title
...
--Notification by K2Panel""".format(
    ...
    def get_cron_info(self, cron_name):
        ...
        try:
            cron_info = (
                public.M("crontab")
                .where("echo=?", (cron_name,))
                .field("name,save_local,notice,notice_channel,id,sType")
                .find()
            )
            return cron_info
        except Exception as e:
    ...
    def send_all_failture_notification(self, backup_type, results, remark=""):
        ...
        if notice == 1 or notice == 2:
            title = self.generate_failture_title(cron_title)
            type_desc = {"site": "site", "database": "database"}
            ...
            if failture_count > 0:
                if self._cloud:
                    remark = public.lang(
                        "Backup to {}, a total of {} {}, and failures {}."
            ...
            msg = self.generate_all_failture_notice(
                task_name, content, backup_type_desc, remark
            )
    ...
    def send_notification(self, channel, title, msg="", total=0, failture_count=0):
        ...
        try:
            ...
            if not push_res["status"] or error_count:
                self.echo_error(
                    "Notification:{} Failed send!".format(",".join(error_channel))
                )
            else:
    ...
    def send_notification2(self, channel, title, msg={}):
        try:
            ...
            send_res = []
            error_count = 0
            channel_names = {"mail": "email", "telegram": "telegram"}
            error_channel = []
            ...
            for td in tongdao:
                ...
                if td == "mail":
                    ...
                    _res = sm.qq_smtp_send(
                        mail_list, title=title, body=msg["mail"].replace("\n", "<br/>")
                    )
                    if not _res:
                        error_count += 1
                ...
                if td == "telegram":
                    ...
                    if not settings["telegram"]["setup"]:
                        continue
                    _res = panel_telegram_bot.panel_telegram_bot().send_by_tg_bot(
                        msg["tg"]
                    )
