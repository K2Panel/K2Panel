...
class apache:
    ...
    def GetApacheStatus(self):
        ...
        try:
            ...
            if re.search(rep, confcontent):
                confcontent = re.sub(
                    rep, "Include conf/extra/httpd-info.conf", confcontent
                )
            ...
            if not restarttime:
                return public.return_msg_gettext(
                    False, public.lang("Get Restart Time False")
                )
            ...
            if not timedetail:
                return public.return_msg_gettext(
                    False, public.lang("Get time detail False")
                )
            ...
            if not total_acc:
                return public.return_msg_gettext(
                    False, public.lang("Get TotalAccesses False")
                )
            data["TotalAccesses"] = total_acc.group(1)
            total_kb = re.search(r"Total kBytes:\s+(\d+)", result)
            if not total_kb:
                return public.return_msg_gettext(
                    False, public.lang("Get TotalKBytes False")
                )
            ...
            if not busywork:
                return public.return_msg_gettext(
                    False, public.lang("Get BusyWorkers False")
                )
            data["BusyWorkers"] = busywork.group(1)
            idlework = re.search(r"IdleWorkers:\s+(\d+)", result)
            if not idlework:
                return public.return_msg_gettext(
                    False, public.lang("Get IdleWorkers False")
                )
    ...
    def GetApacheValue(self):
        ...
        if not "mpm_event_module" in apachempmcontent:
            return public.returnMsg(
                False,
                public.lang(
                    "mpm_event_module conf not found or /www/server/apache/conf/extra/httpd-mpm.conf is empty"
                ),
            )
        apachempmcontent = re.search(
            r"\<IfModule mpm_event_module\>(\n|.)+?\</IfModule\>", apachempmcontent
        ).group()
        ps = [
            "%s，%s" % (public.lang("Second"), public.lang("Request timeout")),
        ...
        for c in conflist:
            if c["name"] == "KeepAlive":
                if not re.search("On|Off", c["value"]):
                    return public.return_msg_gettext(
                        False, public.lang("Parameter ERROR!")
                    )
            else:
                print(c["value"])
    ...
    def add_httpd_access_log_format(self, args):
        ...
        #LOG_FORMAT_END_{n}
""".format(
                n=args.log_format_name, c=" ".join(log_format)
            )
    ...
    def _del_format_log_of_website(self, log_format_name):
        ...
        try:
            for s in site_format_log_status.keys():
                ...
                if not conf:
                    continue
                if not re.search(format_exist_reg, conf):
                    continue
    ...
    def get_httpd_access_log_format(self, args=None):
        try:
            ...
            for i in format_name:
                format_reg = (
                    r"#LOG_FORMAT_BEGIN_{n}(\n|.)+LogFormat\s+\'(.*)\'\s+{n}".format(
                        n=i
                    )
                )
    ...
    def set_httpd_format_log_to_website(self, args):
        ...
        try:
            ...
            for site in all_site:
                ...
                access_log = (
                    re.search(format_exist_reg, conf).groups()[0]
                    + '" '
                    + args.log_format_name
                )
