...
# +-------------------------------------------------------------------
# | Author: lkq <1249648969@binarjoinanalyticnl.nl>
# +-------------------------------------------------------------------
# +--------------------------------------------------------------------
# |   告警消息队列
# +--------------------------------------------------------------------
...
class send_to_user:
    ...
    def __init__(self):
        ...
        if (
            ...
            .where("type=? AND name=?", ("table", "send_settings"))
            .count()
        ):
            public.M("").execute(
        ...
        if (
            not public.M("sqlite_master")
            .where("type=? AND name=?", ("table", "send_msg"))
            .count()
        ):
            public.M("").execute(
    ...
    def inser_send_msg(self, name, send_type, msg, type, inser_time):
        ...
        if (
            public.M("send_msg")
            .where(
                "name=? and send_type=? and type=? and inser_time=?",
                (name, send_type, type, inser_time),
            )
            .count()
        ):
            return False
    ...
    def read_thread(self):
        if not public.M("send_settings").count():
            return False
        send_data = (
            public.M("send_settings")
            .field("id,name,type,path,send_type,inser_time,last_time,time_frame")
            .select()
        )
        for i in send_data:
                ...
                if os.path.exists(i["path"]):
                    ...
                    for i2 in read_file:
                        self.inser_send_msg(
                            ...
                            + i2[1]
                            + ",触发告警时间:"
                            + self.dtchg(int(time.time())),
                            "json",
                            i2[0],
                        )
                    public.writeFile(i["path"], "")
                    public.M("send_settings").where("id=?", (i["id"],)).update(
                        {"last_time": int(time.time())}
                    )
            if i["type"] == "file":
                if os.path.exists(i["path"]):
    ...
    def send_msg(self):
        ...
        send_msg = (
            public.M("send_msg")
            .where("is_send=?", (False,))
            .field("id,name,send_type,msg,is_send,type,inser_time")
            .select()
        )
        count = 1
        for i in send_msg:
            ...
            if i["send_type"] == "mail":
                if public.send_mail(i["name"], i["msg"]):
                    ...
                    public.M("send_msg").where("id=?", (i["id"],)).update(
                        {"is_send": True}
                    )
