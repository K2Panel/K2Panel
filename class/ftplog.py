...
class ftplog:
    ...
    def get_file_list(self, path, is_bakcup=False):
        ...
        if is_bakcup:
            file_name_list = [
                {"file": "/var/log/pure-ftpd.log", "time": int(time.time())}
            ]
        else:
            file_name_list = []
        for i in files:
            ...
            if is_bakcup:
                ...
                if os.path.isfile(file_path) and i.find("messages") != -1:
                    ...
                    file_name_list.append(tmp_dict)
        file_name_list = sorted(file_name_list, key=lambda x: x["time"], reverse=False)
            ...
            if os.path.exists(ftp_file):
                ...
                if os.path.exists(ftp_file):
                    return public.returnMsg(
                        False, public.lang("failed to close the log")
                    )
            self.del_crontab()
    ...
    def get_action_log(self, get):
        ...
        for file in file_list:
            ...
            for line in lines:
                ...
                user = ""
                if (
                    upload_value in line
                    or download_value in line
                    or rename_value in line
                    or delete_value in line
                ):
                    user = line[line.find("(") + 1 : line.find(")")].split("@")[0]
                ...
                # 取下载日志
                if (
                    get.type == "all" or get.type == "download"
                ) and download_value in line:
                    line_list = line.split()
        ...
        if cron_list:
            for i in cron_list:
                if not i:
                    continue
                cron_echo = (
                    public.M("crontab").where("id=?", (i["id"],)).getField("echo")
                )
                args = {"id": i["id"]}
                import crontab
                ...
                public.ExecShell(
                    "crontab -u root -l| grep -v '{}'|crontab -u root -".format(
                        del_cron_file
                    )
                )
    ...
    def add_crontab(self):
        ...
        if (
            ...
            .where("name=?", ("[Do not delete] FTP audit log cutting task",))
            .count()
        ):
