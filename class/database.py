...
class database(datatool.datatools):
    ...
    _DB_BACKUP_DIR = os.path.join(
        public.M("config").where("id=?", (1,)).getField("backup_path"), "database"
    )
    ...
    def AddCloudServer(self, get):
        ...
        if isinstance(res, dict):
            return res
        if (
            public.M("database_servers")
            .where("db_host=? AND db_port=?", (get.db_host, get.db_port))
            .count()
        ):
            return public.return_msg_gettext(
                False,
                "The specified server already exists: [{}:{}]",
    ...
    def RemoveCloudServer(self, get):
        ...
        if not db_find:
            return public.returnMsg(
                False, public.lang("The specified server dose not exists!")
            )
        ...
        if isinstance(result, int):
            public.WriteLog(
                "Database manager",
                "Delete the remote MySQL server [{}:{}]",
    ...
    def ModifyCloudServer(self, get):
        ...
        if (
            db_find["db_user"] != get.db_user
            or db_find["db_password"] != get.db_password
        ):
            _modify = True
    ...
    def AddCloudDatabase(self, get):
        ...
        if isinstance(result, int):
            public.write_log_gettext(
                "Database manager",
                "Add remote MySQL database [{}] successfully",
                (get.db_name,),
            )
            return public.return_msg_gettext(True, public.lang("Setup successfully!"))
        return public.return_msg_gettext(False, "Add failed: {}", (result,))
        ...
        try:
            ...
            for i in result:
                if i[0] == conn_config["db_name"]:
                    return True
            return public.returnMsg(
                False, public.lang("The specified database does not exist!")
            )
    ...
    def GetMySQLError(self, e):
        ...
        if e.args[0] == 1044:
            res = public.gettext_msg(
                "No access rights, or the database does not exist!"
            )
        if e.args[0] == 1062:
    ...
    def AddDatabase(self, get):
        ...
        if not data_name:
            return public.return_msg_gettext(
                False, public.lang("The database name cannot be empty")
            )
        ...
        if len(data_name) > 64:
            return public.return_msg_gettext(
                False, public.lang("Database name cannot be more than 16 characters!")
            )
        reg = r"^[\w\.-]+$"
        username = get.db_user.strip()
        if not username:
            return public.return_msg_gettext(
                False, public.lang("The database user name cannot be empty")
            )
        if not re.match(reg, data_name):
            return public.return_msg_gettext(
                False, public.lang("Database name cannot contain special characters!")
            )
        if not re.match(reg, username):
            return public.return_msg_gettext(
                False, public.lang("Database name is illegal!")
            )
        if not hasattr(get, "db_user"):
        ...
        if username in checks or len(username) < 1:
            return public.return_msg_gettext(
                False, public.lang("Database username is illegal!")
            )
        ...
        if not mysql_obj:
            return public.returnMsg(
                False, public.lang("Failed to connect to the specified database")
            )
        ...
        public.write_log_gettext(
            "Database manager", "Successfully added database [{}]!", (data_name,)
        )
        return {
            **public.return_message(True, 0, public.lang("Setup successfully!")),
            "id": db_id,
        }
...
"""
        ...
        return public.return_msg_gettext(
            True,
            public.lang(
                "Open successfully, take effect after manually restarting the database"
            ),
        )
    ...
    def __CreateUsers(self, dbname, username, password, address, ssl=None):
        ...
        if not ssl:
            mysql_obj.execute(
                "update mysql.user set ssl_type='' where user='%s' and host='localhost'"
                % (username)
            )
        for a in address.split(","):
            mysql_obj.execute(
    ...
    def CheckRecycleBin(self, name):
        try:
            ...
            for n in os.listdir("/www/.Recycle_bin"):
                if n.find("BTDB_" + name + "_t_") != -1:
                    return True
                if n.find("BTDB_" + u_name + "_t_") != -1:
                    return True
            return False
        except:
            return False
    ...
    def IsSqlError(self, mysqlMsg):
        ...
        if "2002," in mysqlMsg or "2003," in mysqlMsg:
            return public.return_msg_gettext(
                False,
                public.lang("ERROR to connect database, pls check database status!"),
            )
        ...
        if "1133" in mysqlMsg:
            return public.return_msg_gettext(
                False, public.lang("Database user does NOT exist!")
            )
        if "3679" in mysqlMsg:
            return public.returnMsg(
                False,
        ...
        if "libmysqlclient" in mysqlMsg:
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Execution failed, attempted auto repair, please try again later!"
                ),
            )
        return None
...
"""
    ...
    def DeleteDatabase(self, get):
        ...
        find = (
            public.M("databases")
            .where("id=?", (id,))
            .field("id,sid,pid,name,username,password,accept,ps,addtime,db_type")
            .find()
        )
        if not find:
            return public.return_msg_gettext(
                False, public.lang("Database [{}] does not exist!", name)
            )
        self.sid = find["sid"]
        if find["db_type"] in ["0", 0] or self.sid:  # 删除本地数据库
            ...
            if not mysql_obj:
                return public.return_msg_gettext(
                    False, public.lang("Database [{}] connection failed", name)
                )
        ...
        public.write_log_gettext(
            "Database manager", "Successfully deleted database [{}]!", (name,)
        )
        return public.return_msg_gettext(True, public.lang("Successfully deleted"))
        # except Exception as ex:
    ...
    def DeleteToRecycleBin(self, name):
        ...
        if isinstance(users, str):
            return public.return_msg_gettext(
                False, public.lang("Delete failed, failed to connect to database!")
            )
        try:
            for us in users:
                panelMysql.panelMysql().execute(
                    "drop user '" + username + "'@'" + us[0] + "'"
                )
        except Exception:
            pass
        ...
        if not os.path.exists(db_path):
            return public.return_msg_gettext(
                False, public.lang("The database data does not exist!")
            )

        public.ExecShell("mv -f {} {}".format(db_path, rm_path))
        if not os.path.exists(rm_path):
            return public.return_msg_gettext(
                False, public.lang("Failed to move database data to the recycle bin!")
            )
        ...
        return public.return_msg_gettext(
            True, public.lang("Database moved to recycle bin!")
        )
    ...
    def DeleteTo(self, filename):
        ...
        if os.path.isfile(filename):
            ...
            if os.path.exists(filename):
                ...
                return public.returnMsg(
                    False, public.lang("Recycle Bin does not exist for this database!")
                )
    ...
    def RecycleDB(self, filename):
        ...
        if not _isdir:
            ...
            if not os.path.exists(db_path):
                return public.return_msg_gettext(
                    False, public.lang("Data recovery failed!")
                )
        ...
        self.__CreateUsers(
            data["name"], data["username"], data["password"], data["accept"]
        )
    ...
    def SetupPassword(self, get):
        ...
        try:
            if not password:
                return public.return_msg_gettext(
                    False, public.lang("Root password cannot be empty")
                )
            rep = r"^[\w@\.\?\-\_\>\<\~\!\#\$\%\^\&\*\(\)]+$"
            if not re.match(rep, password):
                return public.return_msg_gettext(
                    False,
                    public.lang("Database password cannot contain special characters!"),
                )
            self.sid = get.get("sid/d", 0)
            # 修改MYSQL
            mysql_obj = public.get_mysql_obj_by_sid(self.sid)
            ...
            if is_modify:
                ...
                if m_version.find("5.7") == 0 or m_version.find("8.0") == 0:
                    accept = self.map_to_list(
                        mysql_obj.query(
                            "select Host from mysql.user where User='{}'".format(
                                admin_user
                            )
                        )
                    )
                    for my_host in accept:
                        ...
                        mysql_obj.execute(
                            ...
                            % (admin_user, my_host[0], password)
                        )
                # elif m_version.find('10.5.') != -1 or m_version.find('10.4.') != -1:
                elif any(
                    mariadb_ver in m_version
                    for mariadb_ver in [
                        "10.5.",
                        "10.4.",
                        "10.6.",
            ...
            # 修改SQLITE
            if self.sid:
                public.M("database_servers").where("id=?", self.sid).setField(
                    "db_password", password
                )
                public.write_log_gettext(
                    "Database manager", "Change the password of the remote MySQL server"
                )
            else:
                public.M("config").where("id=?", (1,)).setField("mysql_root", password)
                public.write_log_gettext(
                    "Database manager", "Successfully modified root password!"
                )
    ...
    def ResDatabasePassword(self, get):
        ...
        if not newpassword:
            return public.return_msg_gettext(
                False, "Database [{}] password cannot be empty", (username,)
            )
        ...
        if self.sid and username == "root":
            return public.returnMsg(
                False,
                public.lang("Cannot change the root password of the remote database"),
            )
        mysql_obj = public.get_mysql_obj_by_sid(self.sid)
        if not mysql_obj:
            return public.returnMsg(
        ...
        public.write_log_gettext(
            "Database manager",
            "Successfully modifyied password for database [{}]!",
            (name,),
        )
        return public.return_msg_gettext(
            True, "Successfully modifyied password for database [{}]!", (name,)
        )
        # except Exception as ex:
        #     import traceback
    ...
    def ToBackup(self, get):
        ...
        name = db_find["name"]
        fileName = (
            name + "_" + time.strftime("%Y%m%d_%H%M%S", time.localtime()) + ".sql.gz"
        )
        ...
        if db_find["db_type"] in ["0", 0]:
            ...
            try:
                ...
                if not password:
                    return public.returnMsg(
                        False, public.lang("Database password cannot be empty")
                    )
                password = shlex.quote(str(password))
                os.environ["MYSQL_PWD"] = password
                public.ExecShell(
                    mysqldump_bin
                    + " -R -E --triggers=false --default-character-set="
                    + public.get_database_character(name)
                    + ' --force --opt "'
                    + name
                    + '"  -u root -p'
                    + password
            ...
            try:
                ...
                public.ExecShell(
                    ...
                    + conn_config["db_host"]
                    + " -P "
                    + str(int(conn_config["db_port"]))
                    + " -R -E --triggers=false --default-character-set="
                    + public.get_database_character(name)
                    + ' --force --opt "'
                    + str(db_find["name"])
                    + '"  -u '
                    + str(conn_config["db_user"])
                    + " -p"
                    + password
                    + " | gzip > "
                    + backupName
            ...
            try:
                conn_config = (
                    public.M("database_servers").where("id=?", db_find["sid"]).find()
                )
                res = self.CheckCloudDatabase(conn_config)
                if isinstance(res, dict):
                    return res
            ...
            return public.return_msg_gettext(
                False, public.lang("Unsupported database type")
            )
        ...
        public.write_log_gettext(
            "Database manager", "Backup database [{}] succeed!", (name,)
        )
    ...
    def DelBackup(self, get):
        try:
            ...
            public.write_log_gettext(
                "Database manager",
                "Successfully deleted backup [{}] for database [{}]!",
                (db_name, filename),
            )
            return public.return_msg_gettext(True, public.lang("Successfully deleted"))
        except Exception as ex:
            public.write_log_gettext(
                "Database manager",
                "Failed to delete backup [{}] for database [{}]! => {}",
    ...
    def InputSql(self, get):
        ...
        if ext not in exts:
            return public.return_msg_gettext(
                False, public.lang("Select sql/gz/zip file!")
            )
        ...
        if ext != "sql":
            ...
            if not os.path.exists(input_path) or tmpFile == "":
                if tmpFile and os.path.isfile(input_path2):
                    ...
                    return public.return_msg_gettext(
                        False, "Configuration file not exist", (tmpFile,)
                    )
        ...
        public.write_log_gettext(
            "Database manager", "Successfully imported database [{}]", (name,)
        )
        return public.return_msg_gettext(
            True, public.lang("Successfully imported database!")
        )
        # except Exception as ex:
    ...
    def SyncToDatabases(self, get):
        ...
        if type == 0:
            data = (
                sql.field("id,sid,name,username,password,accept,db_type")
                .where("type='MySQL'", ())
                .select()
            )
            for value in data:
            ...
            for value in data:
                find = (
                    sql.where("id=?", (value,))
                    .field("id,sid,name,username,password,accept")
                    .find()
                )
        ...
        if n == 1:
            ...
            return public.return_msg_gettext(
                True, "Sync {} database(s) from server!", (str(n),)
            )
    ...
    def SyncGetDatabases(self, get):
        ...
        for value in data:
            ...
            if b:
                continue
            if sql.where("name=?", (value[0],)).count():
                continue
            host = "127.0.0.1"
            for user in users:
                if value[0] == user[0]:
            ...
            if sql.table("databases").add(
                "name,sid,db_type,username,password,accept,ps,addtime",
                (value[0], self.sid, db_type, value[0], "", host, ps, addTime),
            ):
                n += 1

        return public.return_msg_gettext(
            True, "Obtain {} database(s) from server!", (str(n),)
        )
    ...
    def GetDatabaseAccess(self, get):
        ...
        ssl_type = panelMysql.panelMysql().query(
            "select ssl_type from mysql.user where User='%s'" % name
        )
        ...
        if len(users) < 1:
            return public.return_msg_gettext(
                True, {"permission": "127.0.0.1", "ssl": ssl_type}
            )
    ...
    def SetDatabaseAccess(self, get):
        ...
        if access in [""]:
            return public.return_msg_gettext(
                False, public.lang("The IP address cannot be empty!")
            )
        password = (
            public.M("databases").where("username=?", (name,)).getField("password")
        )
        result = mysql_obj.query("show databases")
        isError = self.IsSqlError(result)
        if isError != None:
    ...
    def SetDataDir(self, get):
        ...
        if len(get.datadir) > 32:
            return public.return_msg_gettext(
                False, public.lang("The data directory length cannot exceed 32 bits")
            )
        ...
        public.ExecShell(
            r"\cp -arf " + mysqlInfo["datadir"] + "/* " + get.datadir + "/"
        )
    ...
    def BinLog(self, get):
        ...
        if mysql_cnf.find("#log-bin=mysql-bin") != -1:
            if hasattr(get, "status"):
                return public.return_msg_gettext(False, public.lang("0"))
        ...
        return {
            "status": True,
            "msg": "{} Binary log successful, Please refresh manually".format(
                "Enable" if not bin_log_status else "Disable"
            ),
            "data": {"binlog_status": not bin_log_status},
        }
    ...
    def SetDbConf(self, get):
        ...
        for g in gets:
            ...
            if m_version.find("8.") == 0 and g in [
                "query_cache_type",
                "query_cache_size",
            ]:
                n += 1
            ...
            if g in ["innodb_log_buffer_size"]:
                ...
                if int(get[g]) < 8:
                    return public.return_msg_gettext(
                        False,
                        public.lang("innodb_log_buffer_size cannot be less than 8MB"),
                    )
    ...
    def GetRunStatus(self, get):
        ...
        try:
            if data[0] == 1045:
                return public.return_msg_gettext(
                    False, public.lang("MySQL password ERROR!")
                )
            for d in data:
                for g in gets:
                    try:
    ...
    def GetSlowLogs(self, get):
        ...
        if not os.path.exists(path):
            return public.return_msg_gettext(
                False, public.lang("Log file does NOT exist!")
            )
    ...
    def GetMySQLBinlogs(self, get):
        ...
        if not os.path.exists(index_file):
            return public.return_msg_gettext(
                False,
                public.lang("Binlog is not enabled or binlog file does not exist!"),
            )
        ...
        if not isinstance(rows, list):
            return public.return_msg_gettext(
                False, public.lang("Mysql status is abnormal!")
            )
        if len(rows) != 0:
        ...
        for item in text.split("\n"):
            ...
            if not os.path.isfile(bin_log_path):
                continue
    ...
    def ClearMySQLBinlog(self, get):
        ...
        if days < 7:
            return public.return_msg_gettext(
                False,
                public.lang(
                    "To ensure data security, recent binlogs cannot be deleted!"
                ),
            )

        rows = panelMysql.panelMysql().query(
            "PURGE BINARY LOGS BEFORE DATE_SUB(NOW(), INTERVAL {days} DAY)".format(
                days=days
            )
        )
    ...
    def GetInfo(self, get):
        ...
        if info:
            ...
            return public.return_msg_gettext(
                False, public.lang("Failed to get databases")
            )
    ...
    def ReTable(self, get):
        ...
        if info:
            return public.return_msg_gettext(
                True, public.lang("Successfully repaired!")
            )
        else:
    ...
    def OpTable(self, get):
        ...
        if info:
            return public.return_msg_gettext(
                True, public.lang("Successfully optimized!")
            )
        else:
            return public.return_msg_gettext(
                False, public.lang("Failed to optimize or already optimized")
            )
        ...
        for id in ids:
            if not is_pid:
                x = (
                    public.M("databases")
                    .where("id=?", id)
                    .field("id,sid,pid,name,type,ps,addtime")
                    .find()
                )
            else:
                x = (
                    public.M("databases")
                    .where("pid=?", id)
                    .field("id,sid,pid,name,ps,type,addtime")
                    .find()
                )
            if not x:
                continue
            x["backup_count"] = (
                public.M("backup").where("pid=? AND type=?", (x["id"], "1")).count()
            )
            if x["type"] == "MySQL":
    ...
    def check_del_data(self, get):
        ...
        for key in db_data:
            ...
            if data["total"] > 0:
                db_list_size.append(data["total"])
            result.append(data)
