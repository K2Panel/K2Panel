...
class plugin_deployment:
    ...
    def GetList(self, get):
        ...
        if not os.path.exists(jsonFile):
            return public.returnMsg(False, "Profile does not exist!")
        data = {}
    ...
    def GetCloudList(self, get):
        try:
            jsonFile = self.__setupPath + "/deployment_list.json"
            if (
                not "package" in session
                or not os.path.exists(jsonFile)
                or hasattr(get, "force")
            ):
    ...
    def GetPackageOther(self, get):
        ...
        jsonFile = self.__setupPath + "/deployment_list_other.json"
        if not os.path.exists(jsonFile):
            public.returnMsg(False, "could not find [%s]" % p_name)
        data = json.loads(public.readFile(jsonFile))
        ...
        try:
            ...
            if sys.version_info[0] == 2:
                ...
                urllib.request.urlretrieve(
                    url, filename=filename, reporthook=self.DownloadHook
                )
            self.WriteLogs(
                json.dumps(
                    {
    ...
    def SetupPackage(self, get):
        ...
        # 取基础信息
        find = (
            public.M("sites").where("name=?", (site_name,)).field("id,path,name").find()
        )
        if not "path" in find:
        ...
        id = pinfo["id"]
        if not pinfo:
            return public.returnMsg(False, "The specified package does not exist.!")
        ...
        if isDownload:
            ...
            if pinfo["versions"][0]["download"]:
                self.DownloadFile(
                    ...
                    + pinfo["versions"][0]["download"],
                    packageZip,
                )

        if not os.path.exists(packageZip):
            return public.returnMsg(False, "File download failed!" + packageZip)

        pinfo = self.set_temp_file(packageZip, path)
        if not pinfo:
            return public.returnMsg(
                False,
                "Cannot find [K2Panel Auto Deployment Configuration File] in the installation package",
            )
        ...
        if "enable_functions" in pinfo:
            try:
                if type(pinfo["enable_functions"]) == str:
                    pinfo["enable_functions"] = (
                        pinfo["enable_functions"].strip().split(",")
                    )
                php_f = (
        ...
        self.WriteLogs(
            json.dumps(
                {"name": "Set URL rewrite", "total": 0, "used": 0, "pre": 0, "speed": 0}
            )
        )
        ...
        self.WriteLogs(
            json.dumps(
                {"name": "Import database", "total": 0, "used": 0, "pre": 0, "speed": 0}
            )
        )
            ...
            if databaseInfo:
                ...
                if os.path.exists(siteConfigFile):
                    ...
                    siteConfig = siteConfig.replace(
                        "BT_DB_USERNAME", databaseInfo["username"]
                    )
                    siteConfig = siteConfig.replace(
                        "BT_DB_PASSWORD", databaseInfo["password"]
                    )
                    siteConfig = siteConfig.replace(
                        "BT_DB_NAME", databaseInfo["username"]
                    )
        ...
        self.WriteLogs(
            json.dumps(
                {"name": "清理多余的文件", "total": 0, "used": 0, "pre": 0, "speed": 0}
            )
        )
        ...
        self.WriteLogs(
            json.dumps(
                {"name": "Ready to deploy", "total": 0, "used": 0, "pre": 0, "speed": 0}
            )
        )
    ...
    def GetSpeed(self, get):
        try:
            if not os.path.exists(self.logPath):
                return public.returnMsg(
                    False, "There are currently no deployment tasks!"
                )
