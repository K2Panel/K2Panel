...
class crontab:
    ...
    def GetCrontab(self, get):
        ...
        for i in range(len(cront)):
            ...
            if cront[i]["type"] == "day":
                ...
                tmp["cycle"] = public.lang(
                    "Run Every {} Minutes", str(cront[i]["where1"])
                )
            elif cront[i]["type"] == "week":
                tmp["type"] = public.lang("Weekly")
                if not cront[i]["where1"]:
                    cront[i]["where1"] = "0"
                tmp["cycle"] = public.lang(
                    "Every {}, run at {} Hour {} Min",
                    self.toWeek(int(cront[i]["where1"])),
                    str(cront[i]["where_hour"]),
                    str(cront[i]["where_minute"]),
                )
        ...
        try:
            ...
            for f in os.listdir(log_file):
                if f[-4:] != ".log":
                    continue
                filename = log_file + "/" + f
                if os.path.getsize(filename) < 1048576 / 2:
                    continue
    ...
    def checkBackup(self):
        ...
        if not os.path.exists(filePath):
            public.downloadFile(
                public.GetConfigValue("home") + "/linux/logsBackup.py", filePath
            )
        # 检查计划任务服务状态
        import system
    ...
    def set_cron_status(self, get):
        ...
        if cronInfo["status"] == status:
            ...
            if not self.sync_to_crond(cronInfo):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Unable to write to file, please check if [System hardening] is enabled!"
                    ),
                )
        ...
        public.WriteLog(
            "TYPE_CRON",
            "Modified cron job [{}] status to [{}]".format(
                cronInfo["name"], str(status_msg[status])
            ),
        )
    ...
    def modify_crond(self, get):
        if len(get["name"]) < 1:
            return public.return_msg_gettext(
                False, public.lang("Name of task cannot be empty!")
            )
        id = get["id"]
        ...
        if cronInfo["status"] == 0:
            return public.return_msg_gettext(
                False,
                public.lang(
                    "The current task is Disable status, please open the task before modifying!"
                ),
            )
        if not self.sync_to_crond(cronInfo):
            return public.return_msg_gettext(
    ...
    def AddCrontab(self, get):
        ...
        public.add_security_logs(
            "TYPE_CRON", "Add Cron tasks [" + get["name"] + "] success" + str(values)
        )
        if type(addData) == str:
    ...
    def GetDataList(self, get):
        ...
        if get["type"] == "databases":
            data["data"] = (
                public.M(get["type"]).where("type=?", "MySQL").field("name,ps").select()
            )
        else:
    ...
    def DelLogs(self, get):
        try:
            ...
            return public.return_msg_gettext(
                False, public.lang("Failed to empty task logs!")
            )
    ...
    def DelCrontab(self, get):
        try:
            ...
            if not find:
                return public.return_msg_gettext(
                    False, public.lang("The specified task does not exist!")
                )

            if not self.remove_for_crond(find["echo"]):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Unable to write to file, please check if [System hardening] is enabled!"
    ...
    def GetShell(self, param):
        ...
        if type == "toFile":
            ...
            if (
                type in ["site", "path"]
                and param["sBody"] != "undefined"
                and len(param["sBody"]) > 1
            ):
            ...
            wheres = {
                ...
                + " "
                + public.GetConfigValue("setup_path")
                + "/panel/script/backup.py database "
                + param["sName"]
                + " "
                + str(param["save"])
                + attach_param,
                ...
                + "/bin/bash "
                + public.GetConfigValue("setup_path")
                + "/panel/script/rememory.sh",
                "webshell": head
                + python_bin
                + " "
                + public.GetConfigValue("setup_path")
                + "/panel/class/webshell_check.py site "
                + param["sName"]
                + " "
                + param["urladdress"],
            }
            ...
            try:
                ...
                if type == "toUrl":
                    shell = (
                        head
                        + "curl -sS --connect-timeout 10 -m 3600 '"
                        + param["urladdress"]
                        + "'"
                    )
                else:
...
"""
    ...
    def WriteShell(self, config):
        ...
        conf = public.readFile(file)
        if type(conf) == bool:
            return public.return_msg_gettext(False, public.lang("Failed to read file!"))
        conf += config + "\n"
        if public.writeFile(file, conf):
            if not os.path.exists(u_file):
                ...
                public.ExecShell(
                    "chmod 600 '" + file + "' && chown root.crontab " + file
                )
            return True
        return public.return_msg_gettext(
            False,
