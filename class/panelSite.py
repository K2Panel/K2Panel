            ...
            """server
{
    ...
    def apacheAddPort(self, port):
        ...
        allConf = allConf.replace(
            listen, listen + "Listen " + port + listen_ipv6 + "\n"
        )
    ...
    def apacheAdd(self):
        ...
        try:
            httpdVersion = public.readFile(
                self.setupPath + "/apache/version.pl"
            ).strip()
        except:
            httpdVersion = ""
        if httpdVersion == "2.2":
...
{{
    ...
    def rep_site_config(self, get):
        ...
        siteInfo = (
            public.M("sites")
            .where("name=?", (self.siteName,))
            .field("id,path,port")
            .find()
        )
        siteInfo["domains"] = (
            public.M("domains")
            .where("pid=?", (siteInfo["id"],))
            .field("name,port")
            .select()
        )
        siteInfo["binding"] = (
    ...
    def openlitespeed_add_site(self, get, init_args=None):
        ...
        if not self.sitePath:
            return public.return_msg_gettext(
                False, public.lang("Not specify parameter [sitePath]")
            )
        if init_args:
...
"""
    ...
    def __create_website_mulitiple(self, websites_info, site_path, get):
        ...
        for data in websites_info:
            ...
            try:
                ...
                data["port"] = (
                    "80" if len(domains[0].split(":")) < 2 else domains[0].split(":")[1]
                )
                get.webname = json.dumps(
                    {"domain": website_name, "domainlist": domains[1:], "count": 0}
                )
                get.path = (
                    data["path"]
                    if "path" in data and data["path"] != "0" and data["path"] != "1"
                    else site_path + "/" + website_name
                )
                get.version = (
                    data["version"]
                ...
                if get.sql == "true":
                    ...
                    create_other["db_user"] = get.datauser = website_name.replace(
                        ".", "_"
                    )
                    create_other["db_status"] = True
                if get.ftp == "true":
                    create_other["ftp_pass"] = get.ftp_password = public.gen_password(
                        16
                    )
                    create_other["ftp_user"] = get.ftp_username = website_name.replace(
                        ".", "_"
                    )
                ...
                create_failed[domains[0]] = public.lang(
                    "There was an error creating, please try again."
                )
        return {
            "status": True,
            "msg": public.lang(
                "Create the website [ {} ] successfully", ",".join(create_successfully)
            ),
            "error": create_failed,
    ...
    def check_php_conf(self):
        try:
            ...
            conf = public.readFile(
                "/www/server/nginx/conf/enable-php-{}.conf".format(php_v[0])
            )
            public.writeFile(file, conf)
        except:
            pass
    ...
    def AddSite(self, get, multiple=None):
        if not get.path:
            return public.return_msg_gettext(
                False, public.lang("Please fill in the website path")
            )
        if get.path == "/":
            return public.return_msg_gettext(
                False, public.lang("The website path cannot be the root directory [/]")
            )
        rep_email = r"[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?"
        if hasattr(get, "email"):
            if not re.search(rep_email, get.email):
                return public.return_msg_gettext(
                    False, public.lang("Please check if the [Email] format correct")
                )
        if hasattr(get, "password") and hasattr(get, "pw_weak"):
            ...
            if not os.path.exists("/www/server/mysql") or not os.path.exists(
                "/www/server/php"
            ):
                return public.return_msg_gettext(
                    False, public.lang("Please install Mysql and PHP first!")
                )
        self.check_default()
        self.check_php_conf()
        ...
        if isError != True:
            return public.return_msg_gettext(
                False,
                'ERROR: %s<br><br><a style="color:red;">'
                % public.lang(
        ...
        if not public.check_site_path(get.path):
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Please do not set the website root directory to the system main directory:<br> {}",
                    "<br>".join(a + c),
                ),
            )
        try:
        ...
        self.siteName = (
            self.ToPunycode(siteMenu["domain"].strip().split(":")[0]).strip().lower()
        )
        self.sitePath = self.ToPunycodePath(
            self.GetPath(get.path.replace(" ", ""))
        ).strip()
        ...
        if not public.checkPort(self.sitePort):
            return public.return_msg_gettext(
                False,
                public.lang("Port range is incorrect! should be between 100-65535"),
            )
        ...
        if not is_phpv:
            return public.return_msg_gettext(
                False, public.lang("Requested PHP version does NOT exist!")
            )
        ...
        if len(self.phpVersion) < 2:
            return public.return_msg_gettext(
                False, public.lang("PHP version cannot be empty")
            )
        reg = r"^([\w\-\*]{1,100}\.){1,24}([\w\-]{1,24}|[\w\-]{1,24}\.[\w\-]{1,24})$"
        if not re.match(reg, self.siteName):
            return public.return_msg_gettext(
                False, public.lang("Format of primary domain is incorrect")
            )
        if self.siteName.find("*") != -1:
            return public.return_msg_gettext(
                False, public.lang("Primary domain cannot be wildcard DNS record")
            )
        if self.sitePath[-1] == ".":
            return public.return_msg_gettext(False, "DIR_END_WITH", ("'.'",))
        ...
        if sql.where("name=?", (self.siteName,)).count():
            return public.return_msg_gettext(
                False, public.lang("The site you tried to add already exists!")
            )
        ...
        if opid:
            if public.M("sites").where("id=?", (opid,)).count():
                return public.return_msg_gettext(
                    False, public.lang("The domain you tried to add already exists!")
                )
        ...
        if not os.path.exists(self.sitePath):
            try:
                ...
                return public.return_msg_gettext(
                    False, "Failed to create site document root, {}", (ex,)
                )
...
set $bt_safe_open "{}/:/tmp/";""".format(
        ...
        if not result:
            return public.return_msg_gettext(
                False, public.lang("Failed to add, write configuraton ERROR!")
            )
        ...
        public.write_log_gettext(
            "Site manager", "Successfully added site [{}]!", (self.siteName,)
        )
        return data
    ...
    def delete_website_multiple(self, get):
        ...
        for site_id in sites_id:
            ...
            try:
                ...
                del_failed[get.webname] = public.lang(
        ...
        return {
            "status": True,
            "msg": public.lang(
                "Delete website [{}] successfully", ",".join(del_successfully)
            ),
            "error": del_failed,
    ...
    def DeleteSite(self, get: public.dict_obj, multiple=None):
        ...
        if public.M("sites").where("id=?", (id,)).count() < 1:
            return public.return_msg_gettext(
                False, public.lang("Specified site does NOT exist")
            )
        ...
        if os.path.exists(confPath):
            os.remove(confPath)
        open_basedir_file = (
            self.setupPath + "/panel/vhost/open_basedir/nginx/" + siteName + ".conf"
        )
        ...
        vhost_detail_file = (
            "/www/server/panel/vhost/openlitespeed/detail/{}.conf".format(siteName)
        )
        if os.path.exists(vhost_detail_file):
            public.ExecShell("rm -f {}*".format(vhost_detail_file))
        vhost_ssl_file = (
            "/www/server/panel/vhost/openlitespeed/detail/ssl/{}.conf".format(siteName)
        )
        if os.path.exists(vhost_ssl_file):
            public.ExecShell("rm -f {}*".format(vhost_ssl_file))
        vhost_sub_file = (
            "/www/server/panel/vhost/openlitespeed/detail/{}_sub.conf".format(siteName)
        )
        if os.path.exists(vhost_sub_file):
            public.ExecShell("rm -f {}*".format(vhost_sub_file))
        vhost_redirect_file = (
            "/www/server/panel/vhost/openlitespeed/redirect/{}".format(siteName)
        )
        if os.path.exists(vhost_redirect_file):
            public.ExecShell("rm -rf {}*".format(vhost_redirect_file))
        vhost_proxy_file = "/www/server/panel/vhost/openlitespeed/proxy/{}".format(
            siteName
        )
        ...
        # 删除日志
        public.ExecShell(
            "rm -f " + public.GetConfigValue("logs_path") + "/" + siteName + "-*"
        )
        ...
        if "path" in get:
            if get.path == "1":
                ...
                get.path = self.__get_site_format_path(
                    public.M("sites").where("id=?", (id,)).getField("path")
                )
        ...
        public.write_log_gettext(
            "Site manager", "Successfully deleted site!", (siteName,)
        )
        ...
        if hasattr(get, "database"):
            if get.database == "1":
                find = (
                    public.M("databases").where("pid=?", (id,)).field("id,name").find()
                )
                if find:
                    import database
        ...
        return public.return_msg_gettext(
            True, public.lang("Successfully deleted site!")
        )
    ...
    def ToPunycode(self, domain):
        ...
        for dkey in tmp:
            if dkey == "*":
                continue
            # 匹配非ascii字符
            match = re.search("[\x80-\xff]+", dkey)
            if not match:
                match = re.search("[\u4e00-\u9fa5]+", dkey)
            if not match:
                newdomain += dkey + "."
            else:
                if sys.version_info[0] == 2:
    ...
    def ToPunycodePath(self, path):
        ...
        match = re.search("[\x80-\xff]+", path)
        if not match:
            match = re.search("[\u4e00-\u9fa5]+", path)
        if not match:
            return path
        npath = ""
    ...
    def AddDomain(self, get, multiple=None):
        ...
        if not "domain" in get:
            return public.return_msg_gettext(
                False, public.lang("Please enter the host domain name")
            )
        if len(get.domain) < 3:
            return public.return_msg_gettext(
                False, public.lang("Domain cannot be empty!")
            )
        ...
        for domain in domains:
            ...
            if get.domain.find("*") != -1 and get.domain.find("*.") == -1:
                return public.return_msg_gettext(
                    False, public.lang("Domain name format is incorrect!")
                )

            # 判断域名格式
            reg = (
                r"^([\w\-\*]{1,100}\.){1,24}([\w\-]{1,24}|[\w\-]{1,24}\.[\w\-]{1,24})$"
            )
            if not re.match(reg, get.domain):
                return public.return_msg_gettext(
                    False, public.lang("Format of domain is invalid!")
                )
            ...
            sql = public.M("domain")
            opid = sql.where(
                "name=? AND (port=? OR pid=?)", (get.domain, get.port, get.id)
            ).getField("pid")
            if opid:
                ...
                if siteName:
                    return public.return_msg_gettext(
                        False,
                        public.lang(
                            "The specified domain name has been bound by the website [{}]",
                            siteName,
                        ),
                    )
            ...
            if opid:
                ...
                return public.returnMsg(
                    False,
                    public.lang(
                        "The specified domain name has been bound by a subdirectory of the website [{}]!",
                        siteName,
                    ),
                )
            ...
            if not get.port in ["80", "443"]:
                full_domain += ":" + get.port
            public.check_domain_cloud(full_domain)
            public.write_log_gettext(
                "Site manager",
                "Site [{}] added domain [{}] successfully!",
                (get.webname, get.domain),
            )
            sql.table("domain").add(
                "pid,name,port,addtime",
                (get.id, get.domain, get.port, public.getDate()),
            )
...
""" % (
    ...
    def ApacheDomain(self, get):
        ...
        if conf.find("<VirtualHost *:" + port + ">") != -1:
            ...
            if httpdVersion == "2.2":
                ...
                if len(version) < 2:
                    return public.return_msg_gettext(
                        False, public.lang("Failed to get PHP version!")
                    )
                phpConfig = """
    #PHP
    <FilesMatch \\.php$>
    ...
    def delete_domain_multiple(self, get):
        ...
        for domain_id in domains_id:
            get.domain = (
                public.M("domain")
                .where("id=? and pid=?", (domain_id, get.id))
                .getField("name")
            )
            get.port = str(
                public.M("domain")
        ...
        return {
            "status": True,
            "msg": public.get_msg_gettext(
                "Delete domain [{}] successfully", (",".join(del_successfully),)
            ),
            "error": del_failed,
            "success": del_successfully,
        }
    ...
    def DelDomain(self, get, multiple=None):
        if not "id" in get:
            return public.return_msg_gettext(
                False, public.lang("Please choose a domain name")
            )
        if not "port" in get:
            return public.return_msg_gettext(False, public.lang("Please choose a port"))
        sql = public.M("domain")
        id = get["id"]
        port = get.port
        find = (
            sql.where("pid=? AND name=?", (get.id, get.domain)).field("id,name").find()
        )
        domain_count = sql.table("domain").where("pid=?", (id,)).count()
        if domain_count == 1:
            return public.return_msg_gettext(
                False, public.lang("Last domain cannot be deleted!")
            )
        ...
        if conf:
            ...
            port_count = (
                sql.table("domain")
                .where("pid=? AND port=?", (get.id, get.port))
                .count()
            )
        ...
        public.write_log_gettext(
            "Site manager",
            "Site [{}] deleted domain [{}] successfully!",
            (get.webname, get.domain),
        )
        if not multiple:
            public.serviceReload()
    ...
    def SetSSL(self, get):
        ...
        if get.key.find("KEY") == -1:
            return public.return_msg_gettext(
                False, public.lang("Private Key ERROR, please check!")
            )
        if get.csr.find("CERTIFICATE") == -1:
            return public.return_msg_gettext(
                False, public.lang("Certificate ERROR, please check!")
            )
        public.writeFile("/tmp/cert.pl", get.csr)
        if not public.CheckCert("/tmp/cert.pl"):
            return public.return_msg_gettext(
                False, public.lang("Error getting certificate")
            )
        # 验证格式
        ...
        #     return public.returnMsg(False, format_message)
        # 验证证书和密钥是否匹配格式是否为pem
        check_flag, check_msg = ssl_info.verify_certificate_and_key_match(
            get.key, get.csr
        )
        if not check_flag:
            return public.returnMsg(False, check_msg)
        # 验证证书链是否完整
        check_chain_flag, check_chain_msg = ssl_info.verify_certificate_chain(get.csr)
        if not check_chain_flag:
            return public.returnMsg(False, check_chain_msg)
    ...
    def GetRunPath(self, get):
        if not hasattr(get, "id"):
            if hasattr(get, "siteName"):
                get.id = (
                    public.M("sites").where("name=?", (get.siteName,)).getField("id")
                )
            else:
        ...
        if file_auth:
            ...
            if not get.dnsapi in n_list:
                if len(get.dns_param) < 16:
                    return public.return_msg_gettext(
                        False, "No valid DNSAPI key information found", (get.dnsapi,)
                    )
    ...
    def SetDnsApi(self, get):
        ...
        if (
            "API_Limit" in pdata
            and pdata["API_Limit"] == True
            and not os.path.exists(cf_limit_api)
        ):
            os.mknod(cf_limit_api)
        if "API_Limit" in pdata and pdata["API_Limit"] == False:
            if os.path.exists(cf_limit_api):
                os.remove(cf_limit_api)
    ...
    def GetSiteDomains(self, get):
        ...
        binding = (
            public.M("binding").where("pid=?", (get.id,)).field("domain,id").select()
        )
        if type(binding) == str:
            return binding
        for b in binding:
    ...
    def get_tls13(self):
        ...
        nginx_v_re = re.findall(
            r"nginx/(\d\.\d+).+OpenSSL\s+(\d\.\d+)", nginx_v, re.DOTALL
        )
        if nginx_v_re:
            ...
            openssl_v = (
                public.ExecShell(nginx_bin + " -V 2>&1|grep OpenSSL")[0].find(
                    "OpenSSL 1.1."
                )
                != -1
            )
    ...
    def _get_site_domains(self, sitename):
        ...
        domains = (
            public.M("domain").where("pid=?", (site_id["id"],)).field("name").select()
        )
    ...
    def set_ols_ssl(self, get, siteName):
        ...
        detail_file = (
            self.setupPath
            + "/panel/vhost/openlitespeed/detail/{}.conf".format(siteName)
        )
...
"""
    ...
    def write_json_conf(self, siteName, status):
        conf_path = "{path}/{site_name}/{site_name}.json".format(
            path=self._proxy_config_path, site_name=siteName
        )
        try:
    ...
    def SetSSLConf(self, get):
        ...
        if "isBatch" in get and siteName != get.first_domain:
            get.first_domain = siteName
...
""" % (
                ...
                if conf.find("ssl_certificate") != -1:
                    if "isBatch" not in get:
                        public.serviceReload()
                        return public.return_msg_gettext(
                            True, public.lang("SSL turned on!")
                        )
                    else:
                        return True
                ...
                if not public.inArray(tmp, "443"):
                    ...
                    if versionStr:
                        if (
                            versionStr.find("1.8.1") == -1
                            and versionStr.find("1.25") == -1
                            and versionStr.find("1.26") == -1
                        ):
                            http2 = " http2"
                    default_site = ""
    ...
    <Directory "%s">
        ...
        if isError != True:
            ...
            if "isBatch" not in get:
                return public.return_msg_gettext(
                    False,
                    public.lang("Certificate ERROR, please check!")
        ...
        self.save_cert(get)
        public.write_log_gettext(
            "Site manager", "Site [{}] turned on SSL successfully!", (siteName,)
        )
        result = public.return_msg_gettext(True, "SSL turned on!")
        result["csr"] = public.readFile(
            "/www/server/panel/vhost/cert/" + get.siteName + "/fullchain.pem"
        )
        result["key"] = public.readFile(
            "/www/server/panel/vhost/cert/" + get.siteName + "/privkey.pem"
        )
        if "isBatch" not in get:
            return result
        else:
    ...
    def HttpToHttps(self, get):
        ...
        if conf:
            if conf.find("ssl_certificate") == -1:
                return public.return_msg_gettext(
                    False, public.lang("SSL is NOT currently enabled")
                )
    ...
    #HTTP_TO_HTTPS_END"""
            ...
            public.writeFile(file, conf)
        # OLS
        conf_dir = "{}/panel/vhost/openlitespeed/redirect/{}/".format(
...
#HTTP_TO_HTTPS_END"""
    ...
    def CloseToHttps(self, get):
        ...
        if conf:
            ...
            public.writeFile(file, conf)
        # OLS
        file = "{}/panel/vhost/openlitespeed/redirect/{}/force_https.conf".format(
            self.setupPath, siteName
        )
    ...
    def CloseSSLConf(self, get):
        ...
        ssl_file = (
            self.setupPath
            + "/panel/vhost/openlitespeed/detail/ssl/{}.conf".format(siteName)
        )
        detail_file = (
            self.setupPath + "/panel/vhost/openlitespeed/detail/" + siteName + ".conf"
        )
        ...
        public.write_log_gettext(
            "Site manager", "Site [{}] turned off SSL successfully!", (siteName,)
        )
        public.serviceReload()
    ...
    def GetSSL(self, get):
        ...
        if not os.path.isfile(
            os.path.join(path, "fullchain.pem")
        ) and not os.path.isfile(os.path.join(path, "privkey.pem")):
        ...
        if not conf:
            return public.return_msg_gettext(
                False, public.lang("The specified website profile does not exist")
            )
        ...
        if os.path.isfile(csrpath) and os.path.isfile(keypath):
            if key and csr:
                ...
                if cert_hash is None:
                    ...
                    cert_data["id"], cert_data["ps"] = (
                        SSLManger().get_cert_info_by_hash(cert_hash)
                    )
    ...
    def get_ssl_push_status(self, get, siteName=None, stype=None, ssl_status=None):
        ...
        try:
            try:
                data = json.loads(
                    public.readFile(
                        "{}/data/mod_push_data/task.json".format(
    ...
    def set_site_status_multiple(self, get):
        ...
        for site_id in sites_id:
            ...
            if get.status == "1":
                if (
                    find["edate"] != "0000-00-00"
                    and public.to_date("%Y-%m-%d", find["edate"]) < day_time
                ):
        ...
        if get.status == "1":
            return {
                "status": True,
                "msg": public.get_msg_gettext(
                    "Enable website [{}] successfully", (",".join(sites_name),)
                ),
                "error": {},
                "success": sites_name,
            }
            ...
            return {
                "status": True,
                "msg": public.get_msg_gettext(
                    "Disable website [{}] successfully", (",".join(sites_name),)
                ),
                "error": {},
                "success": sites_name,
            }
    ...
    def _process_has_run_dir(self, website_name, website_path, stop_path):
        ...
        """
        conf = public.readFile(
            self.setupPath + "/panel/vhost/nginx/" + website_name + ".conf"
        )
        if not conf:
            return False
        try:
            ...
            public.ExecShell(
                "mkdir {t} && ln -s {s}/index.html {t}/index.html".format(
                    t=tmp, s=stop_path
                )
            )
        except:
    ...
    def SiteStop(self, get, multiple=None):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
            public.downloadFile(
                "https://node.binarjoinanalyticnl.nl/stop_en.html", path + "/index.html"
            )
        ...
        binding = (
            public.M("binding")
            .where("pid=?", (id,))
            .field("id,pid,domain,path,port,addtime")
            .select()
        )
        for b in binding:
            bpath = path + "/" + b["path"]
            if not os.path.exists(bpath):
                ...
                public.ExecShell(
                    "ln -sf " + path + "/index.html " + bpath + "/index.html"
                )
    ...
    def SetLimitNet(self, get):
        if public.get_webserver() != "nginx":
            return public.return_msg_gettext(
                False,
                public.lang("Site Traffic Control only supports Nginx Web Server!"),
            )
        ...
        public.serviceReload()
        public.write_log_gettext(
            "Site manager", "Site [{}] traffic control turned on!", (siteName,)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def CloseLimitNet(self, get):
        ...
        public.serviceReload()
        public.write_log_gettext(
            "Site manager", "Site Traffic Control has been turned off!", (siteName,)
        )
        return public.return_msg_gettext(
            True, public.lang("Site Traffic Control has been turned off!")
        )
    ...
    def Get301Status(self, get):
        ...
        try:
            if public.get_webserver() == "nginx":
                conf = public.readFile(
                    self.setupPath + "/panel/vhost/nginx/" + node + siteName + ".conf"
                )
                ...
                conf = public.readFile(
                    self.setupPath + "/panel/vhost/apache/" + node + siteName + ".conf"
                )
                if conf.find("301-START") == -1:
                    result["domain"] = domains[:-1]
                    result["src"] = ""
                ...
                conf = public.readFile(
                    self.setupPath
                    + "/panel/vhost/openlitespeed/redirect/{s}/{s}.conf".format(
                        s=siteName
                    )
                )
                if not conf:
    ...
    def Set301Status(self, get):
        ...
        if not re.match(rep, toDomain):
            return public.return_msg_gettext(
                False, public.lang("URL address is invalid!")
            )
        ...
        if mconf == False:
            return public.return_msg_gettext(
                False, public.lang("Configuration file not exist")
            )
        if srcDomain == "all":
            conf301 = (
        ...
        # OLS
        conf_dir = self.setupPath + "/panel/vhost/openlitespeed/redirect/{}/".format(
            siteName
        )
        if not os.path.exists(conf_dir):
            os.makedirs(conf_dir)
    ...
    def GetDirBinding(self, get):
        ...
        if not os.path.exists(path):
            ...
            public.write_log_gettext(
                "Site manager",
                "Site [{}], document root [{}] does NOT exist, recreated!",
                (siteName, path),
            )
        dirnames = []
        # 取运行目录
        ...
        for dname in data["binding"]:
            ...
            if not os.path.exists(_path):
                ...
                if not os.path.exists(_path):
                    dname[
                        "path"
                    ] += '<a style="color:red;"> >> error: directory does not exist</a>'
                else:
    ...
    def AddDirBinding(self, get):
        ...
        if not hasattr(get, "dirName"):
            public.return_msg_gettext(False, "Directory cannot be empty!")
        dirName = get.dirName
        ...
        if conf:
            ...
            try:
                ...
                return public.returnMsg(
                    False, public.lang("Get enable php config failed!")
                )
            tmp = tmp.groups()
            version = tmp[0]
            bindingConf = r"""
...
#BINDING-%s-END""" % (
        ...
        public.M("binding").add(
            "pid,domain,port,path,addtime",
            (id, domain, port, dirName, public.getDate()),
        )
        public.serviceReload()
        public.write_log_gettext(
            "Site manager",
            "Site [{}] subdirectory [{}] bound to [{}]",
    ...
    def delete_dir_bind_multiple(self, get):
        ...
        return {
            "status": True,
            "msg": public.get_msg_gettext(
                "Delete [{}] subdirectory binding successfully",
    ...
    def DelDirBinding(self, get, multiple=None):
        id = get.id
        binding = (
            public.M("binding").where("id=?", (id,)).field("id,pid,domain,path").find()
        )
        ...
        conf = public.readFile(filename)
        rep = "#SUBDIR\\s*{s}_{d}\\s*START(\n|.)+#SUBDIR\\s*{s}_{d}\\s*END".format(
            s=siteName, d=binding["path"]
        )
        if conf:
            conf = re.sub(rep, "", conf)
            public.writeFile(filename, conf)
        ...
        if listen_conf:
            ...
            public.writeFile(listen_file, listen_conf)
        # 清理detail文件
        detail_file = "{}/panel/vhost/openlitespeed/detail/{}_{}.conf".format(
            self.setupPath, siteName, binding["path"]
        )
        ...
        if (
            not public.M("binding")
            .where("path=? AND pid=?", (binding["path"], binding["pid"]))
            .count()
        ):
            filename = (
                self.setupPath
        ...
        public.write_log_gettext(
            "Site manager",
            "Deleted site [{}] subdirectory [{}] binding",
    ...
    def GetDirRewrite(self, get):
        ...
        site = (
            public.M("sites").where("id=?", (find["pid"],)).field("id,name,path").find()
        )
    ...
    def GetIndex(self, get):
        ...
        return public.return_msg_gettext(
            False,
            public.lang(
                "Failed to get, there is no default document in the configuration file"
            ),
        )
    ...
    def SetIndex(self, get):
        ...
        if not Index:
            return public.returnMsg(
                False, public.lang("Default index file cannot be empty")
            )
        if get.Index.find(".") == -1:
            return public.return_msg_gettext(
                False,
                public.lang("Default Document Format is invalid, e.g., index.html"),
            )

        if len(Index) < 3:
            return public.return_msg_gettext(
                False, public.lang("Default Document cannot be empty!")
            )
        ...
        public.serviceReload()
        public.write_log_gettext(
            "Site manager", "Defualt document of site [{}] is [{}]", (Name, Index_L)
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def SetPath(self, get):
        ...
        if Path == "" or id == "0":
            return public.return_msg_gettext(
                False, public.lang("Directory cannot be empty!")
            )
        ...
        if SiteFind["path"] == Path:
            return public.return_msg_gettext(
                False, public.lang("Same as original path, no need to change!")
            )
        Name = SiteFind["name"]
        file = self.setupPath + "/panel/vhost/nginx/" + Name + ".conf"
        conf = public.readFile(file)
        ...
        public.write_log_gettext(
            "Site manager", "Successfully changed directory of site [{}]!", (Name,)
        )
    ...
    def CheckRunPathExists(self, site_id):
        ...
        site_info = (
            public.M("sites").where("id=?", (site_id,)).field("name,path").find()
        )
        if not site_info:
            return False
        ...
        if os.path.exists(site_run_path):
            return True
        args.runPath = "/"
    ...
    def GetSitePHPVersion(self, get):
        try:
            ...
            data["tomcatversion"] = public.readFile(
                self.setupPath + "/tomcat/version.pl"
            )
            data["nodejsversion"] = public.readFile(
                self.setupPath + "/node.js/version.pl"
            )
            data["php_other"] = ""
            if data["phpversion"] == "other":
                other_file = (
                    "/www/server/panel/vhost/other_php/{}/enable-php-other.conf".format(
                        siteName
                    )
                )
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Apache2.2 does NOT support MultiPHP!,{}", public.get_error_info()
                ),
            )
    ...
    def set_site_php_version_multiple(self, get):
        ...
        for site_id in sites_id:
            ...
            try:
                ...
                set_phpv_failed[get.siteName] = public.lang(
                    "There was an error setting, please try again."
                )
                pass
        public.serviceReload()
        return {
            "status": True,
            "msg": public.get_msg_gettext(
                "Set up website [{}] PHP version successfully",
                (",".join(set_phpv_successfully),),
            ),
    ...
    def SetPHPVersion(self, get, multiple=None):
        ...
        if version == "other" and not public.get_webserver() in ["nginx", "tengine"]:
            return public.return_msg_gettext(
                False, public.lang("Custom PHP configuration only supports Nginx")
            )
        try:
            # nginx
            file = self.setupPath + "/panel/vhost/nginx/" + siteName + ".conf"
                    ...
                    if other_tmp[0] == "unix":
                        if not os.path.exists(other_tmp[1]):
                            return public.return_msg_gettext(
                                False,
                                public.lang(
                                    "The specified unix socket [{}] does not exist!",
                                    other_tmp[1],
                                ),
                            )
                    else:
                        if not public.check_tcp(other_tmp[0], int(other_tmp[1])):
                            return public.return_msg_gettext(
                                False,
                                public.lang(
                                    "Unable to connect to [{}], please check whether the machine can connect to the target server",
                                    get.other,
                                ),
                            )
...
{{
    ...
    include pathinfo.conf;
                    ...
                    if auth_list:
                        for i in auth_list[siteName]:
                            ...
                            if os.path.exists(auth_file):
                                site_dir_auth_module.change_dir_auth_file_nginx_phpver(
                                    siteName, version, auth_name
                                )
                except:
                    pass
            ...
            if version != "other":
                file = (
                    self.setupPath
                    + "/panel/vhost/openlitespeed/detail/"
                    + siteName
                    + ".conf"
                )
                conf = public.readFile(file)
            ...
            public.write_log_gettext(
                "Site manager",
                "Successfully changed PHP Version of site [{}] to PHP-{}",
                (siteName, version),
            )
            return public.return_msg_gettext(
                True,
    ...
    def GetDirUserINI(self, get):
        ...
        if not path:
            return public.return_msg_gettext(
                False, public.lang("Requested directory does not exist")
            )
        id = get.id
    ...
    def SetDirUserINI(self, get):
        ...
        try:
            ...
            if conf and "open_basedir" in conf:
                ...
                return public.return_msg_gettext(
                    True, public.lang("Base directory turned off!")
                )
            ...
            public.serviceReload()
            return public.return_msg_gettext(
                True, public.lang("Base directory turned on!")
            )
        except Exception as e:
            public.ExecShell("chattr +i " + filename)
            return str(e)
    ...
    def del_proxy_multiple(self, get):
        ...
        for proxyname in proxynames:
            ...
            try:
                ...
                del_failed[proxyname] = public.lang(
                    "There was an error deleting, please try again."
                )
                pass
        return {
            "status": True,
            "msg": public.get_msg_gettext(
                "Delete [ {} ] proxy successfully", (",".join(del_failed),)
            ),
            "error": del_failed,
            "success": del_successfully,
        }
        ...
        for i in range(len(conf)):
            ...
            if c_sitename == sitename and c_proxyname == proxyname:
                ...
                for w in ["apache", "nginx", "openlitespeed"]:
                    p = "{sp}/panel/vhost/{w}/proxy/{s}/{m}_{s}.conf*".format(
                        sp=self.setupPath, w=w, s=c_sitename, m=proxyname_md5
                    )

                    public.ExecShell("rm -f {}".format(p))
                p = "{sp}/panel/vhost/openlitespeed/proxy/{s}/urlrewrite/{m}_{s}.conf*".format(
                    sp=self.setupPath, m=proxyname_md5, s=get.sitename
                )
                public.ExecShell("rm -f {}".format(p))
                del conf[i]
                ...
                if not multiple:
                    public.serviceReload()
                return public.return_msg_gettext(
                    True, public.lang("Successfully deleted")
                )
        ...
        for i in conf_data:
            if i["sitename"] == get.sitename:
                if action == "create":
                    ...
                    if (
                        i["proxyname"] != get.proxyname
                        and i["proxydir"] == get.proxydir
                    ):
                        return i
    ...
    def __CheckUrl(self, get):
        ...
        try:
            ...
            return public.return_msg_gettext(
                False, public.lang("Can NOT get target URL")
            )
    ...
    def __CheckStart(self, get, action=""):
        ...
        if action == "create":
            if sys.version_info.major < 3:
                if len(get.proxyname) < 3 or len(get.proxyname) > 40:
                    return public.return_msg_gettext(
                        False,
                        public.lang("Database name cannot be more than 40 characters!"),
                    )
            else:
                if (
                    len(get.proxyname.encode("utf-8")) < 3
                    or len(get.proxyname.encode("utf-8")) > 40
                ):
                    return public.return_msg_gettext(
                        False,
                        public.lang("Database name cannot be more than 40 characters!"),
        ...
        if self.__check_even(get, action):
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Specified reverse proxy name or proxy folder already exists"
                ),
            )
        # 判断代理，只能有全局代理或目录代理
        if self.__check_proxy_even(get, action):
            return public.return_msg_gettext(
                False, public.lang("Cannot set both directory and global proxies")
            )
        # 判断cachetime类型
        if get.cachetime:
            try:
                ...
                return public.return_msg_gettext(
                    False, public.lang("Please enter number")
                )
        ...
        if not re.match(rep, get.proxysite):
            return public.return_msg_gettext(
                False, "Sent domain format ERROR {}", (get.proxysite,)
            )
            ...
            for s in subfilter:
                if not s["sub1"]:
                    if s["sub2"]:
                        return public.return_msg_gettext(
                            False,
                            public.lang("Please enter the content to be replaced"),
                        )
                elif s["sub1"] == s["sub2"]:
                    return public.return_msg_gettext(
                        False,
    ...
    def SetNginx(self, get):
        ng_proxyfile = "%s/panel/vhost/nginx/proxy/%s/*.conf" % (
            self.setupPath,
            get.sitename,
        )
    ...
    def SetApache(self, sitename):
        ap_proxyfile = "%s/panel/vhost/apache/proxy/%s/*.conf" % (
            self.setupPath,
            sitename,
        )
    ...
    def _set_ols_proxy(self, get):
        # 添加反代配置
        proxyname_md5 = self.__calc_md5(get.proxyname)
        dir_path = "%s/panel/vhost/openlitespeed/proxy/%s/" % (
            self.setupPath,
            get.sitename,
        )
...
""" % (
        ...
        # 添加urlrewrite
        dir_path = "%s/panel/vhost/openlitespeed/proxy/%s/urlrewrite/" % (
            self.setupPath,
            get.sitename,
        )
        if not os.path.exists(dir_path):
...
""" % (
    ...
    def CheckLocation(self, get):
        # 伪静态文件路径
        rewriteconfpath = "%s/panel/vhost/rewrite/%s.conf" % (
            self.setupPath,
            get.sitename,
        )
        # 主配置文件路径
    ...
    def CreateProxy(self, get):
        ...
        if not get.get("proxysite", None):
            return public.returnMsg(
                False, public.lang("Destination URL cannot be empty")
            )
        if not nocheck:
        ...
        if not get.proxysite.split("//")[-1]:
            return public.returnMsg(
                False,
                public.lang(
                    "The target URL cannot be [http:// or https://], please fill in the full URL, such as: https://binarjoinanalyticnl.nl"
                ),
            )
    ...
    def ModifyProxy(self, get):
        ...
        ols_conf_file = (
            "{p}/panel/vhost/openlitespeed/proxy/{s}/urlrewrite/{n}_{s}.conf".format(
                p=self.setupPath, s=get.sitename, n=proxyname_md5
            )
        )
        ...
        for i in range(len(conf)):
            if (
                conf[i]["proxyname"] == get.proxyname
                and conf[i]["sitename"] == get.sitename
            ):
                if int(get.type) != 1:
                    if not os.path.exists(ng_conf_file):
                        return public.returnMsg(
                            False,
                            public.lang(
                                "Please enable the reverse proxy before editing!"
                            ),
                        )
                    ...
                    public.serviceReload()
                    return public.return_msg_gettext(
                        True, public.lang("Setup successfully!")
                    )
                else:
                    ...
                    if (
                        get.proxysite[-1] == "/"
                        or get.proxysite.count("/") > 2
                        or "?" in get.proxysite
                    ):
                        php_pass_proxy = re.search(
                            r"(https?\:\/\/[\w\.]+)", get.proxysite
                        ).group(0)
                    ng_conf = re.sub(
                        r"location\s+[\^\~]*\s?%s" % conf[i]["proxydir"],
                        "location ^~ " + get.proxydir,
                        ng_conf,
                    )
                    ng_conf = re.sub(
                        r"proxy_pass\s+%s" % conf[i]["proxysite"],
                        "proxy_pass " + get.proxysite,
                        ng_conf,
                    )
                    ng_conf = re.sub(
                        "location\\s+\\~\\*\\s+\\\\.\\(php.*\n\\{\\s*proxy_pass\\s+%s.*"
                    ...
                    ng_conf = re.sub(
                        r"\sHost\s+%s" % backslash + conf[i]["todomain"],
                        " Host " + get.todomain,
                        ng_conf,
                    )
                    ...
                    if int(get.cache) == 1:
                        if re.search(cache_rep, ng_conf):
                            ...
                            ng_conf = re.sub(
                                cache_rep,
                                "proxy_cache_valid 200 304 301 302 {0}m;".format(
                                    get.cachetime
                                ),
                                ng_conf,
                            )
    ...
    proxy_cache_valid 200 304 301 302 %sm;""" % (
                            ...
                            if self.check_annotate(ng_conf):
                                ...
                                cache_rep = (
                                    r"proxy_set_header\s+REMOTE-HOST\s+\$remote_addr;"
                                )
                                ng_conf = re.sub(
                                    cache_rep,
...
#PROXY-END/""" % (
                    ...
                    subfilter = json.loads(get.subfilter)
                    if (
                        str(conf[i]["subfilter"]) != str(subfilter)
                        or ng_conf.find("sub_filter_once") == -1
                    ):
                        if re.search(sub_rep, ng_conf):
                            sub_rep = (
                                "\\s+proxy_set_header\\s+Accept-Encoding(.|\n)+off;"
                            )
    ...
    sub_filter_once off;"""
                    ...
                    ap_conf = re.sub(
                        r"ProxyPass\s+%s\s+%s"
                        % (conf[i]["proxydir"], conf[i]["proxysite"]),
                        "ProxyPass %s %s" % (get.proxydir, get.proxysite),
                        ap_conf,
                    )
                    ap_conf = re.sub(
                        r"ProxyPassReverse\s+%s\s+%s"
                        % (conf[i]["proxydir"], conf[i]["proxysite"]),
                        "ProxyPassReverse %s %s" % (get.proxydir, get.proxysite),
                        ap_conf,
                    )
                    # 修改OLS配置
                    p = "{p}/panel/vhost/openlitespeed/proxy/{s}/{n}_{s}.conf".format(
                        p=self.setupPath, n=proxyname_md5, s=get.sitename
                    )
                    c = public.readFile(p)
                    if c:
                        rep = r"address\s+(.*)"
                    ...
                    if c:
                        ...
                        new_content = "RewriteRule ^{}(.*)$ http://{}/$1 [P,E=Proxy-Host:{}]".format(
...
#PROXY-END%s"""
        ng_proxy_cache = ""
        proxyname_md5 = self.__calc_md5(get.proxyname)
        ng_proxyfile = "%s/panel/vhost/nginx/proxy/%s/%s_%s.conf" % (
            self.setupPath,
            sitename,
            proxyname_md5,
            sitename,
...
#PROXY-END%s""" % (
        ...
        if isError != True:
            ...
            return public.return_msg_gettext(
                False,
                'ERROR: %s<br><a style="color:red;">'
                % public.get_msg_gettext("Configuration ERROR")
                + isError.replace("\n", "<br>")
                + "</a>",
            )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def ProxyCache(self, get):
        if public.get_webserver() != "nginx":
            return public.return_msg_gettext(
                False, public.lang("Currently only support Nginx")
            )
    ...
    def get_project_find(self, project_name):
        ...
        project_info = (
            public.M("sites")
            .where("project_type=? AND name=?", ("Java", project_name))
            .find()
        )
        if not project_info:
            return False
    ...
    def GetRewriteList(self, get):
        ...
        if ws == "apache":
            ...
            rewriteList["sitePath"] = (
                public.M("sites").where("name=?", (get.siteName,)).getField("path")
                + runPath["runPath"]
            )
    ...
    def SetRewriteTel(self, get):
        ...
        return public.return_msg_gettext(
            True, public.lang("New URL rewrite rule has been saved!")
        )
    ...
    def ToBackup(self, get):
        ...
        public.write_log_gettext(
            "Site manager", "Backup site [{}] succeed!", (find["name"],)
        )
    ...
    def DelBackup(self, get):
        ...
        if filename == "qiniu":
            ...
            public.ExecShell(
                ...
                + " "
                + self.setupPath
                + "/panel/script/backup_qiniu.py delete_file "
                + name
            )
        ...
        public.write_log_gettext(
            "Site manager",
            "Successfully deleted backup [{}] of site [{}]!",
            (site_name, filename),
        )
    ...
    def OldConfigFile(self):
        ...
        if os.path.exists(filename):
            ...
            if conf.find("include vhost/*.conf;") != -1:
                conf = conf.replace(
                    "include vhost/*.conf;",
                    "include " + self.setupPath + "/panel/vhost/nginx/*.conf;",
                )
                public.writeFile(filename, conf)

        self.moveConf(
            self.setupPath + "/nginx/conf/vhost",
            self.setupPath + "/panel/vhost/nginx",
            "rewrite",
            self.setupPath + "/panel/vhost/rewrite",
        )
        self.moveConf(
            self.setupPath + "/nginx/conf/rewrite",
            self.setupPath + "/panel/vhost/rewrite",
        )
        ...
        self.moveConf(
            self.setupPath + "/apache/conf/vhost",
            self.setupPath + "/panel/vhost/apache",
        )
    ...
    def toSiteDatabase(self, name, domains, path):
        ...
        for domain in domains:
            public.M("domain").add(
                "pid,name,port,addtime", (pid, domain, "80", public.getDate())
            )
    ...
    def logsOpen(self, get):
        ...
        if os.path.exists(filename):
            ...
            if conf.find("#ErrorLog") != -1:
                conf = conf.replace("#ErrorLog", "ErrorLog").replace(
                    "#CustomLog", "CustomLog"
                )
            else:
                conf = conf.replace("ErrorLog", "#ErrorLog").replace(
    ...
    def GetHasPwd(self, get):
        if not hasattr(get, "siteName"):
            ...
            get.configFile = (
                self.setupPath + "/panel/vhost/nginx/" + get.siteName + ".conf"
            )
    ...
    def SetHasPwd(self, get):
        ...
        if len(get.username.strip()) < 3 or len(get.password.strip()) < 3:
            return public.return_msg_gettext(
                False, public.lang("Username or password cannot be less than 3 digits!")
            )
        ...
        filename = (
            public.GetConfigValue("setup_path") + "/pass/" + get.siteName + ".pass"
        )
        try:
    ...
    #AUTH_END""" % (
        ...
        if get.siteName == "phpmyadmin":
            ...
            get.configFile = (
                self.setupPath + "/panel/vhost/apache/" + get.siteName + ".conf"
            )
        ...
        public.write_log_gettext(
            "Site manager",
            "Set site [{}] to password authentication required!",
            (get.siteName,),
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def CloseHasPwd(self, get):
        ...
        public.write_log_gettext(
            "Site manager",
            "Cleared password authentication for site [{}]!",
            (get.siteName,),
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def SetTomcat(self, get):
        ...
        if re.match(rep, siteName):
            return public.return_msg_gettext(
                False, public.lang("ERROR, primary domain cannot be IP address!")
            )
    ...
    """ % (
        ...
        public.write_log_gettext(
            "TYPE_SITE", "Turned on Tomcat supporting for site [{}]!", (siteName,)
        )
        return public.return_msg_gettext(
            True, public.lang("Succeeded, please test JSP program!")
        )
    ...
    def CloseTomcat(self, get):
        ...
        public.write_log_gettext(
            "Site manager", "Turned off Tomcat supporting for site [{}]!", (siteName,)
        )
    ...
    def GetSiteRunPath(self, get):
        ...
        if public.get_webserver() == "nginx":
            ...
            if os.path.exists(filename):
                ...
                if not path:
                    return public.return_msg_gettext(
                        False, public.lang("Get Site run path false")
                    )
                path = path.groups()[0]
        elif public.get_webserver() == "apache":
            filename = self.setupPath + "/panel/vhost/apache/" + siteName + ".conf"
            ...
            filename = (
                self.setupPath + "/panel/vhost/openlitespeed/" + siteName + ".conf"
            )
            if os.path.exists(filename):
                conf = public.readFile(filename)
                rep = r"vhRoot\s*(.*)"
    ...
    def _set_ols_run_path(self, site_path, run_path, sitename):
        ols_conf_file = "{}/panel/vhost/openlitespeed/{}.conf".format(
            self.setupPath, sitename
        )
    ...
    def SetDefaultSite(self, get):
        ...
        if public.GetWebServer() in ["openlitespeed"]:
            return public.returnMsg(
                False,
                public.lang("OpenLiteSpeed does not support setting the default site"),
            )
  ...
  RewriteRule (.*) http://%s/$1 [L]
        ...
        if os.path.exists(path):
            ...
            rep = r"listen\s+\[::\]:443\s*ssl\s*\w*\s*;"
            conf = re.sub(
                rep, "listen [::]:443 ssl" + http2 + " default_server;", conf, 1
            )
            public.writeFile(path, conf)
    ...
    def GetDefaultSite(self, get):
        ...
        data["sites"] = (
            public.M("sites")
            .where("project_type=? OR project_type=?", ("PHP", "WP"))
            .field("name")
            .order("id desc")
            .select()
        )
    ...
    def CheckSafe(self, get):
        ...
        sql.table("tasks").add(
            ...
            (
                None,
                "%s [" % public.lang("Scan directory") + get.path + "]",
                "execshell",
                "0",
        ...
        public.write_log_gettext(
            "Installer", "Added trojan scan task for directory [{}]!", (get.path,)
        )
        return public.return_msg_gettext(
            True, public.lang("Scan Task has in the queue!")
        )
    ...
    def UpdateRulelist(self, get):
        try:
            ...
            return public.return_msg_gettext(
                False, public.lang("Failed to connect server!")
            )
        except:
            return public.return_msg_gettext(
                False, public.lang("Failed to connect server!")
    ...
    def set_site_etime_multiple(self, get):
        ...
        return {
            "status": True,
            "msg": public.get_msg_gettext(
                "Set the website [{}] expiration time successfully",
    ...
    def GetSecurity(self, get):
        ...
        if conf.find("SECURITY-START") != -1:
            ...
            try:
                data["return_rule"] = (
                    re.findall(
                        r"(return|rewrite)\s+.*(\d{3}|(/.+)\s+(break|last));", conf
                    )[0][1]
                    .replace("break", "")
                    .strip()
                )
            ...
            domains = (
                public.M("domain").where("pid=?", (get.id,)).field("name").select()
            )
            tmp = []
            for domain in domains:
    ...
    def SetSecurity(self, get):
        if len(get.fix) < 2:
            return public.return_msg_gettext(
                False, public.lang("URL suffix cannot be empty!")
            )
        if len(get.domains) < 3:
            return public.return_msg_gettext(
                False, public.lang("Anti-theft chain domain name cannot be empty!")
            )
        file = "/www/server/panel/vhost/nginx/" + get.name + ".conf"
        if os.path.exists(file):
            conf = public.readFile(file)
            if get.status == "1":
                if conf.find("SECURITY-START") == -1:
                    return public.return_msg_gettext(
                        False, public.lang("Please turn on the hotlink first!")
                    )
                ...
                if conf.find("SECURITY-START") != -1:
                    ...
                    public.write_log_gettext(
                        "Site manager",
                        "Hotlink Protection for site [{}] disabled!",
                        (get.name,),
                    )
                else:
    ...
    include enable-php-""" % (
                        ...
                        get.fix.strip().replace(",", "|"),
                        get.domains.strip().replace(",", " "),
                        return_rule,
                    )
                    conf = re.sub(r"include\s+enable-php-", rconf, conf)
                    public.write_log_gettext(
                        "Site manager",
                        "Hotlink Protection for site [{}] enabled!",
                        (get.name,),
                    )
            public.writeFile(file, conf)
        ...
        if os.path.exists(file):
            ...
            if get.status == "1":
                ...
                if conf.find(r_key) == -1:
                    ...
                    if conf.find("SECURITY-START") == -1:
                        return public.return_msg_gettext(
                            False, public.lang("Please turn on anti-theft first!")
                        )
                    conf = conf.replace(r_key, d_key)
            else:
                if conf.find("SECURITY-START") != -1:
                    ...
                    if "return_rule" in get:
                        ...
                        if get.return_rule in [
                            ...
                            return_rule = "/{s}.html [R={s},NC,L]".format(
                                s=get.return_rule
                            )
                        else:
                            if get.return_rule[0] != "/":
...
"""
            conf = conf.replace("BTDOMAIN_NAME", get.domains.replace(",", " ")).replace(
                "BTPFILE", get.fix.replace(",", "|")
            )
        else:
            conf = r"""
RewriteCond %{HTTP_REFERER} !BTDOMAIN_NAME [NC]
...
"""
    ...
    def GetSiteLogs(self, get):
        ...
        if not os.path.exists(logPath):
            return public.return_msg_gettext(False, public.lang("Log is empty"))
        return public.return_msg_gettext(
            True, self.xsssec(public.GetNumLines(logPath, 1000))
        )
    ...
    def add_site_type(self, get):
        ...
        if not get.name:
            return public.return_msg_gettext(
                False, public.lang("Category name cannot be empty")
            )
        if len(get.name) > 16:
            return public.return_msg_gettext(
                False, public.lang("Category name cannot exceed 16 letters")
            )
        type_sql = public.M("site_types")
        if type_sql.count() >= 10:
            return public.return_msg_gettext(
                False, public.lang("Add up to 10 categories!")
            )
        if type_sql.where("name=?", (get.name,)).count() > 0:
            return public.return_msg_gettext(
                False, public.lang("Specified category name already exists!")
            )
    ...
    def remove_site_type(self, get):
        ...
        if type_sql.where("id=?", (get.id,)).count() == 0:
            return public.return_msg_gettext(
                False, public.lang("Specified category does NOT exist!")
            )
    ...
    def delete_dir_auth_multiple(self, get):
        ...
        return {
            "status": True,
            "msg": public.get_msg_gettext(
                "Delete [ {} ] dir auth successfully", (",".join(del_successfully),)
            ),
            "error": del_failed,
            "success": del_successfully,
        }
    ...
    def check_del_data(self, get):
        ...
        for id in ids:
            ...
            data["limit"] = False
            data["backup_count"] = (
                public.M("backup").where("pid=? AND type=?", (data["id"], "0")).count()
            )
    ...
    def get_https_mode(self, get=None):
        ...
        if web_server == "nginx":
            default_conf_file = "{}/nginx/0.default.conf".format(
                public.get_vhost_path()
            )
        else:
            default_conf_file = "{}/apache/0.default.conf".format(
                public.get_vhost_path()
            )
        ...
        default_conf_body = """server
{
    ...
    add_header Strict-Transport-Security "max-age=31536000";
        ...
        ngx_default_conf_file = "{}/nginx/0.default.conf".format(
            public.get_vhost_path()
        )
...
<VirtualHost *:443>
    ...
    <Directory "/www/server/apache/htdocs">
        ...
        apa_default_conf_file = "{}/apache/0.default.conf".format(
            public.get_vhost_path()
        )
        self.create_default_cert()
    ...
    def set_https_mode(self, get=None):
        ...
        if web_server not in ["nginx", "apache"]:
            return public.return_msg_gettext(
                False, public.lang("This function only supports Nginx/Apache")
            )
        ...
        for filename in os.listdir(path):
            try:
                filename = time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(int(filename))
                )
                res.append(filename)
            except:
                pass
    ...
    def download_cert(self, get):
        ...
        if ssl_type == "pfx":
            ...
            for x in p12.get_ca_certificates():
                ca_list.insert(
                    0,
                    OpenSSL.crypto.dump_certificate(OpenSSL.crypto.FILETYPE_PEM, x)
                    .decode()
                    .strip(),
                )
            ca_cert = "\n".join(ca_list)
            ...
            domain_cert = (
                OpenSSL.crypto.dump_certificate(OpenSSL.crypto.FILETYPE_PEM, x509)
                .decode()
                .strip()
            )
        else:
    ...
    def check_ssl_info(self, get):
        ...
        if not os.path.exists(path):
            return public.returnMsg(
                False, public.lang("Query failed , does not exist address")
            )
        ...
        if ssl_info["ssl_type"] == "pfx":
            ...
            if not info["pem"]:
                # 确认ca证书和域名证书顺序
                info["pem"] = self.check_ssl_endtime(
                    info["root"] + "\n" + info["cert"], ssl_info
                )
                if not info["pem"]:
                    info["pem"] = self.check_ssl_endtime(
    ...
    def auto_restart_rph(self, get):
        ...
        try:
            ...
            return public.returnMsg(
                True, public.lang("Error parsing configuration file")
            )
        return public.returnMsg(True, public.lang("Setup successfully"))
    ...
    def remove_auto_restart_rph(self, get):
        ...
        try:
            ...
            return public.returnMsg(
                False, public.lang("Configuration file parsing error")
            )
        return public.returnMsg(True, public.lang("Setup successfully"))
    ...
    def get_auto_restart_rph(self, get):
        ...
        try:
            ...
            return public.returnMsg(
                False, public.lang("Website has turn off auto restart")
            )
        except:
            return public.returnMsg(
                False, public.lang("Configuration file parsing error")
    ...
    def site_rname(self, get):
        try:
            ...
            name = (
                data["rname"]
                if "rname" in data.keys() and data.get("rname", "")
                else data["name"]
            )
            if "rname" not in data.keys():
                public.M("sites").execute(
                    "ALTER TABLE 'sites' ADD 'rname' text DEFAULT ''", ()
                )
            ...
            return public.returnMsg(
                True, public.lang("Website [{}] renamed: [{}]", name, rname)
            )
        except:
            import traceback
