...
# -------------------------------------------------------------------
# Author: cjxin <bt_ahong@binarjoinanalyticnl.nl>
# -------------------------------------------------------------------
...
class main(logsBase):
    ...
    def get_ftp_logs(self, get):
        ...
        ip_list = []
        log_list = []
        dirs = self.__get_ftp_log_files(
            "{}/ftpServer/Logs".format(public.get_soft_path())
        )
            ...
            while not next_file:
                ...
                for _line in result:
                    ...
                    try:
                        if self.find_line_str(_line, search):
                            # 根据用户名查找
                            if username and not re.search(
                                r"-\s+({})\s+\(".format(username), _line
                            ):
                                continue
        ...
        for arrs in data:
            ...
            if ip_addr:
                if result.get(ip_addr) != None:
                    result[ip_addr]["operation_num"] = (
                        result[ip_addr]["operation_num"] + 1
                    )
                else:
