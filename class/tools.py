...
def set_panel_pwd(password, ncli=False):
    ...
    result = (
        sql.table("users")
        .where("id=?", (1,))
        .setField("password", public.md5(password))
    )
...
"""
    ...
    print(
        "\033[1;32m|-%s\033[0m"
        % public.lang(
            "The panel is packaged successfully. Please do NOT log in to the panel to do any other operations!"
        )
    )
    print(
        "\033[1;41m|-%s: http://{SERVERIP}:"
        % public.lang("Panel initialization address")
        + port
        + "/install\033[0m"
    )
...
def ClearSystem():
    ...
    print(
        "\033[1;32m|-%s\033[0m"
        % public.lang(
            "System rubbish cleared, totally deleted [{}] files, freed disk space [{}]",
            str(count),
            ToSize(total),
        )
    )
...
def ClearMail():
    ...
    for d in os.listdir(rpath):
        ...
        print(
            public.lang(
                "|-Cleared [{}], deleted [{}] files, freed disk space [{}]",
                dpath,
                str(num),
                ToSize(size),
            )
    ...
    print(
        public.lang(
            "|-Spool cleared, deleted [{}] files, freed disk space [{}]",
            str(count),
            ToSize(total),
        )
    )
...
def ClearSession():
    ...
    print(
        public.lang(
            "|-PHP session cleared, deleted [{}] files, freed disk space [{}]",
            str(count),
            ToSize(total),
        )
    )
...
def ClearOther():
    ...
    print(
        public.lang(
            "|-Temporary files and site logs cleared, deleted [{}] files, freed disk space [{}]",
            str(count),
            ToSize(total),
        )
    )
...
def setup_idc():
    try:
        ...
        if os.path.exists(tFile):
            ...
            if title == public.lang("K2Panel") or title == "":
...
def update_to6():
    ...
    print(
        "\033[32m%s\033[0m"
        % public.lang("All plugins successfully updated to 6.0 compatible!")
    )
    print("====================================================")
...
def bt_cli():
    ...
    print(
        "(01) %s           (08) %s"
        % (public.lang("Restart panel"), public.lang("Change panel port"))
    )
    print(
        "(02) %s           (09) %s"
        % (public.lang("Stop panel"), public.lang("Clear panel cache"))
    )
    print(
        "(03) %s           (10) %s"
        % (public.lang("Restart panel"), public.lang("Clear login limit"))
    )
    print(
        "(04) %s           (11) %s"
        % (public.lang("Reload panel"), public.lang("Cancel entrance limit"))
    )
    print(
        "(05) %s           (12) %s"
        % (
            public.lang("Change panel password"),
            public.lang("Cancel domain binding limit"),
        )
    )
    print(
        "(06) %s         (13) %s"
        % (public.lang("Change panel username"), public.lang("Cacel IP access limit"))
    )
    print(
        "(07) %s      (14) %s"
        % (
            public.lang("Forcibly change MySQL root password"),
            public.lang("View panel default info"),
        )
    )
    print(
        "(00) %s                   (15) %s"
        % (public.lang("Task cancelled!"), public.lang("Clear system rubbish"))
    )
    print(raw_tip)
    try:
    ...
    if u_input == 1:
        ...
        if sys.version_info[0] == 2:
            input_user = raw_input(
                public.lang("Pls enter new username(>5 characters): ")
            )
        else:
        ...
        if old_port == input_port:
            print(
                public.lang(
                    "|-ERROR, new port is the same as current panel port, no need to change"
                )
            )
            return
