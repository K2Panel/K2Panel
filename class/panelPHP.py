...
class panelPHP:
    ...
    def exec_php_script(self, args):
        ...
        if not php_bin:
            return public.returnMsg(
                False,
                public.lang("No compatible PHP version found, please install first"),
            )
        # 是否将参数写到文件
        ...
        result = os.popen(
            "cd "
            + self.__plugin_path
            + ' && %s /www/server/panel/class/panel_php_run.php --args_tmp="%s" --plugin_name="%s" --fun="%s"'
            % (php_bin, self.__args_tmp, self.__plugin_name, args.s)
        ).read()
        try:
            # 解析执行结果
    ...
    def get_php_version(self, php_version):
        ...
        for pv in php_vs:
            php_bin = php_path + pv + "/bin/php"
            if os.path.exists(php_bin) and os.path.exists(
                "/tmp/php-cgi-{}.sock".format(pv)
            ):
                php_v = pv
                break
        return php_v
