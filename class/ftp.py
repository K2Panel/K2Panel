...
class ftp:
        ...
        try:
            if not os.path.exists("/www/server/pure-ftpd/sbin/pure-ftpd"):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Please install the Pure-FTPd service in the software store first."
                    ),
                )
            import files, time
            ...
            if get["ftp_username"].strip().find(" ") != -1:
                return public.returnMsg(
                    False, public.lang("Username cannot contain spaces")
                )
            if re.search(r"\W+", get["ftp_username"]):
                return {
                    ...
                    "code": 501,
                    "msg": public.lang(
                        "Username is illegal, special characters are NOT allowed!"
                    ),
                }
            if len(get["ftp_username"]) < 3:
                return {
                    ...
                    "code": 501,
                    "msg": public.lang(
                        "Username is illegal, cannot be less than 3 characters!"
                    ),
                }
            if not fileObj.CheckDir(get["path"]):
                return {
                    ...
                    "code": 501,
                    "msg": public.lang(
                        "System critical directory cannot be used as FTP directory!"
                    ),
                }
            if public.M("ftps").where("name=?", (get.ftp_username.strip(),)).count():
                return public.return_msg_gettext(
                    False, "User [{}] exists!", (get.ftp_username,)
                )
            username = get["ftp_username"].strip()
            if re.search("[\\/\\\\:\\*\\?\"'\\<\\>\\|]+", username):
                return public.return_msg_gettext(
                    False, public.lang('Name cannot contain /\\:*?"<>| symbol')
                )
            password = get["ftp_password"].strip()
            if len(password) < 6:
                return public.return_msg_gettext(
                    False, "Password must be at least [{}] characters", ("6",)
                )
            ...
            public.write_log_gettext(
                "FTP manager", "Successfully added FTP user [{}]!", (username,)
            )
            return public.return_msg_gettext(True, public.lang("Setup successfully!"))
        except Exception as ex:
            public.write_log_gettext(
                "FTP manager", "Failed to add FTP user[{}]! => {}", (username, str(ex))
            )
    ...
    def DeleteUser(self, get):
        try:
            ...
            public.write_log_gettext(
                "FTP manager", "Successfully deleted FTP user[{}]!", (username,)
            )
            return public.return_msg_gettext(True, public.lang("Successfully deleted"))
        except Exception as ex:
            public.write_log_gettext(
                "FTP manager",
                "Faided to delete FTP user[{}]! => {}",
                (username, str(ex)),
            )
            return public.return_msg_gettext(False, public.lang("Failed to delete"))
    ...
    def SetUserPassword(self, get):
        try:
            ...
            public.write_log_gettext(
                "FTP manager",
                "Successfully changed password for FTP user[{}]!",
                (username,),
            )
            return public.return_msg_gettext(True, public.lang("Setup successfully!"))
        except Exception as ex:
            public.write_log_gettext(
                "FTP manager",
                "Failed to change password FTP user[{}]! => {}",
                (username, str(ex)),
            )
    ...
    def SetStatus(self, get):
        ...
        try:
            ...
            if int(status) == 0:
                public.ExecShell(
                    self.__runPath + '/pure-pw usermod "' + username + '" -r 1'
                )
            else:
                public.ExecShell(
                    self.__runPath + '/pure-pw usermod "' + username + "\" -r ''"
                )
            self.FtpReload()
            public.M("ftps").where("id=?", (id,)).setField("status", status)
            public.write_log_gettext(
                "FTP manager", "Successfully {} FTP user [{}]!", (msg, username)
            )
            return public.return_msg_gettext(True, public.lang("Setup successfully!"))
        except Exception as ex:
            public.write_log_gettext(
    ...
    def setPort(self, get):
        try:
            ...
            if not port:
                return public.returnMsg(
                    False, public.lang("Please enter an integer for the port")
                )
            ...
            public.write_log_gettext(
                "FTP manager", "Successfully modified FTP port to [{}]!", (port,)
            )
            # 添加防火墙
            ...
            public.write_log_gettext(
                "FTP manager", "Failed to modify FTP port! => {}", (str(ex),)
            )
            return public.return_msg_gettext(False, public.lang("Failed to modify"))
    ...
    def FtpReload(self):
        public.ExecShell(
            self.__runPath + "/pure-pw mkdb /www/server/pure-ftpd/etc/pureftpd.pdb"
        )
    ...
    def set_user_home(self, get):
        ...
        try:
            ...
            if not auth_conf:
                return public.returnMsg(
                    False, public.lang("FTP account has not been set up")
                )
            ...
            if not macth_conf:
                return public.returnMsg(
                    False, public.lang("FTP account has not been set up1")
                )
            if len(macth_conf) > 1:
                return public.returnMsg(
                    False,
                    public.lang(
                        "Matching multiple configurations, this operation has been stopped!"
                    ),
                )
