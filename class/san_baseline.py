...
class san_baseline:
    ...
    def get_username(self):
        userInfo = (
            public.M("users").where("id=?", (1,)).field("id,username,password").find()
        )
        if (
            userInfo["username"] == "admin"
            or userInfo["username"] == "root"
            or userInfo["username"] == "password"
        ):
            return False
        else:
            return True
        ...
        for i in panel_chome:
            if not self.check_san_baseline(i):
                ret1 = {
                    ...
                    "Suggestions": "加固建议 : %s 权限改为%s 所属用户为%s"
                    % (i["file"], i["chmod"], i["user"]),
                    "repair": "加固建议 : %s 权限改为%s 所属用户为%s"
                    % (i["file"], i["chmod"], i["user"]),
                }
                result.append(ret1)
                ...
                if os.path.isdir(php_path + i):
                    if os.path.exists(php_path + i + "/etc/php.ini"):
                        php_data = {
                            ...
                            "Suggestions": "加固建议, 在%s expose_php的值修改为Off中修改"
                            % (php_path + i + "/etc/php.ini"),
                            "repair": "expose_php = Off",
                            "rule": [
                                {
                                    "re": "\nexpose_php\\s*=\\s*(\\w+)",
                                    "check": {"type": "string", "value": ["Off"]},
                                }
                            ],
                        }
                ...
                if os.path.isdir(php_path + i):
                    if os.path.exists(php_path + i + "/etc/php.ini"):
                        php_data = {
                            ...
                            "file": php_path + i + "/etc/php.ini",
                            "Suggestions": "加固建议, 在%s 中 disable_functions= 修改成如下:"
                            % (php_path + i + "/etc/php.ini"),
    ...
    def redis_security(self):
        ...
        redis_server_ip = {
            ...
            "Suggestions": "加固建议, 在%s 中的监听IP设置为127.0.0.1 例如"
            % ("/www/server/redis/redis.conf"),
            "repair": "bind 127.0.0.1",
            "rule": [
                {
                    "re": "\nbind\\s*(.+)",
                    "check": {"type": "string", "value": ["0.0.0.0"]},
                }
            ],
        }
        ...
        redis_server_not_pass = {
            ...
            "name": "Redis 查看是否设置密码",
            "file": "/www/server/redis/redis.conf",
            "Suggestions": "加固建议, 在%s 中的为未设置密码 例如"
            % ("/www/server/redis/redis.conf"),
        ...
        redis_server_pass = {
            ...
            "name": "Redis 存在弱密码",
            "file": "/www/server/redis/redis.conf",
            "Suggestions": "加固建议, 在%s 中requirepass 设置为强密码"
            % ("/www/server/redis/redis.conf"),
            "repair": "requirepass requirepassQWERQQQQQQQ",
            "rule": [
                {
    ...
    def user_security(self):
        ...
        if len(self.user_not_password()) >= 1:
            user_len = {
                ...
                "name": "系统存在空密码的用户",
                "file": "/etc/login.defs ",
                "Suggestions": "加固建议  为如下%s这些用户添加密码"
                % self.user_not_password(),
                "repair": "（如果用户不用可以删除）",
            }
            ...
            if not os.path.exists(path):
                ...
                if not ssl:
                    ...
                    if tls:
                        ...
                        site["name"] = (
                            "%s该站点启用了不安全的SSL协议LSv1 或者LSv1.1" % i["name"]
                        )
                        site["ps"] = (
                            "%s该站点启用了不安全的SSL协议LSv1 或者LSv1.1" % i["name"]
                        )
    ...
    def check_san_baseline(self, base_json):
        if base_json["type"] == "file":
            ...
            if os.path.exists(base_json["file"]):
                ...
                if (
                    ret["chown"] in base_json["user"]
                    and int(ret["chmod"]) in base_json["chmod"]
                    and ret["group"] in base_json["group"]
                ):
                    return True
                else:
                    return False
    ...
    def start(self, get):
        os.system(
            public.get_python_bin() + " /www/server/panel/class/san_baseline.py &"
        )
    ...
    def repair_san_baseline(self, base_json):
        ...
        if base_json["type"] == "chmod":
            if os.path.exists(base_json["file"]):
                os.system(
                    "chown %s:%s %s"
                    % (base_json["user"], base_json["group"], base_json["file"])
                )
