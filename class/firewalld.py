...
class firewalld:
    ...
    def AddAcceptPort(self, port, pool="tcp"):
        # 检查是否存在
        if self.CheckPortAccept(pool, port):
            return True
    ...
    def DelAcceptPort(self, port, pool="tcp"):
        # 检查是否存在
        if not self.CheckPortAccept(pool, port):
            return True
        mlist = self.__ROOT.getchildren()
        m = False
        for p in mlist:
    ...
    def CheckPortAccept(self, pool, port):
        for p in self.GetAcceptPortList():
            if p["port"] == port and p["protocol"] == pool:
                return True
        return False
    ...
    def DelDropAddress(self, address):
        # 检查是否存在
        if not self.CheckIpDrop(address):
            return True
        mlist = self.__ROOT.getchildren()
        for ip in mlist:
            if ip.tag != "rule":
    ...
    def Add_Port_IP(self, port, address, pool, type):
        if type == "accept":
            # 判断是否存在
            if self.Chekc_info(port, address, pool, type):
                return True
            attr = {"family": "ipv4"}
            rule = Element("rule", attr)
            attr = {"address": address}
                ...
                if i.tag == "rule":
                    ...
                    for c in i.getchildren():
                        ...
                        if tmp["type"]:
                            if (
                                tmp["port"] == port
                                and tmp["address"] == address
                                and tmp["type"] == type
                                and tmp["protocol"] == pool
                            ):
    ...
    def GetServiceStatus(self):
        ...
        for pid in psutil.pids():
            if psutil.Process(pid).name() == "firewalld":
                return True
        return False
