...
def check_run():
    pip = public.ExecShell(
        'btpip freeze | grep -E "istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap'
        "-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python"
        '-openssl|python-sqlite|virtualnv|mateplotlib|request="'
    )[0].strip()
    ...
    if pip:
        ...
        return (
            False,
            "【{}】security risk in the python library, please deal with it as soon as possible".format(
                "、".join(pip)
            ),
        )
    else:
