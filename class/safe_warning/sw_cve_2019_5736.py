...
def check_run():
    ...
    """
    docker = public.ExecShell("docker version --format='{{ .Server.Version }}'")[
        0
    ].strip()
    ...
    if len(docker[0]) < 2:
        return (
            False,
            "Risky，The current docker version has security risks and needs to be upgraded to a safe version",
        )
