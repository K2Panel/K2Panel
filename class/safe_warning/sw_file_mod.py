...
def check_run():
    ...
    for d in dir_list:
        ...
        if u_mode["user"] != d[2]:
            not_mode_list.append(
                "{} Current permissions: {} : {} Suggested changes to: {} : {}".format(
                    d[0], u_mode["mode"], u_mode["user"], d[1], d[2]
                )
            )
    ...
    if not_mode_list:
        return (
            False,
            "The following critical files or directories have permission errors: <br />"
            + ("<br />".join(not_mode_list)),
        )
    else:
