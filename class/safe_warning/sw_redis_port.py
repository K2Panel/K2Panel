...
def check_run():
    ...
    if not tmp:
        return (
            False,
            "Reids allows public internet connection, but no Redis password is set, which is extremely dangerous, please deal with it immediately",
        )
    ...
    if not is_strong_password(redis_pass):
        return (
            False,
            "Redis access password is too simple, and there are security risks",
        )
