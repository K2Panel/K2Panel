...
def check_run():
    ...
    for site_info in site_list:
        ...
        cert_file = "/www/server/panel/vhost/cert/{}/fullchain.pem".format(
            site_info["name"]
        )
        ...
        if s_time > cert_timeout:
            not_ssl_list.append(
                site_info["name"]
                + " Expiration: "
                + public.format_date("%Y-%m-%d", cert_timeout)
            )

    if not_ssl_list:
        return False, "The following sites SSL certificate has expired: <br />" + (
            "<br />".join(not_ssl_list)
        )
