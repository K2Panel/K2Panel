...
class setPanelLets:
    ...
    def __create_lets(self, get):
        ...
        if "private_key" not in cert_info:
            return public.returnMsg(
                False,
                public.lang(
                    "Failed to apply for a certificate, please try to manually apply for a certificate for the panel domain name on the site management page"
                ),
            )
    ...
    def __read_site_cert(self, domain_cert):
        try:
            key_file = "{path}{domain}/{key}".format(
                path=self.__vhost_cert_path,
                domain=domain_cert["subject"],
                key="privkey.pem",
            )
            cert_file = "{path}{domain}/{cert}".format(
                path=self.__vhost_cert_path,
                domain=domain_cert["subject"],
                cert="fullchain.pem",
            )
        except:
            key_file = "/www/server/panel/{}/privkey.pem".format(
                domain_cert["save_path"]
            )
            cert_file = "/www/server/panel/{}/fullchain.pem".format(
                domain_cert["save_path"]
            )
        ...
        if not os.path.exists(key_file):
            return public.returnMsg(
                False, public.lang("Can not found the ssl file! {}", key_file)
            )
    ...
    def __save_cert_source(self, domain, email):
        public.writeFile(
            self.__panel_cert_path + "lets.info",
            json.dumps({"domain": domain, "cert_type": "2", "email": email}),
        )
    ...
    def copy_cert(self, domain_cert):
        ...
        if (
            panel_cert_data["key"] != self.__tmp_key
            and panel_cert_data["cert"] != self.__tmp_cert
        ):
            self.__write_panel_cert()
            return public.returnMsg(True, public.lang("1"))
    ...
    def set_lets(self, get):
        ...
        if not domain:
            return public.returnMsg(
                False,
                public.lang(
                    "You need to bind the domain name to the panel before you can apply for the Lets Encrypt certificate."
                ),
            )
