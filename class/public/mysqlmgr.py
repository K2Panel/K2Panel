...
def backup(database_id: int) -> aap_t_simple_result:
    ...
    return aap_t_simple_result(
        True,
        M("backup")
        .where("type = 1 and pid=?", (database_id,))
        .order("id desc")
        .getField("filename"),
    )
...
def restore(db_name: str, bak_file: str) -> aap_t_simple_result:
    ...
    return aap_t_simple_result(
        int(data.get("status", 0)) == 0, data.get("message", {})["result"]
    )
...
def del_bak(bak_file: str) -> aap_t_simple_result:
    # k2panel内部备份
    bak_id_dict = (
        M("backup").where("`type`=1 and `filename`=?", (bak_file,)).field("id").find()
    )
...
# 数据库导出
def dumpsql_with_aap(
    database_id: int, backup_path: typing.Optional[str] = None
) -> aap_t_mysql_dump_info:
    import shlex
    ...
    if not isinstance(db_find, dict):
        raise HintException(
            get_msg_gettext("Table {} has been corrupted", ("databases",))
        )

    if backup_path is None:
        backup_path_tmp = (
            M("config").order("`id` desc").limit(1).field("backup_path").find()
        )

        if not isinstance(backup_path_tmp, dict):
            raise HintException(
                get_msg_gettext("Table {} has been corrupted", ("config",))
            )
    ...
    if db_find["db_type"] in ["0", 0]:
        ...
        if not database_obj.mypass(True, root):
            raise HintException(
                get_msg_gettext(
                    "Database configuration file failed to get checked, please check "
        ...
        try:
            ...
            if not password:
                raise HintException(
                    get_msg_gettext("Database password cannot be empty")
                )
        ...
        try:
            ...
            if isinstance(res, dict):
                raise HintException(
                    res.get("msg", get_msg_gettext("Cannot connect to remote MySQL"))
                )
    ...
    return aap_t_mysql_dump_info(
        db_name=str(db_find["name"]), file=backupName, dump_time=int(time.time())
    )
