...
# 简单网站信息
aap_t_simple_site_info = collections.namedtuple(
    "aap_t_simple_site_info", ["site_id", "database_id"]
)
...
def get_site_php_version(siteName: str) -> str:
    try:
        ...
        conf = readFile(
            "{setup_path}/panel/vhost/{webserver}/{siteName}.conf".format(
                setup_path=setup_path, webserver=webserver, siteName=siteName
            )
        )
...
def restore_files(site_id: int, bak_file: str) -> aap_t_simple_result:
    ...
    data = panel_restore().restore_website_backup(
        to_dict_obj({"site_id": site_id, "file_name": os.path.basename(bak_file)})
    )
    return aap_t_simple_result(
...
def del_bak(bak_file: str) -> aap_t_simple_result:
    # k2panel内部备份
    bak_id_dict = (
        M("backup").where("`type`=0 and `filename` = ?", (bak_file,)).field("id").find()
    )
...
def create_php_site_with_mysql(
    ...
    if int(data.get("databaseStatus", 0)) != 1:
        ...
        raise HintException(
            public.lang(
                "Database creation failed. Please check mysql running status and try again."
            )
        )
...
def remove_site(site_id: int) -> public.aap_t_simple_result:
    ...
    if not isinstance(site_info, dict):
        return public.aap_t_simple_result(
            False, public.lang("No found site-info with id {}", site_id)
        )

    from panel_site_v2 import panelSite

    data = panelSite().DeleteSite(
        to_dict_obj(
            {
                "id": site_id,
