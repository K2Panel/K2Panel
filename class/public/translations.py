...
def load_translations():
    ...
    scan_pattern_dir = "{}/BTPanel/static/vite/lang/{}".format(
        public.get_panel_path(), key
    )
    if not os.path.exists(scan_pattern_dir):
        key = "en"

    # 改获取用户设置的语言 没有使用默认英语
    scan_pattern = "{}/BTPanel/static/vite/lang/{}/*.json".format(
        public.get_panel_path(), key
    )
    ...
    try:
        ...
        translations2 = {
            ...
            if k == key or k == translations.get(key)
        }
...
def load_login_translations():
    ...
    scan_pattern = "{}/BTPanel/static/vite/lang/*/login.json".format(
