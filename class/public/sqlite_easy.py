...
# 匹配表名的正则表达式
match_table_name_reg = re.compile(
    r"^([\w`]+)(?:(?:\s+AS\s+|\s+)([\w`]+))?$", flags=re.IGNORECASE
)
...
# 匹配查询字段名称的正则表达式
match_row_key_reg = re.compile(
    r"^(?:distinct\s+)?(?:[\w`]+\.)?([\w`]+)(?:(?:\s+AS\s+|\s+)([\w`]+))?$",
    flags=re.IGNORECASE,
)
...
# 匹配子查询的正则表达式
search_subquery_reg = re.compile(
    r"^(\([\s\S]+?\))(?:(?:\s+AS\s+|\s+)([\w`]+))?$", flags=re.IGNORECASE
)
...
def _add_backtick_for_field(field):
    ...
    @return string
    """
    return match_field_reg.sub(
        lambda m: "{}{}".format(
            "" if m.group(1) is None else "`%s`." % m.group(1), "`%s`" % m.group(2)
        ),
        str(field).strip(),
    )
...
class Order:
    ...
    def add_order(self, field, ordering="ASC", params=()):
        ...
        self.__ORDERS.append(
            "{} {}".format(_add_backtick_for_field(field), ordering.upper())
        )
...
class Field:
    ...
    def set_fields(self, *fields):
        ...
        self.__FIELDS = list(
            map(
                lambda field: _add_backtick_for_field(field),
...
class Join:
    ...
    def add_join(self, expression, condition, join_type="INNER", table_prefix=""):
        ...
        if m:
            expression = "{}{}".format(
                _add_backtick_for_field(
                    "{}{}".format(table_prefix, m.group(1).strip("`"))
                ),
                (
                    ""
                    if m.group(2) is None
                    else " AS {}".format(_add_backtick_for_field(m.group(2)))
                ),
            )

        self.__JOINS.append(
            "{} JOIN {} ON {}".format(
                join_type.upper(), str(expression).strip(), str(condition).strip()
            )
        )
        return self
...
class Update:
    ...
    def increment(self, field, step=1):
        ...
        self.__UPDATES.append(
            "{field} = {field} + {step}".format(
                field=_add_backtick_for_field(field), step=str(int(step))
            )
        )
        return self
    ...
    def exp(self, field, exp):
        ...
        """
        self.__UPDATES.append(
            "{field} = {exp}".format(field=_add_backtick_for_field(field), exp=str(exp))
        )
        return self
...
class AlterTable:
    ...
    def rename_table(self, new_table_name):
        ...
        @return self
        """
        self.__RENAME_TABLE = " RENAME TO {}".format(
            _add_backtick_for_field(new_table_name)
        )
        return self
    ...
    def rename_column(self, col_name, new_col_name):
        ...
        if self.__column_exists(col_name):
            self.__ALTERS.append(
                " RENAME COLUMN {} TO {}".format(
                    _add_backtick_for_field(col_name),
                    _add_backtick_for_field(new_col_name),
                )
            )
    ...
    def add_column(self, col_name, prop, force=False):
        ...
        if not self.__column_exists(col_name):
            self.__ALTERS.append(
                " ADD COLUMN {} {}".format(_add_backtick_for_field(col_name), prop)
            )
    ...
    def drop_column(self, col_name):
        ...
        if self.__column_exists(col_name):
            self.__ALTERS.append(
                " DROP COLUMN {}".format(_add_backtick_for_field(col_name))
            )
            self.__COLUMNS.remove(col_name)
        return self
    ...
    def build(self, table_name):
        ...
        ret += "\n".join(
            list(
                map(lambda x: "ALTER TABLE {}{};".format(table_name, x), self.__ALTERS)
            )
        )
        ret += (
            "ALTER TABLE {}{};\n".format(table_name, self.__RENAME_TABLE)
            if self.__RENAME_TABLE is not None
            else "\n"
        )
        ret += "commit;"
...
class DbConnection:
    ...
    def __init__(self, db_name, engine=None):
        ...
        if self.__DB_PATH != ":memory:":
            self.__DB_LOCK_FILE = "/tmp/aap_locks/{}.lock".format(
                self.__DB_PATH.replace("/", "____")
            )
    ...
    def connect(self):
        ...
        self.__CONN = self.ENGINE.connect(
            self.__DB_PATH, timeout=15, check_same_thread=False
        )
    ...
    def commit(self) -> bool:
        ...
        """
        if (
            isinstance(self.__CONN, self.ENGINE.Connection)
    ...
    def __sqlite_retry_help(self, fn, *args, **kwargs):
        ...
        while retries > 0:
            try:
                ...
                SystemError,
                KeyError,
                self.ENGINE.InterfaceError,
                self.ENGINE.InternalError,
                self.ENGINE.OperationalError,
            ) as e:
                # 数据库操作错误，不是锁协议错误，直接抛出异常
                if isinstance(e, self.ENGINE.OperationalError) and str(e) not in [
                    "locking protocol",
                    "database is locked",
                ]:
                    raise e
    ...
    def query(self, sql, params=(), take_first=False):
        ...
        with self.__rlock():
            ...
            try:
                ret = self.__sqlite_retry_help(
                    self.__query_help, sql, params, take_first
                )
                e_time = time.time()
                return ret
            finally:
                if e_time - s_time > 1:
                    _log2(
                        "{}s {}".format(
                            round(e_time - s_time, 2),
                            reduce(
                                lambda x, y: search_question_reg.sub(
                                    str(y) if is_number(y) else "'%s'" % y, x, 1
                                ),
                                params,
                                sql,
                            ),
    ...
    def __query_help(self, sql, params=(), take_first=False):
        with _auto_repair_context(self.__DB_PATH):
            ...
            try:
                ...
                if e_time - s_time > 1:
                    _log2(
                        "{}s format_cursor".format(round(e_time - s_time, 2)),
                        "sqlite_slow.log",
                    )
    ...
    # 导出数据库
    def dump(
        self, dest_file: str, row_check_func: typing.Optional[callable] = None
    ) -> bool:
        """
        @name 导出数据库
...
class Db:
    ...
    def __init__(self, db_name, engine=None):
        ...
        if str(db_name).startswith(":memory:"):
            ...
            self.__DB_NAME = os.path.join(
                self.__DB_ROOT_DIR, re.sub(r"\.db$", "", db_name)
            )  # 数据库名称(全路径 不带.db)
    ...
    def backup(self, dest_db):
        ...
        if self.is_debug():
            self.debug_log(
                "backup to {}".format(dest_db.db_name()),
                self.db_name(),
                time.time() - s_time,
            )
    ...
    def dump(
        ...
        if self.is_debug():
            self.debug_log(
                "dump to {}".format(dest_file), self.db_name(), time.time() - s_time
            )
    ...
    def debug_log(self, content, db_name, cost_time):
        ...
        with open("{}/{}.log".format(log_dir, cur_datetime[8:10]), "a") as fp:
            fp.write(
                "[{}]{} {}ms {}\n".format(
                    cur_datetime, db_name, round(cost_time * 1000, 2), content
                )
            )
...
class SqliteEasy:
    ...
    def set_where_obj(self, where_obj):
        ...
        if not isinstance(where_obj, Where):
            raise PanelError(
                "where_obj must a instance of core.include.sqlite_server.Where"
            )
    ...
    def set_limit_obj(self, limit_obj):
        ...
        if not isinstance(limit_obj, Limit):
            raise PanelError(
                "limit_obj must a instance of core.include.sqlite_server.Limit"
            )
    ...
    def set_order_obj(self, order_obj):
        ...
        if not isinstance(order_obj, Order):
            raise PanelError(
                "order_obj must a instance of core.include.sqlite_server.Order"
            )
    ...
    def set_field_obj(self, field_obj):
        ...
        if not isinstance(field_obj, Field):
            raise PanelError(
                "field_obj must a instance of core.include.sqlite_server.Field"
            )
    ...
    def set_group_obj(self, group_obj):
        ...
        if not isinstance(group_obj, Group):
            raise PanelError(
                "group_obj must a instance of core.include.sqlite_server.Group"
            )
    ...
    def set_having_obj(self, having_obj):
        ...
        if not isinstance(having_obj, Having):
            raise PanelError(
                "having_obj must a instance of core.include.sqlite_server.Having"
            )
    ...
    def set_join_obj(self, join_obj):
        ...
        if not isinstance(join_obj, Join):
            raise PanelError(
                "join_obj must a instance of core.include.sqlite_server.Join"
            )
    ...
    def set_update_obj(self, update_obj):
        ...
        if not isinstance(update_obj, Update):
            raise PanelError(
                "update_obj must a instance of core.include.sqlite_server.Update"
            )
    ...
    def set_duplicate_obj(self, duplicate_obj):
        ...
        if not isinstance(duplicate_obj, Duplicate):
            raise PanelError(
                "duplicate_obj must a instance of core.include.sqlite_server.Duplicate"
            )
    ...
    def set_alter_table_obj(self, alter_table_obj):
        ...
        if not isinstance(alter_table_obj, AlterTable):
            raise PanelError(
                "alter_table_obj must a instance of core.include.sqlite_server.AlterTable"
            )
    ...
    def query(self, raw_sql, params=(), take_first=False, clear_conditions=True):
        ...
        if self.__DB.is_debug():
            self.__DB.debug_log(
                self.__to_raw_sql(raw_sql, _to_tuple(params)),
                self.__DB.db_name(),
                time.time() - s_time,
            )
    ...
    def insert(self, data, option=None, get_rowid=True):
        ...
        if option is not None and str(option).upper() not in self.__CONFLICT_OPTIONS:
            raise PanelError(
                "option must be one of: {}".format(", ".join(self.__CONFLICT_OPTIONS))
            )
    ...
    def insert_all(self, data_list, clear_conditions=True, option=None):
        ...
        # 绑定参数
        params = list(
            map(lambda x: _to_tuple(list(map(lambda y: x[y], ks))), data_list)
        )
        ...
        if self.__DB.is_debug():
            ...
            self.__DB.debug_log(
                self.__to_raw_sql(debug_raw_sql, _to_tuple(debug_params)),
                self.__DB.db_name(),
                time.time() - s_time,
            )
    ...
    def update(self, data=None):
        ...
        raw_sql = (
            "UPDATE {table_name}{join_condition} SET {exprission}"
    ...
    def column(self, field, dict_key=None):
        ...
        if self.__OPT_FIELD.is_empty() and field is not None:
            self.__OPT_FIELD.set_fields(
                *filter(lambda x: x is not None, [field, dict_key])
            )
    ...
    def get_pk(self):
        ...
        ret = self.query(
            "PRAGMA TABLE_INFO({})".format(self.__build_table_name(False)),
            take_first=True,
    ...
    def drop_index(self, idx_name):
        ...
        """
        self.execute(
            "DROP INDEX IF EXISTS {} ON {}".format(
                _add_backtick_for_field(idx_name), self.__build_table_name(False)
            )
        )
    ...
    def restore_from_snapshot(self, snapshot):
        ...
        if not isinstance(snapshot, Snapshot):
            raise PanelError(
                "snapshot must a instance of core.include.sqlite_server.Snapshot"
            )
    ...
    def explain_raw_sql(self, raw_sql, bind_params=()):
        ...
        """
        ret = self.query(
            "EXPLAIN QUERY PLAN {}".format(raw_sql), _to_tuple(bind_params)
        )
    ...
    def __build_table_name(self, contain_alias=True):
        ...
        table_name = (
            self.__DB_TABLE
            if self.__FROM_SUB_QUERY
            else _add_backtick_for_field(self.__DB_TABLE)
        )
        table_alias = ""
        ...
        if contain_alias:
            table_alias = (
                ""
                if self.__OPT_ALIAS is None
                else " AS {}".format(_add_backtick_for_field(self.__OPT_ALIAS))
            )
