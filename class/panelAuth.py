...
class panelAuth:
    ...
    def create_serverid(self, get):
        try:
            ...
            return public.return_msg_gettext(
                False, public.lang("Please login with account first")
            )
    ...
    def get_plugin_price(self, get):
        try:
            ...
            data = self.send_cloud(
                "{}/api/product/pricesV3".format(self.__official_url), params
            )
            if not data:
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Please log in to your K2Panel account on the panel first!"
                    ),
                )
            if not data["success"]:
            ...
            return public.return_msg_gettext(
                False,
                "Syncing information, please try again!\n {}",
    ...
    def get_plugin_list(self, get):
        try:
            if not session.get("get_product_bay") or not os.path.exists(
    ...
    def get_buy_code(self, get):
        ...
        data = self.send_cloud(
            "{}/api/order/product/create".format(self.__official_url), params
        )
        if not data["success"]:
    ...
    def get_stripe_session_id(self, get):
        ...
        data = self.send_cloud(
            "{}/api/order/product/pay".format(self.__official_url), params
        )
    ...
    def get_paypal_session_id(self, get):
        ...
        data = self.send_cloud(
            "{}/api/paypal/create_order".format(self.__official_url), params
        )
        session["focre_cloud"] = True
        data2 = {
    ...
    def check_paypal_status(self, get):
        ...
        data = self.send_cloud(
            "{}/api/paypal/capture_order".format(self.__official_url), params
        )
        # session['focre_cloud'] = True
        data2 = {
            "status": data.get("success", False),
    ...
    def get_business_plugin(self, get):
        try:
            if not session.get("get_product_list") or not os.path.exists(
                self.__product_list_path
            ):
                data = self.send_cloud(
                    "{}/api/product/chargeProducts".format(self.__official_url), {}
                )
                if data["success"]:
                    public.writeFile(self.__product_list_path, json.dumps(data["res"]))
    ...
    def get_voucher_plugin(self, get):
        ...
        params["status"] = "0"
        data = self.send_cloud(
            "{}/api/user/productVouchers".format(self.__official_url), params
        )
        if not data:
            return []
        return data["res"]
    ...
    def get_cpuname(self):
        return public.ExecShell("cat /proc/cpuinfo|grep 'model name'|cut -d : -f2")[
            0
        ].strip()
    ...
    def get_product_auth(self, get):
        ...
        data = self.send_cloud(
            "{}/api/user/productAuthorizes".format(self.__official_url), params
        )
        if not data:
            return []
        if not data["success"]:
    ...
    def auth_activate(self, get):
        ...
        params["environment_info"] = json.dumps(public.fetch_env_info())
        data = self.send_cloud(
            "{}/api/authorize/product/activate".format(self.__official_url), params
        )
        if not data["success"]:
            return public.return_msg_gettext(False, public.lang("Activate Failed"))
        session["focre_cloud"] = True
    ...
    def renew_product_auth(self, get):
        ...
        if hasattr(get, "coupon_id") and get.pay_channel == "10":
            params["coupon_id"] = get.coupon_id
        data = self.send_cloud(
            "{}/api/authorize/product/renew".format(self.__official_url), params
        )
    ...
    def free_trial(self, get):
        ...
        params["environment_info"] = json.dumps(public.fetch_env_info())
        data = self.send_cloud(
            "{}/api/product/obtainProfessionalMemberFree".format(self.__official_url),
            params,
        )
        session["focre_cloud"] = True
        # 使用抵扣券续费直接返回续费结果
    ...
    def get_plugin_remarks(self, get):
        ...
        if not hasattr(get, "product_id"):
            return public.return_msg_gettext(
                False, public.lang("product_id Parameter ERROR!")
            )
        ...
        if not data:
            return public.returnMsg(
                False, public.lang("Failed to connect to the server!")
            )
        session[ikey] = data
        return data
