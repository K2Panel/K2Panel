...
class SiteDirAuth:
    ...
    def set_dir_auth(self, get):
        ...
        if public.get_webserver() == "openlitespeed":
            return public.return_msg_gettext(
                False, public.lang("OpenLiteSpeed is currently not supported")
            )
        ...
        if not get.site_dir:
            return public.return_msg_gettext(
                False, public.lang("Please enter the directory to be protected")
            )
        if not get.name:
            return public.return_msg_gettext(
                False, public.lang("Please enter the Name")
            )
        ...
        if self._check_dir_auth(site_name, name, site_dir):
            return public.return_msg_gettext(
                False, public.lang("Directory has been protected")
            )
        ...
        # 配置独立认证文件
        self.set_dir_auth_file(
            site_info["site_path"], site_name, name, username, site_dir, auth_file
        )
        # 配置站点主文件
    ...
    def get_site_php_version(self, siteName):
        try:
            ...
            return public.return_msg_gettext(
                False, public.lang("Apache2.2 does NOT support MultiPHP!")
            )
    ...
    def change_dir_auth_file_nginx_phpver(self, site_name, phpv, auth_name):
        ...
        if phpv == "other":
            php_conf = "include /www/server/panel/vhost/other_php/{}/enable-php-other.conf;".format(
                site_name
            )
        else:
    ...
    # 设置独立认证文件
    def set_dir_auth_file(
        self, site_path, site_name, name, username, site_dir, auth_file
    ):
        php_ver = self.get_site_php_version(site_name)
        php_conf = ""
        if php_ver:
            if php_ver == "other":
                php_conf = "include /www/server/panel/vhost/other_php/{}/enable-php-{}.conf;".format(
                    site_name, php_ver
                )
            else:
    ...
    #Require all granted
        DirectoryIndex index.php index.html index.htm default.php default.html default.htm
            ...
            conf_file = file_path.format(
                setup_path=self.setup_path, site_name=site_name, webserver=i
            )
    ...
    def set_conf(self, site_name, act):
        for i in ["nginx", "apache"]:
            dir_auth_file = "%s/panel/vhost/%s/dir_auth/%s/*.conf" % (
                self.setup_path,
                i,
                site_name,
            )
        ...
        if isError != True:
            ...
            return public.return_msg_gettext(
                ...
                public.lang(
                    ...
                    % public.lang("Configuration ERROR")
                    + isError.replace("\n", "<br>")
                    + "</a>"
                ),
            )
    ...
    def delete_dir_auth(self, get):
        ...
        if site_name not in conf:
            return public.return_msg_gettext(
                False,
                "The website does not exist in the configuration：{}",
                (site_name,),
            )
    ...
    def __check_param(self, get):
        ...
        if hasattr(get, "password"):
            ...
            if len(password) < 3:
                return public.returnMsg(
                    False, public.lang("Password cannot be less than 3 characters")
                )
            if re.search(r"\s", password):
                return public.returnMsg(
                    False, public.lang("Password cannot contain spaces")
                )
        ...
        if hasattr(get, "username"):
            ...
            if len(username) < 3:
                return public.returnMsg(
                    False, public.lang("Username cannot be less than 3 characters")
                )
        ...
        if hasattr(get, "name"):
            ...
            if len(name) < 3:
                return public.returnMsg(
                    False, public.lang("Name cannot be less than 3 characters")
                )
            if re.search(r"\s", name):
                return public.returnMsg(
                    False, public.lang("Name cannot contain spaces")
                )
            if re.search("[\\/\"'\\!@#$%^&*()+={}\\[\\]\\:\\;\\?><,./\\\\]+", name):
                return public.returnMsg(
                    False, public.lang("Name format must be [ aaa_bbb ]")
                )
            values["name"] = name
