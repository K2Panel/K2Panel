...
class one_key_wp:
    ...
    def get_plugin_page(self):
        ...
        self.__plugin_page_content = self.__wp_session.get(
            _get_ajax_nonce_url, headers=self.get_headers(), verify=False
        ).text
    ...
    def __login_wp(self):
        ...
        self.__session_resp = retry(
            lambda: self.__wp_session.post(
                wp_login, headers=headers1, data=datas, verify=False
            )
        )
    ...
    def download_file(self):
        try:
            ...
            print(
                "Download the installation package: {} --> {}".format(
                    self.wp_package_url, self.package_zip
                )
            )
            self.write_logs(
                "|-Download the installation package: {} --> {}".format(
    ...
    def create_wp_table(self):
        if (
            ...
            .where("type=? AND name=?", ("table", "wordpress_onekey"))
            .count()
        ):
            public.M("").execute(
                """CREATE TABLE "wordpress_onekey" (
    ...
    def request_setup_3(self, values):
        ...
        url = "http://{}/wp-admin/install.php?language={}".format(
            values["site_name"], values["language"]
        )
        param = {
            "url": url,
            "headers": {"Host": values["domain"]},
    ...
    def get_update_wp_nonce(self):
        ...
        response = retry(
            lambda: self.get_wp_nonce(self.action_plugin_get(url), _wp_nonce_rep)
        )
        return response
    ...
    def action_plugin_post(self, param):
        ...
        if "upgrade" in param["data"]:
            ...
            headers["referer"] = "http://{}/wp-admin/update-core.php".format(
                param["domain"]
            )

        response = retry(
            lambda: self.__wp_session.post(
                param["url"], data=param["data"], headers=headers, verify=False
            ).text
        )
        return response
    ...
    def action_plugin_get(self, url, headers=None):
        ...
        response = retry(
            lambda: self.__wp_session.get(url, headers=headers, verify=False).text
        )
        return response
    ...
    def get_smart_http_expire_form_nonce(self, url):
        ...
        smart_http_expire_form_nonce = (
            '"smart_http_expire_form_nonce\\"\\svalue=\\"(\\w+)'
        )
        return self.get_wp_nonce(
            self.action_plugin_get(url), smart_http_expire_form_nonce
        )
    ...
    def set_nginx_helper(self, values):
        ...
        param = {
            ...
            "data": {
                ...
                "log_level": "INFO",
                "log_filesize": "5",
                "smart_http_expire_form_nonce": self.get_smart_http_expire_form_nonce(
                    url
                ),
    ...
    def act_nginx_helper_active(self, values):
        ...
        url = "http://{}/wp-admin/{data}".format(
            self.__domain, data=str(str(res[0]).split('"')[5])
        )
        url = url.replace("amp;", "")
    ...
    def generate_wp_passwd(self, site_path, new_pass):
        hash_password_code = public.readFile(
            "{}/wp-includes/class-phpass.php".format(site_path)
        )
        extra_code = (
            """
         $passwordValue = "%s";
        ...
        return public.return_msg_gettext(
            False, public.lang("Generated password detection failed!")
        )
    ...
    def get_wp_username(self, get):
        ...
        db_name = (
            public.M("databases").where("id=?", (db_info["d_id"],)).field("name").find()
        )
        ...
        res = mysql_obj.query(
            "select * from {}.{}users".format(db_name, db_info["prefix"])
        )
    ...
    def is_update(self, get):
        ...
        if str(online_v) != str(local_v):
            update = True
        data = {"online_v": online_v, "local_v": local_v, "update": update}
    ...
    def purge_all_cache(self, get):
        ...
        if public.get_webserver() != "nginx":
            return public.return_msg_gettext(
                False, public.lang("This feature currently only supports Nginx")
            )
    ...
    def set_fastcgi_cache(self, get):
        ...
        return fast_cgi().set_website_conf(
            values["version"], values["sitename"], values["act"]
        )
    ...
    def update_wp(self, get):
        ...
        param = {
            "url": "http://{}/wp-admin/update-core.php?action=do-core-upgrade".format(
                self.__domain
            ),
    ...
    def check_param(self, args):
        ...
        if hasattr(args, "d_id"):
            if re.search(r"\d+", args.d_id):
                ...
                return public.return_msg_gettext(
                    False,
                    "Please check if the [{}] format is correct For example: {}",
                    ("d_id", "99"),
                )
    ...
    def deploy_wp(self, get):
     ...
     """.format(
            ...
            return public.return_msg_gettext(
                True, public.lang("Deployment was successful!")
            )
        except:
            self.del_site(get)
...
class optimize_php:
 ...
 """.format(
                get.max_children,
                get.start_servers,
                get.min_spare_servers,
                get.max_spare_servers,
                get.pm,
                get.listen,
            )
        )
        ...
        if not result["status"]:
            ...
            return public.return_msg_gettext(
                False, "PHP FPM Optimization failed: {}", (result,)
            )
        one_key_wp().write_logs("|-PHP FPM optimization succeeded")
        return public.return_msg_gettext(
            True, public.lang("PHP FPM optimization succeeded")
        )
...
class optimize_db:
 ...
 """.format(
                ...
                get.innodb_log_buffer_size,
                get.sort_buffer_size,
                get.read_buffer_size,
                get.read_rnd_buffer_size,
                get.join_buffer_size,
                get.thread_stack,
                get.binlog_cache_size,
                get.thread_cache_size,
                get.table_open_cache,
                get.max_connections,
                get.query_cache_type,
                get.max_heap_table_size,
            )
        )
        import database
...
class fast_cgi:
 ...
 """
        ...
        if "#K2PANEL_FASTCGI_CONF_BEGIN" in content:
            one_key_wp().write_logs(
                "|-Nginx FastCgi cache configuration already exists"
            )
            print("Nginx FastCgi cache configuration already exists")
            return public.return_msg_gettext(
                True, public.lang("Nginx FastCgi cache configuration already exists")
            )
        rep = "http\\s*\n\\s*{"
 ...
 """
        ...
        if "#K2PANEL_FASTCGI_CONF_BEGIN" in content_init:
            one_key_wp().write_logs(
                "|-Nginx init FastCgi cache configuration already exists"
            )
            print("Nginx init FastCgi cache configuration already exists")
            return public.return_msg_gettext(
                True,
                public.lang("Nginx init FastCgi cache configuration already exists"),
            )
        # content_init = re.sub(r"\$NGINX_BIN -c \$CONFIGFILE", + conf2, content_init)
        rep2 = r"\$NGINX_BIN -c \$CONFIGFILE"
        content_init = re.sub(
            rep2, conf2 + "        $NGINX_BIN -c $CONFIGFILE", content_init
        )
        ...
        if conf_pass == False:
            ...
            one_key_wp().write_logs(
                "|-Nginx init FastCgi configuration error! {}".format(conf_pass)
            )
            print("Nginx init FastCgi configuration error! {}".format(conf_pass))
            return public.return_msg_gettext(
                False, public.lang("Nginx init FastCgi configuration error!")
            )
        ...
        return public.return_msg_gettext(
            True, public.lang("Nginx init FastCgi cache configuration complete")
        )

    def set_fastcgi_php_conf(self, version):
        conf_path = "/www/server/nginx/conf/enable-php-{}-wpfastcgi.conf".format(
            version
        )
        if os.path.exists(conf_path):
            one_key_wp().write_logs(
                "|-Nginx FastCgi PHP configuration already exists..."
            )
    ...
    def set_website_conf(self, version, sitename, act=None):
        ...
        if not conf:
            ...
            one_key_wp().write_logs(
                "|-Website configuration file does not exist: {}".format(conf_path)
            )
            return False
        if act == "disable":
            ...
            if fastcgi_conf not in conf:
                ...
                one_key_wp().write_logs(
                    "|-FastCgi configuration does not exist in website configuration, skip"
                )
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "FastCgi configuration does not exist in website configuration"
                    ),
                )
        ...
        if conf_pass != True:
            ...
            one_key_wp().write_logs(
                "|-Website FastCgi configuration error: {}", (conf_pass,)
            )
            return public.return_msg_gettext(
                False, public.lang("Website FastCgi configuration error！")
            )
        print("Website FastCgi configuration complete")
        one_key_wp().write_logs("|-Website FastCgi configuration complete...")
        return public.return_msg_gettext(
            True, public.lang("Website FastCgi configuration complete")
        )
 ...
 """
        ...
        if not conf:
            ...
            one_key_wp().write_logs(
                "|-Wordpress configuration file does not exist: {}".format(conf_file)
            )
            return public.return_msg_gettext(
                False, "Wordpress configuration file does not exist: {}", (conf_file,)
            )
        if "K2PANEL_FASTCGICACHE_BEGIN" in conf:
            one_key_wp().write_logs(
                "|-Cache cleaning configuration already exists, skip"
            )
    ...
    def set_userini(self, site_path):
        ...
        if not conf:
            print(
                "Anti-cross-site configuration file does not exist: {}".format(
                    conf_file
                )
            )
            one_key_wp().write_logs(
            ...
            return public.return_msg_gettext(
                False,
                "Anti-cross-site configuration file does not exist: {}",
                (conf_file,),
            )
