...
"""
    ...
    def log_analysis(self, get):
        ...
        if not os.path.exists(path):
            return public.ReturnMsg(False, "No log file")
        if os.path.getsize(path) > 9433107294:
            return public.ReturnMsg(False, "The log file is too large!")
        if os.path.getsize(path) < 10:
            return public.ReturnMsg(False, "log is empty")
        ...
        if self.get_log_format(path):
            public.ExecShell(
                "cd %s && bash %s san_log %s %s &"
                % (self.path, self.log_analysis_path, path, log_path)
            )
        else:
            public.ExecShell(
                "cd %s && bash %s san %s %s &"
    ...
    def speed_log(self, get):
        ...
        if not os.path.exists(speed):
            return public.ReturnMsg(False, "The directory was not scanned")
        try:
            data = public.ReadFile(speed)
            data = int(data)
            if data == 100:
                time_data, start_time, status = public.ReadFile(
                    self.path + "/log/" + log_path + ".time"
                ).split("[]")
                public.WriteFile(
                    speed + ".time",
                    str(time.time() - float(time_data))
                    + "[]"
                    + start_time
                    + "[]"
                    + "1",
    ...
    def get_log_count(self, path, is_body=False):
        ...
        if is_body:
            ...
            with open(path, "r") as f:
                for i in f:
                    count += 1
                    data = data.replace("<", "&lt;").replace(">", "&gt;") + i.replace(
                        "<", "&lt;"
                    ).replace(">", "&gt;")
                    if count >= 300:
                        break
    ...
    def get_detailed(self, get):
        ...
        if get.type not in type_list:
            return public.ReturnMsg(False, "Type mismatch")
        if not os.path.exists(speed + get.type + ".log"):
            return public.ReturnMsg(False, "Record does not exist")
