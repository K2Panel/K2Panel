...
class ssh_authentication:
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.__config_pl):
            public.ExecShell(
                "echo  '%s' >>%s" % (public.GetRandomString(32), self.__config_pl)
            )
    ...
    def get_pam_dir(self):
        ...
        if os.path.exists("/etc/redhat-release"):
            ...
            if isinstance(version, str):
                if version.find(" 7.") != -1:
                    return "auth  requisite  %s/pam_btssh_authentication.so" % (
                        self.__python_pam
                    )
    ...
    def install_pam_python(self, check):
        ...
        public.ExecShell(
            "/usr/local/curl/bin/curl -k -o %s https://download.bt.cn/btwaf_rule/pam_python_so/%s"
            % (so_path, so_name)
        )
        ...
        if isinstance(file, str):
            if file.find("PasswordAuthentication") != -1:
                ...
                return public.returnMsg(
                    False, public.lang("No password authentication")
                )
    ...
    def start_password(self):
        ...
        if isinstance(file, str):
            if file.find("PasswordAuthentication") != -1:
                ...
                self.restart_ssh()
                return public.returnMsg(
                    True, public.lang("Turn on password authentication successfully")
                )
            else:
                file_result = file + "\nPasswordAuthentication yes"
                self.wirte(self.__SSH_CONFIG, file_result)
    ...
    def start_ssh_authentication_two_factors(self):
        if not self.get_pam_dir():
            return public.returnMsg(False, public.lang("The system is not supported"))
        check = self.isPamSoExists()
        if not check:
            return "False"
    ...
    def download_so(self):
        ...
        if os.path.exists(config_data[2]):
            if os.path.getsize(config_data[2]) > 10240:
                return public.returnMsg(
                    True, public.lang("Download the file successfully")
                )
