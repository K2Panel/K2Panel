...
class main(panelBase):
    ...
    def get_update_logs(self, get):
        try:
            ...
            res = public.httpPost(
                "https://wafapi2.binarjoinanalyticnl.nl/Api/getUpdateLogs?type=Linux",
                {},
            )
    ...
    def get_public_config(self, args):
        ...
        data["ipv6"] = ""
        if _config_obj.get_ipv6_listen(None):
            data["ipv6"] = "checked"
        data["is_local"] = ""
        if public.is_local():
            data["is_local"] = "checked"
        ...
        data["total"] = os.path.exists(
            "/www/server/panel/plugin/total"
        ) or os.path.exists("/www/server/panel/plugin/monitor")
    ...
    def set_backup_path(get):
        ...
        if not os.path.exists(backup_path):
            return public.returnMsg(
                False, public.lang("The specified directory does not exist")
            )
        ...
        if not fs.CheckDir(get.backup_path):
            return public.returnMsg(
                False,
                public.lang(
                    "You cannot use the system critical directory as the default backup directory"
                ),
            )
    ...
    def get_soft_status(self, get):
        ...
        if (
            os.path.exists("/www/server/{}".format(name))
            and len(os.listdir("/www/server/{}".format(name))) > 2
        ):
            return public.returnMsg(True, "accordWith")
        if name == ["mysql", "pgsql", "sqlserver", "mongodb", "redis"]:
            count = (
                public.M("database_servers")
                .where("LOWER(db_type)=LOWER(?)", (name,))
                .count()
            )
            if count > 0:
                return public.returnMsg(True, "accordWith")
