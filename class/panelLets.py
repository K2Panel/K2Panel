...
class panelLets:
    ...
    def dump_pkcs12(self, key_pem=None, cert_pem=None, ca_pem=None, friendly_name=None):
        ...
        if ca_pem:
            ret = p12.set_ca_certificates(
                (crypto.load_certificate(crypto.FILETYPE_PEM, ca_pem.encode()),)
            )
        if friendly_name:
    ...
    def get_error(self, error):
        if error.find("Max checks allowed") >= 0:
            return public.lang(
                "CA can't verify your domain name, please check if the domain name resolution is correct, or wait 5-10 minutes and try again."
            )
        elif (
            error.find("Max retries exceeded with") >= 0
            or error.find("status_code=0 ") != -1
        ):
            return public.lang(
                "The CA server connection timed out, please try again later."
            )
            ...
            return public.lang(
                "The DNS server connection failed. Please check if the key is correct."
            )
        elif "too many certificates already issued for exact set of domains" in error:
            return public.lang(
                "The signing failed, the domain name exact set of domains: (.+): {} exceeded the weekly number of repeated issuances!",
                error,
            )
        elif (
            ...
            return public.lang(
                "The signing failed, the current server IP has reached the limit of creating up to 10 accounts every 3 hours.."
            )
        elif "DNS problem: NXDOMAIN looking up A for" in error:
            return public.lang(
                "The verification failed, the domain name was not resolved, or the resolution did not take effect.!"
            )
        elif "Invalid response from" in error:
            return public.lang(
                "Authentication failed, domain name resolution error or verification URL could not be accessed!"
            )
        elif error.find("TLS Web Server Authentication") != -1:
            public.restart_panel()
            return public.lang(
                "Failed to connect to CA server, please try again later."
            )
        elif error.find("Name does not end in a public suffix") != -1:
            return public.lang(
                "Unsupported domain name {}, please check if the domain name is correct!",
                re.findall('Cannot issue for "(.+)":', error),
            )
        elif error.find("No valid IP addresses found for") != -1:
            return public.lang(
                "The domain name {} did not find a resolution record. Please check if the domain name is resolved.!",
                re.findall("No valid IP addresses found for (.+)", error),
            )
        elif error.find("No TXT record found at") != -1:
            return public.lang(
                "If a valid TXT resolution record is not found in the domain name {}, please check if the TXT record is correctly parsed. If it is applied by DNSAPI, please try again in 10 minutes.!",
                (re.findall("No TXT record found at (.+)", error),),
            )
        elif error.find("Incorrect TXT record") != -1:
            return public.lang(
                "Found the wrong TXT record on {}: {}, please check if the TXT resolution is correct. If it is applied by DNSAPI, please try again in 10 minutes.!",
                re.findall("found at (.+)", error),
                re.findall('Incorrect TXT record "(.+)"', error),
            )
        elif error.find("Domain not under you or your user") != -1:
            return public.lang(
                "This domain name does not exist under this dnspod account. Adding parsing failed.!"
            )
        elif error.find("SERVFAIL looking up TXT for") != -1:
            return public.lang(
                "If a valid TXT resolution record is not found in the domain name {}, please check if the TXT record is correctly parsed. If it is applied by DNSAPI, please try again in 10 minutes.!",
                (re.findall("looking up TXT for (.+)", error),),
            )
        elif error.find("Timeout during connect") != -1:
            return public.lang(
                "Connection timed out, CA server could not access your website!"
            )
        elif error.find("DNS problem: SERVFAIL looking up CAA for") != -1:
            return public.lang(
                "The domain name {} is currently required to verify the CAA record. Please manually resolve the CAA record, or try again after 1 hour.!",
                re.findall("looking up CAA for (.+)", error),
            )
        elif error.find("Read timed out.") != -1:
            return public.lang(
                "Verification timeout, please check whether the domain name is correctly resolved. If dns is resolved, the connection between the server and Let'sEncrypt may be abnormal. Please try again later!"
            )
            ...
            return public.lang(
                "More than 5 verification failures in 1 hour, application is temporarily banned, please try again later!"
            )
    ...
    def get_dns_class(self, data):
        if data["dnsapi"] == "dns_ali":
            ...
            dns_class = panelDnsapi.AliyunDns(
                key=data["dns_param"][0], secret=data["dns_param"][1]
            )
            return dns_class
        elif data["dnsapi"] == "dns_dp":
            dns_class = sewer.DNSPodDns(
                DNSPOD_ID=data["dns_param"][0], DNSPOD_API_KEY=data["dns_param"][1]
            )
            return dns_class
            ...
            public.mod_reload(panelDnsapi)
            dns_class = panelDnsapi.CloudxnsDns(
                key=data["dns_param"][0], secret=data["dns_param"][1]
            )
    ...
    def renew_lest_cert(self, data):
        ...
        if not os.path.exists(path):
            return public.return_msg_gettext(
                False,
                public.lang(
                    "The renewal failed and the certificate directory does not exist."
                ),
            )
        ...
        if not os.path.exists(account_path):
            return public.return_msg_gettext(
                False, public.lang("Renewal failed, missing account_key.")
            )
        ...
        if not certificate["status"]:
            return public.return_msg_gettext(False, certificate["msg"])

        # 存储证书
        public.writeFile(path + "/privkey.pem", certificate["key"])
        public.writeFile(
            path + "/fullchain.pem", certificate["cert"] + certificate["ca_data"]
        )
        public.writeFile(
            path + "/account_key.key", certificate["account_key"]
        )  # 续签KEY
        ...
        try:
            ...
            p12 = ssl_info.ssl_info().dump_pkcs12_new(
                certificate["key"],
                certificate["cert"] + certificate["ca_data"],
                certificate["ca_data"],
                data["first_domain"],
        ...
        return public.return_msg_gettext(
            True, "[ {} ] The certificate renewal was successful.", (data["siteName"],)
        )
    ...
    def apple_lest_cert(self, get):
        ...
        self.write_log(
            public.lang("Ready to apply for SSL, domain name {}", data["domains"])
        )
        self.write_log("=" * 50)
        if len(data["domains"]) <= 0:
            return public.return_msg_gettext(
                False, public.lang("The list of applied domain names cannot be empty.")
            )
        ...
        if hasattr(get, "dnsapi"):
            ...
            if get.dnsapi == "dns":
                ...
                if hasattr(get, "renew"):  # 验证
                    ...
                    result["msg"] = public.lang(
                        "Get successful, please manually resolve the domain name"
                    )
                    result["code"] = 2
                    return result
        ...
        try:
            ...
            if not cron_id:
                ...
                shell = (
                    ...
                    + " %s/panel/class/panelLets.py renew_lets_ssl " % (self.setupPath)
                )
    ...
    def crate_let_by_oper(self, data):
        ...
        try:
            ...
            if not "renew" in data:
                ...
                authorizations, finalize_url = (
                    BTPanel.dns_client.apply_for_cert_issuance()
                )
                responders = []
                ...
                for url in authorizations:
                    ...
                    acme_keyauthorization, domain_dns_value = (
                        BTPanel.dns_client.get_keyauthorization(dns_token)
                    )

                    acme_name = self.get_acme_name(dns_name)
                    dns_names_to_delete.append(
                        {
                            "dns_name": public.de_punycode(dns_name),
                            "acme_name": acme_name,
                ...
                for i in responders:
                    self.write_log(
                        public.lang(
                            "|-Requesting CA to verify domain name [{}]...",
                            i["dns_name"],
                        )
                    )
                ...
                for i in responders:
                    self.write_log(
                        public.lang(
                            "|-Get CA verification results [{}]...", i["dns_name"]
                        )
                    )
                    BTPanel.dns_client.check_authorization_status(
                ...
                if certificate:
                    ...
                    result["msg"] = public.lang(
                        "Certificate acquisition failed, please try again later."
                    )

        except Exception as e:
            self.write_log(
                public.lang("|-Error: {}, exited the application process.", e)
            )
            self.write_log("=" * 50)
            res = str(e).split(">>>>")
            err = False
    ...
    def crate_let_by_dns(self, data):
        ...
        if not dns_class:
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "An error occurred while requesting a certificate using dns"
                ),
            )
        ...
        try:
            ...
            try:
                ...
                for url in authorizations:
                    ...
                    dns_challenge_url = identifier_auth["dns_challenge_url"]
                    acme_keyauthorization, domain_dns_value = (
                        client.get_keyauthorization(dns_token)
                    )
                    self.write_log(
                        public.lang(
                    ...
                    dns_class.create_dns_record(
                        public.de_punycode(dns_name), domain_dns_value
                    )
                    dns_names_to_delete.append(
                        {
                            "dns_name": public.de_punycode(dns_name),
                ...
                try:
                    for i in responders:
                        ...
                        self.check_dns(
                            self.get_acme_name(i["dns_name"]), i["domain_dns_value"]
                        )
                        self.write_log(
                            public.lang(
                    ...
                    for i in responders:
                        self.write_log(
                            public.lang(
                                "|-Check CA verification results [{}]...", i["dns_name"]
                            )
                        )
                    ...
                    self.write_log(
                        public.lang("|-An error occurred, try again [{}]", str(ex))
                    )
                    for i in responders:
                        self.write_log(
                            public.lang(
                ...
                try:
                    for i in dns_names_to_delete:
                        self.write_log(
                            public.lang(
                                "|-Clearing resolve history [{}]", i["dns_name"]
                            )
                        )
                        dns_class.delete_dns_record(
            ...
            try:
                for i in dns_names_to_delete:
                    self.write_log(
                        public.lang("|-Clearing resolve history [{}]", i["dns_name"])
                    )
            ...
            self.write_log(
                public.get_msg_gettext(
                    "|-Error: {}, exit the application process.",
    ...
    def crate_let_by_file(self, data):
        ...
        try:
            ...
            for url in authorizations:
                ...
                self.write_log(
                    public.get_msg_gettext(
                        "|-Writing verification file [{}]...", (wellknown_path,)
                    )
                )
                ...
                result["clecks"].append(
                    {"wellknown_url": wellknown_url, "http_token": http_token}
                )
                is_check = False
                n = 0
                self.write_log(
                    public.get_msg_gettext(
                        "|-Attempt to verify file contents via HTTP [{}]...",
                        (wellknown_url),
                    )
                )
                while n < 5:
            ...
            if len(sucess_domains) > 0:
                ...
                for i in responders:
                    ...
                    if auth_status_response.json()["status"] == "pending":
                        client.respond_to_challenge(
                            i["acme_keyauthorization"], i["http_challenge_url"]
                        ).json()

                for i in responders:
                    self.write_log(
                        public.get_msg_gettext(
                            "|-Check CA verification results [{}]...", (i["http_name"],)
                        )
                    )
                    client.check_authorization_status(
                        i["authorization_url"], ["valid", "invalid"]
    ...
    def get_identifier_authorization(self, client, url):
        ...
        get_identifier_authorization_response = requests.get(
            url, timeout=client.ACME_REQUEST_TIMEOUT, headers=headers, verify=False
        )
        if get_identifier_authorization_response.status_code not in [200, 201]:
            raise ValueError(
                "Error getting identifier authorization: status_code={status_code}".format(
                ...
                for j in ns.response.answer:
                    for i in j.items:
                        ...
                        if txt_value == value:
                            self.write_log(
                                public.get_msg_gettext(
                                    "|-Successful verification, domain name [{}], record type [{}], record value [{}]!",
                                    (domain, type, txt_value),
                                )
                            )
    ...
    def get_renew_lets_bytimeout(self, cron_list):
        ...
        for siteName in cron_list:
            ...
            if os.path.exists(spath + "/account_key.key"):
                if public.M("sites").where("name=?", (siteName,)).count():
                    ...
                    timeout = int(
                        time.mktime(time.strptime(data["timeout"], "%Y%m%d%H%M%S"))
                    )
        ...
        # 清理过期配置
        public.writeFile(
            self.setupPath + "/panel/vhost/cert/crontab.json", json.dumps(new_list)
        )
        return nlist
    ...
    def renew_lets_ssl(self):
        ...
        if not os.path.exists(cpath):
            ...
            print(
                public.get_msg_gettext(
                    r"|-[{}] Not expired or the site does not use the Let\s Encrypt certificate.",
                    (",".join(tlist),),
                )
            )
            print(
            ...
            print(
                public.get_msg_gettext(
                    "|-After the task is completed, a total of renewals are required.[{}], renewal success [%s], renewal failed [{}]. ",
                    (str(len(cron_list)), str(len(sucess_list)), str(len(err_list))),
                )
            )
            if len(sucess_list) > 0:
                print(
                    public.get_msg_gettext(
                        "|-Renewal success：{}", (",".join(sucess_list),)
                    )
                )
            if len(err_list) > 0:
