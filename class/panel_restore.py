...
# -------------------------------------------------------------------
# Author: zhwwen <zhw@binarjoinanalyticnl.nl>
# -------------------------------------------------------------------
#
# ------------------------------
# 网站恢复
...
class panel_restore:
    ...
    def _build_aws_backup_path(self, btype, file_name, domain):
        ...
        backup_path = (
            conf.split("|")[-1].strip() + btype + "/" + domain + "/" + file_name
        )
        return backup_path
    ...
    def _get_backfile_method(self, filename):
        backup_info = (
            public.M("backup").where("name=?", (filename,)).getField("filename")
        )
    ...
    def _restore_backup(self, local_backup_file_path, site_info, args):
        ...
        if not os.path.exists(local_backup_file_path):
            self._progress_rewrite(
                "No backup file found: {}".format(str(local_backup_file_path))
            )
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Panel does not find the backup file: {}", local_backup_file_path
                ),
            )
        # 将网站目录移至回收站
        self._progress_rewrite(
        ...
        if not os.path.exists(args.path):
            self._progress_rewrite(
                "Create an empty directory for the site: {}".format(str(args.path))
            )
        ...
        self._progress_rewrite(
            "The decompression command is: {}".format(str(uncompress_comand))
        )
        self._progress_rewrite("Start to restore data......")
        public.ExecShell(
            "cd {} && {} {} >> /tmp/restore_site.log".format(
    ...
    def _download_aws_file(self, args, btype="site"):
        ...
        download_file = self._build_aws_backup_path(
            btype, args.file_name, args.obj_name
        )
        self._progress_rewrite("The download path is:{}".format(download_file))
        self._local_file = self._local_file.format(args.file_name)
        self._progress_rewrite(
            "Backup file will be downloaded to:{}".format(self._local_file)
        )
    ...
    def _download_google_cloud_file(self, args, btype="site"):
        ...
        download_file = self._build_google_backup_path(
            btype, args.file_name, args.obj_name
        )
        self._progress_rewrite("The download path is:{}".format(download_file))
        self._local_file = self._local_file.format(args.file_name)
    ...
    def restore_website_backup(self, args):
        ...
        self._progress_rewrite(
            "Get the site path:{}".format(str(site_info["site_path"]))
        )
        local_backup_path = self._get_local_backup_path()
        local_backup_file_path = local_backup_path + "/site/" + args.file_name
        self._progress_rewrite(
            "Get the local backup file path: {}".format(str(local_backup_path))
        )
        ...
        if backup_method == "local":
            self._progress_rewrite(
                "Start to restore local backup files: {}".format(
            ...
            if result:
                self._progress_rewrite(
                    "Recovery failed: {}".format(str(site_info["site_path"]))
                )
                return result
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Currently only supports restoring local, Google storage and AWS S3 backups"
                ),
            )
        ...
        if result:
            ...
            return result
        self._progress_rewrite(
            "Successful recovery: {}".format(str(site_info["site_path"]))
        )
    ...
    def restore_db_backup(self, args):
        ...
        try:
            ...
            self._progress_rewrite(
                "Get the backup method: {}".format(str(backup_method))
            )
            ...
            public.ExecShell(
                "mv {} {}/database".format(
                    self._local_file, self._get_local_backup_path()
                )
            )
            return public.returnMsg(True, public.lang("success"))
        except:
