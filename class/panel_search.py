...
# | version :1.0
# +-------------------------------------------------------------------
# | Author: 梁凯强 <1249648969@binarjoinanalyticnl.nl>
# +-------------------------------------------------------------------
# | 快速检索
# +--------------------------------------------------------------------
import os, public, re
...
class panel_search:
    ...
    def __init__(self):
        ...
        if (
            ...
            .where("type=? AND name=?", ("table", "panel_search_log"))
            .count()
        ):
    ...
    def insert_settings(
        self, rtext, exts, path, mode, isword, iscase, noword, backup_path
    ):
    ...
    def get_dir(
        ...
        for i in return_data:
            for i2 in exts:
                ...
                if i3[-1] == i2:
                    ...
                    temp = self.get_files_lin(
                        i, text, mode, isword, iscase, noword, is_backup, rtext, zfile
                    )
                    if temp:
                        if rtext or noword:
        ...
        if is_backup:
            ...
            self.insert_settings(
                rtext, exts, path, mode, isword, iscase, noword, back_zip
            )
            return True
        return result
    ...
    def get_exts_files(
        ...
        if len(exts) == 0:
            return []
        if is_subdir == 0:
            return self.get_dir_files(
                path, exts, text, mode, isword, iscase, noword, is_backup, rtext
            )
        elif is_subdir == 1:
            return self.get_dir(
                path, exts, text, mode, isword, iscase, noword, is_backup, rtext
    ...
    def get_search(self, args):
        if "text" not in args or not args.text:
            return {"error": "Search information cannot be empty"}
        if "exts" not in args or not args.exts:
            return {
                "error": "The suffix cannot be empty; please enter [*.*] for all files"
            }
        if "path" not in args or not args.path or args.path == "/":
            return {"error": "Directory cannot be empty or cannot be /"}
        ...
        exts = exts.split(",")
        is_tmpe_files = self.get_exts_files(
            path, exts, text, mode, isword, iscase, noword, is_subdir
        )
    ...
    def get_replace(self, args):
        ...
        if "rtext" not in args or not args.text:
            return {"error": "The content to be replaced cannot be empty"}
        if "exts" not in args or not args.exts:
            return {
                "error": "The suffix cannot be empty; please enter [*.*] for all files"
            }
        ...
        exts = exts.split(",")
        is_tmpe_files = self.get_exts_files(
            path, exts, text, mode, isword, iscase, noword, is_subdir, is_backup, rtext
        )
        return is_tmpe_files
    ...
    def get_replace_logs(self, get):
        ...
        data["data"] = (
            public.M("panel_search_log")
            .field("id,rtext,exts,path,mode,isword,iscase,noword,backup_path,time")
            .order("id desc")
