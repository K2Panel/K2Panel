...
class panelPlugin:
    ...
    def __init__(self):
        ...
        self.__error_html = (
            "/www/server/panel/BTPanel/templates/default/block_error.html"
        )
        self.__sub_rules = []
        self.__replace_rule = []
    ...
    def check_install_limit(self, get):
        ...
        if not pluginInfo:
            return public.return_msg_gettext(
                False, public.lang("The specified plugin does not exist!")
            )
        self.mutex_title = pluginInfo["mutex"]
        if not self.check_mutex(pluginInfo["mutex"]):
            return public.return_msg_gettext(
                False, public.lang("Please uninstall [{}] first", self.mutex_title)
            )
        if not hasattr(get, "id"):
            if not self.check_dependent(pluginInfo["dependent"]):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Depends on the following software, please install [{}] first",
                ...
                if get.type == "0":
                    if not self.check_cpu_limit(versionInfo["cpu_limit"]):
                        return public.return_msg_gettext(
                            False,
                            public.lang(
                                "At least [{0}] CPU cores are required to install",
                    ...
                    if not self.check_mem_limit(versionInfo["mem_limit"]):
                        return public.return_msg_gettext(
                            False,
                            public.lang(
                                "At least [{0} MB] memory is required to install",
                                versionInfo["mem_limit"],
                            ),
                        )
                ...
                if not hasattr(get, "id"):
                    if not self.check_dependent(versionInfo["dependent"]):
                        return public.return_msg_gettext(
                            False,
                            public.lang(
                                "Depend on the following software, please install first [{}]",
                                versionInfo["dependent"],
                            ),
                        )
    ...
    def install_plugin(self, get):
        ...
        str3 = public.lang(
            "2. If Yunsuo is installed, please turn off [System Hardening] feature"
        )
        str4 = public.lang(
            "3. If Safedog is installed, please turn off [System Protection] feature"
        )
        ...
        if not "sName" in get:
            return public.return_msg_gettext(
                False, public.lang("Please specify the software name!")
            )
        # 处理ols还不支持php81的情况
    ...
    def install_sync(self, pluginInfo, get):
        ...
        if "download" in pluginInfo["versions"][0]:
            ...
            public.downloadFile(
                "{}/api/plugin/download?filename={}&token={}".format(
                    self.__official_url, pluginInfo["versions"][0]["download"], token
                ),
                toFile,
            )
            if public.FileMd5(toFile) != pluginInfo["versions"][0]["md5"]:
                return public.return_msg_gettext(
                    False,
                    public.lang("File hash verification failed, stop installation!"),
                )
            update = False
    ...
    def set_pyenv(self, filename):
        ...
        rep_path = [
            "PATH={}/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/local/sbin:~/bin".format(
                env_py + ":"
            )
        ]
    ...
    def install_async(self, pluginInfo, get):
        ...
        if get.sName == "nginx":
            if get.version == "1.8":
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Nginx 1.8.1 is too old, no longer available, please choose another version!"
                    ),
                )
        ...
        if "php" == get.sName and os.path.exists("/usr/local/lsws/bin/lswsctrl"):
            ...
            ols_execstr = (
                " &> /tmp/panelExec.log && /bin/bash install_soft.sh {} {} "
                + ols_sName
                + " "
                + ols_version
            )
        ...
        if get.sName == "phpmyadmin":
            ...
            if os.path.exists("{}/data/ssl.pl".format(public.get_panel_path())):
                with open(
                    "{}/data/phpmyadmin_ssl.mark".format(public.get_panel_path()), "w"
                ) as fp:
                    fp.write("1")
        ...
        public.write_log_gettext(
            "Installer",
            "Successfully added intallation task [{}-{}]",
            (get.sName, get.version),
        )
        return public.return_msg_gettext(
            True, public.lang("Installation task added to queue")
        )
    ...
    def uninstall_plugin(self, get):
        ...
        if pluginInfo["type"] != 5:
            ...
            public.write_log_gettext(
                "Installer",
                "Successfully uninstalled software [{}]",
                (pluginInfo["title"],),
            )
            return public.return_msg_gettext(
                True, public.lang("Uninstallaton succeeded")
            )
        else:
            ...
            if pluginInfo["name"] == "mysql":
                if public.M("databases").where("db_type=?", 0).count() > 0:
                    return public.return_msg_gettext(
                        False,
                        public.lang(
                            "The database list is not empty. For your data security, please backup and delete the existing database.<br>Forced uninstall command: rm -rf /www/server/mysql"
                        ),
                    )
            ...
            if get.sName.find("php-") != -1:
                get.sName = get.sName.split("-")[0]
            execstr = (
                "cd /www/server/panel/install && /bin/bash install_soft.sh "
                + get.type
                + " uninstall "
                + get.sName.lower()
                + " "
            ...
            public.write_log_gettext(
                "Installer", "Successfully unintalled [{}-{}]", (get.sName, get.version)
            )
            return public.return_msg_gettext(
                True, public.lang("Uninstallation succeeded")
            )
        ...
        if hasattr(get, "query"):
            if get.query:
                ...
                for softInfo in softList["list"]:
                    if (
                        softInfo["name"].lower().find(get.query) != -1
                        or softInfo["title"].lower().find(get.query) != -1
                        or softInfo["ps"].lower().find(get.query) != -1
                    ):
    ...
    def add_expire_msg(self, title, level, name, expire_day, pid, endtime):
        ...
        if pm.is_level(level):  # 是否忽略
            if level != name:  # 到期还是即将到期
                msg_last = "您的【{}】授权还有{}天到期".format(
                    title, int(expire_day) + 1
                )
            else:
            ...
            if name in ["pro", "ltd"]:
                pl_msg = "false"
            renew_msg = (
                "<a class=\"btlink\" onclick=\"bt.soft.product_pay_view({name:'%s',pid:%s,limit:'%s',plugin:%s,renew:%s});\">立即续费</a>"
    ...
    def expire_msg(self, data):
        ...
        if not data["ltd"] in [-1]:
            ...
            if data["pro"] < 0 or (data["pro"] - s_time) / 86400 < 15:
                ...
                self.add_expire_msg(
                    "企业版", level, "ltd", expire_day, 100000046, data["ltd"]
                )
        ...
        if not data["pro"] in [-1, 0]:
            ...
            self.add_expire_msg(
                "专业版", level, "pro", expire_day, 100000030, data["pro"]
            )
    ...
    def set_score(self, args):
        try:
            ...
            if 1 < pdata["num"] > 5:
                return public.return_msg_gettext(
                    False, public.lang("Scoring range [1-5]")
                )
            if not pdata["pid"]:
                return public.return_msg_gettext(
                    False, public.lang("The specified plugin does not exist!")
            ...
            result = public.httpPost(
                public.GetConfigValue("home") + "/api/panel/plugin_score", pdata, 10
            )
    ...
    def get_score(self, args):
        try:
            ...
            result = public.httpPost(
                public.GetConfigValue("home") + "/api/panel/get_plugin_socre" + u_args,
                pdata,
                10,
            )
            result = json.loads(result)
    ...
    def check_accept(self, get):
        ...
        for p in p_list["list"]:
            if p["name"] == get.name:
                if int(p_list["pro"]) < 0 and int(p["endtime"]) < 0:
                    return False
                break
        ...
        for p in p_list["list"]:
            ...
            if p["name"] == get.name:
                if not "endtime" in p:
                    continue
                if int(p_list["ltd"]) < 1 and int(p["endtime"]) < 1:
                    return False
                break
        return True
    ...
    def get_soft_list(self, get=None):
        ...
        if not softList:
            ...
            if not softList:
                return public.return_msg_gettext(
                    False, public.lang("Failed to get software list ({})", "401")
                )
    ...
    def add_index(self, get):
        ...
        if sName in indexList:
            return public.return_msg_gettext(
                False, public.lang("Please do NOT repeat adding")
            )
        if len(indexList) >= 12:
            softList = self.get_cloud_list(get)["list"]
            softList = self.set_coexist(softList)
            for softInfo in softList:
                ...
                if softInfo["name"] == "php":
                    for i in softInfo["versions"]:
                        ...
                        if (
                            not os.path.exists(
                                "/www/server/php/{}".format(i["m_version"])
                            )
            ...
            if len(indexList) >= 12:
                return public.return_msg_gettext(
                    False, public.lang("Dashboard only display up to 12 software!")
                )
    ...
    def add_link(self, get):
        ...
        if len(indexList) >= 5:
            return public.return_msg_gettext(
                False, public.lang("Shortcut Bar only display up to 5 software!")
            )
        indexList.append(sName)
                ...
                if sInfo["version_coexist"] == 1 and "versions" in sInfo:
                    for versionA in sInfo["versions"]:
                        try:
                            ...
                            sTmp["manager_version"] = sTmp["manager_version"].replace(
                                "{VERSION}", v
                            )
                            sTmp["install_checks"] = sTmp["install_checks"].replace(
                                "{VERSION}", v
                            )
    ...
    def check_status(self, softInfo):
        ...
        if softInfo["setup"]:
            ...
            softInfo["versions"] = self.tips_version(
                softInfo["versions"], softInfo["version"]
            )
            softInfo["admin"] = os.path.exists(
                "/www/server/panel/plugin/" + softInfo["name"]
            )
            ...
            if softInfo["name"] == sName:
                if sName == "phpmyadmin":
                    ...
                    if os.path.exists(
                        ...
                        "{}/phpmyadmin".format(public.get_setup_path())
                    ):
                        os.remove(
                            "{}/data/phpmyadmin_ssl.mark".format(
                    ...
                    if softInfo["ext"] and pmd:
                        port = (
                            softInfo["ext"]["ssl_port"]
                            if softInfo["ext"].get("ssl_enabled", False)
                            else pmd[1]
                        )
                        softInfo["ext"]["url"] = (
                            "http"
                            + ("s" if softInfo["ext"].get("ssl_enabled", False) else "")
                            + "://"
                            + public.GetHost()
                            + ":"
                            + port
                ...
                if "php-" in sName:
                    ...
                    if public.get_webserver() == "openlitespeed":
                        ...
                        if os.path.exists("/etc/redhat-release"):
                            softInfo["php_ini"] = (
                                "/usr/local/lsws/lsphp{}/etc/php.ini".format(v1)
                            )
                    else:
                        softInfo["php_ini"] = "/www/server/php/{}/etc/php.ini".format(
                            v1
                        )
    ...
    def install(self, get):
        ...
        if not pluginInfo:
            ...
            pluginInfo = json.loads(
                public.readFile(self.__install_path + "/" + get.name + "/info.json")
            )

        if pluginInfo["tip"] == "lib":
            if not os.path.exists(self.__install_path + "/" + pluginInfo["name"]):
                public.ExecShell(
                    "mkdir -p " + self.__install_path + "/" + pluginInfo["name"]
                )
            ...
            if self.checksSetup(
                pluginInfo["name"], pluginInfo["checks"], pluginInfo["versions"]
            )[0]["status"] or os.path.exists(self.__install_path + "/" + get.name):
                public.write_log_gettext(
                    "Installer",
                ...
                return public.return_msg_gettext(
                    True, public.lang("Installation succeeded!")
                )
            return public.return_msg_gettext(False, public.lang("Installation failed!"))
        else:
            import db, time
            ...
            public.writeFile("/var/bt_apacheVersion.pl", apacheVersion)
            public.writeFile(
                "/var/bt_setupPath.conf", public.GetConfigValue("root_path")
            )
        ...
        for pluinfo in self.__plugin_list:
            if pluinfo["product"] == pluginName:
                if not pluinfo["endtime"] or not pluinfo["state"]:
                    return public.lang("To be paid")
                if pluinfo["endtime"] < time.time():
                    return public.lang("Expired")
    ...
    def getPluginList(self, get):
        ...
        for i in range(len(arr)):
            ...
            if arr[i]["tip"] == "lib":
                arr[i]["path"] = (
                    self.__install_path + "/" + arr[i]["name"].replace("_soft", "")
                )
        ...
        for v in versArr:
            ...
            for tm in tmp:
                if name == "php":
                    ...
                    if os.path.exists(path + "/bin/php") and not os.path.exists(
                        path + "/version.pl"
                    ):
                        public.ExecShell(
                            "echo `"
            ...
            if name == "memcached":
                if os.path.exists("/etc/init.d/memcached"):
                    ...
                    if not v1:
                        v1 = public.ExecShell("memcached -V|awk '{print $2}'")[
                            0
                        ].strip()
            ...
            if name == "apache":
                if os.path.exists("/www/server/apache/bin/httpd"):
                    ...
                    if not v1:
                        v1 = public.ExecShell(
                            r"/www/server/apache/bin/httpd -v|grep Apache|awk '{print $3}'|sed 's/Apache\///'"
                        )[0].strip()
                        session["httpdv"] = v1
            # if name == 'mysql':
        ...
        if name == "php":
            ...
            for i in range(len(versions)):
                if versions[i]["status"]:
                    ...
                    versions[i]["display"] = os.path.exists(
                        path + "/" + v4 + "/display.pl"
                    )
                    if len(versions) < 5:
                        versions[i]["run"] = True
    ...
    def getPHPMyAdminStatus(self):
        ...
        if os.path.exists(configFile):
            ...
            # SSL配置文件查看
            ssl_config_file = "{}/vhost/nginx/phpmyadmin.conf".format(
                public.get_panel_path()
            )
            if (
                os.path.exists(ssl_config_file)
                and os.path.getsize(ssl_config_file) > 10
            ):
            ...
            if not os.path.exists(configFile):
                public.writeFile(
                    configFile,
                    public.readFile(setupPath + "/nginx/conf/enable-php-54.conf"),
                )
            conf = public.readFile(configFile)
        ...
        if os.path.exists(configFile):
            ...
            # SSL配置文件查看
            ssl_config_file = "{}/vhost/apache/phpmyadmin.conf".format(
                public.get_panel_path()
            )
            if (
                os.path.exists(ssl_config_file)
    ...
    def getConfigHtml(self, get):
        ...
        if not os.path.exists(filename):
            return public.return_msg_gettext(
                False, public.lang("This plugin does NOT have template!")
            )
        mimetype = "text/html"
    ...
    def creatab_open_total_table(self, sql):
        ...
        if (
            ...
            .where("type=? AND name=?", ("table", "open_total"))
            .count()
        ):
...
`num` INTEGER
    ...
    def plugin_open_total(self, plugin_name):
        ...
        num = (
            sql.table("open_total").where("plugin_name=?", plugin_name).getField("num")
        )
        if not num:
        ...
        for version in find["versions"].split(","):
            ...
            if get.name == "php":
                tmp["status"] = os.path.exists(
                    path + "/" + version.replace(",", "") + "/display.pl"
                )
            else:
    ...
    def setPluginStatus(self, get):
        if get.name == "php":
            ...
            if get.status == "0":
                ...
                public.ExecShell(
                    "rm -f " + path + "/" + get.version.replace(".", "") + "/display.pl"
                )
                ...
                public.writeFile(
                    path + "/" + get.version.replace(".", "") + "/display.pl", "True"
                )
    ...
    def getCloudPlugin(self, get):
        if session.get("getCloudPlugin") and get != None:
            return public.return_msg_gettext(
                True, "Your plugin list is already the latest version {}!", ("-1",)
            )
        import json

        if not session.get("download_url"):
            session["download_url"] = "http://node.binarjoinanalyticnl.nl"
    ...
    def update_zip(self, get=None, tmp_file=None, update=False):
        ...
        if tmp_file:
            if not os.path.exists(tmp_file):
                return public.return_msg_gettext(
                    False, public.lang("File download failed!")
                )
        ...
        try:
            ...
            if not "author" in data:
                data["author"] = public.lang("Unknown")
            if not "home" in data:
            ...
            return public.return_msg_gettext(
                False,
                public.lang(
                    "No plugin info found in the archive, please check the plugin package!"
                ),
            )
    ...
    def input_zip(self, get):
        if not os.path.exists(get.tmp_path):
            return public.return_msg_gettext(
                False, public.lang("Temporary file does NOT exist, please re-upload!")
            )
        ...
        public.ExecShell(
            "cd " + plugin_path + " && bash install.sh install &> /tmp/panelShell.pl"
        )
        ...
        if p_info:
            ...
            icon_dfile = (
                "/www/server/panel/BTPanel/static/img/soft_ico/ico-{}.png".format(
                    get.plugin_name
                )
            )
    ...
    def export_zip(self, get):
        ...
        if not os.path.exists(get.dfile):
            return public.return_msg_gettext(
                False, public.lang("Export failed, please check permissions!")
            )
    ...
    def add_make_args(self, get):
        ...
        if not re.match(r"^\w+$", get.args_name):
            return public.return_msg_gettext(
                False,
                public.lang(
                    "Non-compliant names can only be numbers, letters, underscores"
                ),
            )
        ...
        public.write_log_gettext(
            "Software manager",
            "Add custom compilation parameters: {}:{}",
            (get.name, get.args_name),
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def del_make_args(self, get):
        ...
        if not os.path.exists(config_path):
            return public.return_msg_gettext(
                False,
                public.lang(
                    "The specified custom compilation parameters do not exist!"
                ),
            )
        ...
        public.write_log_gettext(
            "Software manager",
            "Remove custom compilation parameters: {}:{}",
            (get.name, get.args_name),
        )
        return public.return_msg_gettext(True, public.lang("Successfully deleted"))
    ...
    def set_make_args(self, get):
        ...
        public.write_log_gettext(
            "Software manager",
            "Setup software: Custom compilation parameters for {} are configured as: {}".format(
                get.name, config_data
            ),
        )
        return public.return_msg_gettext(True, public.lang("Setup successfully!"))
    ...
    def __get_plugin_info(self, upgrade_plugin_name):
        ...
        @return dict
        """
        plugin_info_file = "{}/{}/info.json".format(
            self.__plugin_path, upgrade_plugin_name
        )
    ...
    def __get_update_msg(self, upgrade_plugin_name, upgrade_version):
        ...
        for _version_info in plugin_info["versions"]:
            l_version = "{}.{}".format(
                _version_info["m_version"], _version_info["version"]
            )
        ...
        if "not_os_bit" in plugin_data_info:
            if public.get_sysbit() == int(plugin_data_info["not_os_bit"]):
                return public.returnMsg(
                    False,
                    "Not supported for {}bit systems.".format(
                        plugin_data_info["not_os_bit"]
                    ),
                )
    ...
    def __download_plugin(self, upgrade_plugin_name, upgrade_version):
        ...
        pdata["os"] = "Linux"
        pdata["environment_info"] = json.dumps(
            public.fetch_env_info(), ensure_ascii=False
        )
        ...
        if not cache.get(pkey):
            ...
            try:
                ...
                if "Name or service not known" in str_ex:
                    return public.returnMsg(
                        False, public.lang("Error: Name or service not known")
                    )
                    ...
                    return public.returnMsg(
                        False, public.lang("Error: Connection refused")
                    )
                ...
                try:
                    for download_chunk in download_res.iter_content(
                        ...
                        if download_chunk:
                            ...
                            pre_text = "{}/{}/{}".format(
                                res_down_size, headers_total_size, res_sec_speed
                            )
                    ...
                    if "Read timed out" in ex_str:
                        return public.returnMsg(
                            False, public.lang("Download timeout: {}", ex_str)
                        )
                    if "No space left on device" in ex_str:
                        return public.returnMsg(
                            False, public.lang("No available disk space.")
                        )
                finally:
                    with_res_f.close()
            if cache.get(pkey):
            ...
            if public.FileMd5(filename) != download_res.headers["Content-md5"]:
                return public.returnMsg(
                    False, public.lang("Verify package checksum failed.")
                )
        else:
            while True:
                time.sleep(1)
    ...
    def __get_download_speed(self, upgrade_plugin_name):
        ...
        if not pre_text:
            return public.returnMsg(
                False, public.lang("Cannot get progress bar for this plugin.")
            )
        result = {"status": True}
