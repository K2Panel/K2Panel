...
class mailUnsubscribe:
    ...
    def Unsubscribe(self, get):
        ...
        try:
            ...
            return {
                "status": True,
                "msg": public.lang("The unsubscribe of email {} is successful", email),
            }
        except jwt.ExpiredSignatureError:
            # return public.lang('Operation failed,The token expires')
            return {
                "status": False,
                "msg": public.lang("Operation failed,The token expires"),
            }
            ...
            return {
                "status": False,
                "msg": public.lang("Operation failed,Invalid tokens"),
            }
    ...
    def Subscribe(self, get):
        ...
        with public.S("mail_unsubscribe", "/www/vmail/mail_unsubscribe.db") as obj:
            # 有退订 改为订阅
            exit_un = (
                obj.where("recipient", email)
                .where("etype", etype)
                .where("active", 0)
                .count()
            )
            if exit_un:
                obj.where("recipient", email).where("etype", etype).update(
                    {"active": 1}
                )
                return
                ...
                for etype in etype_list:
                    ...
                    if exit:
                        ...
                        bb = (
                            ...
                            .where("etype", etype)
                            .update(
                                {"active": 0, "task_id": task_id, "created": created}
                            )
                        )
    ...
    def get_mail_type_list(self, get):
        ...
        with public.S("mail_unsubscribe", "/www/vmail/mail_unsubscribe.db") as obj:
            ...
            etypes = (
                obj.where("active", 1)
                .where("recipient like ?", "{}%".format(email))
                .field("etype")
                .select()
            )
            ...
            for j in etypes:
                if typelist.get(str(j["etype"]), None):
                    mail_type.append(
                        {"id": j["etype"], "mail_type": typelist[str(str(j["etype"]))]}
                    )
