...
class panelSSL:
    ...
    def GetToken(self, get):
        ...
        try:
            ...
            return public.return_msg_gettext(
                False,
                "%s<br>%s" % (public.lang("Failed to connect server!"), str(rtmp)),
            )
    ...
    def DelToken(self, get):
        ...
        try:
            ...
            public.writeFile(
                self.__UPATH, json.dumps({"server_id": uinfo["server_id"]})
            )
        except:
    ...
    def GetUserInfo(self, get):
        ...
        try:
            if self.__userInfo:
                userTmp = {}
                userTmp["username"] = (
                    self.__userInfo["email"][0:3]
                    + "****"
                    + self.__userInfo["email"][-4:]
                )
                result["status"] = True
    ...
    # 检查CAA记录是否正确
    def check_ssl_caa(
        self, domains, clist=["sectigo.com", "digicert.com", "comodoca.com"]
    ):
        """
        @name 检查CAA记录是否正确
        @param domains 域名列表
        ...
        try:
            ...
            if data:
                ...
                result["status"] = False
                result["msg"] = (
                    "error: There is a CAA record in the DNS resolution of the domain name. Please delete it and apply again "
                )
    ...
    def set_verify_info(self, args):
        ...
        for domain in verify_info["domains"]:
            ...
            if is_file_verify:
                ...
                if not siteRunPath:
                    # if domain[:4] == 'www.': domain = domain[4:]
                    verify_info["paths"].append(
                        verify_info["path"].replace("example.com", domain)
                    )
                    continue
                verify_path = siteRunPath + "/.well-known/pki-validation"
                if not os.path.exists(verify_path):
    ...
    def check_url_txt(self, args, timeout=5):
        ...
        if (
            ...
            or result.find("not found") != -1
            or res.status_code in [404]
        ):
            return -2
        ...
        if (
            result.find("301") != -1
            or result.find("302") != -1
            or result.find("Redirecting...") != -1
            or res.status_code in [301, 302]
        ):
            return -4
        if result == content:
            return 1
    ...
    def get_verify_result(self, args):
        ...
        is_file_verify = (
            "CNAME_CSR_HASH" != verify_info["data"]["dcvList"][0]["dcvMethod"]
        )
        verify_info["paths"] = []
        verify_info["hosts"] = []
        if verify_info["data"]["application"]["status"] == "ongoing":
            return public.return_msg_gettext(
                False,
                public.lang(
                    "In verification, please contact K2Panel if the audit still fails after 24 hours"
                ),
            )
        ...
        for dinfo in verify_info["data"]["dcvList"]:
            ...
            if is_file_verify:
                ...
                dinfo["domainName"] = domain
                verify_info["hosts"].append(
                    verify_info["data"]["DCVdnsHost"] + "." + domain
                )
    ...
    def ApplyDVSSL(self, get):
        ...
        if not "orgName" in get:
            return public.returnMsg(False, public.lang("missing parameter: orgName"))
        if not "orgPhone" in get:
            return public.returnMsg(False, public.lang("missing parameter: orgPhone"))
        if not "orgPostalCode" in get:
            return public.returnMsg(
                False, public.lang("missing parameter: orgPostalCode")
            )
        if not "orgRegion" in get:
            return public.returnMsg(False, public.lang("missing parameter: orgRegion"))
        if not "orgCity" in get:
            return public.returnMsg(False, public.lang("missing parameter: orgCity"))
        if not "orgAddress" in get:
            return public.returnMsg(False, public.lang("missing parameter: orgAddress"))
        if not "orgDivision" in get:
            return public.returnMsg(
                False, public.lang("missing parameter: orgDivision")
            )
        ...
        if get.domain[:4] == "www.":
            if (
                not public.M("domain")
                .where("name=? AND pid=?", (get.domain[4:], get.id))
                .count()
            ):
                return public.returnMsg(
                    False,
        ...
        # 判断是否是Java项目
        if (
            public.M("sites").where("id=?", (get.id,)).getField("project_type")
            == "Java"
            or public.M("sites").where("id=?", (get.id,)).getField("project_type")
            == "Go"
            or public.M("sites").where("id=?", (get.id,)).getField("project_type")
            == "Other"
        ):
        ...
        if not self.CheckDomain(get):
            if not os.path.exists(authfile):
                return public.returnMsg(
                    False, public.lang("Unable to write validation file: {}", authfile)
                )
            else:
        ...
        try:
            ...
            if "authKey" in result["data"]:
                ...
                return public.returnMsg(
                    False, public.lang(" Failed to get the validation file!")
                )
    ...
    def request(self, dname):
        ...
        url_headers = {"authorization": "bt {}".format(self.__userInfo["token"])}

        result = public.return_msg_gettext(
        ...
        try:
            ...
            response_data = public.httpPost(
                self.__APIURL + "/" + dname, data=self.__PDATA, headers=url_headers
            )
        except Exception as ex:
            raise public.error_conn_cloud(str(ex))
        try:
    ...
    def GetOrderList(self, get):
        if hasattr(get, "siteName"):
            ...
            if os.path.exists(path):
                ...
                path = (
                    "/www/server/panel/vhost/cert/" + get.siteName + "/partnerOrderId"
                )
                if os.path.exists(path):
                    self.__PDATA["data"]["partnerOrderId"] = public.readFile(path)
        ...
        try:
            ...
            return public.return_msg_gettext(
                False, public.lang("Failed to get, please try again later!")
            )

        result["data"] = self.En_Code(result["data"])
        for i in range(len(result["data"])):
            result["data"][i]["endtime"] = self.add_months(
                result["data"][i]["createTime"], result["data"][i]["validityPeriod"]
            )
        return result
    ...
    def add_months(self, dt, months):
        ...
        return (
            time.mktime(dt.replace(year=year, month=month, day=day).timetuple()) + 86400
        ) * 1000
    ...
    def CheckDomain(self, get):
        try:
            ...
            result = http_requests.get(
                self._check_url, s_type="curl", timeout=6, headers={"host": get.domain}
            ).text
            self.__test = result
            if result == epass:
                return True
    ...
    def Completed(self, get):
        ...
        if hasattr(get, "siteName"):
            ...
            try:
                ...
                if not os.path.exists(spath):
                    public.ExecShell("mkdir -p '" + spath + "'")
                public.writeFile(
                    spath + "/" + sslInfo["data"]["authKey"],
                    sslInfo["data"]["authValue"],
                )
            except:
                return public.return_msg_gettext(
                    False, public.lang("Verification error!")
                )
        try:
            result = json.loads(
                public.httpPost(self.__APIURL + "user/Completed", self.__PDATA)
            )
            if "data" in result:
                result["data"] = self.En_Code(result["data"])
        except:
        ...
        while True:
            ...
            time.sleep(5)
            rRet = json.loads(
                public.httpPost(self.__APIURL + "user/SyncOrder", self.__PDATA)
            )
            n += 1
    ...
    def GetSSLInfo(self, get):
        ...
        time.sleep(3)
        result = json.loads(
            public.httpPost(self.__APIURL + "user/GetSSLInfo", self.__PDATA)
        )
        result["data"] = self.En_Code(result["data"])
        ...
        if hasattr(get, "siteName"):
            try:
                ...
                public.ExecShell(
                    "rm -rf /etc/letsencrypt/archive/" + get.siteName + "-00*"
                )
                public.ExecShell(
                    "rm -f /etc/letsencrypt/renewal/" + get.siteName + ".conf"
                )
                public.ExecShell(
    ...
    def GetSiteDomain(self, get):
        ...
        for site in all_sites:
            ...
            domain_list = (
                public.M("domain").where("pid=?", (pid,)).field("name").select()
            )
    ...
    def RemoveCert(self, get):
        try:
            ...
            if not os.path.exists(vpath):
                return public.return_msg_gettext(
                    False, public.lang("Certificate does NOT exist!")
                )
    ...
    def SaveCert(self, get):
        try:
            ...
            if not certInfo:
                return public.return_msg_gettext(
                    False, public.lang("Certificate parsing failed")
                )
            ...
            return public.return_msg_gettext(
                True, public.lang("Successfully saved certificate!")
            )
        except:
            return public.return_msg_gettext(
                False, public.lang("Failed to save certificate!")
            )
    ...
    def GetSSLProduct(self, get):
        self.__PDATA["data"] = self.De_Code(self.__PDATA["data"])
        result = json.loads(
            public.httpPost(self.__APIURL + "user/GetSSLProduct", self.__PDATA)
        )
        result["data"] = self.En_Code(result["data"])
        return result
    ...
    def renew_lets_ssl(self, get):
        if not os.path.exists("vhost/cert/crontab.json"):
            return public.return_msg_gettext(
                False, public.lang("There are currently no certificates to renew!")
            )
    ...
    def renew_cert_order(self, args):
        ...
        if not "pdata" in args:
            return public.returnMsg(
                False, public.lang("The pdata parameter cannot be empty!")
            )
        pdata = json.loads(args.pdata)
        self.__PDATA["data"] = pdata
    ...
    def check_ssl_method(self, get):
        ...
        if (
            public.M("sites")
            .where(
                "id=?", (public.M("domain").where("name=?", (domain)).getField("pid"),)
            )
            .getField("project_type")
            == "Java"
        ):
        ...
        res = {}
        msg = [
            " domain name [{}] validation file cannot be accessed correctly".format(
                domain
            ),
            "Probable cause",
    ...
    def get_cert_info(get):
        ...
        try:
            ...
            if target is None:
                return public.returnMsg(
                    False, public.lang("No certificate information was obtained")
                )
    ...
    def get_cert_list(get):
        ...
        try:
            ...
            if "force_refresh" in get and get.force_refresh.strip() in (
                "1",
                1,
                "True",
                True,
        ...
        try:
            res_list = SSLManger().get_cert_list(
                param=param, force_refresh=force_refresh
            )
        ...
        try:
            ...
            for j in test_cert_data["data"]:
                ...
                try:
                    ...
                    data["exclude_hash"].update(
                        {
                            j["partnerOrderId"]: self._hash(
                                certificate=certInfo["cert"] + certInfo["certCa"]
                            )
                        }
                    )
                except Exception as e:
