...
class panelApi:
    ...
    def login_for_app(self, get):
        ...
        tid = get.tid
        if len(tid) != 32:
            return public.return_msg_gettext(False, public.lang("Invalid login key1"))
        session_id = cache.get(tid)
        if not session_id:
            return public.return_msg_gettext(
                False, public.lang("The specified key does not exist or has expired1")
            )
        if len(session_id) != 64:
            return public.return_msg_gettext(False, public.lang("Invalid login key2"))
        try:
            if not os.path.exists("/www/server/panel/data/app_login_check.pl"):
                return public.returnMsg(False, public.lang("Invalid login key3"))
            key, init_time, tid2, status = public.readFile(
                "/www/server/panel/data/app_login_check.pl"
            ).split(":")
            if session_id != key:
                return public.returnMsg(False, public.lang("Invalid login key4"))
            if tid != tid2:
                return public.returnMsg(
                    False,
                    public.lang("The specified key does not exist or has expired5"),
                )
            if time.time() - float(init_time) > 60:
                return public.returnMsg(
                    False, public.lang("QR code validity time expired6")
                )
            ...
            return public.return_msg_gettext(
                True, public.lang("Scan code successfully, log in!")
            )
        except:
    ...
    def check_bind(self, args):
        if (
            not "bind_token" in args
            or not "client_brand" in args
            or not "client_model" in args
        ):
            return 0
        ...
        if bind["token"] != args.bind_token:
            return public.lang(
                "The current QR code has expired, please refresh the page and rescan the code!"
            )

        apps = self.get_apps()
        if len(apps) >= self.max_bind:
            return public.lang(
                "This server is bound to a maximum of {} devices, which has reached the limit!",
                self.max_bind,
            )
    ...
    def add_bind_app(self, args):
        ...
        if len(apps) >= self.max_bind:
            return public.return_msg_gettext(
                False,
                public.lang("A server allows up to {} device bindings!", self.max_bind),
            )
    ...
    def set_token(self, get):
        if "request_token" in get:
            return public.return_msg_gettext(
                False, public.lang("Cannot configure API through API interface")
            )
        data = self.get_api_config()
        if get.t_type == "1":
            ...
            if not "token_crypt" in data:
                token = public.GetRandomString(32)
                data["token"] = public.md5(token)
                data["token_crypt"] = public.en_crypt(data["token"], token).decode(
                    "utf-8"
                )
            public.write_log_gettext(
                "API configuration", "{} API interface", (stats[data["open"]],)
            )
            ...
            public.write_log_gettext(
                "API configuration", "Change IP limit to [{}]", (get.limit_addr,)
            )
            public.add_security_logs(
                "API configuration", "Change IP limit to [{}]", (get.limit_addr,)
            )
