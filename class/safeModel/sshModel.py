...
class main(safeBase):
    ...
    def get_ssh_intrusion(self, get):
        ...
        for sfile in self.get_ssh_log_files(None):
            for stype in result.keys():
                count = 0
                if sfile in data[stype] and not sfile in [
                    "/var/log/auth.log",
                    "/var/log/secure",
                ]:
                    ...
                    try:
                        if stype == "error":
                            ...
                            try:
                                num1 = int(
                                    public.ExecShell(
                                        "cat %s|grep -a 'Failed password for' |grep -v 'invalid' |wc -l"
                                        % (sfile)
                                    )[0].strip()
                                )
                            except:
                            ...
                            try:
                                num2 += int(
                                    public.ExecShell(
                                        "cat %s|grep -a 'Connection closed by authenticating user' |grep -a 'preauth' |wc -l"
                                        % (sfile)
                                    )[0].strip()
                                )
                            except:
                            ...
                            count = int(
                                public.ExecShell(
                                    "cat %s|grep -a 'Accepted' |wc -l" % (sfile)
                                )[0].strip()
                            )
                    except:
                        pass
    ...
    def SetSshPort(self, get):
        ...
        if port in ports:
            return public.returnMsg(
                False, public.lang("Please dont use default ports for common programs!")
            )
