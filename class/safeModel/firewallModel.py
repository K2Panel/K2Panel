...
class main(safeBase):
    ...
    def SetPing(self, get):
        ...
        if conf.find("net.ipv4.icmp_echo") != -1:
            ...
            conf = re.sub(
                rep, "net.ipv4.icmp_echo_ignore_all=" + get.status + "\n", conf
            )
        else:
    ...
    def firewall_admin(self, get):
        ...
        if self.__isUfw:
            ...
            return public.returnMsg(
                True, public.lang("firewall has {}", result[get.status])
            )
        if self.__isFirewalld:
    ...
    def FirewallReload(self):
        if self.__isUfw:
            public.ExecShell(
                "/usr/sbin/ufw reload"
            )  # 兼容安装了多个防火墙的情况 hezhihong  # return
            # 解决deian系统启动ufw后禁ping失1效 by wzz/2023-06-14
            filename = "/etc/sysctl.conf"
            conf = public.readFile(filename)
            if conf.find("net.ipv4.icmp_echo") != -1:
        ...
        for port in port_list:
            if port.find("-") != -1:
                ...
                if not re.search(rep1, ports[0]):
                    return public.returnMsg(
                        False, public.lang("Port range is incorrect!")
                    )
    ...
    def create_rules2(self, get):
        ...
        if address:
            ...
            for source_ip in _ips:
                if not re.search(
                    rep2, source_ip
                ) and not self.is_ipv6_network_segment_or_ipv6_address(source_ip):
                    return public.returnMsg(
                        False, public.lang("IP address you've input is illegal!")
                    )
    ...
    def create_rules(self, get):
        ...
        domain_total = (
            "" if ("domain" not in get or not get.domain) else get.domain.strip()
        )
        domain = (
            ""
            if ("domain" not in get or not get.domain)
            else get.domain.strip() + "|" + address
        )
        result = self.check_port(port_list)  # 检测端口
        if result:
            return result
        ...
        for source_ip in allow_ips:
            for port in port_list:
                ...
                if query_result:
                    new_query_result = {
                        "ports": query_result["ports"],
                        "address": query_result["address"],
                        "protocol": query_result["protocol"],
                ...
                if domain:
                    ...
                    public.M("firewall_new").where("id=?", (self._add_sid,)).save(
                        "sid", domain_sid
                    )
        if len(allow_ips) > 0:
            self.FirewallReload()
        ...
        public.WriteLog(
            "system firewall",
            "Add port rules: Protocol:{}, Port:{}, Policy:{}, IP:{}".format(
                protocol, ports, strategy, log_ip
            ),
        )
        # 如果有忽略的端口，返回忽略的端口
        if ignore_list:
    ...
    def remove_rules(self, get):
        ...
        if not self.get_firewall_status():
            return public.returnMsg(
                False, public.lang("Please enable the firewall before proceeding.")
            )
        ...
        public.WriteLog(
            "system firewall",
            "Delete port rules: Protocol:{}, Port:{}, Policy:{}, IP:{}".format(
                get.protocol, get.ports, types, log_ip
            ),
        )
    ...
    def modify_rules(self, get, addtime=None):
        ...
        if address:
            ...
            if not re.search(
                rep, get.source
            ) and self.is_ipv6_network_segment_or_ipv6_address(get.source):
                return public.returnMsg(
                    False, public.lang("IP address you've input is illegal!")
                )
        data = (
            public.M("firewall_new")
            .where("id=?", (id,))
            .field("id,address,protocol,ports,types,brief,addtime,domain")
            .find()
        )
        if data:
        ...
        self._modify_firewall_rules(
            _address, _protocol, _port, _type, address, protocol, ports, types
        )
        addtime = time.strftime("%Y-%m-%d %X", time.localtime())
        ...
        public.WriteLog(
            "system firewall",
            "Modify port rules： Protocol:{}, Port:{}, Policy:{}, IP:{}".format(
                get.protocol, get.ports.strip(), get.types, log_ip
            ),
        )
        return public.returnMsg(True, public.lang("Added successfully!"))
    ...
    def add_ufw_rule(self, address, protocol, ports, types):
        ...
        if address == "":
            ...
            if protocol.find("/") != -1:
                ...
                public.ExecShell(
                    "ufw " + rule + " from " + address + " to any port " + ports + ""
                )
            else:
                public.ExecShell(
                    "ufw "
    ...
    def del_ufw_rule(self, address, protocol, ports, types):
        ...
        if address == "":
            ...
            if protocol.find("/") != -1:
                ...
                public.ExecShell(
                    "ufw delete " + rule + " from " + address + " to any port " + ports
                )
            else:
                public.ExecShell(
                    "ufw delete "
    ...
    def edit_ufw_rule(
        ...
        if address == "":
            ...
            if protocol.find("/") != -1:
                ...
                public.ExecShell(
                    "ufw " + rules + " from " + address + " to any port " + ports
                )
            else:
                public.ExecShell(
                    "ufw "
    ...
    def check_a_ip(self, address):
        ...
        if address:
            ...
            if address[-1] == ".":
                address = address[:-1]
            if public.is_domain(address):
                return self.get_a_ip(address)
        return address
    ...
    def get_a_ip(self, hostname):
        ...
        if not self.install_dnspython():
            return public.returnMsg(
                False,
                public.lang(
                    "Please install dnspython module first: btpip install dnspython"
                ),
            )
        import dns.resolver
    ...
    def del_domain_ip(self, args):
        ...
        if not public.M("firewall_domain").count():
            pdata = (
                ...
                .where(
                    "name=?",
                    "[Do not delete] System firewall domain name resolution detection task",
                )
                .select()
            )
            if pdata:
    ...
    def add_crontab(self):
        ...
        if (
            ...
            cmd = "{} {}".format(
                python_path, "/www/server/panel/script/firewall_domain.py"
            )
            args = {
                "name": "[Do not delete] System firewall domain name resolution detection task",
                "type": "minute-n",
                "where1": "5",
                "hour": "",
                "minute": "",
    ...
    def set_domain_ip2(self, args):
        ...
        if not pay:
            return public.returnMsg(
                False,
                public.lang(
                    "Current features are exclusive to the professional version"
                ),
            )
        if not args.domain:
            return public.returnMsg(False, public.lang("Please enter domain name"))
        ports = ""
        ...
        return public.returnMsg(
            True,
            public.lang("Domain name {} resolution added successfully", args.domain),
        )
    ...
    def set_domain_ip(self, args):
        ...
        if (
            public.M("firewall_domain")
            .where(
                "domain=? and types=? and port=? and protocol=?",
                (
                    args.domain,
                    args.types,
                    ports,
            ...
            return public.returnMsg(
                False, public.lang("Domain name {} already exists", args.domain)
            )
            ...
            if int(args.sid) > 0:
                ...
                if choose == "domain":
                    ...
                    # 当修改为相同域名时
                        ...
                        public.M("firewall_domain").where("id=?", pdata["id"]).update(
                            pdata
                        )
    ...
    def handle_firewall_ip(self, address, types):
        ...
        if isinstance(ip_list, dict):
            return
        public.ExecShell(
            "firewall-cmd --permanent --zone=public --new-ipset="
            + address
            + " --type=hash:net"
        )
    ...
    def create_ip_rules(self, get):
        ...
        domain = (
            ""
            if ("domain" not in get or not get.domain)
            else get.domain.strip() + "|" + _address
        )
        ...
        # 先处理用户的IP地址
        old_login_ip = (
            public.M("firewall_ip")
            .where("brief=?", ("IP that allows users to log in",))
            .field("id, address")
            .select()
        )
        ...
        for ip_range in address_list:
            ...
            if self.ip_in_range(user_ip, ip_range):
                ...
                self.add_rule(
                    user_ip,
                    "accept",
                    "IP that allows users to log in",
                    domain,
                    domain_total,
                )
        ...
        public.WriteLog(
            "system firewall",
            "Add IP rules: IP: {}, policy: {}".format(_address, original_types),
        )
    ...
    def add_rule(self, address, types, brief, domain, domain_total):
        if (
            public.M("firewall_ip")
            .where("address=? and types=? and domain=?", (address, types, domain))
            .count()
            > 0
        ):
            return
        if self.__isUfw:
            ...
            if address.find("-") != -1:
                ...
                is_debian = (
                    True
                    if public.get_os_version().lower().find("debian") != -1
                    else False
                )
                if not is_debian:
                    if _rule == "allow":
        ...
        self._add_sid = public.M("firewall_ip").add(
            "address,types,brief,addtime,domain,sid",
            (
                address,
                types,
                public.xsssec(brief),
        ...
        if domain:
            ...
            public.M("firewall_ip").where("id=?", (self._add_sid,)).save(
                "sid", domain_sid
            )
    ...
    def remove_ip_rules(self, get):
        ...
        public.WriteLog(
            "system firewall",
            "Delete IP rules: IP:{}, policy:{}".format(get.address, strategy),
        )
        return public.returnMsg(True, public.lang("Delete successfully!"))
    ...
    def modify_ip_rules(self, get):
        ...
        data = (
            public.M("firewall_ip")
            .where("id=?", (id,))
            .field("id,address,types,brief,addtime")
            .find()
        )
        _address = data.get("address", "")
        _type = data.get("types", "")
        ...
        if domain:
            public.M("firewall_domain").where("id=?", (sid,)).save(
                "sid,types,brief,domain_total", (id, types, brief, domain_total)
            )
        self.FirewallReload()
        old_strategy = ""
        if types == "accept":
        ...
        public.WriteLog(
            "system firewall",
            "修改规则, IP:{}, 策略:{} -> IP:{}, 策略:{}".format(
                _address, old_strategy, get.address.strip(), get.types
            ),
        )
    ...
    def create_forward(self, get):
        ...
        if public.M("firewall_trans").where("start_port=?", (s_port,)).count() > 0:
            return public.returnMsg(
                False,
                public.lang("This port already exists, please do not add it again!"),
            )
        if self.__isUfw:
        ...
        public.WriteLog(
            "system firewall",
            "Add port forwarding rules: Start port: {}, Destination port: {}, Destination IP: {}".format(
                s_port, d_port, d_ip
            ),
        )
        return public.returnMsg(True, public.lang("Added successfully!"))
    ...
    def modify_forward(self, get):
        ...
        if not re.search(rep1, d_port):
            return public.returnMsg(False, public.lang("Port range is incorrect!"))
        data = (
            public.M("firewall_trans")
            .where("id=?", (id,))
            .field("id,start_port,ended_ip,ended_port,protocol,addtime")
            .find()
        )
        ...
        if d_ip:
            ...
            if d_ip in ["127.0.0.1", "localhost"]:
                d_ip = ""
        if self.__isUfw:
            content = self.ufw_handle_update(
                start_port, ended_ip, ended_port, protocol, s_port, d_ip, d_port, pool
            )
            self.save_profile(self._ufw_before, content)
        else:
            if self.__isFirewalld:
                self.firewall_handle_update(
                    start_port,
                    ended_ip,
                    ended_port,
                    protocol,
    ...
    def ufw_handle_add(self, s_port, d_port, d_ip, protocol):
        ...
        if d_ip == "":
            if protocol.find("/") != -1:
                ...
                _string = (
                    _string
                    + "-A PREROUTING -p udp --dport {1} -j REDIRECT --to-port {2}".format(
                        s_port, d_port
                    )
                )
            else:
                _string = (
                    "-A PREROUTING -p {0} --dport {1} -j REDIRECT --to-port {2}".format(
            ...
            ) + "-A POSTROUTING -d {0} -j MASQUERADE".format(
                d_ip
            )
    ...
    def ufw_handle_del(self, s_port, d_port, d_ip, protocol):
        ...
        if d_ip == "":
            _string = (
                "-A PREROUTING -p {0} --dport {1} -j REDIRECT --to-port {2}\n".format(
                    protocol, s_port, d_port
                )
            )
        else:
    ...
    # 处理ufw的端口转发修改
    def ufw_handle_update(
        self, start_port, ended_ip, ended_port, protocol, s_port, d_ip, d_port, pool
    ):
        content = self.get_profile(self._ufw_before)
        if ended_ip == "":
            s_string = (
    ...
    def open_close_forward(self, get):
        ...
        if self.__isUfw:
            ...
            if get.status == "open":
                ...
                content1 = content1.replace(
                    'DEFAULT_FORWARD_POLICY="ACCEPT"', 'DEFAULT_FORWARD_POLICY="DROP"'
                )
                content2 = content2.replace(
                    "net/ipv4/ip_forward=1", "#net/ipv4/ip_forward=1"
                )
            ...
            self.FirewallReload()
            return public.returnMsg(
                True, public.lang("Enable" if get.status == "open" else "Disable")
            )
        if self.__isFirewalld:
            if get.status == "open":
                public.ExecShell("firewall-cmd --add-masquerade --permanent")
    ...
    def create_countrys(self, get):
        try:
            if not hasattr(get, "country"):
                return public.returnMsg(
                    False, public.lang("Please enter the country name!")
                )
    ...
    def create_country(self, get, is_mutil=False, _ips_paths=None):
        ...
        for add in add_list:
            if not add["ports"]:
                return public.returnMsg(
                    False,
                    public.lang(
                        "This area has already been added, please do not add it again!"
                    ),
                )
        ...
        if ports:
            ...
            for port in port_list:
                ...
                if (
                    public.M("firewall_country")
                    .where("country=? and ports=?", (country, port))
                    .count()
                    > 0
                ):
        ...
        if not ip_list:
            ...
            return public.returnMsg(
                True, public.lang("Please enter the correct area name!")
            )
        if self.__isUfw:
        ...
        public.WriteLog(
            "system firewall",
            "Add regional rules: Region:{}, Policy:{}, Port:{}".format(
                get.country, get.types, log_port
            ),
        )
        return public.returnMsg(True, public.lang("Added successfully!"))
    ...
    def remove_country(self, get):
        ...
        if self.__isUfw:
            ...
            if (
                not public.M("firewall_country").where("country=?", (country,)).count()
                > 0
            ):
        ...
        public.WriteLog(
            "system firewall",
            "Delete zone rules: Region:{}, Policy:{}, Port:{}".format(
                get.country, strategy, log_port
            ),
        )
        return public.returnMsg(True, public.lang("Delete successfully!"))
    ...
    def modify_country(self, get):
        ...
        data = (
            public.M("firewall_country")
            .where("id=?", (id,))
            .field("id,country,types,brief,ports,addtime")
            .find()
        )
    ...
    def GetList(self):
        try:
            ...
            for i in range(len(result)):
                if "address" not in result[i].keys():
                    continue
                tmp = self.check_db_exists(
                    result[i]["ports"], result[i]["address"], result[i]["types"]
                )
                ...
                if not tmp:
                    if ports:
                        ...
                        public.M("firewall_ip").add(
                            "address,types,brief,addtime", (address, types, "", addtime)
                        )
        ...
        try:
            for dt in data:
                ...
                if port.find(".") != -1:
                    ...
                    if not tmp:
                        public.M("firewall_ip").add(
                            "address,types,brief,addtime", (port, "drop", "", addtime)
                        )
                else:
    ...
    def check_db_exists(self, ports, address, types):
        if ports:
            data = (
                public.M("firewall_new")
                .field("id,ports,protocol,address,types,brief,addtime")
                .select()
            )
            for dt in data:
                if dt["ports"] == ports:
            ...
            data = (
                public.M("firewall_ip").field("id,address,types,brief,addtime").select()
            )
            for dt in data:
                if dt["address"] == address and dt["types"] == types:
                    return dt
            return False
    ...
    def import_rules(self, get):
        try:
            rule_name = (
                get.rule_name
            )  # 规则名:[port_rule, ip_rule, trans_rule, country_rule]
            file_name = (
                get.file_name
            )  # 文件命:[port.json, ip.json, trans.json, country.json]
    ...
    def check_table(self):
        if (
            public.M("sqlite_master")
            .where("type=? AND name=?", ("table", "firewall_new"))
            .count()
        ):
            if (
                public.M("sqlite_master")
                .where("type=? AND name=?", ("table", "firewall_ip"))
                .count()
            ):
                if (
                    public.M("sqlite_master")
                    .where("type=? AND name=?", ("table", "firewall_trans"))
                    .count()
                ):
                    if (
                        public.M("sqlite_master")
                        .where("type=? AND name=?", ("table", "firewall_country"))
                        .count()
                    ):
                        return True
        return Sqlite()
    ...
    def sync_must_ports(self, get):
        ...
        print(ports)
        if not ports:
            return public.returnMsg(False, public.lang("Port cannot be empty!"))
    ...
    def get_listening_processes(get: public.dict_obj):
        ...
        @return:
        """
        if "port" not in get.get_items().keys():
            return public.returnMsg(
                False,
                public.lang("Parameter passing error, please pass the port field"),
            )
        if len(get.port) == 0:
            return public.returnMsg(False, public.lang("Port cannot be empty"))
        if get.port.find("-") != -1 or get.port.find(":") != -1:
            return public.returnMsg(False, public.lang("Range ports not supported"))
        if not get.port.isdigit():
            return public.returnMsg(False, public.lang("Port must be numeric"))
        if int(get.port) < 1 or int(get.port) > 65535:
            return public.returnMsg(False, public.lang("The port range is 1-65535"))
        ...
        cmd = (
            "lsof -i:{}|grep LISTEN|grep -v COMMAND".format(get.port)
            + "|awk '{print $1,$2}'"
        )
        ...
        if len(info_list) == 2:
            ...
            process_cmd = (
                public.ExecShell(cmd_ps + cmd_awk)[0].split("\n")[0].strip(" ")
            )
    ...
    def get_panel_firewall_rules(self):
        ...
        all_ports = (
            public.M("firewall_new")
            .field("protocol,ports,types,address")
            .order("addtime desc")
            .select()
        )
        ...
        for item1 in list1:
            ...
            try:
                for item2 in list2:
                    ...
                    if all(
                        item1[key] == item2[key]
                        for key in ("protocol", "ports", "types", "address")
                    ):
                        found = True
                        break
            except KeyError:
                continue
            if not found:
                list1_not_in_list2.append(item1)
    ...
    def _get_diff_rules(self, panel_firewall_rules, firewall_rules):
        ...
        if not firewall_diff_rules:
            data = {
                "panel_not_in_sys_fw_diff_list": self._diff_dict_list(
                    panel_firewall_rules, firewall_rules
                ),
                "sys_not_in_panel_fw_diff_list": self._diff_dict_list(
                    firewall_rules, panel_firewall_rules
                ),
                "panel_exclude": [],
                "sys_exclude": [],
            }
        ...
        # 将差异规则写入文件并返回
        firewall_diff_rules["panel_not_in_sys_fw_diff_list"] = (
            panel_not_in_sys_fw_diff_list
        )
        firewall_diff_rules["sys_not_in_panel_fw_diff_list"] = (
            sys_not_in_panel_fw_diff_list
        )
    ...
    def exclude_diff_rules(self, get):
        ...
        try:
            panel_excludes = (
                get.panel_exclude if "panel_exclude" in get.get_items().keys() else {}
            )
            sys_excludes = (
                get.sys_exclude if "sys_exclude" in get.get_items().keys() else {}
            )
    ...
    def _modify_firewall_rules(
        self, address, protocol, port, type, source_ip, source_protocol, ports, types
    ):
        """
        修改防火墙规则1
        @param address:
        ...
        if self.__isUfw:
            self.edit_ufw_rule(
                address, protocol, port, type, source_ip, source_protocol, ports, types
            )
        elif self.__isFirewalld:
            self.edit_firewall_rule(
                address, protocol, port, type, source_ip, source_protocol, ports, types
            )
        else:
            self.edit_iptables_rule(
                address, protocol, port, type, source_ip, source_protocol, ports, types
            )
    ...
    def get_anti_scan_logs(self, get):
        ...
        for key in server_lists:
            ...
            if type(logs_result["msg"]) is dict:
                result_dict["currently_failed"] += int(
                    logs_result["msg"]["currently_failed"]
                )
                result_dict["total_failed"] += int(logs_result["msg"]["total_failed"])
                result_dict["currently_banned"] += int(
                    logs_result["msg"]["currently_banned"]
                )
    ...
    def set_anti_scan_status(self, get):
        ...
        if not os.path.exists(_set_up_path + "/fail2ban_main.py"):
            return public.returnMsg(
                False, public.lang("fail2ban plugin is not installed")
            )
...
class Sqlite:
    ...
    def create_table(self):
        ...
        if (
            not public.M("sqlite_master")
            .where("type=? AND name=?", ("table", "firewall_domain"))
            .count()
        ):
            public.M("").execute(
                """CREATE TABLE "firewall_domain" (
                "id" INTEGER PRIMARY KEY AUTOINCREMENT,
                "types" TEXT,
                "domain" TEXT,
                "domain_total" TEXT,
            ...
            public.M("").execute(
                "CREATE INDEX firewall_domain_addr ON firewall_domain (domain);"
            )
        ...
        if "domain" not in create_table_str:
            public.M("firewall_new").execute(
                'ALTER TABLE "firewall_new" ADD "domain" TEXT DEFAULT ""'
            )
        if "sid" not in create_table_str:
            public.M("firewall_new").execute(
                'ALTER TABLE "firewall_new" ADD "sid"  int DEFAULT 0'
            )
        ...
        if "sid" not in create_table_str:
            public.M("firewall_ip").execute(
                'ALTER TABLE "firewall_ip" ADD "sid"  int DEFAULT 0'
            )
        if "domain" not in create_table_str:
            public.M("firewall_ip").execute(
                'ALTER TABLE "firewall_ip" ADD "domain" TEXT DEFAULT ""'
            )
        ...
        if "ports" not in create_table_str:
            public.M("firewall_country").execute(
