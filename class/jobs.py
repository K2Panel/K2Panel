...
def acme_crond_reinit():
    ...
    """
    try:
        lets_config_file = os.path.join(
            public.get_panel_path(), "config/letsencrypt_v2.json"
        )
        if not os.path.exists(lets_config_file):
            return
...
def rep_websocket_conf():
    ...
    if os.path.exists(conf_file):
        ...
        if conf_body.find("map $http_upgrade $connection_upgrade") != -1:
            return
...
def upgrade_polkit():
    ...
    if os.path.exists(tip_file):
        return
    os.system(
        "nohup {} {}/script/polkit_upgrade.py &> {}".format(
            public.get_python_bin(), public.get_panel_path(), upgrade_log_file
        )
    )
...
def clear_other_files():
    ...
    if os.path.exists(p_file):
        public.ExecShell("rm -f class/*.so")
    public.ExecShell(
        "chmod -R  600 /www/server/panel/data;chmod -R  600 /www/server/panel/config;chmod -R  700 /www/server/cron;chmod -R  600 /www/server/cron/*.log;chown -R root:root /www/server/panel/data;chown -R root:root /www/server/panel/config;chown -R root:root /www/server/phpmyadmin;chmod -R 755 /www/server/phpmyadmin"
    )
    ...
    if os.path.exists(node_service_src):
        public.ExecShell("chmod 700 " + node_service_src)
...
def sql_pacth():
    ...
    if (
        ...
        .where("type=? AND name=?", ("table", "load_average"))
        .count()
    ):
...
`addtime` INTEGER
    ...
    if (
        ...
        .where("type=? AND name=? AND sql LIKE ?", ("table", "sites", "%type_id%"))
        .count()
    ):
    ...
    if (
        ...
        .where(
            ...
            ("table", "database_servers", "%db_type%"),
        )
        .count()
    ):
        public.M("databases").execute(
            "alter TABLE database_servers add db_type REAL DEFAULT 'mysql'", ()
        )

    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "sites", "%edate%"))
        .count()
    ):
    ...
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "sites", "%project_type%"))
        .count()
    ):
        public.M("sites").execute(
            "alter TABLE sites add project_type STRING DEFAULT 'PHP'", ()
        )

    if (
        not public.M("sqlite_master")
        .where(
            "type=? AND name=? AND sql LIKE ?", ("table", "sites", "%project_config%")
        )
        .count()
    ):
        public.M("sites").execute(
            "alter TABLE sites add project_config STRING DEFAULT '{}'", ()
        )

    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "backup", "%ps%"))
        .count()
    ):
    ...
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "databases", "%db_type%"))
        .count()
    ):
        public.M("databases").execute(
            "alter TABLE databases add db_type integer DEFAULT '0'", ()
        )

    if (
        not public.M("sqlite_master")
        .where(
            "type=? AND name=? AND sql LIKE ?", ("table", "databases", "%conn_config%")
        )
        .count()
    ):
        public.M("databases").execute(
            "alter TABLE databases add conn_config STRING DEFAULT '{}'", ()
        )

    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "databases", "%sid%"))
        .count()
    ):
        public.M("databases").execute(
            "alter TABLE databases add sid integer DEFAULT 0", ()
        )

    ndb = (
        public.M("databases")
        .order("id desc")
        .field("id,pid,name,username,password,accept,ps,addtime,type")
        .select()
    )
    if type(ndb) == str:
        public.M("databases").execute(
            "alter TABLE databases add type TEXT DEFAULT MySQL", ()
        )

    # 计划任务表处理
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%status%"))
        .count()
    ):
    ...
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%save%"))
        .count()
    ):
        public.M("crontab").execute(
    ...
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%backupTo%"))
        .count()
    ):
        public.M("crontab").execute(
    ...
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%sName%"))
        .count()
    ):
        public.M("crontab").execute("ALTER TABLE 'crontab' ADD 'sName' TEXT", ())
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%sBody%"))
        .count()
    ):
        public.M("crontab").execute("ALTER TABLE 'crontab' ADD 'sBody' TEXT", ())
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%sType%"))
        .count()
    ):
        public.M("crontab").execute("ALTER TABLE 'crontab' ADD 'sType' TEXT", ())
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%urladdress%"))
        .count()
    ):
        public.M("crontab").execute("ALTER TABLE 'crontab' ADD 'urladdress' TEXT", ())
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%save_local%"))
        .count()
    ):
        public.M("crontab").execute(
            "ALTER TABLE 'crontab' ADD 'save_local' INTEGER DEFAULT 0", ()
        )
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%notice%"))
        .count()
    ):
        public.M("crontab").execute(
            "ALTER TABLE 'crontab' ADD 'notice' INTEGER DEFAULT 0", ()
        )
    if (
        not public.M("sqlite_master")
        .where(
            "type=? AND name=? AND sql LIKE ?", ("table", "crontab", "%notice_channel%")
        )
        .count()
    ):
        public.M("crontab").execute(
            "ALTER TABLE 'crontab' ADD 'notice_channel' TEXT DEFAULT ''", ()
        )

    sql = db.Sql()
    if (
        not sql.table("sqlite_master")
        .where("type=? AND name=?", ("table", "site_types"))
        .count()
    ):
...
`ps` REAL
    ...
    if (
        not sql.table("sqlite_master")
        .where("type=? AND name=?", ("table", "wp_site_types"))
        .count()
    ):
...
`ps` TEXT
    ...
    if (
        not sql.table("sqlite_master")
        .where("type=? AND name=?", ("table", "download_token"))
        .count()
    ):
...
`addtime` INTEGER
    ...
    if (
        not sql.table("sqlite_master")
        .where("type=? AND name=?", ("table", "messages"))
        .count()
    ):
...
`addtime` INTEGER
    ...
    if (
        not sql.table("sqlite_master")
        .where("type=? AND name=?", ("table", "temp_login"))
        .count()
    ):
...
`addtime` INTEGER
    ...
    if (
        not sql.table("sqlite_master")
        .where("type=? AND name=?", ("table", "database_servers"))
        .count()
    ):
...
`addtime` INTEGER
    ...
    if (
        not sql.table("sqlite_master")
        .where("type=? AND name=?", ("table", "security"))
        .count()
    ):
    ...
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "logs", "%username%"))
        .count()
    ):
    ...
    public.M("users").where(
        "email=? or email=?", ("287962566@qq.com", "amw_287962566@qq.com")
    ).setField("email", "test@message.com")
    ...
    if (
        not public.M("sqlite_master")
        .where("type=? AND name=? AND sql LIKE ?", ("table", "messages", "%retry_num%"))
        .count()
    ):
        public.M("messages").execute(
            "alter TABLE messages add send integer DEFAULT 0", ()
        )
        public.M("messages").execute(
            "alter TABLE messages add retry_num integer DEFAULT 0", ()
        )
...
def install_pycountry():
    ...
    whl_file = "{}/script/pycountry-24.6.1-py3-none-any.whl".format(
...
def install_pyroute2():
    ...
    if os.path.exists(script_file) and not os.path.exists(tip_file):
        ...
        os.system("bash {}".format(script_file))
        public.restart_panel()
    whl_file = "{}/script/pyroute2-0.7.12-py3-none-any.whl".format(
        public.get_panel_path()
    )
    if os.path.exists(whl_file):
        os.remove(whl_file)
...
def deb_bashrc():
    ...
    if not isinstance(profile_body, str):
        return
...
def null_html():
    null_files = [
        "/www/server/nginx/html/index.html",
        "/www/server/apache/htdocs/index.html",
        "/www/server/panel/data/404.html",
    ]
    null_new_body = """<html>
...
<hr><center>nginx</center>
    ...
    for null_file in null_files:
        ...
        if not null_body:
            continue
        if (
            null_body.find("没有找到站点") != -1
            or null_body.find("您请求的文件不存在") != -1
        ):
...
def set_php_cli_env():
    ...
    for php_version in php_versions_list:
        ...
        if os.path.exists(php_bin):
            # 设置每个版本的环境变量
            if not os.path.exists(env_php_bin):
                os.symlink(php_bin, env_php_bin)
            if not os.path.exists(php_ize) and os.path.exists(php_ize_src):
                os.symlink(php_ize_src, php_ize)
            if not os.path.exists(php_fpm) and os.path.exists(php_fpm_src):
                os.symlink(php_fpm_src, php_fpm)
            if not os.path.exists(php_pecl) and os.path.exists(php_pecl_src):
                os.symlink(php_pecl_src, php_pecl)
            if not os.path.exists(php_pear) and os.path.exists(php_pear_src):
                os.symlink(php_pear_src, php_pear)
            public.ExecShell(
                r"\cp -f {} {}".format(php_ini, php_cli_ini)
            )  # 每次复制新的php.ini到php-cli.ini
            public.ExecShell(
                'sed -i "/disable_functions/d" {}'.format(php_cli_ini)
            )  # 清理禁用函数
            bashrc_body += "alias php{}='php{} -c {}'\n".format(
                php_version, php_version, php_cli_ini
            )  # 设置别名
        else:
            # 清理已卸载的环境变量
            if os.path.exists(env_php_bin):
                os.remove(env_php_bin)
            if os.path.exists(php_ize):
                os.remove(php_ize)
            if os.path.exists(php_fpm):
                os.remove(php_fpm)
            if os.path.exists(php_pecl):
                os.remove(php_pecl)
            if os.path.exists(php_pear):
                os.remove(php_pear)
...
def check_enable_php():
    ...
    for php_v in php_versions:
        ngx_php_conf = (
            public.get_setup_path() + "/nginx/conf/enable-php-{}.conf".format(php_v)
        )
        if os.path.exists(ngx_php_conf):
            continue
        enable_conf = r"""
    location ~ [^/]\.php(/|$)
	{{
...
def run_script():
    try:
        ...
        for sname in os.listdir(run_config):
            ...
            exec_script = "nohup {} {} &> {} &".format(
                _bin, script_info["script_file"], exec_log_file
            )
...
def update_py37():
    ...
    if os.path.exists(pyenv) or os.path.exists(pyenv_exists):
        return False
    download_url = public.get_url()
    public.ExecShell(
        "nohup curl {}/install/update_panel_en.sh|bash &>/tmp/panelUpdate.pl &".format(
            download_url
        )
    )
...
def update_py312():
    ...
    public.ExecShell(
        "nohup curl -k {}/install/update_7.x_en.sh|bash &>/tmp/panelUpdate.pl &".format(
            download_url
        )
    )
    return True
...
def check_dnsapi():
    ...
    if tmp.find("CloudFlare") == -1:
        cloudflare = {
            ...
            "title": "CloudFlare",
            "data": [
                {"value": "", "key": "SAVED_CF_MAIL", "name": "E-Mail"},
                {"value": "", "key": "SAVED_CF_KEY", "name": "API Key"},
            ],
...
def check_firewall():
    try:
        if not os.path.exists("/usr/sbin/firewalld"):
            return False
...
def run_new():
    try:
        ...
        if not port:
            return False
        cmd_line = public.ExecShell(
            "lsof -P -i:{}|grep LISTEN|grep -v grep".format(int(port))
        )[0]
        if len(cmd_line) < 20:
            return False
        if cmd_line.find("BT-Panel") != -1:
            return False
...
def clean_php_log():
    ...
    for name in os.listdir(path):
        if name not in php_list:
            continue
        filename = path + "/" + name + "/var/log/php-fpm.log"
        if os.path.exists(filename):
            clean_max_log(filename)
...
def remove_tty1():
    ...
    if not os.path.islink(file_path):
        return False
    if os.readlink(file_path) != "/dev/null":
        return False
    try:
        os.remove(file_path)
    except:
...
def disable_putenv(fun_name):
    try:
        ...
        if os.path.exists(is_set_disable):
            return True
        ...
        for pv in php_vs:
            ...
            if disable_functions.find(fun_name) != -1:
                continue
