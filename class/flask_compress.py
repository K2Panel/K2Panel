...
class Compress(object):
    ...
    def after_request(self, response):
        ...
        if "rm_ssl" in g:
            ...
            response.set_cookie(
                session_name, "", expires="Thu, 01-Jan-1970 00:00:00 GMT", path="/"
            )
            response.set_cookie(
                session_name, session_id, path="/", max_age=86400 * 30, httponly=True
            )

            request_token = request.cookies.get("request_token", "")
            if request_token:
                response.set_cookie(
                    "request_token", request_token, path="/", max_age=86400 * 30
                )
