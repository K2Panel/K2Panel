...
class data:
    ...
    def get_site_ssl_info(self, siteName):
        try:
            ...
            ssl_info["endtime"] = int(
                int(
                    time.mktime(time.strptime(ssl_info["notAfter"], "%Y-%m-%d"))
                    - time.time()
                )
                / 86400
            )
    ...
    def get_database_size(self, databaseName):
        try:
            if not self.DB_MySQL:
                self.DB_MySQL = panelMysql.panelMysql()
            db_size = self.map_to_list(
                self.DB_MySQL.query(
    ...
    def getData(self, get):
        ...
        if user_Data != "all" and table in ["sites", "databases", "ftps"]:
            data["data"] = [
                i for i in data["data"] if str(i["id"]) in user_Data.get(table, [])
            ]
        ...
        try:
            ...
            if table == "backup":
                ...
                backup_path = (
                    public.M("config").where("id=?", (1,)).getField("backup_path")
                )
                for i in range(len(data["data"])):
                    if data["data"][i]["size"] == 0:
                        ...
                        if not os.path.exists(data["data"][i]["filename"]):
                            if (
                                (
                                    data["data"][i]["filename"].find("/www/") != -1
                                    or data["data"][i]["filename"].find(backup_path)
                                    != -1
                                )
                                and data["data"][i]["filename"][0] == "/"
                                and data["data"][i]["filename"].find("|") == -1
                            ):
                                data["data"][i]["size"] = 0
                                data["data"][i]["ps"] = "文件不存在"
                    if data["data"][i]["ps"] in ["", "无"]:
                        if data["data"][i]["name"][:3] == "db_" or (
                            data["data"][i]["name"][:4] == "web_"
                            and data["data"][i]["name"][-7:] == ".tar.gz"
                        ):
                            data["data"][i]["ps"] = "自动备份"
                        else:
                            data["data"][i]["ps"] = "手动备份"
                    ...
                    data["data"][i]["localexist"] = (
                        0 if os.path.isfile(data["data"][i]["local"]) else 1
                    )
                ...
                for i in range(len(data["data"])):
                    ...
                    try:
                        backup_count = (
                            SQL.table("backup")
                            .where("pid=? AND type=?", (data["data"][i]["id"], type))
                            .count()
                        )
                    except:
                        pass
                    ...
                    if table == "databases":
                        data["data"][i]["conn_config"] = json.loads(
                            data["data"][i]["conn_config"]
                        )
                    data["data"][i]["quota"] = self.get_database_quota(
                        data["data"][i]["name"]
                    )
                ...
                if table == "sites":
                    for i in range(len(data["data"])):
                        ...
                        data["data"][i]["site_ssl"] = (
                            ssl_info["endtime"] if ssl_info != -1 else -1
                        )

                        data["data"][i]["php_version"] = self.get_php_version(
                            data["data"][i]["name"]
                        )
                        data["data"][i]["attack"] = self.get_analysis(
                            get, data["data"][i]
                        )
                        ...
                        if data["data"][i]["project_type"] == "WP":
                            ...
                            data["data"][i][
                                "cache_status"
                            ] = one_key_wp.one_key_wp().get_cache_status(
                                data["data"][i]["id"]
                            )
                        if not data["data"][i]["status"] in ["0", "1", 0, 1]:
                            data["data"][i]["status"] = "1"
                        data["data"][i]["quota"] = self.get_site_quota(
                            data["data"][i]["path"]
                        )
                        site1 = (
                            SQL.table("sites")
                ...
                for i in range(len(data["data"])):
                    if (
                        ...
                        or data["data"][i]["port"].find(".") != -1
                        or data["data"][i]["port"].find("-") != -1
                    ):
                        data["data"][i]["status"] = -1
                    else:
                        data["data"][i]["status"] = self.CheckPort(
            ...
            if get.table == "sites":
                if isinstance(data, dict):
                    file_path = os.path.join(
                        public.get_panel_path(), "data/sort_list.json"
                    )
                    if os.path.exists(file_path):
                        ...
                        if top_list:
                            top_list = top_list[::-1]
                        top_data = [
                            item for item in data["data"] if str(item["id"]) in top_list
                        ]
                        data1 = [
                            item
                            for item in data["data"]
                            if str(item["id"]) not in top_list
                        ]
                        top_data.sort(key=lambda x: top_list.index(str(x["id"])))
                        data["data"] = top_data + data1
            public.set_search_history(
                get.table, get.search_key, get.search
            )  # 记录搜索历史
            # 字段排序
                        ...
                        if os.path.exists(os.path.join(user_authority, str(uid))):
                            try:
                                data = json.loads(
                                    self._decrypt(
                                        public.ReadFile(
                                            os.path.join(user_authority, str(uid))
                                        )
                                    )
                                )
    ...
    def get_sort_data(self, data):
        ...
        if "plist" in data:
            ...
            data["data"] = sorted(
                data["data"], key=lambda x: x[sort_key], reverse=reverse
            )
    ...
    def GetSql(self, get, result="1,2,3,4,5,8"):
        ...
        if hasattr(get, "search"):
            ...
            if get.table == "sites" and get.search:
                ...
                pid = (
                    SQL.table("domain")
                    .where(
                        "name LIKE ?{}".format(conditions), ("%{}%".format(get.search),)
                    )
                    .getField("pid")
                )
                if pid:
        ...
        if get.table == "sites":
            search_key = "php"
            if where:
                where = "({}) AND (project_type='PHP' OR project_type='WP')".format(
                    where
                )
            else:
        ...
        if o_list[0] in self.__SORT_DATA:
            data["data"] = (
                SQL.table(get.table).where(where, param).field(field).select()
            )
    ...
    def GetWhere(self, tableName, search):
        ...
        wheres = {
            "sites": (
                "name LIKE ? OR ps LIKE ?{}".format(conditions),
                ("%" + search + "%", "%" + search + "%"),
            ),
            "ftps": (
                "name LIKE ? OR ps LIKE ?{}".format(conditions),
            ...
            "logs": (
                "username=? OR type LIKE ?{} OR log LIKE ?{}".format(
