...
class ssl_info:
    ...
    def load_ssl_info_by_data(self, pem_data: str):
        ...
        if is_openssl:
            ...
            res = public.ExecShell("openssl x509 -in {} -noout -text".format(pem_file))[
                0
            ]
    ...
    def __get_cert_info(self, pem_data):
        ...
        result["endtime"] = int(
            int(
                time.mktime(time.strptime(result["notAfter"], "%Y-%m-%d")) - time.time()
            )
            / 86400
        )
        return result
    ...
    def dump_pkcs12_new(
        self, key_pem=None, cert_pem=None, ca_pem=None, friendly_name=""
    ):
        try:
    ...
    def verify_certificate_and_key_match(self, key_pem, cert_pem, password=None):
        ...
        try:
            ...
            if not private_key:
                return (
                    False,
                    "Key error, please check if it is the correct PEM format private key",
                )
            if not cert:
                return (
                    False,
                    "Certificate error, please check if it is the correct PEM format certificate",
                )
            # 使用私钥对消息进行签名
            try:
        ...
        try:
            ...
            for i in range(len(cert_chain) - 1):
                ...
                try:
                    ...
                    return (
                        False,
                        "Certificate chain verification failed, please check if the certificate chain is complete.",
                    )
