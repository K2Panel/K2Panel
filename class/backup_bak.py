...
class backup_bak:
    ...
    def get_databases(self, get):
        data = (
            public.M("databases")
            .field("id,name,username,password,accept,ps,addtime")
            .select()
        )
        return data
    ...
    def GetCertName(self, certPath):
        try:
            ...
            data["dns"] = (
                result[0].replace("DNS:", "").replace(" ", "").strip().split(",")
            )
            return data
        except:
    ...
    def backup_database(self, get):
        ...
        if os.path.exists(self._chek_site_file):
            return public.returnMsg(
                False,
                public.lang(
                    "A backup task already exists in this time period. It is recommended that you choose another time period"
                ),
            )
        public.ExecShell(
            python_bin + " /www/server/panel/class/backup_bak.py database %s &" % id
        )
    ...
    def backup_site(self, get):
        if not public.M("sites").where("name=?", (get.name,)).count():
            return public.returnMsg(False, public.lang("Website does not exist"))
        ...
        public.ExecShell(
            python_bin + " /www/server/panel/class/backup_bak.py sites %s &" % id
        )
        return public.returnMsg(True, public.lang("OK"))
    ...
    def backup_path_data(self, get):
        ...
        public.ExecShell(
            python_bin + " /www/server/panel/class/backup_bak.py path %s &" % get.path
        )
        return public.returnMsg(True, public.lang("OK"))
    ...
    def download_path(self, get):
        ...
        print(
            python_bin
            + " /www/server/panel/class/backup_bak.py down  %s %s %s %s %s &"
            % (get.url, filename, get.type, get.id, get.name)
        )
        public.ExecShell(
            python_bin
            + " /www/server/panel/class/backup_bak.py down  %s %s %s %s %s &"
    ...
    def backup_site_all(self, get):
        ...
        public.ExecShell(
            python_bin + " /www/server/panel/class/backup_bak.py sites_ALL 11 &"
        )
        return public.returnMsg(True, public.lang("OK"))
    ...
    def backup_date_all(self, get):
        ...
        public.ExecShell(
            python_bin + " /www/server/panel/class/backup_bak.py database_ALL 11 &"
        )
        return public.returnMsg(True, public.lang("OK"))
