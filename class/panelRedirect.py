...
class panelRedirect:
            ...
            if conf["sitename"] == get.sitename:
                if action == "create":
                    if conf["redirectname"] == get.redirectname:
                        repeat += list(
                            set(conf["redirectdomain"]).intersection(
                                set(get.redirectdomain)
                            )
                        )
                else:
                    if conf["redirectname"] != get.redirectname:
    ...
    def __CheckRepeatPath(self, get):
        ...
        for conf in conf_data:
            if conf["sitename"] == get.sitename and get.redirectpath != "":
                if (
                    conf["redirectname"] != get.redirectname
                    and conf["redirectpath"] == get.redirectpath
                ):
        ...
        if action == "create":
            ...
            if sys.version_info.major < 3:
                if len(get.redirectname) < 3 or len(get.redirectname) > 15:
                    return public.return_msg_gettext(
                        False,
                        public.lang("Database name cannot be more than 16 characters!"),
                    )
            else:
                if (
                    len(get.redirectname.encode("utf-8")) < 3
                    or len(get.redirectname.encode("utf-8")) > 15
                ):
                    return public.return_msg_gettext(
                        False,
                        public.lang("Database name cannot be more than 16 characters!"),
            ...
            if self.__CheckRedirect(get.sitename, get.redirectname, is_error_page):
                return public.return_msg_gettext(
                    False, public.lang("Specified redirect name already exists")
                )
        # 检测目标URL格式
        ...
        if "errorpage" not in get:
            ...
            if get.domainorpath == "domain":
                if not json.loads(get.redirectdomain):
                    return public.return_msg_gettext(
                        False, public.lang("Please select redirected domain")
                    )
            else:
                if not get.redirectpath:
                    return public.return_msg_gettext(
                        False, public.lang("Please enter redirected path")
                    )
                # repte = "[\\?\\=\\[\\]\\)\\(\\*\\&\\^\\%\\$\\#\\@\\!\\~\\`{\\}\\>\\<\\,\',\"]+"
                # 检测路径格式
                if "/" not in get.redirectpath:
                    return public.return_msg_gettext(
                        False,
                        public.lang("Path format is incorrect, the format is /xxx"),
                    )
            ...
            if repeatdomain:
                return public.return_msg_gettext(
                    False, "Redirected domain already exists {}", (repeatdomain,)
                )
            # 检测路径是否有存在配置文件
            repeatpath = self.__CheckRepeatPath(get)
            if repeatpath:
                return public.return_msg_gettext(
                    False, "Redirected domain already exists {}", (repeatpath,)
                )
            # 检测目标URL是否可用
                ...
                for d in json.loads(get.redirectdomain):
                    ...
                    if d == tu:
                        return public.return_msg_gettext(
                            False,
                            public.lang(
                                "Domain name {} is the same as the target domain name, please deselect it",
                                d,
                            ),
                        )
            ...
            if get.domainorpath == "path":
                ...
                for d in domains:
                    ...
                    if tu == ad:
                        return public.lang(
                            "{}, the target URL is the same as the redirected path", tu
                        )
        ...
        # 404页面重定向检测项
            ...
            if "tourl" not in get and "topath" not in get:
                return public.returnMsg(
                    False, public.lang("Please select where you need to redirect to")
                )
            # 网站首页访问检测
            if "topath" in get and get.topath == "/":
                ...
                if not self.__firsturl:
                    return public.returnMsg(
                        False,
                        public.lang(
                            "The website cannot be accessed, please check whether the website is working properly"
                        ),
                    )
    ...
    def ModifyRedirect(self, get):
        ...
        for i in range(len(redirectconf)):
            domainorpath = ""
            if "domainorpath" not in get or not get.domainorpath:
                domainorpath = "domain" if get.tourl else "path"
            if not domainorpath:
                domainorpath = get.domainorpath
            if (
                redirectconf[i]["redirectname"] == get.redirectname
                and redirectconf[i]["sitename"] == get.sitename
            ):
                redirectconf[i]["tourl"] = (
                    get.tourl if "tourl" in get and get.tourl else ""
                )
                redirectconf[i]["redirectdomain"] = (
                    ""
                    if "redirectdomain" not in get
                    else json.loads(get.redirectdomain)
                )
                redirectconf[i]["redirectpath"] = (
                    "" if "redirectpath" not in get else get.redirectpath
                )
                redirectconf[i]["redirecttype"] = (
                    "" if "redirecttype" not in get else get.redirecttype
                )
                ...
                redirectconf[i]["holdpath"] = (
                    999 if "holdpath" not in get else int(get.holdpath)
                )
                redirectconf[i]["errorpage"] = (
                    1 if "errorpage" in get and get.errorpage in [1, "1"] else 0
                )
        self.__write_config(self.__redirectfile, redirectconf)
        redirect_path = (
            get.tourl.strip() if "tourl" in get and get.tourl else get.topath.strip()
        )
        # 404页面重定向
        ...
        if "errorpage" in get and get.errorpage in [1, "1"]:
            ...
            if web_type == "nginx":
                ...
                self.get_apache_conf(
                    redirect_path,
                    get.sitename,
                    get.redirectname,
                    str(get.redirecttype),
                    is_del,
                )
            else:
                return public.returnMsg(
                    False, public.lang("web server not installed or unknown web server")
                )
        # 非404页面重定向
        else:
            self.SetRedirect(get)
    ...
    def set_error_redirect(self, get):
        ...
        redirect_path = (
            get.tourl
            if "tourl" in get and get.tourl and get.tourl.strip()
            else get.topath.strip()
        )
        redirectconf.append(
            {
                "sitename": site_name,
                ...
                "redirectpath": "",
                "topath": (
                    get.topath.strip() if "topath" in get and get.topath.strip() else ""
                ),
                "redirecttype": get.redirecttype,
                "type": int(get.type),
        ...
        if web_type == "nginx":
            ...
            self.get_nginx_conf(
                redirect_path, get.redirecttype, site_name, get.redirectname
            )
        elif web_type == "apache" or web_type == "openlitespeed":
            self.SetRedirectApache(get.sitename)
            self.get_apache_conf(
                redirect_path, site_name, get.redirectname, str(get.redirecttype)
            )
        else:
            return public.returnMsg(
                False, public.lang("web server not installed or unknown web server")
    ...
    def get_nginx_conf(
        self, redirect_path, redirecttype, site_name, redirectname, is_del=False
    ):
        """
        @name 设置nginx 404重定向
        @author hezhihong
        ...
        if os.path.isfile(file_path):
            public.ExecShell("rm -f %s" % file_path)
        if not is_del:
            public.writeFile(file_path, add_str)

    def get_apache_conf(
        self, redirect_path, site_name, redirectname="", r_type="301", is_del=False
    ):
        """
...
"""
            ...
            for w in ["nginx", "apache"]:
                ...
                redirectdir = "%s/panel/vhost/%s/redirect/%s" % (
                    self.setupPath,
                    w,
                    get.sitename,
                )
            ...
            if isError != True:
                ...
                for i in range(len(p_conf) - 1, -1, -1):
                    if (
                        get.sitename == p_conf[i]["sitename"]
                        and p_conf[i]["redirectname"]
                    ):
                        del p_conf[i]
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        '%s<br><a style="color:red;">'
                        % public.lang("Sorry, something went wrong")
                        + isError.replace("\n", "<br>")
                        + "</a>"
                    ),
                )
    ...
    def del_redirect_multiple(self, get):
        ...
        for redirectname in redirectnames:
            ...
            try:
                ...
                del_failed[redirectname] = public.lang(
        ...
        return {
            "status": True,
            "msg": public.lang(
                "Delete redirects [{}] successfully", ",".join(del_successfully)
            ),
            "error": del_failed,
    ...
    def DeleteRedirect(self, get, multiple=None):
        ...
        for i in range(len(redirectconf)):
            if (
                redirectconf[i]["sitename"] == sitename
                and redirectconf[i]["redirectname"] == redirectname
            ):
                proxyname_md5 = self.__calc_md5(redirectconf[i]["redirectname"])
                public.ExecShell(
                    "rm -f %s/panel/vhost/nginx/redirect/%s/%s_%s.conf"
                    % (
                        self.setupPath,
                        redirectconf[i]["sitename"],
                        proxyname_md5,
                        redirectconf[i]["sitename"],
                    )
                )
        ...
        for i in redirectconf:
            if i["sitename"] == sitename:
                if (
                    "errorpage" in get
                    and "errorpage" in i
                    and int(get.errorpage) != int(i["errorpage"])
                ):
                    continue
                if "errorpage" in i and i["errorpage"] in [1, "1"]:
                    i["redirectdomain"] = ["404 page"]
    ...
    def GetRedirectFile(self, get):
        ...
        get.path = "%s/panel/vhost/%s/redirect/%s/%s_%s.conf" % (
            self.setupPath,
            get.webserver,
            sitename,
            proxyname_md5,
        ...
        for i in conf:
            if (
                redirectname == i["redirectname"]
                and sitename == i["sitename"]
                and i["type"] != 1
            ):
                return public.return_msg_gettext(
                    False, public.lang("Redirection suspended")
                )
        f = files.files()
