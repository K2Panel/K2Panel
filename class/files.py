...
session.save_handler = files""".format(
        ...
        try:
            if self.get_real_len(args.f_name) > 256:
                return public.return_msg_gettext(
                    False, public.lang("The file name contains more than 256 bytes")
                )
        except:
            pass
        if not self.f_name_check(args.f_name):
            return public.return_msg_gettext(
                False,
                public.lang("No special characters can be included in the file name!"),
            )

        if args.f_path == "/":
            return public.return_msg_gettext(
                False, public.lang("Cannot upload files to the system document root!")
            )
    ...
    def GetDir(self, get: public.dict_obj):
        ...
        if not os.path.isdir(get.path):
            return public.return_msg_gettext(
                False, public.lang("This is not a directory")
            )
        ...
        if not hasattr(get, "reverse"):
            get.reverse = "False"
        if not hasattr(get, "sort"):
            get.sort = "off"
        reverse = bool(get.reverse)
        if get.reverse == "False":
            reverse = False

        info["count"], _nlist = self.files_list(
            get.path, search, my_sort=get.sort, reverse=reverse
        )
        # 改1
        ...
        for file_info in _nlist:
            ...
            try:
                ...
                r_file = (
                    self.__filename_flater(file_info["name"])
                    + ";"
                    + str(file_info["size"])
                    + ";"
                    + str(file_info["mtime"])
                    + ";"
                    + str(file_info["accept"])
                    + ";"
                    + file_info["user"]
                    + ";"
                    + file_info["link"]
                    + ";"
                    + self.get_download_id(filename)
                    + ";"
                    + self.is_composer_json(filename)
                    + ";"
                    + favorite
                    + ";"
                    + self.__check_share(filename)
                )
    ...
    def GetDirNew(self, get):
        ...
        n_count, list_data = self.files_list(
            get.path, search, my_sort=get.sort, reverse=reverse
        )
        # 包含分页类
        import page
        ...
        info = {
            ...
            "row": (
                ...
                if hasattr(get, "showRow")
                    ...
                    2000
                    if "disk" in get and get.disk == "true"
                    else 5000 if "share" in get and get.share else 500
                )
            ),
            "p": int(get.get("p", 1)),
        ...
        for file_info in list_data[page.SHIFT : page.SHIFT + page.ROW]:
            ...
            content = (
                ...
                else public.to_dict_obj(
                    {"st_size": 0, "st_mtime": 0, "st_mode": 0, "st_uid": 0}
                )
            )
            ...
            try:
                rmk = (
                    public.readFile("data/files_ps/" + public.Md5(filename))
                    if os.path.exists("data/files_ps/" + public.Md5(filename))
                    else pss.get(filename, "")
                )
            except:
                rmk = ""
            ...
            try:
                r_file = {
                    ...
                    "user": user_name,  # 用户
                    "lnk": (
                        "->" + str(os.readlink(filename))
                        if os.path.islink(filename)
                        else ""
                    ),  # 链接
                    "durl": str(self.download_token_list.get(filename, "")),  # 下载链接
                    "cmp": (
                        1 if os.path.exists(filename + "/composer.json") else 0
                    ),  # 是否包含composer.json
                    "fav": self.__check_favorite(filename, data["store"]),  # 是否为收藏
                    "rmk": rmk,  # 备注
                    "top": (
                        1 if html.unescape(filename) in top_data else 0
                    ),  # 文件或者目录是否置顶
                    "sn": file_info[0],
                }
                if os.path.isdir(filename):
    ...
    def SearchFilesNew(self, get):
        ...
        if "sort" in get and get.sort:
            sort = (
                ...
                if get.sort == "name"
                    ...
                    "sz"
                    if get.sort == "size"
                    else "mt" if get.sort == "mtime" else "nm"
                )
            )
        ...
        data["page"] = public.get_page(
            len(my_dirs) + len(my_files), 1, max, "GetFiles"
        )["page"]
        data["store"] = self.get_files_store(None)
    ...
    def _check_tamper(self, data):
        ...
        data["tamper_data"] = PluginLoader.plugin_run(
            "tamper_core", "check_dir_safe", args
        )
    ...
    def get_file_ps(self, filename):
        ...
        if filename in pss:
            return "PS：" + pss[filename]
        try:
            if not self.recycle_list:
                self.recycle_list = public.get_recycle_bin_list()
        except:
            pass
        if filename + "/" in self.recycle_list:
            "PS: Recycle Bin Directory"
        if filename in self.recycle_list:
            return "PS: Recycle Bin Directory"
        return ""
    ...
    def set_file_ps(self, args):
        ...
        if ps_body:
            ...
            public.write_log_gettext(
                "File manager", "Set the file name [{}], notes: {}", (f_name, ps_body)
            )
        else:
            if os.path.exists(f_key):
                os.remove(f_key)
                public.write_log_gettext(
                    "File manager", "Clear file notes [{}]", (f_name)
                )
    ...
    def __format_stat_old(self, filename, path):
        try:
            ...
            tmp_stat = stat.split(";")
            file_info = {
                "name": self.xssencode(tmp_stat[0].replace("/", "")),
                "size": int(tmp_stat[1]),
                "mtime": int(tmp_stat[2]),
                "accept": int(tmp_stat[3]),
                "user": tmp_stat[4],
                "link": tmp_stat[5],
            }
            return file_info
    ...
    def CreateFile(self, get):
        ...
        for data in str(df_data).split("\n"):
            ...
            if (
                int(use_size) < 1024
                and str(size).rstrip("%") == "100"
                and disk_path in ["/", "/www"]
            ):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "File creation failed! The disk is full! please clear the space first!"
                    ),
                )
        ...
        try:
            ...
            if get.path[-1] == ".":
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "It is not recommended to use [ . ] at the end of the file because there may be security risks"
                    ),
                )
            if not self.CheckFileName(get.path):
                return public.return_msg_gettext(
                    False, public.lang("File names can NOT contain special characters!")
                )
            if os.path.exists(get.path):
                return public.return_msg_gettext(
                    False, public.lang("Requested file exists!")
                )
            ...
            public.write_log_gettext(
                "File manager", "Successfully created file [{}]!", (get.path,)
            )
            return public.return_msg_gettext(
                True, public.lang("Successfully created file!")
            )
        except:
            return public.return_msg_gettext(
                False, public.lang("Failed to create file!")
            )
    ...
    def CreateLink(self, get):
        ...
        if os.path.exists(get.dfile):
            return public.return_msg_gettext(
                False, public.lang("The specified soft link file name already exists")
            )
        l_name = os.path.basename(get.dfile)
        if re.match(r"^[\w\-\.]+$", l_name) == None:
            return public.returnMsg(False, public.lang("Link file name is illegal!"))
        if get.dfile[0] != "/":
            return public.return_msg_gettext(
                False,
                public.lang(
        ...
        if not os.path.exists(get.dfile):
            return public.return_msg_gettext(
                False, public.lang("Softlink file creation failed")
            )
        public.write_log_gettext(
            "Firewall manager", "Create softlink: {} -> {}", (get.dfile, get.sfile)
        )
        return public.return_msg_gettext(
            True, public.lang("The softlink file was created successfully")
        )
        ...
        try:
            if get.path[-1] == ".":
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "It is not recommended to use [ . ] at the end of the directory, because there may be safety risks"
                    ),
                )
            if not self.CheckFileName(get.path):
                return public.return_msg_gettext(
                    False,
                    public.lang("Directory names cannot contain special characters!"),
                )
            if os.path.exists(get.path):
                return public.return_msg_gettext(
                    False, public.lang("Requested directory exists!")
                )
            os.makedirs(get.path)
            self.SetFileAccept(get.path)
            public.write_log_gettext(
                "File manager", "Successfully created directory [ {} ]!", (get.path,)
            )
            return public.return_msg_gettext(
                True, public.lang("Successfully created directory!")
            )
        except:
            return public.return_msg_gettext(
                False, public.lang("Failed to create directory!")
            )
    ...
    def DeleteDir(self, get):
        ...
        if not self.CheckDir(get.path):
            return public.return_msg_gettext(
                False,
                public.lang("Editing this directory may cause service exceptions!"),
            )
        ...
        try:
            ...
            if hasattr(get, "empty"):
                if not self.delete_empty(get.path):
                    return public.return_msg_gettext(
                        False, public.lang("Cannot delete non-empty directory!")
                    )
            ...
            if os.path.exists("data/recycle_bin.pl") and session.get("debug") != 1:
                if self.Mv_Recycle_bin(get):
                    ...
                    return public.return_msg_gettext(
                        True, public.lang("Directory moved to recycle bin!")
                    )
            ...
            public.add_security_logs("Del dir", "Delete directory: " + get.path)
            public.WriteLog(
                "TYPE_FILE", "Successfully deleted directory [{}]!", (get.path,)
            )
            self.remove_file_ps(get)
            return public.return_msg_gettext(
                True, public.lang(" Successfully deleted directory!")
            )
        except:
            return public.return_msg_gettext(
                False, public.lang("Failed to delete directory!")
            )
        ...
        try:
            if os.path.exists("data/recycle_bin.pl") and session.get("debug") != 1:
                if self.Mv_Recycle_bin(get):
                    ...
                    return public.return_msg_gettext(
                        True, public.lang("File moved to recycle bin!")
                    )
            os.remove(get.path)
            self.site_path_safe(get)
            public.write_log_gettext(
                "File manager",
                "Successfully permanent deleted file: [{}]!",
                (get.path,),
            )
            public.add_security_logs("Del file", "Delete file: " + get.path)
            self.remove_file_ps(get)
            return public.return_msg_gettext(
                True, public.lang("Successfully deleted file!")
            )
        except:
            return public.return_msg_gettext(
                False, public.lang("Failed to delete file!")
            )
    ...
    def Mv_Recycle_bin(self, get):
        ...
        rFile = os.path.join(
            rPath, get.path.replace("/", "_bt_") + "_t_" + str(time.time())
        )
        try:
            import shutil
            ...
            public.write_log_gettext(
                "File manager",
                "Successfully moved file [{}] to recycle bin!",
                (get.path,),
            )
            return True
        except:
    ...
    def Re_Recycle_bin(self, get):
        ...
        try:
            ...
            if os.path.isdir(get.path) and os.path.exists(dFile):
                shutil.move(
                    dFile, dFile + "_{}.bak".format(public.format_date("%Y%m%d%H%M%S"))
                )
            shutil.move(get.path, dFile)
            public.write_log_gettext(
                "File manager",
                "Successfully recovered [{}] from recycle bin!",
                (dFile,),
            )
            return public.return_msg_gettext(True, public.lang("Recovery succeeded!"))
        except:
            public.write_log_gettext(
                "File manager", "Failed to recover [{}] from recycle bin!", (dFile,)
            )
            return public.return_msg_gettext(False, public.lang("Recovery failed!"))
        ...
        for rPath in recycle_bin_list:
            ...
            for file in os.listdir(rPath):
                try:
                    ...
                    if tmp["dname"].find("@") != -1:
                        tmp["dname"] = "BTDB_" + tmp["dname"][5:].replace(
                            "@", "\\u"
    ...
    def Del_Recycle_bin(self, get):
        ...
        if not _ok:
            return public.returnMsg(False, "Error deleting file : {}", (tfile,))
        ...
        if not self.CheckDir(filename):
            return public.return_msg_gettext(
                False, public.lang("Never trouble troubles till troubles trouble you!")
            )
        ...
        public.write_log_gettext(
            "File manager", "Parmanently deleted {} from recycle bin!", (tfile,)
        )
        return public.return_msg_gettext(
            True, public.lang("Parmanently deleted {} from recycle bin!", tfile)
        )
    ...
    def Recycle_bin(self, get):
        ...
        if os.path.exists(c):
            ...
            return public.return_msg_gettext(
                True, public.lang("Recycle bin feature turned off!")
            )
        else:
            public.writeFile(c, "True")
            public.write_log_gettext("File manager", "Recycle bin feature turned on!")
            return public.return_msg_gettext(
                True, public.lang("Recycle bin feature turned on!")
            )
    ...
    def CopyFile(self, get):
        ...
        try:
            ...
            return public.return_msg_gettext(
                True, public.lang("Successfully copied file!")
            )
        except:
    ...
    def CopyDir(self, get):
        ...
        try:
            ...
            os.chown(get.dfile, stat.st_uid, stat.st_gid)
            public.write_log_gettext(
                "File manager", "Successfully copied directory!", (get.sfile, get.dfile)
            )
            return public.return_msg_gettext(
                True, public.lang("Successfully copied directory!")
            )
        except:
            return public.return_msg_gettext(
                False, public.lang("Failed to copy directory!")
            )
    ...
    def MvFile(self, get):
        ...
        if hasattr(get, "rename"):
            if os.path.exists(get.dfile):
                return public.return_msg_gettext(
                    False, public.lang("The target file name already exists!")
                )
        ...
        if get.dfile == get.sfile:
            return public.return_msg_gettext(
                False, public.lang("Meaningless operation")
            )
        ...
        try:
            ...
            if hasattr(get, "rename"):
                public.write_log_gettext(
                    "File manager", "[{}] renamed to [{}]", (get.sfile, get.dfile)
                )
                return public.return_msg_gettext(
                    True, public.lang("Successfully renamed!")
                )
            else:
                public.write_log_gettext(
                    "File manager", "File moved!", (get.sfile, get.dfile)
                )
    ...
    def GetFileBody(self, get):
        ...
        if get.path.find("/rewrite/null/") != -1:
            ...
            get.path = get.path.replace(
                "/rewrite/null/", "/rewrite/{}/".format(webserver)
            )
        ...
        if self.__get_ext(get.path) in [
            ...
            return public.return_msg_gettext(
                False, public.lang("The file format does not support online editing!")
            )
        ...
        if os.path.isdir(get.path):
            return public.return_msg_gettext(
                False, public.lang("Writing verification file failed: {}")
            )
        ...
        if data["size"] > 3145928:
            try:
                ...
                if info_data == "":
                    return public.return_msg_gettext(
                        False,
                        "The file encoding is not compatible, the file cannot be read correctly!",
                    )
                data["data"] = info_data
                data["only_read"] = True
                return data
                    ...
                    try:
                        ...
                        try:
                            ...
                            return public.return_msg_gettext(
                                False,
                                public.lang(
                                    "File encoding is not compatible and cannot be read correctly!"
                                ),
                            )
            else:
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "Failed to open file, file may be occupied by other processes!"
                    ),
                )
    ...
    def SaveFileBody(self, get):
        if not "path" in get:
            return public.return_msg_gettext(
                False, public.lang("[path] parameter cannot be empty!")
            )
        if sys.version_info[0] == 2:
            get.path = get.path.encode("utf-8")
        ...
        if not os.path.exists(get.path):
            ...
            return public.returnMsg(
                False, public.lang("Files larger than 3MB cannot be edited online!")
            )
            ...
            if get.data.find("#SSL-START") != -1 and get.data.find("#SSL-END") != -1:
                if get.data.find("#error_page 404/404.html;") == -1:
                    ...
                    return public.get_msg_gettext(
                        False,
                        public.lang(
                            '{}:<p style="color:red;">{}</p><p>:</p><pre>#ERROR-PAGE-START  Error page configuration, allowed to be commented</pre> {}',
                            str1,
                            str2,
                            str3,
                        ),
        ...
        if "st_mtime" in get:
            ...
            if st_mtime != get["st_mtime"]:
                return public.returnMsg(
                    False,
                    public.lang(
                        "Failed to save, {} file has been changed, please refresh the content and modify it again.",
                        get.path,
                    ),
                )
        ...
        if get.path.find(his_path) != -1:
            return public.return_msg_gettext(
                False, public.lang("Cannot modify history copy directly!")
            )
        try:
            if "base64" in get:
                import base64
            ...
            if data == "undefined":
                return public.return_msg_gettext(
                    False, public.lang("Wrong file content, please save again!")
                )
            userini = False
            if get.path.find(".user.ini") != -1:
                userini = True
            ...
            try:
                if sys.version_info[0] == 2:
                    ...
                    data = data.encode(get.encoding, errors="ignore").decode(
                        get.encoding
                    )
            ...
            public.write_log_gettext(
                "File manager", "Successfully saved file [{}]!", (get.path,)
            )
    ...
    def save_history(self, filename):
        ...
        try:
            ...
            if is_write:
                public.writeFile(
                    save_path + "/" + str(int(time.time())),
                    public.readFile(filename, "rb"),
                    "wb",
                )
        except:
            pass
    ...
    def re_history(self, args):
        ...
        if not os.path.exists(args.path):
            return public.return_msg_gettext(
                False, public.lang("The specified historical copy does not exist!")
            )
        import shutil
    ...
    def auto_save_temp(self, args):
        ...
        return public.return_msg_gettext(
            True, public.lang("Automatically saved successfully!")
        )
    ...
    def Zip(self, get):
        ...
        if get.z_type == "rar":
            if os.uname().machine != "x86_64":
                return public.return_msg_gettext(
                    False,
                    public.lang("RAR component does not support aarch 64 platform"),
                )
        ...
        for file_name in get.sfile.split(","):
            ...
            status, total_size, total_num = self.is_max_size(
                path, max_size, max_num, total_size, total_num
            )
            if not status:
                break
        ...
        if not status:
            return task_obj._zip(
                get.path, get.sfile, get.dfile, "/tmp/zip.log", get.z_type
            )
        ...
        task_obj.create_task(
            ...
            3,
            get.path,
            json.dumps({"sfile": get.sfile, "dfile": get.dfile, "z_type": get.z_type}),
        )
        public.WriteLog("TYPE_FILE", "ZIP_SUCCESS", (get.sfile, get.dfile))
        return public.returnMsg(
            True, public.lang("A compaction task has been added to the message queue!")
        )
    ...
    def UnZip(self, get):
        ...
        if not os.path.exists(get.sfile):
            return public.returnMsg(
                False, public.lang("The specified archive does not exist!")
            )
        if not os.path.exists(get.dfile):
        ...
        task_obj.create_task(
            public.get_msg_gettext("Decompress the file"),
            2,
            get.sfile,
        ...
        public.write_log_gettext(
            "File manager",
            "Successfully uncompressed file from [{}] to [{}]!",
            (get.sfile, get.dfile),
        )
        return public.return_msg_gettext(
            True, public.lang("Decompression task added to the message queue!")
        )
    ...
    def SetFileAccess(self, get, all="-R"):
        ...
        try:
            if not self.CheckDir(get.filename):
                return public.return_msg_gettext(
                    False,
                    public.lang("Never trouble troubles till troubles trouble you!"),
                )
            ...
            public.ExecShell(
                "chmod " + all + " " + get.access + " '" + get.filename + "'"
            )
            public.ExecShell(
                "chown "
                + all
            ...
            public.write_log_gettext(
                "File manager",
                "Set [{}]'s permission to [{}] and authorized user to [{}]",
    ...
    def CloseLogs(self, get):
        ...
        public.ExecShell(
            "rm -rf " + public.GetConfigValue("logs_path") + "/history_backups/*"
        )
    ...
    def SetBatchData(self, get: public.dict_obj):
        ...
        if get.type == "1" or get.type == "2":
            ...
            return public.return_msg_gettext(
                True,
                public.lang(
                    "Successfully marked, please click Paste All button in the target directory!"
                ),
            )
        elif get.type == "3":
            for key in json.loads(get.data):
                try:
                    ...
                    continue
            public.write_log_gettext(
                "File manager", "Batch setting permission successful!"
            )
            return public.return_msg_gettext(
                True, public.lang("Batch setting permission successful!")
            )
        else:
            isRecyle = (
                os.path.exists("data/recycle_bin.pl") and session.get("debug") != 1
            )
            path = get.path
            ...
            if not isRecyle:
                ...
                return public.return_msg_gettext(
                    True, public.lang("Batch deleting successful!")
                )
            else:
                public.write_log_gettext(
                    "File manager",
    ...
    def BatchPaste(self, get):
        ...
        if not "selected" in session:
            return public.return_msg_gettext(
                False,
                public.lang(
                    "The operation failed, please re-copy the copy or cut process"
                ),
            )
        i = 0
        if not "selected" in session:
            return public.return_msg_gettext(
                False, public.lang("The operation failed, please re-operate")
            )
            ...
            for key in myfiles:
                ...
                if os.path.commonpath([dfile, sfile]) == sfile:
                    return public.return_msg_gettext(
                        False,
                        public.lang(
                            "Wrong copy logic, from {} copy to {} has an inclusive relationship, there is an infinite loop copy risk!",
                            sfile,
                            dfile,
                        ),
                    )
        ...
        return public.return_msg_gettext(
            True,
            "Batch operating succeeded [{}], failed [{}]",
            (str(i), str(errorCount)),
        )
    ...
    def DownloadFile(self, get):
        ...
        task_obj = panelTask.bt_task()
        task_obj.create_task(
            public.get_msg_gettext("Download file"),
            1,
            get.url,
            get.path + "/" + get.filename,
        )
        ...
        public.write_log_gettext(
            "File manager", "Downloaded file [{}] to [{}]", (get.url, get.path)
        )
        return public.return_msg_gettext(
            True, public.lang("Download task added into the queue!")
        )
    ...
    def InstallSoft(self, get):
        ...
        execstr = (
            "cd "
            + public.GetConfigValue("setup_path")
            + "/panel/install && /bin/bash install_soft.sh "
            + get.type
            + " install "
            + get.name
            + " "
        ...
        public.writeFile(isTask, "True")
        public.write_log_gettext(
            "Installer", "Download task added to queue!", (get.name, get.version)
        )
        time.sleep(0.1)
        return public.return_msg_gettext(
            True, public.lang("Download task added to queue!")
        )
...
do
    ...
    def UninstallSoft(self, get):
        ...
        if public.get_webserver() == "openlitespeed":
            ...
            if get.name.lower() in default_ext:
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "This extension is the default extension of OLS and cannot be uninstalled"
                    ),
                )
        execstr = (
            "cd "
        ...
        public.write_log_gettext(
            "Installer", "Uninstallaton succeeded", (get.name, get.version)
        )
    ...
    def GetTaskSpeed(self, get):
        ...
        find = (
            db.Sql()
            .table("tasks")
            .where("status=? OR status=?", ("-1", "0"))
            .field("id,type,name,execstr")
            .find()
        )
        if type(find) == str:
        ...
        if find["type"] == "download":
            try:
                ...
                if len(tmp) < 10:
                    return public.return_msg_gettext(
                        False, "NO_TASK_AT_LINEUP", ("-3",)
                    )
                echoMsg["msg"] = json.loads(tmp)
                echoMsg["isDownload"] = True
            except:
                db.Sql().table("tasks").where("id=?", (find["id"],)).save(
                    "status", ("0",)
                )
    ...
    def ExecShell(self, get):
        ...
        if tmp[0] in disabled:
            return public.return_msg_gettext(
                False, "Sorry, [{}] command is NOT supported!", (tmp[0],)
            )
        shellStr = """#!/bin/bash
...
""" % (
    ...
    def install_rar(self, get):
        ...
        public.ExecShell(
            "tar xvf " + tmp_file + " -C {}".format(public.get_setup_path())
        )
    ...
    def add_files_store(self, get):
        ...
        if not os.path.exists(path):
            return public.return_msg_gettext(
                False, public.lang("File or directory does not exist!")
            )
        data = self.get_store_data()
        if path in data:
            return public.return_msg_gettext(
                False, public.lang("Do not add it repeatedly!")
            )
        data.append(path)
    ...
    def del_files_store(self, get):
        ...
        if not path in data:
            ...
            if not is_go:
                return public.return_msg_gettext(
                    False, public.lang("This favorite object could not be found!")
                )
        data.remove(path)
        if len(data) <= 0:
            data = []
    ...
    def get_download_url_find(self, get):
        ...
        if not data:
            return public.return_msg_gettext(
                False, public.lang("The specified address does not exist!")
            )
        return data
        ...
        if "password" in get:
            ...
            if len(pdata["password"]) < 4 and len(pdata["password"]) > 0:
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "The length of the extracted password cannot be less than 4 digits"
                    ),
                )
            if not re.match(r"^\w+$", pdata["password"]):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "The password only supports a combination of uppercase and lowercase letters and numbers"
                    ),
                )
    ...
    def create_download_url(self, get):
        ...
        # 更新 or 插入
        token = (
            public.M(my_table).where("filename=?", (get.filename,)).getField("token")
        )
        if token:
    ...
    def __get_php_bin(self, php_version=None):
        ...
        if not os.path.exists(php_ini):
            ...
            php_ini_body = re.sub(
                r"disable_functions\s*=.*", "disable_functions = ", php_ini_body
            )
    ...
    def exec_git(self, get):
        if get.git_action == "option":
            ...
            public.ExecShell(
                "nohup git clone {} &> /tmp/panelExec.pl &".format(get.giturl)
            )
    ...
    def get_composer_bin(self):
        ...
        download_addr = "wget -O {} {}/install/src/composer.phper -T 5".format(
            composer_bin, public.get_url()
        )
    ...
    def exec_composer(self, get):
        ...
        if not composer_bin:
            return public.return_msg_gettext(
                False, public.lang("No composer available!")
            )
        ...
        if not php_bin:
            return public.return_msg_gettext(
                False,
                public.lang(
                    "No available PHP version was found, or the specified PHP version was not installed!"
                ),
            )
        ...
        if get.composer_cmd == "":
            if not os.path.exists(get.path + "/composer.json"):
                return public.return_msg_gettext(
                    False,
                    public.lang(
                        "The composer.json configuration file was not found in the specified directory!"
                    ),
                )
        ...
        if "repo" in get:
            if get.repo != "repos.packagist":
                public.ExecShell(
                    "export COMPOSER_HOME=/tmp && {}{} {} config -g repo.packagist composer {}".format(
                        user, php_bin, composer_bin, get.repo
                    )
                )
            else:
                public.ExecShell(
                    "export COMPOSER_HOME=/tmp && {}{} {} config -g --unset repos.packagist".format(
                        user, php_bin, composer_bin
                    )
                )
        # 执行composer命令
        if not get.composer_cmd:
            composer_exec_str = "{} {} {} -vvv".format(
                php_bin, composer_bin, get.composer_args
            )
        else:
            if (
                get.composer_cmd.find("composer ") == 0
                or get.composer_cmd.find("/usr/bin/composer ") == 0
            ):
                composer_cmd = get.composer_cmd.replace("composer ", "").replace(
                    "/usr/bin/composer ", ""
                )
                composer_exec_str = "{} {} {} -vvv".format(
                    php_bin, composer_bin, composer_cmd
                )
            else:
                composer_exec_str = "{} {} {} {} -vvv".format(
                    php_bin, composer_bin, get.composer_args, get.composer_cmd
                )
        ...
        public.write_log_gettext(
            "Composer",
            "Execute composer [{}] in the directory: [{}]",
    ...
    def get_composer_version(self, get):
        ...
        try:
            ...
            if not php_bin:
                return public.return_msg_gettext(
                    False, public.lang("No available PHP version found!")
                )
            composer_exec_str = (
                ...
                + " "
                + composer_bin
                + " --version 2>/dev/null|grep 'Composer version'|awk '{print $3}'"
            )
    ...
    def update_composer(self, get):
        ...
        composer_exec_str = (
            "export COMPOSER_HOME=/tmp && {} {} self-update -vvv".format(
        ...
        if version1 == version2:
            ...
            msg = public.get_msg_gettext(
                "Upgrade composer from {} to {}", (version1, version2)
            )
    ...
    def get_file_attribute(self, args):
        ...
        attribute["md5"] = public.get_msg_gettext(
            "Do not count files or directories larger than 100MB"
        )  # 文件MD5
        attribute["sha1"] = public.get_msg_gettext(
            "Do not count files or directories larger than 100MB"
        )  # 文件sha1
        ...
        for fname in file_list:
            try:
                ...
                cache_file = os.path.join(
                    cache_path,
                    public.md5("{}_{}_{}_{}".format(filename, width, height, f_size)),
                )
                if not os.path.exists(filename):
                    # 移除缓存文件
                ...
                if args.return_type == "base64":
                    b64_data = "data:{};base64,".format(mimetype) + b64encode(
                        out.read()
                    ).decode("utf-8")
                    data[fname] = b64_data
                    out.close()
    ...
    def set_rsync_data(self, data):
        ...
        """
        public.writeFile(
            "{}/data/file_rsync.json".format(public.get_panel_path()), json.dumps(data)
        )
        return True
   ...
   first_path CHAR
    ...
    def back_path_permissions(self, get):
        ...
        if self._get_total_back() >= back_limit:
            return public.return_msg_gettext(
                False,
                public.lang(
                    "The number of backup versions has exceeded {} ,Please go to the upper right corner [Backup Permissions] to clean up the old backup before operating",
                    back_limit,
                ),
            )
        if not os.path.exists(get.path):
            return public.return_msg_gettext(
                False, public.lang("Path is incorrect {}", get.path)
            )
        path = get.path
    ...
    def _recursive_restore_file_perm(self, path, p_i):
        file_permissions = self._operate_db(
            "SELECT owner,mode from TB_NAME where pid='{}' and type='{}'".format(
                p_i[0], "first_file"
            ),
            "file",
        ).fetchall()
    ...
    def _restore_subdir_perm(self, path, date):
        path_info = self._operate_db(
            "SELECT id,owner,mode,type from TB_NAME where path='{}' and date='{}'".format(
                path, date
            ),
            "path",
        ).fetchall()
        ...
        file_permissions = self._operate_db(
            "SELECT path,owner,mode from TB_NAME where pid='{}' and date='{}'".format(
                p_i[0], date
            ),
            "file",
        ).fetchall()
    ...
    def _restore_dir_perm(self, path_full, restore_sub_dir, date):
        tb_name = self._operate_db(
            "select permissions_tb from TB_NAME where date='{}'".format(date)
        ).fetchall()
        main_dir_data = self._operate_db(
            "select path,owner,mode from TB_NAME where path='{}'".format(path_full),
            permissions_tb=tb_name[0][0],
        ).fetchall()
        if main_dir_data:
        ...
        if tb_name:
            data = self._operate_db(
                "select path,owner,mode from TB_NAME", permissions_tb=tb_name[0][0]
            ).fetchall()
            for d in data:
                if ".user.ini" in d[0]:
                    continue
        ...
        return public.return_msg_gettext(
            True, public.lang("Permission restored successfully")
        )
    ...
    def restore_single_file_perm(self, path_full, date):
        ...
        return public.return_msg_gettext(
            False, public.lang("The file does not have backup permissions")
        )
    ...
    def get_path_premissions(self, get):
        ...
        for tb_name in exist_tbs:
            data = self._operate_db(
                "select path,owner,mode,date from TB_NAME where path='{}'".format(
                    path_full
                ),
                permissions_tb=tb_name,
            ).fetchall()
            ...
            if data:
                index_data = self._operate_db(
                    "select id,remark from index_tb where permissions_tb='{}'".format(
                        tb_name
                    )
                ).fetchall()
                d_l = []
    ...
    def del_path_premissions(self, get):
        p_tb = self._operate_db(
            "select permissions_tb from index_tb where id='{}'".format(get.id)
        ).fetchall()
        # 删除引导行
    ...
    def get_all_back(self, get):
        data = self._operate_db(
            "select id,remark,date,first_path from index_tb"
        ).fetchall()
    ...
    def fix_permissions(self, get):
        ...
        if os.path.isfile(path):
            ...
            os.chmod(path, 0o644)
            return public.return_msg_gettext(
                True, public.lang("Permission repair succeeded")
            )
        os.chown(path, get.uid, get.gid)
        os.chmod(path, 0o755)
