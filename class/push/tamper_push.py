...
class tamper_push(base_push):
    ...
    def get_module_config(self, get: public.dict_obj):
        ...
        item = self.__push.format_push_data(
            push=["mail", "dingding", "weixin", "feishu", "wx_account", "tg"],
            project="tamper",
            type="",
        )
        item["cycle"] = 30
    ...
    def set_push_config(self, get: public.dict_obj):
        if self.__config is None:
            return public.returnMsg(
                False,
                public.lang(
                    'Configuration error, cannot be set, please try use the command fix: "bash /www/server/panel/plugin/tamper_core/install.sh install"'
                ),
            )
        try:
            id = int(get.id)
            if id != 0 and id not in self.__config:
                return public.returnMsg(
                    False, public.lang("No protection directory specified")
                )
        except ValueError:
            return public.returnMsg(
                False, public.lang("No protection directory specified")
        ...
        if "tamper_push" not in data:
            data["tamper_push"] = {}
        if not str(id) in data["tamper_push"]:
            if id != 0:
                public.WriteLog(
                    "Tamper-proof for Enterprise",
                    "Protected directory: {} set tampering alarm ".format(
                        self.__config[id]["path"]
                    ),
                )
            else:
                public.WriteLog(
                    "Tamper-proof for Enterprise",
                    "All directory is set, tampering alarm ",
                )
        else:
            if id != 0:
                public.WriteLog(
                    "Tamper-proof for Enterprise",
                    "Protected directory: {} Changed alert config".format(
                        self.__config[id]["path"]
                    ),
                )
            else:
                public.WriteLog(
                    "Tamper-proof for Enterprise", " All directory alarm config changed"
                )
    ...
    def get_push_data(self, data, total):
        ...
        @data dict 推送数据
            ...
            keys:检测键值
        """
        if not self._log_check(data["id"], data["cycle"], data["count"]):
            return None
        if data["push_count"] <= self._get_today_push_count(data["id"]):
            return None
        result = {
            "index": time.time(),
        }
        tamper_path = (
            "All protected directory "
            if not int(data["id"])
            else "Protected Directory:{}".format(self.__config[int(data["id"])]["path"])
        )
        for m_module in data["module"].split(","):
            if m_module == "sms":
                continue
            s_list = [
                ">Notification: Tamper-proof for Enterprise alarm",
                ">Alarm content: <font color=#ff0000>Last {} minutes, {} has been tampered with more than {} times, and it has been successfully intercepted, please pay attention to the website situation , and deal with it in timely manner.</font> ".format(
                    data["cycle"], tamper_path, data["count"]
                ),
            ]
    ...
    def _log_check(self, id, cycle, count):
        ...
        tday, yday = datetime.now().strftime("%Y-%m-%d"), (
    ...
    def _get_today_push_count(self, id):
        ...
        today_tip = "{}/data/push/tips/tamper_today.json".format(
