...
class panel_push:
    ...
    def get_module_config(self, get):
        ...
        item = self.__push.format_push_data(
            push=["mail", "dingding", "weixin", "feishu"], project="ssl", type="ssl"
        )
        item["cycle"] = 30
        item["title"] = "SSL到期提醒"
        item["helps"] = [
            "SSL到期提醒一天只发送一次",
            "证书夹内【剩余天数不足30天】的所有证书.",
        ]
        data.append(item)

        item = self.__push.format_push_data(
            push=["mail", "dingding", "weixin", "feishu"],
            project="endtime",
            type="endtime",
        )
        item["cycle"] = 5
        ...
        for x in services:
            ...
            if x == "other":
                ...
                if self.get_server_status(ser_name) == -1:
                    continue
            ...
            item["helps"] = [
                "部分服务停止可能会造成业务中断.",
                '短信推送需提前购买，如需购买请联系[<a href="http://q.url.cn/CDfQPS?_type=wpa&amp;qidian=true" target="_blank" class="btlink">宝塔客服</a>]',
            ]
            data.append(item)
        import json
    ...
    def get_push_data(self, data, total):
        if data["type"] == "services":
            ...
            if softs["pro"] == 0:
                return public.returnMsg(False, public.lang("永久专业版，跳过."))
            ...
            if m_module in ["dingding", "weixin", "feishu", "mail"]:
                ...
                result[m_module]["msg"] = "".join(
                    (
                        ...
                        ">到期时间："
                        + public.format_date(times=pro_data["endtime"])
                        + newline,
    ...
    def __get_ssl_result(self, data, clist):
        ...
        for m_module in data["module"].split(","):
            ...
            if m_module in ["dingding", "weixin", "feishu", "mail"]:
                ...
                for x in clist:
                    p_msg += (
                        "  到期：{}  域名：{}".format(x["notAfter"], x["subject"])
                        + newline
                    )
    ...
    def __get_service_result(self, data):
        ...
        if s_idx < data["index"] + data["interval"]:
            return public.returnMsg(
                False, public.lang("The interval is not reached, skip.")
            )
