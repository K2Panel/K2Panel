...
class site_push:
    ...
    def get_push_cycle(self, data):
        ...
        for skey in data:
            ...
            if m_type in ["endtime", "ssl", "site_endtime"]:
                m_cycle.append(
                    "1 time per day when {} days remain".format(data[skey]["cycle"])
                )
    ...
    def set_push_config(self, get):
        ...
        if pdata["type"] in ["ssl"]:
            for x in data[module]:
                item = data[module][x]
                if (
                    item["type"] == pdata["type"]
                    and item["project"] == pdata["project"]
                ):
                    is_create = False
                    data[module][x] = pdata
            ...
            if len(p_module) > 1:
                return public.returnMsg(
                    False,
                    public.lang("The panel login alarm only supports one alarm mode."),
                )
            ...
            if not pdata["status"]:
                return public.returnMsg(
                    False,
                    public.lang(
                        "It does not support suspending the panel login alarm, if you need to suspend, please delete it directly."
                    ),
                )
            ...
            if len(p_module) > 1:
                return public.returnMsg(
                    False, public.lang("SSH login alarm only supports one alarm mode.")
                )
            ...
            if not pdata["status"]:
                return public.returnMsg(
                    False,
                    public.lang(
                        "It does not support suspending the SSH login alarm. If you need to suspend, please delete it directly."
                    ),
                )
    ...
    def get_site_ssl_info(self, webType, siteName, project_type=""):
        ...
        if webType in ["nginx", "apache"]:
            ...
            if public.get_os("windows"):
                ...
                ssl_file = "{}/{}/conf/ssl/{}/fullchain.pem".format(
                    path, webType, siteName
                )
            else:
                conf_file = "{}/vhost/{}/{}{}.conf".format(
                    public.get_panel_path(), webType, project_type, siteName
                )
                ssl_file = "{}/vhost/cert/{}/fullchain.pem".format(
                    public.get_panel_path(), siteName
                )
            ...
            if (
                conf.find("SSLCertificateFile") >= 0
                or conf.find("ssl_certificate") >= 0
            ):
    ...
    def get_ssl_push_data(self, data):
        ...
        if time.time() < data["index"] + 86400:
            return public.returnMsg(
                False, public.lang("SSL is pushed once a day, skipped.")
            )
        ...
        if data["project"] == "all":
            ...
            if not find:
                return public.returnMsg(False, public.lang("no site available."))
    ...
    def get_panel_update_data(self, data):
        ...
        try:
            ...
            if not res:
                return public.returnMsg(
                    False, public.lang("Failed to get update information.")
                )
        except:
            pass
        ...
        if not old_ver:
            ...
            if old_ver == n_ver:
                ...
                data["tips_list"] = []
                try:
                    tips_path = "{}/data/push/tips/{}".format(
                        public.get_panel_path(), data["id"]
                    )
                    os.remove(tips_path)
        ...
        if public.version() != n_ver:
            for m_module in data["module"].split(","):
                ...
                s_list = [
                    "> Notification Type: Panel Version Update",
                    ">current version:{} ".format(public.version()),
                    ">The latest version of:{}".format(n_ver),
                ]
    ...
    def get_panel_safe_push(self, data, result):
        s_list = []
        # 面板登录用户安全
        t_add, t_del, total = self.get_records_calc(
            "login_user_safe", public.M("users")
        )
        if t_add > 0 or t_del > 0:
            s_list.append(
                ">Login user change:<font color=#ff0000> total {}, add {}, delete {}</font>.".format(
                    total, t_add, t_del
                )
            )
        ...
        # 面板日志发生删除
        t_add, t_del, total = self.get_records_calc(
            "panel_logs_safe", public.M("logs"), 1
        )
        if t_del > 0:
            s_list.append(
                ">The panel log is deleted, the number of deleted items:<font color=#ff0000>{} </font>".format(
                    t_del
                )
            )
        ...
        if not tmp:
            ...
            if str(debug_status) != tmp:
                s_list.append(
                    ">Panel developer mode changed, current status:{}".format(debug_str)
                )
                public.set_cache_func(skey, debug_status)
        ...
        if find:
            ...
            if not tmp:
                ...
                if user_str != tmp:
                    s_list.append(">K2Panel login account or password changed")
                    public.set_cache_func(skey, user_str)

        if len(s_list) > 0:
            sdata = public.get_push_info("K2Panel security warning", s_list)
            for m_module in data["module"].split(","):
                if m_module == "sms":
                    continue
                result[m_module] = sdata
    ...
    def get_push_data(self, data, total):
        ...
        if data["type"] in ["ssl"]:
            ...
            mEdate = public.format_date(
                format="%Y-%m-%d", times=stime + 86400 * int(data["cycle"])
            )
            web_list = (
                public.M("sites")
                .where(
                    "edate>? AND edate<? AND (status=? OR status=?)",
                    ("0000-00-00", mEdate, 1, "Running"),
                )
                .field("id,name,edate")
                .select()
            )
            ...
            if len(web_list) > 0:
                for m_module in data["module"].split(","):
                    ...
                    s_list = [
                        ">Expiring:<font color=#ff0000>{} website</font>".format(
                            len(web_list)
                        )
                    ]
                    for x in web_list:
                        if x["name"] in data["tips_list"]:
                            continue
                        result["push_keys"].append(x["name"])

                        s_list.append(
                            ">Website: {} Expires: {}".format(x["name"], x["edate"])
                        )

                    sdata = public.get_push_info(
                        "K2Panel Website Expiration Reminder", s_list
                    )
                    result[m_module] = sdata
                return result
            ...
            if res["expire"] > 0 and res["expire_day"] < data["cycle"]:
                for m_module in data["module"].split(","):
                    ...
                    s_list = [
                        ">Alarm type: Login password is about to expire",
                        ">Days remaining: <font color=#ff0000>{} days</font>".format(
                            res["expire_day"]
                        ),
                    ]
                    sdata = public.get_push_info(
                        "K2Panel password expiration reminder", s_list
                    )
                    result[m_module] = sdata
                return result
            ...
            if not old_str:
                ...
                if user_str != old_str:
                    ...
                    for m_module in data["module"].split(","):
                        ...
                        s_list = [
                            ">Alarm type: panel binding account change",
                            ">Currently bound account:{}****{}".format(
                                uinfo["username"][:3], uinfo["username"][-4:]
                            ),
                        ]
                        sdata = public.get_push_info(
                            "Panel binding account change reminder", s_list
                        )
                        result[m_module] = sdata
            ...
            for m_module in data["module"].split(","):
                ...
                s_list = [
                    ">alert type:Trojan detects alarms",
                    ">Content of notification: <font color=#ff0000> Found suspected Trojan files {}</font>".format(
                        len(f_list)
                    ),
                    ">listed files:[{}]".format("、".join(f_list)),
                ]
                sdata = public.get_push_info("K2Panel trojan detects alarms", s_list)
                result[m_module] = sdata
            return result
        ...
        # 登录失败次数
            ...
            if type(res) == list:
                ...
                if (
                    public.to_date(times=last_info["time"])
                    >= time.time() - data["cycle"] * 60
                ):
                    for m_module in data["module"].split(","):
                        if m_module == "sms":
                            continue
                        ...
                        s_list = [
                            ">Notification type: SSH login failure alarm",
                            ">Alarm content: <font color=#ff0000>login failed more than {} times within {} minutes</font> ".format(
            ...
            if status > 0:
                ...
                return public.returnMsg(
                    False, public.lang("service not installed，Skip.")
                )
    ...
    def __get_ssl_result(self, data, clist, push_keys=[]):
        ...
        if len(clist) == 0:
            return public.returnMsg(
                False, public.lang("Expired certificate not found, skipping.")
            )
        ...
        for m_module in data["module"].split(","):
            if m_module in self.__push_model:
                ...
                if m_module in ["sms"]:
                    ...
                    sdata["sm_type"] = "ssl_end|K2Panel SSL Expiration Reminder"
                    ...
                    s_list = [
                        ">Expiring soon: <font color=#ff0000>{} copies</font>".format(
                            len(clist)
                        )
                    ]
                    for x in clist:
                        s_list.append(
                            ">Website: {} Expires: {}".format(
                                x["siteName"], x["notAfter"]
                            )
                        )

                    sdata = public.get_push_info(
                        "K2Panel SSL expiration reminder", s_list
                    )
    ...
    def __get_service_result(self, data):
        ...
        for m_module in data["module"].split(","):
            ...
            if m_module in ["dingding", "weixin", "mail", "wx_account", "feishu"]:
                ...
                result[m_module]["sm_args"] = {
                    "name": "{}".format(public.GetConfigValue("title")),
                    "product": data["project"],
                    "product1": data["project"],
                }
