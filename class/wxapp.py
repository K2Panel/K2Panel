...
class wxapp:
    ...
    def is_scan_ok(self, get):
        if os.path.exists(self.app_path + "app_login_check.pl"):
            try:
                key, init_time, tid, status = public.readFile(
                    self.app_path + "app_login_check.pl"
                ).split(":")
                if time.time() - float(init_time) > 60:
                    return public.returnMsg(False, public.lang("QR code expired"))
                session_id = public.get_session_id()
                if cache.get(session_id) == public.md5(
                    uuid.UUID(int=uuid.getnode()).hex
                ):
                    return public.returnMsg(
                        True, public.lang("Scan QRCORE successfully")
                    )
            except:
    ...
    def login_qrcode(self, get):
        ...
        qrcode_str = (
            "https://app.bt.cn/app.html?&panel_url="
            + public.getPanelAddr()
            + "&v="
            + public.GetRandomString(3)
            + "?login&tid="
            + tid
        )
    ...
    def check_app_login(self, get):
        ...
        if not btapp_info:
            return public.returnMsg(False, public.lang("Unbound!"))
        if not btapp_info["open"]:
            return public.returnMsg(False, public.lang("API is not turned on"))
        if not "apps" in btapp_info:
            return public.returnMsg(False, public.lang("Unbound phone"))
        ...
        try:
            ...
            if not os.path.exists(self.app_path + "app_login_check.pl"):
                return public.returnMsg(
                    False, public.lang("Waiting for APP scan code login 1")
                )
            ...
            if len(session_id) != 64:
                return public.returnMsg(
                    False, public.lang("Waiting for APP scan code login 2")
                )
            ...
            if time.time() - float(init_time) > 60:
                return public.returnMsg(
                    False, public.lang("Waiting for APP scan code login")
                )
            import uuid

            if status != uuid.UUID(int=uuid.getnode()).hex[-12:]:
                return public.returnMsg(False, public.lang("当前二维码失效222"))
            cache.delete(session_id)
            cache.delete(tid)
