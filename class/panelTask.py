...
# K2Panel
# -------------------------------------------------------------------
# Copyright (c) 2019-2099 K2Panel(binarjoinanalyticnl.nl) All rights reserved.
# -------------------------------------------------------------------
...
class bt_task:
  ...
  `addtime`			INTEGER
    ...
    def get_task_list(self, status=-3):
        ...
        data = sql.field(
            "id,name,type,shell,other,status,exectime,endtime,addtime"
        ).select()
        return data
    ...
    def get_task_lists(self, get):
        ...
        data = (
            sql.field("id,name,type,shell,other,status,exectime,endtime,addtime")
            .order("id asc")
            .limit("10")
            .select()
        )
    ...
    def create_task(self, task_name, task_type, task_shell, other=""):
        self.clean_log()
        task_id = public.M(self.__table).add(
            "name,type,shell,other,addtime,status",
    ...
    def get_task_find(self, id):
        data = (
            public.M(self.__table)
            .where("id=?", (id,))
            .field("id,name,type,shell,other,status,exectime,endtime,addtime")
            .find()
        )
        return data
    ...
    def execute_task(self, id, task_type, task_shell, other=""):
        ...
        if task_type == 0:  # 执行命令
            ...
            public.ExecShell(
                "wget -O '{}' '{}' --no-check-certificate -T 30 -t 5 -d &> {}".format(
                    other, task_shell, log_file
                )
            )
    ...
    def get_task_log(self, id, task_type, num=5):
        ...
        if not os.path.exists(log_file):
            ...
            if task_type == "1":
                data = {
                    "name": public.lang("Download file"),
                    "total": 0,
                    "used": 0,
                    "pre": 0,
                    "speed": 0,
        ...
        if task_type == "1":
            ...
            if not speed_total:
                data = {
                    "name": public.lang("Download file {}", filename),
                    "total": 0,
                    "used": 0,
                    "pre": 0,
                    "speed": 0,
    ...
    def _zip(self, path, sfile, dfile, log_file, z_type="tar.gz"):
        ...
        if z_type == "zip":
            ...
            if not _7z_bin:
                ...
                public.WriteLog(
                    "File manager",
                    "Failed to compress file, reason: {}, file: {}".format(
                        err_msg, sfile
                    ),
                )
            ...
            public.ExecShell(
                "cd {} && {} a -t7z {} {} -y &> {}".format(
                    path, _7z_bin, dfile, sfiles, log_file
                )
            )
        else:
            return public.return_msg_gettext(
                False, public.lang("Specified compression format is not supported!")
            )
        ...
        public.write_log_gettext(
            "File manager", "Compressed file [ {} ] to [ {} ] success", (sfiles, dfile)
        )
    ...
    def _unzip(self, sfile, dfile, password, log_file):
        ...
        if sfile[-4:] == ".zip":
            ...
            public.ExecShell(
                "tar zxvf '" + sfile + "' -C '" + dfile + "' &> " + log_file
            )
        elif sfile[-4:] == ".rar":
            ...
            public.ExecShell(
                "tar jxvf '" + sfile + "' -C '" + dfile + "' &> " + log_file
            )
        elif sfile[-3:] == ".7z":
            ...
            public.ExecShell(
                '{} x "{}" -o"{}" -y {} &> {}'.format(
                    _7zbin, sfile, dfile, pass_opt, log_file
                )
            )
        else:
        ...
        if log_msg:
            if (
                log_msg.find("incorrect password") != -1
                ...
                public.WriteLog(
                    "File manager",
                    "Unzip file failed, reason: {}, file: {}".format(err_msg, sfile),
                )
                ...
                public.ExecShell(
                    "tar xvf '" + sfile + "' -C '" + dfile + "' &> " + log_file
                )
        ...
        public.write_log_gettext(
            "File manager", "unzip file [ {} ] -> [ {} ] success", (sfile, dfile)
        )
    ...
    def backup_site(self, id, log_file):
        ...
        backupPath = (
            public.M("config").where("id=?", (1,)).getField("backup_path") + "/site"
        )
        ...
        public.WriteLog(
            "TYPE_SITE", "SITE_BACKUP_SUCCESS", (find["name"],), not_web=self.not_web
        )
    ...
    def backup_database(self, id, log_file):
        ...
        public.ExecShell(
            '/www/server/mysql/bin/mysqldump --force --opt "'
            + name
            + '" | gzip > '
            + backupName
        )
        ...
        public.WriteLog(
            "TYPE_DATABASE", "DATABASE_BACKUP_SUCCESS", (name,), not_web=self.not_web
        )
        return public.return_msg_gettext(True, public.lang("Backup Succeeded!"))
    ...
    def input_database(self, id, file, log_file):
        ...
        if ext != "sql":
            ...
            if isgzip:
                public.ExecShell(
                    "cd " + backupPath + " && gzip " + file.split("/")[-1][:-3]
                )
            else:
