...
class nginx:
    ...
    def GetNginxValue(self):
        ...
        for i in [[ngconfcontent, self.nginxconf], [proxycontent, self.proxyfile]]:
            if not i[0]:
                return public.return_msg_gettext(
                    False, public.lang("Can not find nginx config file [ {} ]", i[1])
                )
        unitrep = "[kmgKMG]"
        conflist = []
        ps = [
            "%s,%s"
            % (public.lang("Worker processes"), public.lang("Auto means automatic")),
        ...
        for i in gets:
            ...
            if not k:
                return public.return_msg_gettext(
                    False, public.lang("Get key {} False", k)
                )
            k = k.group(1)
            v = re.search(rep, ngconfcontent)
            if not v:
                return public.return_msg_gettext(
                    False, public.lang("Get value {} False", v)
                )
            v = v.group(2)
        ...
        for c in conflist:
            ...
            if c["name"] == "worker_processes" or c["name"] == "gzip":
                if not re.search(r"auto|on|off|\d+", c["value"]):
                    return public.return_msg_gettext(
                        False, public.lang("Parameter ERROR! -1")
                    )
            else:
                if not re.search(r"\d+", c["value"]):
                    return public.return_msg_gettext(
                        False, public.lang("Parameter ERROR! -2")
                    )
...
""".format(
            ...
            if not conf:
                return public.return_msg_gettext(
                    False, public.lang("Nginx configuration file does not exist!")
                )
            reg = r"http(\n|\s)+{"
        ...
        try:
            ...
            for site in all_site:
                ...
                if not access_log:
                    continue
                access_log = (
                    "access_log " + access_log + " " + args.log_format_name + ";"
                )
    ...
    def _del_format_log_of_website(self, log_format_name):
        ...
        try:
            for s in site_format_log_status.keys():
                ...
                format_exist_reg = r"access_log\s+/www.*\.log\s+{};".format(
                    log_format_name
                )
                conf = public.readFile(website_conf_file)
                if not conf:
