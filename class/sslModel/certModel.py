...
class main(sslBase):
    ...
    def check_and_add_ps_column(self):
        try:
            ...
            if "no such column: group_id" in str(e):
                try:
                    public.M("ssl_info").execute(
                        "ALTER TABLE 'ssl_info' ADD 'group_id' INTEGER NOT NULL DEFAULT 0",
                        (),
                    )
                except Exception as e:
                    pass
        ...
        try:
            ...
            if "no such column: group_id" in str(e):
                try:
                    public.M("ssl_info").execute(
                        "ALTER TABLE 'ssl_info' ADD 'ps' INTEGER NOT NULL DEFAULT ''",
                        (),
                    )
                except Exception as e:
                    pass
    ...
    def get_cert_to_site(self, pure=False):
        ...
        for cert in site_data:
            if cert["project_type"] in ["PHP", "proxy", "WP", "WP2"]:
                ...
                if not SSLManger._get_site_ssl_info(
                    cert["name"], cert["project_type"].lower()
                ):
                    continue
    ...
    def get_cert_list(self, get):
        ...
        if cert_type in ("4", "0"):
            ...
            if status_id != 0:
                cert_data = (
                    public.M("ssl_info")
                    .field("id,hash,dns,cloud_id,not_after,auth_info,info,ps,group_id")
                    .select()
                )
            ...
            for cert in cert_data:
                ...
                for f_str in format_time_strs:
                    try:
                        end_time = int(
                            (
                                datetime.strptime(cert["not_after"], f_str).timestamp()
                                - today_time
                            )
                            / (60 * 60 * 24)
                        )
                ...
                cert["group_name"] = (
                    group_name
                    if group_name
                    else group_name_dic["3"].get(str(cert["id"]), "默认分组")
                )
                ...
                if (
                    search.lower() not in domainName.lower()
                    and search.lower() not in cert["title"].lower()
                ):
                    continue
        ...
        if cert_type in ("1", "0"):
            ...
            for cert in _cert_data:
                ...
                if 0 < cert["endDay"] <= 30:
                    will_num += 1
                elif cert["endDay"] <= 0 and cert["orderStatus"] in (
                    "COMPLETE",
                    "EXPIRED",
                ):
                    end_num += 1
                if status_id == 0 and cert["orderStatus"] not in ("PENDING", ""):
                    continue
                elif status_id == 1 and (
                    cert["endDay"] <= 0 or cert["orderStatus"] != "COMPLETE"
                ):
                    continue
                elif status_id == 2 and (
                    (cert["endDay"] > 30 or cert["endDay"] <= 0)
                    or cert["orderStatus"] != "COMPLETE"
                ):
                    continue
                elif status_id == 3 and (
                    cert["endDay"] > 0
                    or cert["orderStatus"] not in ("COMPLETE", "EXPIRED")
                ):
                    continue
                ...
                if cert["orderStatus"] in ("COMPLETE", "EXPIRED"):
                    ...
                    if title:
                        ...
                        if certInfo["certificate"] and certInfo["caCertificate"]:
                            _info = ssl_info.ssl_info().load_ssl_info_by_data(
                ...
                cert["group_name"] = (
                    group_name
                    if group_name
                    else group_name_dic["1"].get(str(cert["id"]), "默认分组")
                )
                domainName = ",".join(cert["domainName"] or [])
                if (
                    search.lower() not in domainName.lower()
                ...
                if cert["orderStatus"] == "":
                    ...
                    cert["sort"] = 99999
                cert["download_status"] = (
                    True
                    if cert["orderStatus"] == "COMPLETE" and cert["status"] == 1
                    else False
                )
                data.append(cert)
        ...
        if cert_type in ("2", "0"):
            ...
            for cert in test_cert_data.get("data", []):
                ...
                if status_id == 0 and cert["stateCode"] != "WF_DOMAIN_APPROVAL":
                    ...
                    cert["endDay"] <= 0 or cert["stateCode"] != "COMPLETED"
                ):
                    continue
                elif status_id == 2 and (
                    (cert["endDay"] > 30 or cert["endDay"] <= 0)
                    or cert["stateCode"] != "COMPLETED"
                ):
                    continue
                elif status_id == 3 and (
                    cert["endDay"] > 0 or cert["stateCode"] != "COMPLETED"
                ...
                cert["group_name"] = (
                    group_name
                    if group_name
                    else group_name_dic["2"].get(str(cert["id"]), "默认分组")
                )
                cert["title"] = "TrustAsia RSA DV TLS CA G2"
                if (
                    search.lower() not in cert["authDomain"].lower()
                    and search.lower() not in cert["title"].lower()
                ):
                    continue
        ...
        if cert_type in ("3", "0"):
            ...
            for cert in let_cert_data:
                ...
                # 状态
                if status_id == 0 and (
                    cert["status"] != "pending" or cert["endDay"] <= 0
                ):
                    continue
                elif status_id == 1 and (
                    cert["endDay"] <= 0 or cert["status"] == "pending"
                ):
                    continue
                elif status_id == 2 and (cert["endDay"] > 30 or cert["endDay"] <= 0):
                    continue
                ...
                cert["group_name"] = (
                    group_name
                    if group_name
                    else group_name_dic["3"].get(str(cert["index"]), "默认分组")
                )
                    ...
                    if cert["index"] in crontab["sBody"]:
                        ...
                        try:
                            renew_data = json.loads(
                                public.readFile(
                                    "{}/config/letsencrypt_auto_renew.json".format(
                            ...
                            renew_data = {}
                        cert["crontab_data"] = renew_data.get(
                            str(cert["index"]), {"status": 1, "error_msg": ""}
                        )
                        break
    ...
    def get_cert_list_to_push(self, get):
        items = [
            {
                "title": "{} | {}".format(
                    i["title"], ",".join(i.get("domainName", []) or "无")
                ),
                "value": i["ssl_id"],
            }
            for i in self.get_cert_list(public.to_dict_obj({"status_id": 1}))["data"]
            if i.get("endDay")
        ]
        items.insert(0, {"title": "所有证书", "value": "all"})
        return items
    ...
    def remove_cloud_cert(self, get):
        ...
        try:
            ...
            if get.get("oid"):
                ...
                finish_list = panelSSL.panelSSL().batch_soft_release(
                    public.to_dict_obj({"oid": get.oid})
                )["finish_list"]
            import acme_v2

            _return = acme_v2.acme_v2()._delete_order(
                public.to_dict_obj(
                    {
                        "index": index,
                        "ssl_hash": ssl_hash,
        ...
        for cert_path in os.listdir(path):
            ...
            if cert_hash == ssl_hash:
                if cert_path in no_site_list:
                    panelSite.panelSite().CloseSSLConf(
                        public.to_dict_obj({"siteName": cert_path})
                    )
    ...
    def remove_cert(
        ...
        if user_info is None:
            raise ValueError(
                public.lang(
                    "The panel is not logged in, so cloud upload is not possible!"
                )
            )
            ...
            if no_site_list and not force:
                raise ValueError(
                    public.lang(
                        ...
                        "with other certificates before deletion.".format(
                            ",".join(no_site_list)
                        )
                    )
                )
        ...
        if target["cloud_id"] != -1 and cloud:
            url = (
                "https://wafapi2.binarjoinanalyticnl.nl/api/Cert_cloud_deploy/del_cert"
            )
            try:
                res_text = public.httpPost(
                    url,
                    {
                ...
                raise ValueError(
                    public.lang(
                        "Failed to connect to the cloud. Unable to delete cloud data."
                    )
                )

            public.M("ssl_info").where("id = ?", (target["id"],)).update(
                {"cloud_id": -1}
            )
    ...
    def upload_cert(self, ssl_id=None, ssl_hash=None):
        ...
        if key is None or iv is None:
            raise ValueError(
                False,
                public.lang(
                    "The panel is not logged in, and upload to the cloud is not possible!"
                ),
            )
        ...
        if not target:
            raise ValueError(
                public.lang("There is no specified certificate information.")
            )
        ...
        # 对接云端
        url = (
            "https://wafapi2.binarjoinanalyticnl.nl/api/Cert_cloud_deploy/cloud_deploy"
        )
    ...
    def find_ssl_info(ssl_id=None, ssl_hash=None):
        ...
        target["endtime"] = int(
            (
                datetime.strptime(target["not_after"], "%Y-%m-%d").timestamp()
    ...
    def GetCert(self, get):
        ...
        vpath = os.path.join(
            "/www/server/panel/vhost/ssl", get.certName.replace("*.", "")
        )
        if not os.path.exists(vpath):
            return public.returnMsg(
                False, public.lang("The certificate does not exist!")
            )
        data = {}
    ...
    def SetCertToSite(self, get):
        ...
        try:
            ...
            public.ExecShell(
                "rm -f /etc/letsencrypt/renewal/" + get.siteName + "-00*.conf"
            )
    ...
    def SetBatchCertToSite(self, get):
        ...
        if isinstance(ssl_list, list):
            ...
            for Info in ssl_list:
                ...
                get.siteName = set_result["siteName"] = str(
                    Info["siteName"]
                )  # 站点名称必定为字符串
                get.isBatch = True
                if "ssl_hash" in Info:
            ...
            if hasattr(get, "set_https_mode") and get.set_https_mode.strip() in (
                True,
                1,
                "1",
                "true",
    ...
    def dump_pkcs12(self, key_pem=None, cert_pem=None, ca_pem=None, friendly_name=None):
        ...
        try:
            ...
            result = ssl_info.ssl_info().dump_pkcs12_new(
                key_pem, cert_pem, ca_pem, friendly_name
            )
        return result
    ...
    def download_cert(self, get):
        ...
        if not target:
            return public.returnMsg(
                False, public.lang("There is no certificate information.")
            )
        ...
        domain_cert = (
            csr.split("-----END CERTIFICATE-----")[0] + "-----END CERTIFICATE-----\n"
        )
    ...
    def batch_download_cert(self, get):
            ...
            for index in index_list:
                try:
                    ...
                    finish_list.append(
                        {
                            "status": False,
                            "cert": {"info": json.dumps({"issuer": "let's Encrypt"})},
                        }
                    )
        ...
        if "oid" in get and get.oid.strip():
            ...
            for oid in oid_list:
                ...
                try:
                    result = panelSSL.panelSSL().download_cert(
                        public.to_dict_obj({"oid": oid})
                    )
                    if (
                        not result
                        or not result.get("data")
                        or not result.get("filename")
                    ):
                        finish_list.append(
                            {
                                "status": False,
                                "cert": {"info": json.dumps({"issuer": oid})},
                            }
                        )
                        continue
                    with open(
                        rpath + "/{}_{}".format(oid, result["filename"]), "wb"
                    ) as f:
                        f.write(base64.b64decode(result["data"]))
                    finish_list.append(
                        {"status": True, "cert": {"info": json.dumps({"issuer": oid})}}
                    )
                except:
                    finish_list.append(
                        {"status": False, "cert": {"info": json.dumps({"issuer": oid})}}
                    )
    ...
    def parse_certificate(self, get):
        ...
        try:
            ...
            try:
                # 提取品牌（证书颁发对象）
                brand = cert.issuer.get_attributes_for_oid(x509.NameOID.COMMON_NAME)[
                    0
                ].value
                # 提取 CA 名称
                ca_name = cert.issuer.get_attributes_for_oid(
    ...
    def save_cert(self, get):
        ...
        if os.path.exists(path):
            return {
                "status": True,
                "msg": public.lang("The certificate already exists."),
                "creat_order": create_order,
                "ssl_hash": hash_data,
            }
        ...
        return {
            "status": True,
            "msg": public.lang("Save successfully"),
            "creat_order": create_order,
            "ssl_hash": hash_data,
        }
    ...
    def create_order(self, get):
        try:
            ...
            if not ssl_info:
                return public.returnMsg(
                    False, public.lang("Certificate information not found")
                )
            if ssl_info.get("issuer_O") != "Let's Encrypt":
                return public.returnMsg(
                    False,
                    public.lang("The certificate is not issued by Let's Encrypt."),
                )
            ...
            order_data = {
                ...
                "index": index,
                "cert_timeout": datetime.strptime(
                    ssl_info.get("notAfter"), "%Y-%m-%d"
                ).timestamp(),
                "renew_time": 0,
                "retry_count": 0,
                "next_retry_time": 0,
            ...
            pem = public.readFile(
                "vhost/ssl_saved/{}/fullchain.pem".format(get.ssl_hash)
            )
            acm_obj.set_exclude_hash(index, pem)
            return {
                "status": True,
                "msg": public.lang("Create successful!"),
                "index": index,
            }
        except:
    ...
    def update_cert_from_cloud(self, get):
        ...
        if key is None or iv is None:
            return public.returnMsg(
                False,
                public.lang(
                    "The panel is not logged in, "
                    "so it's impossible to connect to the cloud!"
                ),
            )
        # 获取本地证书
        local_cert = public.M("ssl_info").field("hash").select()
        ...
        # 对接云端
        url = (
            "https://wafapi2.binarjoinanalyticnl.nl/api/Cert_cloud_deploy/get_cert_list"
        )
        try:
            res_text = public.httpPost(
                url,
            ...
            if res_data["status"] is False:
                return public.returnMsg(
                    False, public.lang("Failed to obtain cloud data")
                )
        ...
        for data in res_list:
            try:
                ...
                if hash_data in hash_list:
                    # 删除本地证书
                    public.ExecShell(
                        "rm -rf /www/server/panel/vhost/ssl_saved/" + hash_data
                    )
                self.save_cert(get)
                public.M("ssl_info").where("hash = ?", (hash_data,)).update(
                    {"cloud_id": cloud_id}
                )
                x += 1
            except Exception as e:
                pass
        return public.returnMsg(
            True, "{} certificates have been synchronized.".format(str(x))
        )
    ...
    def get_report_task(self):
        ...
        if report_data:
            report_data_dic = {
                ...
                if i["source"] == "cert_endtime" and i["status"]
            }
    ...
    def create_report_task(self, get):
        ...
        if not get.ssl_id in [i["ssl_id"] for i in data]:
            return public.returnMsg(
                False, public.lang("Certificate information was not found")
            )
        ...
        task_data = {
            ...
            "sender": sender_lsit,
            "number_rule": {"day_num": 0, "total": int(get.total)},
    ...
    def renewal_cert(self, get):
        ...
        if "index" not in get:
            return public.returnMsg(
                False, public.lang("Required parameters are missing.")
            )
    ...
    def add_renewal_task(self, get):
        ...
        get.sBody = "/www/server/panel/pyenv/bin/python3 -u /www/server/panel/class/acme_v2.py --renew=1 --index={} --cycle={}".format(
            get.index, get.cycle
        )
        get.type = "day"
        get.week = 1
    ...
    def get_cert_content(self, get):
        ...
        if not ssl_hash:
            return public.returnMsg(
                False, public.lang("No certificate information was found.")
            )
        ssl_info = self.find_ssl_info(ssl_hash=ssl_hash)
        ssl_info["key"] = public.readFile(
            "/www/server/panel/vhost/ssl_saved/{}/privkey.pem".format(ssl_hash)
        )
        ssl_info["cert"] = public.readFile(
            "/www/server/panel/vhost/ssl_saved/{}/fullchain.pem".format(ssl_hash)
        )
