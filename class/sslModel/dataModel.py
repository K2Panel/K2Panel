...
class main(sslBase):
    ...
    def run_fun(self, get):
        ...
        if not get.fun_name in [
            ...
            return public.returnMsg(
                False, public.lang("wrong params，undefined func name")
            )
        ...
        try:
            ...
            return public.returnMsg(
                False, public.lang("Execution failed:{}".format(str(e)))
            )
    ...
    def get_domain_list(self, get):
        ...
        dns_data = self.get_dns_data(get)
        data = (
            public.M("ssl_domains")
            .field("id,domain,type_id,dns_id,endtime,ps")
            .where("1=1 {}".format(filter_sql), ())
        ...
        if report_data:
            report_data_dic = {
                ...
                if i["source"] == "domain_endtime" and i["status"]
            }
        ...
        for val in data:
            ...
            # 获取子域名数量
            val["sbd_count"] = (
                record_data.get(val["domain"], {})
                .get("info", {})
                .get("record_total", "-")
            )
            _data.append(val)
        try:
            public.M("ssl_domains").where(
                "id in ({})".format(",".join(remove_ids)), ()
            ).delete()
        except Exception as e:
            print(e)
    ...
    def set_domain_dns(self, get):
        ...
        if not ids:
            return public.returnMsg(
                False, public.lang("Please select the domain name you want to set!")
            )
        ...
        if dns_id not in dns_data:
            return public.returnMsg(
                False, public.lang("The specified DNS does not exist!")
            )
        ...
        res = public.M("ssl_domains").executemany(
            "update ssl_domains set dns_id=? where id=?", params
        )
        if type(res) != int:
            return public.returnMsg(False, public.lang("setting fail!"))
        ...
        return public.returnMsg(
            True,
            public.lang(
                "The setting was successful, and {} records have been updated!".format(
                    res
                )
            ),
        )
    ...
    def set_domain_endtime(self, get):
        ...
        res = public.M("ssl_domains").executemany(
            "update ssl_domains set endtime=? where id=?", params
        )
        if type(res) != int:
            return public.returnMsg(False, public.lang("setting fail!"))
    ...
    def set_domain_type(self, get):
        ...
        res = public.M("ssl_domains").executemany(
            "update ssl_domains set type_id=? where id=?", params
        )
        if type(res) != int:
    ...
    def __init_data(self):
        ...
        if os.path.exists(check_path) and os.path.exists(check_path_docker):
            if m_time == public.readFile(
                check_path
            ) and m_time_docker == public.readFile(check_path_docker):
                return
        ...
        root_domains = {
            i["domain"] for i in public.M("ssl_domains").field("domain").select()
        }
        site_domains = {
            self.extract_zone(i["name"])[0]
        ...
        add_domains = (
            (site_domains | docker_site_domains) - root_domains - set(skip_domains)
        )
        # del_domains = root_domains - (docker_site_domains | site_domains)

        # 添加域名
        for domain in add_domains:
            public.M("ssl_domains").add(
                "domain,dns_id,type_id,endtime,ps", (domain, 0, 0, 0, "")
            )
...
""",
    ...
    def func_models(self, get, def_name):
        ...
        sfile = "{}/class/sslModel/{}Model.py".format(
            public.get_panel_path(), get.dns_type
        )
        if not os.path.exists(sfile):
            raise Exception(
                public.lang("The module file {} does not exist.".format(sfile))
            )
    ...
    def get_domain_dns_config(self, get):
        ...
        data = (
            public.M("domain")
            .where(where_sql, param)
            .field("name,pid as site_id")
            .select()
            + public.M("docker_domain")
            .where(where_sql, param)
            .field("name,pid as site_id")
            .select()
        )
    ...
    def get_site_list(self, get):
        return (
            public.M("sites").field("name,id").select()
            + public.M("docker_sites").field("name,id").select()
        )
    ...
    def add_dns_value_by_domain(
        self, domain, dns_value, record_type="TXT", is_let_txt=False
    ):
        ...
        for dns_config in self.get_dns_data(public.dict_obj()).values():
            if root in dns_config.get("domains", []) or str(dns_id) == dns_config["id"]:
                ...
                if not _return["status"] and "记录已存在" not in _return["msg"]:
                    raise Exception(
                        public.lang(
                            "Failed to set the DNS record：{}".format(_return["msg"])
                        )
                    )
                else:
                    return _return
        raise Exception(
            public.lang(
                "No valid DNS API key information was found for the domain name {}.".format(
                    domain
                )
            )
        )
    ...
    def del_dns_value_by_domain(self, domain, record_type="TXT", is_let_txt=False):
        ...
        for record_id in ids:
            ...
            if not _return["status"]:
                raise Exception(
                    public.lang(
                        "Failed to delete the DNS record：{}".format(_return["msg"])
                    )
                )
    ...
    def add_domain(self, get):
        ...
        if data:
            return public.returnMsg(
                False, public.lang("The domain name already exists.")
            )
        public.M("ssl_domains").add(
            "domain,dns_id,type_id,endtime,ps", (domain_name, dns_id, 0, 0, "")
        )
