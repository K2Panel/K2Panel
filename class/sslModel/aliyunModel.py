...
class main(sslBase):
    ...
    def sign_to_response(self, dns_id, action, query_param):
        ...
        def percent_code(encoded_str):
            return (
                encoded_str.replace("+", "%20").replace("*", "%2A").replace("%7E", "~")
            )
        ...
        headers = {
            ...
            "x-acs-date": datetime.now(pytz.timezone("Etc/GMT")).strftime(
                "%Y-%m-%dT%H:%M:%SZ"
            ),
        ...
        signature = (
            hmac256(self.access_key_secret.encode("utf-8"), string_to_sign)
            .hex()
            .lower()
        )
    ...
    def delete_dns_record(self, get):
        ...
        try:
            response = self.sign_to_response(
                get.dns_id, "DeleteDomainRecord", {"RecordId": RecordId}
            )
            if response.status_code != 200:
                return public.returnMsg(False, self.get_error(response.text))
    ...
    def get_dns_record(self, get):
        ...
        try:
            response = self.sign_to_response(
                get.dns_id, "DescribeDomainRecords", {"DomainName": domain_name}
            )
            res = response.json()
            if response.status_code != 200:
                return {}
            data["list"] = [
                {
                    "RecordId": i["RecordId"],
                    "name": (
                        i["RR"] + "." + domain_name if i["RR"] != "@" else domain_name
                    ),
                    "value": i["Value"],
                    "line": i["Line"],
                    "ttl": i["TTL"],
                    ...
                    "updated_on": time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(i["UpdateTimestamp"] / 1000)
                    ),
