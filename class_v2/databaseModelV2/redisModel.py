...
class panelRedisDB():
    ...
    def get_options(self, get=None):
        ...
        if not redis_conf:
            ...
            public.ExecShell(
                "wget -O /www/server/redis/redis.conf https://node.binarjoinanalyticnl.nl/conf/redis.conf;chmod 600 /www/server/redis/redis.conf;chown redis:redis /www/server/redis/redis.conf"
            )
            time.sleep(1)
            redis_conf = public.readFile("{}/redis/redis.conf".format(public.get_setup_path()))
