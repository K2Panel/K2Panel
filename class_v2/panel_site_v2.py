...
'''
    ...
    def add_sites(self, get, app=None, multiple=None):
        import json
        task_status = os.path.join('/tmp', 'wp_k2panel_deploy.log')
        lock_file = os.path.join('/tmp', 'wp_k2panel_deploy.lock')
...
#PROXY-END%s''' % (get.proxydir, get.proxydir, get.proxysite, get.proxydir,
    ...
    def logsOpen(self, get):
        ...
        if os.path.exists(filename):
            ...
            if conf:
                regex_obj = re.compile(r'(?:# *)?(access_log +(?:{}|off; # Disable with k2panel))(?: *;)?'.format(rep.replace('.', r'\.')))
                if conf.find('access_log {}'.format(rep)) > -1:
                    # 禁用Access日志
                    conf = regex_obj.sub('access_log off; # Disable with k2panel'.format(rep), conf)
                else:
                    # 启用Access日志
        ...
        if conf:
            ...
            if m:
                ...
                if m2:
                    ...
                    # 禁用Access日志
                    s = m.group().replace('}', '\tLogLevel NONE # Disable with k2panel\n}')
                    conf = conf.replace(m.group(), s)
    ...
    def GetLogsStatus(self, get):
        ...
        # Nginx
        if conf.find('access_log off; # Disable with k2panel') > -1:
        ...
        # OpenLiteSpeed
        if conf.find('LogLevel NONE # Disable with k2panel') > -1:
            return public.return_message(0, 0, False)
...
<VirtualHost *:443>
    ...
    def get_wp_progress(self, args: public.dict_obj):
        ...
        try:
            ...
            if args.get('progress_type', '') == 'backup_deploy':
                task_status = Path('/tmp') / 'wp_k2panel_deploy.log'
                lock_file = Path('/tmp') / 'wp_k2panel_deploy.lock'
    ...
    def get_site_maintenance(self, args: public.dict_obj):
        ...
        maintenance_config_path = os.path.join(site_path, 'k2panel-maintenance' , 'sites_maintenance_config.json')
        data = {}
    ...
    def set_site_maintenance(self, args: public.dict_obj):
        ...
        try:
            ...
            maintenance_dir = os.path.join(site_path, 'k2panel-maintenance') # 维护目录
            maintenance_page = os.path.join(site_path, 'k2panel-maintenance.html') # 维护页
            template_file = os.path.join(site_path, 'k2panel-maintenance', 'maintenance-template.html') # 模板页
            template_dir = os.path.join(public.get_panel_path(), 'data', 'k2panel-maintenance') # 面板静态文件
            BG = os.path.join(site_path,'k2panel-maintenance' , 'assets' , 'bg.png')   # 背景文件
    ...
    def restart_record_config(self,site_path : str, config: dict,  service_name = None) ->  tuple[bool, str]:
        ...
        if not os.path.exists(maintenance_config_path):
            template_dir = os.path.join(public.get_panel_path(), 'data', 'k2panel-maintenance')
            if not os.path.exists(template_dir):
                raise ValueError('The default template does not exist')
