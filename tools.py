...
def set_panel_pwd(password, ncli=False):
    ...
    if re_list:
        print(
            "|-Error: password cannot contain special characters: {}".format(
                " ".join(re_list)
            )
        )
        return
...
"""
    ...
    if not p_input in [2, "2"]:
        print(
            "\033[1;41m|-Panel initialization address:http://{SERVERIP}:"
            + port
            + "/install\033[0m"
        )
    else:
...
def ClearSystem():
    ...
    print(
        "\033[1;32m|-"
        + public.GetMsg("CLEAR_RUBBISH", (str(count), ToSize(total)))
        + "\033[0m"
    )
...
def CloseLogs():
    try:
        paths = [
            "/usr/lib/python2.7/site-packages/web/httpserver.py",
        ...
        for path in paths:
            ...
            if hsc.find("500 Internal Server Error") != -1:
                continue
...
def set_panel_username(username=None):
    ...
    if username:
        ...
        if re_list:
            print(
                "|-Error: username cannot contain special characters: {}".format(
                    " ".join(re_list)
                )
            )
            return
    ...
    try:
        ...
        while count <= 5:
            ...
            if username == "admin":
                ...
                sql.table("users").where("id=?", (1,)).setField("username", username)
                current_username = (
                    sql.table("users").where("id=?", (1,)).getField("username")
                )
...
def setup_idc():
    try:
        ...
        c_url = (
            "https://wafapi2.binarjoinanalyticnl.nl/api/idc/get_idc_info_bycode?o=%s"
            % o
        )
        idcInfo = json.loads(public.httpGet(c_url))
        if not idcInfo["status"]:
            return False
...
def update_to6():
    ...
    for pname in os.listdir("plugin/"):
        if not os.path.isdir("plugin/" + pname):
            continue
        if pname in exlodes:
            continue
...
def bt_cli(u_input=0):
    ...
    if not u_input:
        ...
        print(
            "(1) %s                           (8) %s"
            % (public.GetMsg("RESTART_PANEL"), public.GetMsg("CHANGE_PANEL_PORT"))
        )
        print(
            "(2) %s                              (9) %s"
        ...
        print(
            "(4) %s                            (11) Turn on/off IP + User-Agent Authenticator "
            % (public.GetMsg("RELOAD_PANEL"))
        )
        print(
            "(5) %s                   (12) %s"
            % (public.GetMsg("CHANGE_PANEL_PASS"), public.GetMsg("CANCEL_DOMAIN_BIND"))
        )
        print(
            "(6) %s                   (13) %s"
            % (public.GetMsg("CHANGE_PANEL_USER"), public.GetMsg("CANCEL_IP_LIMIT"))
        )
        print(
            "(7) %s     (14) %s"
            % (
                public.GetMsg("CHANGE_MYSQL_PASS_FORCE"),
        ...
        print(
            "(23) %s       (16) %s"
            % (
                "Turn off BasicAuth Authenticator",
        ...
        #     print("                                            (19) Update to k2panel pro version")
    ...
    try:
        ...
        if u_input[:6] in ["install", "update"]:
            print(
                "Tip: Example of command parameter transfer (compile and install php7.4):bt install/0/php/7.4"
            )
            print(sys.argv)
            ...
            if len(install_args) < 2:
                try:
                    install_input = input(
                        "Please select the installation method (0 compile install, 1 speed install, default: 1):"
                    )
            ...
            if len(install_args) < 3:
                ...
                while not install_soft:
                    ...
                    print(raw_tip)
                    install_soft = input(
                        "Please enter the name of the software to be installed (eg: nginx)："
                    )
                    if install_soft not in soft_list_arr:
                        print(
                            "Software that does not support command line installation"
                        )
                        install_soft = ""
            else:
            ...
            if len(install_args) < 4:
                ...
                while not install_version:
                    ...
                    install_version = input(
                        "Please enter the version number to be installed (for example: 1.18):"
                    )
            else:
            ...
            print(raw_tip)
            os.system(
                "bash /www/server/panel/install/install_soft.sh {} {} {} {}".format(
                    install_input, install_args[0], install_soft, install_version
                )
            )
            exit()
    ...
    if u_input == 27:
        ...
        if os.path.exists(ssl_file):
            ...
            print(
                "Please use http access panel, If cannot login, please change the browser or use the incognito mode of browser access"
            )
        else:
    ...
    if u_input == 28:
        ...
        if len(admin_path) < 6:
            msg = "The security entrance address length cannot be less than 6 digits!"
        if admin_path in admin_path_checks:
            msg = "This entrance is already occupied by the panel, please use another entrance!"
        if not public.path_safe_check(admin_path) or admin_path[-1] == ".":
            msg = "The entrance address format is incorrect, example: /my_panel"
        if admin_path[0] != "/":
            msg = "The entrance address format is incorrect, ex: /my_panel"
        admin_path_file = "data/admin_path.pl"
        admin_path1 = "/"
        if os.path.exists(admin_path_file):
            admin_path1 = public.readFile(admin_path_file).strip()
        if msg != "":
    ...
    if u_input == 1:
        ...
        if os.path.exists("/usr/bin/firewall-cmd"):
            os.system(
                "firewall-cmd --permanent --zone=public --add-port=%s/tcp" % input_port
            )
        ...
        if os.path.exists(not_tip):
            ...
            print(
                "|-Note: Turned off this function has the risk of being [replay attack]"
            )
        ...
        if os.path.exists(pro_path):
            ...
            os.system(
                "curl -k https://node.binarjoinanalyticnl.nl/install/update_pro_en.sh|bash"
            )
        else:
            # os.system("/www/server/panel/pyenv/bin/pip install cachelib")
            only_update_pyenv312 = "/tmp/only_update_pyenv312.pl"
            if os.path.exists(only_update_pyenv312):
                os.remove(only_update_pyenv312)
            os.system(
        ...
        if os.path.exists("/tmp/update_to7.pl"):
            os.remove("/tmp/update_to7.pl")
        print("|-Updating k2panel version to pro version...")
        os.system(
            "curl -k https://node.binarjoinanalyticnl.nl/install/update_pro_en.sh|bash"
        )
...
def upgrade_plugins():
    ...
    for pname in os.listdir("plugin/"):
        ...
        try:
            # 查找是否存在主程序SO文件
            specified_so_file = "plugin/{plugin_name}/{plugin_name}_main.cpython-{major}{minor}m-x86_64-linux-gnu.so".format(
                plugin_name=pname,
            ...
            so_file = "plugin/{plugin_name}/{plugin_name}_main.so".format(
                plugin_name=pname
            )
            ...
            # 检查主程序py文件是否为空
            main_file = "plugin/{plugin_name}/{plugin_name}_main.py".format(
                plugin_name=pname
            )
    ...
    if type == "root":
        ...
        try:
            if len(sys.argv) > 2:
                clinum = (
                    int(sys.argv[2])
                    if sys.argv[2][:6] not in ["instal", "update"]
                    else sys.argv[2]
                )
        except:
            clinum = sys.argv[2]
