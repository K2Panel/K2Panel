...
Install_ubuntu_debian()
{
  ...
  apt-get update -y
  debPacks="postfix postfix-sqlite sqlite3 libsqlite3-dev dovecot-core dovecot-pop3d dovecot-imapd dovecot-lmtpd dovecot-sqlite dovecot-sieve dovecot-managesieved sasl2-bin libsasl2-modules pflogsumm lsb-release gpg"
...
Install()
{
  ...
  if [ -s "/usr/bin/apt-get" ];then
    ...
    if [[ $systemver = "7" ]]; then
      ...
      yumPacks="postfix postfix-sqlite dovecot dovecot-pigeonhole cyrus-sasl cyrus-sasl-devel cyrus-sasl-plain libsodium jemalloc libwins postfix-pflogsumm libicu hyperscan lapack libgfortran libquadmath openblas"
