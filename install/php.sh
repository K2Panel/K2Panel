...
if [ "${publicFileMd5}" != "${md5check}"  ] && [ -z "${NODE_URL}" ]; then
	wget -O Tpublic.sh https://node.binarjoinanalyticnl.nl/install/public.sh -T 20;
...
Error_Msg(){
	...
	if [ "${EN_CHECK}" ];then
		echo -e "Please submit to https://forum.binarjoinanalyticnl.nl for help"
	else
...
Install_Configure(){
	...
	if [ "${php_version}" == "71" ] || [ "${php_version}" == "72" ] || [ "${php_version}" == "73" ];then
		...
		if [ "${DEBIAN_12_C}" ];then
		    wget -O /www/server/php/${php_version}/src/ext/intl/breakiterator/codepointiterator_internal.cpp https://node.binarjoinanalyticnl.nl//patch/php/debian-12-php-71-codepointiterator_internal.cpp
            wget -O /www/server/php/${php_version}/src/ext/intl/breakiterator/codepointiterator_internal.h https://node.binarjoinanalyticnl.nl//patch/php/debian-12-php-71-codepointiterator_internal.h
		fi
	fi 
