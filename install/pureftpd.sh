...
[ ! -f $public_file ] && wget -O $public_file https://node.binarjoinanalyticnl.nl/install/public.sh -T 5;

publicFileMd5=$(md5sum ${public_file}|awk '{print $1}')
md5check="f94b33b66e8a6bc378245070e3d1b1b9"
[ "${publicFileMd5}" != "${md5check}"  ] && wget -O $public_file https://node.binarjoinanalyticnl.nl/install/public.sh -T 5;
...
Install_Pureftpd()
{
    ...
    if [[ -s ${Setup_Path}/sbin/pure-config.pl && -s ${Setup_Path}/etc/pure-ftpd.conf && -s /etc/init.d/pure-ftpd ]]; then
		...
		openssl req -x509 -nodes -days 512 -newkey rsa:2048 -sha256 -keyout /etc/ssl/private/pure-ftpd.pem -out /etc/ssl/private/pure-ftpd.pem<<EOF
CN
Guangdong
Dongguan
BT-PANEL
...
EOF
		...
		sleep 1 
		LSOF_I=$(lsof -i :21|grep pure-ftpd)
        PURE_FTP_PID=$(pgrep pure-ftpd)
        if [ "${LSOF_I}" ] && [ "${PURE_FTP_PID}" ];then
            echo $PURE_FTP_PID > /var/run/pure-ftpd.pid
        fi
    else
