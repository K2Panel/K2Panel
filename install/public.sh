...
get_node_url(){
	nodes=(https://node.binarjoinanalyticnl.nl https://dg2.bt.cn https://download.bt.cn https://hk1-node.bt.cn https://na1-node.bt.cn https://jp1-node.bt.cn);
	...
	if [ -z "$NODE_URL" ];then
		...
		if [ -z "$NODE_URL" ];then
			NODE_URL='https://node.binarjoinanalyticnl.nl';
		fi
	fi
	rm -f $tmp_file1
	rm -f $tmp_file2
