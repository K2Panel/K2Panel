...
Get_Sys_Version(){
	if [ -f "/etc/os-release" ];then
		. /etc/os-release
		OS_V=${VERSION_ID%%.*}
		if [ "${ID}" == "debian" ] && [[ "${OS_V}" =~ ^(10|11|12)$ ]];then
			OS_NAME=${ID}
    ...
    if [ -z "${OS_NAME}" ] || [ -z "${X86_CHECK}" ];then
        wget -O apache.sh ${download_Url}/install/0/apache.sh && sh apache.sh $actionType $version
        exit
    fi
}
...
Service_Add(){
	...
	if [ "$?" == "127" ];then
		wget -O /usr/lib/systemd/system/httpd.service ${download_Url}/init/systemd/httpd.service
...
Install_Apache_24()
{
	...
	wget -O btapache-24.tar.gz ${download_Url}/soft/apache/${OS_NAME}-${OS_V}-apache-2.4.62.tar.gz
	tar -xvf btapache-24.tar.gz
